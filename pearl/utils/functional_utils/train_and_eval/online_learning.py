...
def online_learning(
    ...
    model_path: str = None,
    # 以下是新增用于保存的参数---------------------------------------------------
    moving_average_window: int = 5,  # 添加滑动平均窗口长度参数
    save_path: str = "./saved_models",  # 指定模型保存路径
    ...
    if not os.path.exists(save_path):
        os.makedirs(save_path)  # 确保保存路径存在
    ...
    # 以下是新增用于保存的代码------------------------------------------------
    returns = []  # 存储所有episode的返回值，用于计算滑动平均
    highest_moving_average = -np.inf  # 初始化最高滑动平均返回值为负无穷大
    ...
    while True:
        ...
        if reload_model==True:
            agent = torch.load(model_path)  # 加载模型
            reload_model=False
        ...
        #--------------------------保存模型逻辑--------------------------------
        episode_return = episode_info["return"]
        returns.append(episode_return)

        # 当累积的episode足够计算滑动平均时
        # if len(returns) >= moving_average_window:
        #     current_moving_average = np.mean(returns[-moving_average_window:])
        #     if current_moving_average > highest_moving_average:
        #         highest_moving_average = current_moving_average
        #         model_path = os.path.join(save_path,
        #                                   f"model_ep{total_episodes}_avg{highest_moving_average:.2f}_ret{episode_return:.2f}.pt")
        ...
        #这个地方在while循环里面，每个episode结束后都会执行一次================================================================
        # ==========================打印episode结束传回的info====================
        # plt.rcParams['font.sans-serif'] = ['SimSun']
        # 传回列表=feather.read_dataframe(r"C:\Users\joshua\Desktop\Pearl-main\pearl\外汇交易\episode")
        # 净值列表=传回列表["List_of_net_asset_value_changes"].values
        # 不计摩擦列表=传回列表["不计摩擦成本净值变化列表"].values
        # 只计盈利列表=传回列表["只计盈利的净值变化列表"].values
        #
        # # 创建 x 值（时间步）和 y 值（净值变化）
        # time_steps = list(range(len(净值列表)))
        #
        # # 创建一个图形
        # fig, ax1 = plt.subplots(figsize=(8, 4))  # 调整 figsize 以适应单个子图的大小
        #
        # # 绘制净值变化
        # ax1.plot(time_steps, 净值列表, label="净值变化")
        # ax1.set_xlabel("时间步")  # 将 X 轴标签移到这里
        # ax1.set_ylabel("净值变化")
        # ax1.legend()
        #
        # # 显示图形
        ...
        #
        # # 第二个子图：绘制不计摩擦净值变化
        # ax2.plot(time_steps, 不计摩擦列表, label="不计摩擦净值变化", color='orange')
        # ax2.set_ylabel("不计摩擦净值变化")
        # ax2.legend()
        #
        # # 第三个子图：绘制只计盈利净值变化
        # ax3.plot(time_steps, 只计盈利列表, label="只计盈利净值变化", color='green')
        # ax3.set_xlabel("时间步")
        # ax3.set_ylabel("只计盈利净值变化")
        # ax3.legend()
        #
        # # 显示图形
        # plt.show()
...
def run_episode(
    ...
    while not done:
        ...
        episode_steps += 1
        #agent完成了一个episode中的一个完整的step================================================================
        try:
            with open('提前终止训练.txt', 'r') as file:
                content = file.read()
                if content == '1':
                    print('训练已提前终止。')
                    保存名字 = r"C:\Users\joshua\Desktop\Pearl-main\pearl\外汇交易手动保存.pt"
                    torch.save(agent, 保存名字)
                    print(
                        "===========================================================================================================================")
                    print(
                        f"Model saved to {保存名字} ")
                    print(
                        "===========================================================================================================================")
