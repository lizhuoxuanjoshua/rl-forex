...
#开始
    ...
    A1=((C-llv(L,21))/(hhv(H,21)-llv(L,21)))*100

    A2=(MA(3*sma(A1,6,1)-2*sma(sma(A1,5,1),5,1),2))

    A3=(C-llv(LOW,34))/(hhv(HIGH,34)-llv(LOW,34))*100

    A4=sma(sma(A3,3,1),3,1)

    A5=3*sma(A3,3,1)-2*A4

    A6=EMA(A5,5)

    A7=EMA(100*(C-llv(LOW,34))/(hhv(H,34)-llv(LOW,34)),3)

    A8=((3*sma((((C-llv(LOW,20))/(hhv(HIGH,20)-llv(LOW,20)))*100),5,1))-(2*sma(sma((((C-llv(LOW,20))/(hhv(HIGH,20)-llv(LOW,20)))*100),5,1),3,1)))

    a=(A2<3) * (A6<3) * (A7<8.5) * (A8<5) * (REF(A2,1)>3)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
