...
#开始
    ...
    AA=(REF(C<O,1) ) * ( C>O ) * ( C/REF(C,1)>1.03)
    AB=(4*sma((CLOSE-llv(LOW,5))/(hhv(HIGH,5)-llv(LOW,5))*100,5,1)-3*sma(sma((CLOSE-llv(LOW,5))/(hhv(HIGH,5)-llv(LOW,5))*100,5,1),3.2,1))
    CC=(REF(C<O,2) ) * ( H/C<1.01 ) * ( CROSS(AB,8))
    a=( AA ) * ( CC)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
