...
#开始
    ...
    VAR9=((VAR8-MA(VAR8,14))/((0.015)*(AVEDEV(VAR8,14))))
    VAR10=((VAR8-MA(VAR8,70))/((0.015)*(AVEDEV(VAR8,70))))
    VAR11=((VAR9>=150) ) * ( (VAR9<200) ) * ( (VAR10>=150) ) * ( (VAR10<200))
    tt=((VAR9<=(-150)) ) * ( (VAR9>(-200)) ) * ( (VAR10<=(-150)) ) * ( (VAR10>(-200)))
    VAR12=(np.where(tt,(-10),VAR11))
    VAR13=(((CLOSE-MA(CLOSE,13))/(MA(CLOSE,13)))*(100))
    VAR14=(100-np.abs(VAR13))
    VAR15=(np.where((VAR14<90),VAR14,100))

    VAR17=  ( (VAR15<150))
    a=(VAR17 ) * ( (count(VAR17,20)==1))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
