...
#开始
    ...
    VAR4G=((VAR3G ) * ( (count(VAR3G,5) > 1)))
    VARAA=((VAR4G ) * ( ((VAR1G) ) + ( (VAR2G)))*2)
    VARBB=(C/REF(C,25)<=1.1)
    VARCC=((sma(np.maximum(C-REF(C,2),0),7,1)/sma(np.abs(C-REF(C,2)),7,1)*100)<15)
    AAA=(sma((CLOSE-llv(LOW,8))/(hhv(HIGH,8)-llv(LOW,8))*100,2,1))
    AA=(AAA>REF(AAA,1) ) * ( REF(AAA,1)<REF(AAA,2) ) * ( AAA<23)
    a=(VARAA ) * ( VARBB ) * ( VARCC ) * ( AA)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
