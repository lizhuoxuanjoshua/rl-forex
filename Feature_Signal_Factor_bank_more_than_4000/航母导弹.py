...
#开始
    ...
    VAR2=np.where((EMA((C+L)/2,3)-VAR1)*5>0,np.power((EMA((C+L)/2,3)-VAR1)*5,2)/10,-1*np.power((EMA((C+L)/2,3)-VAR1)*5,2)/10)

    VAR3=np.where(VAR2>0,VAR2-0,0)

    a=(VAR3>0 ) * ( REF(VAR3,1)==0 ) * ( (C+L)/2>MA((C+L)/2,8))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
