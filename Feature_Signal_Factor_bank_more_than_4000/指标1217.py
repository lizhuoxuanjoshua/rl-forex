...
#开始
    ...
    LWR1=(sma(RSV,2,1))
    LWR2=(sma(LWR1,2,1))
    M=(CROSS(LWR2,LWR1) ) * ( LWR1<REF(LWR1,1))
    DF=((hhv(C,21)-C)/hhv(C,21)*100>=7)
    A0=((count(CLOSE*AMOUNT,360)/count(AMOUNT,360)))
    A1=((count(CLOSE*AMOUNT,300)/count(AMOUNT,300)))
    A2=((count(CLOSE*AMOUNT,240)/count(AMOUNT,240)))
    A4=((A0+A1+A2)/3)
    A5=(C<=A4*1.1)
    a=(count(REF(BRDL,1),5) ) * ( DF ) * ( M ) * ( A5 ) * ( LWR1>=60)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
