...
#开始
    ...
    VAR11=(np.where(1,VAR10,VAR10))
    VAR12=(LOW)
    VAR13=(REF(LOW,1))
    VAR14=(sma(np.abs(VAR12-VAR13),3,1)/sma(np.maximum(VAR12-VAR13,0),N,1)*100)
    VAR15=(MA(np.where(CLOSE*1.35<=VAR11,VAR14*10,VAR14/10),N))
    VAR16=(llv(LOW,30))
    VAR17=(hhv(VAR15,30))
    VAR18=(1990831)
    VAR19=(np.where(1,MA(np.where(LOW<=VAR16,(VAR15+VAR17*2)/2,0),N),100000000))
    VAR20=((hhv(H,9)-C)/(hhv(H,9)-llv(L,9))*100)
    VAR21=(sma(VAR20,3,1))
    VAR22=(sma(VAR21,3,1))
    VAR23=(3*VAR21-2*VAR22)
    VAR24=(np.where(VAR23>=100,100,np.where(VAR23<=0,0,VAR23)))
    VAR25=((CLOSE-MA(CLOSE,60))/MA(CLOSE,60)*100)
    VAR26=(100+(-np.abs(VAR25)))
    VAR27=((CLOSE-llv(LOW,21))/(hhv(HIGH,21)-llv(LOW,21))*100)
    VAR28=(sma(sma(VAR27,9,4),9,4)/7.77777)
    VAR29=(EMA(VAR28,5))
    VAR30=(3*VAR28-2*VAR29)
    a=(CROSS(VAR30,VAR28) ) * ( VAR28<2.7 ) * ( C>O)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
