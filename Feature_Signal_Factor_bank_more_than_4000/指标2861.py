...
#开始
    ...
    D=(sma(K,5,1))
    A=(sma(VOL,10,1))
    WR00=(-100*(hhv(hhv(A,1),14)-A)/(hhv(hhv(A,1),14)-llv(llv(A,1),14)))
    W11=(-100*(hhv(hhv(A,1),34)-A)/(hhv(hhv(A,1),34)-llv(llv(A,1),34)))
    W22=(-100*(hhv(hhv(A,1),55)-A)/(hhv(hhv(A,1),55)-llv(llv(A,1),55)))
    W33=(-100*(hhv(hhv(A,1),5)-A)/(hhv(hhv(A,1),5)-llv(llv(A,1),5)))
    量=((WR00+W11+W22+W33)/4)
    WR=(-100*(hhv(HIGH,55)-CLOSE)/(hhv(HIGH,55)-llv(LOW,55)))
    a=((((CROSS(量,-45))) ) + ( ((CROSS(量,-50)))) ) * ( ((CROSS(WR,-45)) ) + ( ((CROSS(WR,-50)))))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
