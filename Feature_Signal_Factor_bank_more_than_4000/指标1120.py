...
#开始
    ...
    MID=(MA(CLOSE,21))
    UPPER=(MID + 1.96*STD(CLOSE,21))
    LOWER=(MID - 1.96*STD(CLOSE,21))
    UP=(MID + 2.58*STD(CLOSE,21))
    LOOW=(MID - 2.58*STD(CLOSE,21))
    AAA=((C<MID))
    AAB=(UP-LOOW)
    AAC=(((C-llv(C,30))/(hhv(C,30)-llv(C,30))<0.4))
    AA=((AAA ) * ( barslast(REF(AAB,1)==hhv(AAB,50))<=5 ) * ( LOOW>REF(LOOW,1) ) * ( C <MID) ) * ( AAC)
    a=(FILTER(AA,5))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
