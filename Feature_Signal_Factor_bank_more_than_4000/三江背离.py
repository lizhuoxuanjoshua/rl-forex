...
#开始
    ...
    BA1=((CLOSE-MA(CLOSE,6))/MA(CLOSE,6)*100)
    BA2=((CLOSE-MA(CLOSE,12))/MA(CLOSE,12)*100)
    BA3=((CLOSE-MA(CLOSE,24))/MA(CLOSE,24)*100)
    VAR1=(llv(LOW,36))
    VAR2=(hhv(HIGH,30))
    VAR3=(EMA((CLOSE-VAR1)/(VAR2-VAR1)*4,4)*25)
    MM=((BA1+2*BA2+3*BA3)/6)
    MN=(MA(MM,3))
    MT=(MA(MM,5))
    DBL=(np.where(MN<-5,1,0))
    a=(CROSS(MN,MT) ) * ( DBL)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
