...
#开始
    ...
    VR1=(0.1)
    VR2=(0.1)
    CS1=((2*CLOSE+HIGH+LOW)/4*(VR1+0.001))
    CS2=(EMA(CS1,4)*(VR2))
    CS3=(EMA(CS2,4))
    CS4=(EMA(CS3,4))
    JS0=((CS4-REF(CS4,1))/REF(CS4,1)*100*(VR2))
    短波0=(JS0)
    JS1=(MA(JS0,2))
    短波1=(JS1)
    JS2=(MA(JS0,1))
    短波2=(JS2)
    红实波=(JS0>=0 ) * ( JS0>REF(JS0,1))
    红空波=(JS0>=0 ) * (( JS0<=REF(JS0,1)
))
    绿波=(JS0<0 ) * ( JS1>=JS2)
    蓝波=(JS0<0 ) * ( JS1<=JS2)
    涨=(短波0>REF(短波0,1))
    跌=(短波0<REF(短波0,1))
    连跌=(barslast(短波0<REF(短波0,1) ) * ( REF(短波0,1)>REF(短波0,2))*跌+1*跌)
    久跌买=(连跌>=TNB ) * ( C/REF(C,1)>1)
    a=(久跌买 ) * ( count(久跌买,TNB)==1)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
