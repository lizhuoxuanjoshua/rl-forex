...
#开始
    ...
    VAR13=(EMA(CLOSE, 12)-EMA(CLOSE, 26))
    VAR14=(EMA(VAR13, 9))
    VAR15=((CLOSE-llv(LOW, 9))/(hhv(HIGH, 9)-llv(LOW, 9))*100)
    VAR16=(sma(VAR15, 3, 1))
    VAR17=(sma(VAR16, 3, 1))
    VAR18=(EMA(np.where(VAR14>REF(VAR14, 1), 1, (-1)), 14)*14)
    VAR19=(EMA(np.where(VAR17>REF(VAR17, 1), 1, (-1)), 14)*14*2)
    VAR20=((MA(CLOSE, 5)/MA(CLOSE, 70)-1)*100)
    VARAB=((VAR20+VAR19+VAR18)/3)
    VARAA=(MA(CLOSE, 80)-MA(CLOSE,10)*4)
    a=(CROSS(VARAB,VARAA))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
