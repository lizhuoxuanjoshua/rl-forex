...
#开始
    ...
    HD=((HIGH - REF(HIGH,1)))
    LD=((REF(LOW,1) - LOW))
    DMP=(count(np.multiply((HD > 0) , (HD > LD)),10))
    DMM=(count(np.multiply((LD > 0) , (LD > HD)),10))
    TR = SUM(MAX(MAX(HIGH - LOW, ABS(HIGH - REF(CLOSE, 1))), ABS(REF(CLOSE, 1) - LOW)), 10)
    PDI=(((DMP * 100) / TR))
    MDI=(((DMM * 100) / TR))
    ADX=(MA(((np.abs((MDI - PDI)) / (MDI + PDI)) * 100),5))
    WR10=(((100 * (hhv(HIGH,10) - CLOSE)) / (hhv(HIGH,10) - llv(LOW,10))))
    AA=((2*RSI5+ADX-WR10))
    a=(CROSS(AA,0) ) * ( C/REF(C,1)>1.02)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
