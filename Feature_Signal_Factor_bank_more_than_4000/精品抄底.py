...
#开始
    ...
    CC=(count(np.where((P > REF(P,1)),(P)*(VOL),0),4))/(count(np.where((P < REF(P,1)),(P)*(VOL),0),4))
    ...
    VAR6A=((hhv(HIGH,4) - P)/(hhv(HIGH,4) - llv(LOW,4)))*(100) - 72

    VAR7A=((P - llv(LOW,4))/(hhv(HIGH,4) - llv(LOW,4)))*(100)

    VAR8A=sma(VAR7A,2,1)

    CCI=((P - MA(P,14))*(100))/((1.5)*(AVEDEV(P,14)))*高

    RSI1=(((sma(np.maximum(CLOSE - REF(CLOSE,1),0),6,1))/(sma(np.abs(CLOSE - REF(CLOSE,1)),6,1)))*(100))*底
