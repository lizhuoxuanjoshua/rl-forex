...
#开始
    ...
    TJ1=(REF(CROSS(MA(C,5),C),1) ) * ( CROSS(C,MA(C,5)))
    TJ2=(REF(CROSS(MA(C,10),C),1) ) * ( CROSS(C,MA(C,10)))
    TJ3=(REF(CROSS(MA(C,20),C),1) ) * ( CROSS(C,MA(C,20)))
    TJ4=((TJ1) ) + ( (TJ2) ) + ( (TJ3
))
    TJ5=(REF(C,2)/REF(C,1)>=1.045 ) * ( REF(C,1)<REF(O,1) ) * ( C/REF(C,1)>1.09 ) * ( C==H ) * ( TJ4)
    A=((3*C+L+O+H)/6)
