...
#开始
    ...
    XA_2=1
    XA_3=(1)
    XA_4=(CLOSE/REF(CLOSE,1)>1.065)
    XA_5=(count(XA_4,10)>0)
    XA_6=(CLOSE/REF(CLOSE,1)<=1.03)
    XA_7=((CLOSE-MA(CLOSE,3))/MA(CLOSE,3)*(-100))
    XA_8=(100*(hhv(HIGH,3)-CLOSE)/(hhv(HIGH,3)-llv(LOW,3)))
    XA_9=((CLOSE-MA(CLOSE,5))/MA(CLOSE,5)*100)
    XA_10=(9)
    XA_11=(REF(CLOSE,1))
    XA_12=(sma(np.maximum(CLOSE-XA_11,0),XA_10,1)/sma(np.abs(CLOSE-XA_11),XA_10,1)*100)
    XA_13=(XA_12>40)
    XA_14=((HIGH+LOW+CLOSE)/3)
    XA_15=((XA_14-MA(XA_14,11))/(0.015*AVEDEV(XA_14,11))*(-1))
    XA_16=(CROSS(XA_15,(-68)))
    XA_17=(XA_15<125)
    XA_18=(CROSS(XA_8,60))
    XA_19=(CROSS(XA_7,1.85))
    XA_20=(XA_15<=REF(XA_15,1))
    XA_21=(XA_15<=(-70))
    XA_22=(XA_7>1.05)
    XA_23=(XA_15>(-100))
    XA_24=(CROSS(XA_9,(-3)))
    XA_25=(XA_8>60)
    XA_26=(XA_7>1.35)
    XA_27=(count((CLOSE-REF(CLOSE,1))/REF(CLOSE,1)*100,3)<5)
    XA_28=(VOL)
    XA_29=(XA_28)
    XA_30=(MA(XA_28,4))
    XA_31=(VOL<=REF(VOL,1))
    ...
    XA_39=(XA_35>XA_36)
    XA_40=(LOW<XA_37)
    XA_41=(XA_1==1 ) * ( XA_3 ) * ( XA_5 ) * ( XA_6 ) * ( XA_38)
    XA_42=(XA_1==1 ) * ( XA_3 ) * ( XA_5 ) * ( XA_6 ) * ( XA_38)
    XA_43=(XA_1==1 ) * ( XA_3 ) * ( XA_5 ) * ( XA_6 ) * ( XA_40 ) * ( XA_39)
    XA_44=((XA_41) ) + ( (XA_42
))
    XA_45=(count(XA_44,10)>0)
    XA_46=(CLOSE/REF(CLOSE,1)>=1.001)
    XA_47=(CLOSE/REF(CLOSE,2)-1>(-0.11))
    XA_48=(CROSS(CLOSE,REF(CLOSE,1)))
    XA_49=(MA(CLOSE,5)>=MA(CLOSE,15))
    XA_50=(count((CLOSE-REF(CLOSE,1))/REF(CLOSE,1)*100,3)>=15)
    XA_51=(CLOSE/REF(CLOSE,1)>1.09)
    XA_52=(count(XA_51,10)>0)
    XA_53=(count(XA_50,10)>0)
    XA_54=(XA_45 ) * ( XA_46 ) * ( XA_47 ) * ( XA_48 ) * ( XA_49 ) * ( XA_3 ) * ( XA_2)
    XA_55=(XA_52 ) * ( XA_53 ) * ( XA_48 ) * ( XA_47 ) * ( XA_49 ) * ( XA_3 ) * ( XA_2)
    a=(XA_54 ) + ( XA_55
)
#结束
    买入 = a
