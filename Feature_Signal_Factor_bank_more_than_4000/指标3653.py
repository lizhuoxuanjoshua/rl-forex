...
#开始
    ...
    MA21=(MA(CLOSE,21))
    MA65=(MA(CLOSE,65))
    GX=(np.maximum(MA5,np.maximum(MA21,MA65)))
    DX=(np.minimum(MA5,np.minimum(MA21,MA65)))
    AA=(np.abs(GX/DX-1)*100<=15)
    TPX=(np.minimum(MA65*1.15,MA21*1.13))
    a=(CROSS(C,TPX) ) * ( AA ) * ( MA65>REF(MA65,1))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
