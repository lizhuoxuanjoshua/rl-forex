...
#开始
    ...
    A0=(CROSS(C,REF(EMA(DMA((((H+L)+(C*2))/4.15),0.9),3),1)))
    A1=(3*sma(RSV,3,1)-2*sma(sma(RSV,3,1),3,1)>REF(3*sma(RSV,3,1)-2*sma(sma(RSV,3,1),3,1),1) ) * ( REF(3*sma(RSV,3,1)-2*sma(sma(RSV,3,1),3,1),1)<REF(3*sma(RSV,3,1)-2*sma(sma(RSV,3,1),3,1),2))
    A2=(sma(RSV,3,1)>REF(sma(RSV,3,1),1) ) * ( REF(sma(RSV,3,1),1)<REF(sma(RSV,3,1),2))
    A3=(MA(EMA(((2*CLOSE+HIGH+LOW)/4-llv(LOW,5))/(hhv(HIGH,5)-llv(LOW,5))*100,5),2))
    A4=(np.where(EMA(((2*CLOSE+HIGH+LOW)/4-llv(LOW,5))/(hhv(HIGH,5)-llv(LOW,5))*100,5)<=A3,0,1))
    B1=( EMA(((2*CLOSE+HIGH+LOW)/4-llv(LOW,34))/(hhv(HIGH,34)-llv(LOW,34))*100,8)>50)
    B2=(A3<40 ) * ( A4<1 ) * ( B1)
    B3=(CROSS(1.1,(llv(LOW,5)*hhv(HIGH,5))/C/C))
    B4=(REF(O>C,2) ) * ( (B2<1) ) * ( (REF(B2,1)>0) ) * ( REF(O>C,1) ) * ( (C-REF(C,1))/REF(C,1)*100>=3)
    a=( A0 ) * ( A1  ) * ( A2 ) * ( B3 ) * ( B4)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
