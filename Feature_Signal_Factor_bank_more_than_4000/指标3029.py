...
#开始
    ...
    VAR1=(1)
    VAR2=((CLOSE-MA(CLOSE,30))/MA(CLOSE,30)*100)
    VAR3=(CLOSE>OPEN ) * ( OPEN-LOW>HIGH-CLOSE)
    VAR4=(REF(VAR2,1)<-20 ) * ( VAR3)
    VAR5=(REF(VAR2,1)<-20 ) * ( CLOSE>OPEN ) * ( (CLOSE-REF(CLOSE,1))/REF(CLOSE,1)*1000>20)
    VAR6=((VAR4) ) + ( (VAR5
))
    VAR7=(OPEN>CLOSE ) * ( HIGH-np.where(CLOSE>OPEN,CLOSE,OPEN)>np.where(OPEN<CLOSE,OPEN,CLOSE)-LOW)
    VAR8=(REF(VAR2,1)>np.abs(-20) ) * ( VAR7)
    VAR9=(REF(VAR2,1)>np.abs(-20) ) * ( CLOSE<OPEN ) * ( (REF(CLOSE,1)-CLOSE)/REF(CLOSE,1)*1000>20)
    VARA=((VAR8) ) + ( (VAR9
))
    VARB=(CLOSE>OPEN ) * ( OPEN-LOW>HIGH-CLOSE)
    VARC=(REF(VAR2,1)<-13 ) * ( VAR3)
    VARD=(REF(VAR2,1)<-13 ) * ( CLOSE>OPEN ) * ( (CLOSE-REF(CLOSE,1))/REF(CLOSE,1)*1000>20)
    VARE=((VARC) ) + ( (VARD
))
    a=(VARE*VAR1>0)
#结束
