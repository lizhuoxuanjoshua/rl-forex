...
#开始
    ...
    M90=(MA(C,90))
    OOO=((REF(L,1)+O*2)/3)
    M4=(np.minimum(OOO,M4M))
    BB0=(np.minimum(M2,M4))
    B0=(np.minimum(M5,BB0))
    B1=(np.minimum(M7,M5))
    B2=(np.minimum(M10,B1))
    B3=(np.minimum(M13,B2))
    B4=(np.minimum(M20,B3))
    B5=(np.minimum(M25,B4))
    B6=(np.minimum(M30,B5))
    a=(CROSS(C,B1) ) * ( B6>REF(B6,1))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
