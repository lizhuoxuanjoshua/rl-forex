...
#开始
    ...
    S1=(hhv(MA(HIGH,2),2))
    S2=(llv(MA(HIGH,2),1))
    SPPP=((S1-S2)/REF(C,1)*100)
    SPTEM1=((np.minimum(O,C)-L)/(H-L)*100)
    SPTEM2=((H-np.maximum(O,C))/(H-L)*100)
    SPTEM3=((C-O)/(H-L)*100)
    SKJ1=(((SPTEM3>49 ) * ( SPTEM1>0.3*SPTEM2)) ) + ( (SPTEM3>80
))
    SKJ2=((SPTEM3<=49 ) * ( SPTEM3>20 ) * ( SPTEM1>SPTEM2))
    SKJ3=((SPTEM3<=20 ) * ( SPTEM3>-20 ) * ( SPTEM1>1.2*SPTEM2))
    SKJ4=((SPTEM3<-20 ) * ( SPTEM3>-40 ) * ( SPTEM1>2*SPTEM2))
    SKJ=(((SKJ1) ) + ( (SKJ2) ) + ( (SKJ3) ) + ( (SKJ4)) ) * ( (SPPP>2 ) + ( (REF(SPPP,1)>2 )))
    a=(count(SPPP>0,5)==5 ) *((SKJ1)  + ( (SKJ2) ) + ( (SKJ3) ) + ( (SKJ4)))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
