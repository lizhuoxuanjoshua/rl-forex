...
#开始
    ...
    X_9=((HIGH+LOW+OPEN+2*CLOSE)/5)
    X_13=(CLOSE/REF(CLOSE,1)>1.095 ) * ( CLOSE==HIGH)
    X_14=(REF(count(X_13,3)==0,1))
    X_16=(MA(CLOSE,5)>MA(CLOSE,10) ) * ( MA(CLOSE,10)>MA(CLOSE,20) ) * ( MA(CLOSE,20)>MA(CLOSE,60) ) * ( MA(CLOSE,60)>MA(CLOSE,250))
    X_17=(X_16 ) * ( LOW<MA(CLOSE,10) ) * ( X_14 ) * ( LOW<MA(CLOSE,10))
    a=(FILTER(X_17,15))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
