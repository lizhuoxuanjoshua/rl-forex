...
#开始
    ...
    B1=(AA*1.02)
    B2=(AA*0.98)
    CC=(np.abs((2*C+H+L)/4-MA(C,20))/MA(C,20))
    DD=(DMA(C,CC))
    B3=(1.07*DD)
    BB=((B3-REF(B3,5))/REF(B3,5)*1000)
    BB2=((B2-REF(B2,5))/REF(B2,5)*100)
    上穿=(B1>B3)
    连升=(count(BB>=REF(BB,1),2)>=2)
    量能=(MA(V,10)>=MA(V,55))
    阳线=(count(C>O,7)>=4)
    GZ=(上穿  ) * ( 连升 ) * ( 量能 ) * ( 阳线)
    关注=(np.where(GZ,150,0))
    三天关注=(count(关注==150,3)>=1)
    通道二三=(B1>B3 ) * ( B3>=REF(B3,1))
    最大涨幅=(hhv(C,20)/llv(C,20)<1.5)
    多头排列=(MA(C,5)>MA(C,10) ) * ( MA(C,10)>MA(C,20) ) * ( MA(C,20)>=REF(MA(C,20),1) ) * ( MA(C,55)>=REF(MA(C,55),1))
    JR5=(L<=B2*1.006)
    昨日股价=((REF(MA(C,5),1)>=REF(MA(C,5),2)) ) + ( (REF(C,1)>=REF(MA(C,5),1)
))
    JR7=(REF(B1,1)>REF(B3,1))
    JR=(三天关注 ) * ( 通道二三 ) * ( 最大涨幅 ) * ( 多头排列 ) * ( JR5 ) * ( 昨日股价 ) * ( JR7)
    介入Z=np.where(JR,100,0)
    介入=(FILTER(介入Z,2))
    JR6=((C>O ) * ( C>B3 ) * ( C==hhv(C,5) ) * ( V==hhv(V,5)))
    完美介入Z=(JR ) * ( JR6)
    完美介入=FILTER(完美介入Z,2)
    a=(介入 ) + ( 完美介入)
#结束
