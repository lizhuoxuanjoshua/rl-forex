...
#开始
    ...
    XA_1=((CLOSE-llv(LOW,9))/(hhv(HIGH,9)-llv(LOW,9))*100)
    XA_2=(sma(XA_1,3,1))
    XA_3=(sma(XA_2,3,1))
    XA_4=(3*XA_2-2*XA_3)
    XA_5=(REF(CLOSE,barslast(REF(CROSS(XA_2,XA_3),1))+1)>CLOSE ) * ( REF(XA_2,barslast(REF(CROSS(XA_2,XA_3),1))+1)<XA_2 ) * ( CROSS(XA_2,XA_3))
    XA_6=(EMA(CLOSE,12)-EMA(CLOSE,26))
    XA_7=(EMA(XA_6,9))
    XA_8=((XA_6-XA_7)*2)
    XA_9=(XA_8>XA_6 ) * ( XA_8>XA_7)
    XA_10=(XA_8>0)
    XA_11=(1)
    a=(XA_5 ) * ( XA_9 ) * ( XA_10 ) * ( XA_11)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
