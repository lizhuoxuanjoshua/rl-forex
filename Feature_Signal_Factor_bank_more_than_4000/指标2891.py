...
#开始
    ...
    MA4=(MA(CLOSE,90))
    MA5=(MA(CLOSE,288))
    MA7=(sma(CLOSE,2,1)/sma(CLOSE,13,2))
    条件1=(MA7>1.05 ) * ( MA7<1.1)
    MA8=(sma(CLOSE,5,1)/sma(CLOSE,15,1))
    条件2=(MA8>0.45 ) * ( MA8<1.09)
    AA=(np.where(条件1,sma(HIGH,21,5)*1.05,sma(HIGH,21,2)*1.05))

    AB=(np.where(条件2,sma(LOW,10,1),sma(LOW,5,1)))
    条件3=(MA(VOL,2)>MA(VOL,3)/2 ) * ( MA(VOL,3)/2>MA(VOL,5))
    AC=(np.where(条件3, sma(HIGH,2,1),sma(CLOSE,3,1)))
    a=((CROSS(AC,AA)) ) + ( (CROSS(MA4,AB)) ) + ( (CROSS(AA,MA5)
))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
