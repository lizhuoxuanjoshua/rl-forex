...
#开始
    ...
    VAR5=(VAR4*(1+VAR3))
    VAR14=((L<hhv(H,60)*0.6) ) + ( (L<hhv(H,50)*0.65) ) + ( (L<hhv(H,40)*0.7) ) + ( (L<hhv(H,30)*0.75) ) + ( (L<hhv(H,20)*0.8) ) + ( (L<hhv(H,10)*0.85 )) * ( (H-L)/REF(C,1)>0.08 ) * ( V>MA(V,135) ) * ( VAR2<40 ) * ( VAR5>REF(VAR5,1))
    VAR15=((H<MA(C,20)) ) + ( (CROSS(MA(C,5),MA(C,10))
))

    VAR18=(CROSS((sma(np.maximum(CLOSE-(REF(CLOSE,1)),0),6,1)/sma(np.abs(CLOSE-(REF(CLOSE,1))),6,1)*100),20))
    a=(VAR14 ) * (( VAR15) ) + ( (VAR18))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
