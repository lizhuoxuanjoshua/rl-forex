...
#开始
    ...
    VAR3=((-100)*(hhv(H,34)-C)/(hhv(H,34)-llv(L,34))+100)
    VAR4=((-100)*(hhv(H,50)-C)/(hhv(H,50)-llv(L,50))+100)
    VAR5=((-100)*(hhv(H,70)-C)/(hhv(H,70)-llv(L,70))+100)
    VAR6=(((C-MA(C,34))/MA(C,34)*100+10)*5)
    VAR7=(((C-MA(C,50))/MA(C,50)*100+10)*5)
    VAR8=(((C-MA(C,70))/MA(C,70)*100+10)*5)
    VAR9=((VAR3+VAR4+VAR5+VAR6+VAR7+VAR8)/6)
    VARA=(EMA(VAR9,3))
    VARB=(np.abs(VAR6-VAR8))
    VARC=((VAR9+VARB)/2)
    a=(VARC<0 ) * ( C==llv(C,5) ) * ( C/REF(C,1)>0.905 ) * ( C/REF(C,1)<1.05)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
