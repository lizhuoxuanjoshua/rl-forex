...
#开始
    ...
    VAR2=(REF(VAR1,1))
    VAR3=(sma(np.maximum(VAR1-VAR2,0),10,1)/sma(np.abs(VAR1-VAR2),10,1)*100)
    KDB=(np.where(count(VAR3<=14,2),30,0))
    ...
    VAR20=(VAR17<=25 ) * ( VAR18<-95 ) * ( VAR1F>20 ) * ( VAR1B<-30 ) * ( VAR14<25 ) * ( VAR15<30 ) * ( VAR16>50)
    FTD=(VAR20)
    ITO=(np.where(VARB<90,VARB,100))
    T6=(FTD>50 ) * ( ITO<90)
    FUTI=(T6 ) * ( count(T6,30)==1)
    QFD=(CROSS(VAR3,KDB) ) * ( VOL>1.5*MA(VOL,5))
    a=((FUTI ) + ( QFD))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
