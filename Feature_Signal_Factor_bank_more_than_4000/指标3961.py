...
#开始
    ...
    涨停=((C-REF(C,1))/REF(C,1)*100>9.7)
    涨停过=((H-REF(C,1))/REF(C,1)*100>8)
    第一次涨停过=(涨停过)
    首涨三一位=(np.where(第一次涨停过,REF(C,1)+(H-L)*1/3,0))
    三一位判定=(REF(首涨三一位,barslast(首涨三一位)))
    没有破首次涨停=(REF(count(L<三一位判定,REF(barslast(第一次涨停过)+1,1)),1)==0)
    二板=(涨停 ) * (( REF(涨停,1)) ) + ( ((涨停 ) * ( REF(涨停过,1))) ) + ( ( (涨停过 ) * ( REF(涨停,1)) 
))
    a=(涨停 ) * ( (( count(二板,13)>0) ) + ( ((涨停 ) * ( REF(count(涨停过,8)>0,1) ) )) * ( 没有破首次涨停 ))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
