...
#开始
    ...
    牛线=(((20*MID+19*REF(MID,1)+18*REF(MID,2)+17*REF(MID,3)+16*REF(MID,4)+15*REF(MID,5)+14*REF(MID,6)+13*REF(MID,7)+12*REF(MID,8)+11*REF(MID,9)+10*REF(MID,10)+9*REF(MID,11)+8*REF(MID,12)+7*REF(MID,13)+6*REF(MID,14)+5*REF(MID,15)+4*REF(MID,16)+3*REF(MID,17)+2*REF(MID,18)+REF(MID,20))/210#CO)L) + (R(ED}
))
    A=(np.maximum(MA(C,60),MA(C,120)))
    B=(np.minimum(MA(C,60),MA(C,120)))
    E=(np.maximum(MA(C,45),MA(C,90)))
    F=(np.minimum(MA(C,45),MA(C,90)))
    J=(np.maximum(MA(C,30),MA(C,60)))
    K=(np.minimum(MA(C,30),MA(C,60)))
    AA=(C>J ) * ( C>E ) * ( C>=A ) * ( C>牛线 ) * ( (M3>=M13 ) * ( M34>=REF(M34,1)))
    a=(AA ) * ( REF(AA,1)==0)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
