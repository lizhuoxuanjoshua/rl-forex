...
#开始
    ...
    VAR1=(((llv(LOW,5) == llv(LOW,45)) ) * ( (CLOSE >= (OPEN * 1.04))))
    VAR2=((CLOSE > ((REF(HIGH,1) + REF(LOW,1)) / 2)))
    VAR3=(REF(CLOSE,1))
    VAR4=(((sma(np.maximum((CLOSE - VAR3),0),6,1) / sma(np.abs((CLOSE - VAR3)),6,1)) * 100))
    VAR5=(np.maximum(VAR4,5))
    VAR6=(np.maximum(VAR5,10))
    VAR7=(np.maximum(VAR6,20))
    VAR8=(np.maximum(VAR7,30))
    a=(VAR7<VAR8 ) * ( VAR4<30 ) * ( VAR4>20)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
