...
#开始
    ...
    J0=((HIGH+LOW)/2)
    QJ=(EMA(J0,3))
    J1=(np.where(CLOSE>QJ,VOL,0))
    J2=(np.where(CLOSE<QJ,VOL,0))
    BB=(MA(count(J1-J2,22),3))
    AA=(21*BB+20*REF(BB,1)+19*REF(BB,2)+18*REF(BB,3)+17*REF(BB,4)+16*REF(BB,5)+15*REF(BB,6)+14*REF(BB,7)+13*REF(BB,8)+12*REF(BB,9)+11*REF(BB,10)+10*REF(BB,11)+9*REF(BB,12)+8*REF(BB,13)+7*REF(BB,14)+6*REF(BB,15)+5*REF(BB,16)+4*REF(BB,17)+3*REF(BB,18)+2*REF(BB,19)+REF(BB,20))
    EMA1=(AA/(21+1)/21*2)
    CC=(EMA1)
    VAR3=((BB-CC))
    VAR4=(sma(RV,3,1))
    ...
    KK=(EMA(sma(RG,3,1),2))
    DD=(sma(KK,3,1))
    a=(CROSS(W,-2) ) * ( CROSS(VAR4,VAR5) ) * ( CROSS(KK,DD))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
