...
#开始
    ...
    X_7=(X_5>X_6)
    X_8=(REF(CLOSE,1))
    X_9=(sma(np.maximum(CLOSE-X_8,0),9,1)/sma(np.abs(CLOSE-X_8),9,1)*100)
    X_10=(sma(np.maximum(CLOSE-X_8,0),26,1)/sma(np.abs(CLOSE-X_8),26,1)*100)
    X_11=(X_9>X_10)
    X_12=(sma(X_4,3,1))
    X_13=(sma(X_12,3,1))
    X_14=(X_12>X_13)
    ...
    X_18=(100*EMA(EMA(X_17,9),9)/EMA(EMA(np.abs(X_17),9),3))
    X_19=(100*EMA(EMA(X_17,26),12)/EMA(EMA(np.abs(X_17),26),12))
    X_20=(X_18>X_19)
    X_21=(EMA((HIGH+LOW+CLOSE*2)/4,1))
    X_22=(REF(EMA(X_21,3),1))
    X_23=(VOL/((HIGH-LOW)*2-np.abs(CLOSE-OPEN)))
    X_24=(np.where(CLOSE>OPEN,X_23*(HIGH-LOW),np.where(CLOSE<OPEN,X_23*(HIGH-OPEN+CLOSE-LOW),VOL/2))+np.where(CLOSE>OPEN,0-X_23*(HIGH-CLOSE+OPEN-LOW),np.where(CLOSE<OPEN,0-X_23*(HIGH-LOW),0-VOL/2)))
    X_25=(X_24/20/1.15)
    ...
    X_38=(EMA(X_37,9/2))
    X_41=(X_3 ) * ( X_7 ) * ( X_11 ) * ( X_14 ) * ( X_16 ) * ( X_20)
    ...
    X_57=(EMA(X_56,2))
    X_58=((CLOSE-MA(CLOSE,13))/MA(CLOSE,13)*20)
    X_59=(sma(X_58,3,1))
    X_60=((CLOSE-MA(CLOSE,4))/MA(CLOSE,4)*20)
    X_61=(sma(X_60,3,1))
    X_62=(X_35 ) * ( X_41 ) * ( REF(X_56-X_57>0 ) * ( REF(X_56-X_57<0,1)) ) * ( (X_56-X_57>0 ) * ( REF(X_56-X_57>0)) ) * ( (X_59>REF(X_59,1) ) * ( X_59>0) ) * ( (X_61<REF(X_61,1) ) * ( X_61>0)==0)
    a=(X_62 ) * ( REF(X_62,1)==0)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
