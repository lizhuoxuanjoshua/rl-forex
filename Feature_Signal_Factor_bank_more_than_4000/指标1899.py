...
#开始
    ...
    a=(REF(CLOSE,2)>REF(OPEN,2) ) * ( (REF(CLOSE,2)-REF(OPEN,2))/REF(OPEN,2)>=2/100 ) * ( REF(OPEN,1)>=REF(CLOSE,1)) * ( (REF(OPEN,1)-REF(CLOSE,1))/REF(CLOSE,1)<=2/100 ) * ( REF(OPEN,1)<=REF(CLOSE,2) ) * ( REF(CLOSE,1)>=REF(OPEN,2) ) * ( C>=OPEN)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
