...
#开始
    ...
    LXSL=(((VOL<REF(VOL,1) ) * ( REF(VOL,1)<REF(VOL,2))) ) + ( (VOL<REF(VOL,1)/1.8
))
    SLSJ=(barslast(LXSL))
    GJZX=((O+C+H+L)/4)
    LXXD=(((C<REF(C,1) ) * ( REF(C,1)<REF(C,2))) ) + ( ((GJZX<REF(GJZX,1) ) * ( REF(GJZX,1)<REF(GJZX,2))) ) + ( ((H<REF(H,1) ) * ( REF(H,1)<REF(H,2))) ) + ( ((L<REF(L,1) ) * ( REF(L,1)<REF(L,2) ) * ( GJZX<REF(GJZX,1))
))
    XDSJ=(barslast(LXXD))
    QRSP=((REF(CLOSE,1)<REF(CLOSE,2)) ) + ( (REF(CLOSE,1)/REF(CLOSE,2)<=1.035
))
    LXSY=(((H/REF(CLOSE,1)>=1.05 ) * ( REF(CLOSE,1)>REF(CLOSE,2)*1.02 ) * ( REF(CLOSE,2)>=REF(CLOSE,3)*1.02)) ) + ( (H/REF(CLOSE,3)>=1.15
))
    FLSZ=(VOL>REF(VOL,1)*1.5 ) * ( VOL>MA(VOL,5) ) * ( CLOSE/REF(CLOSE,1)>=1.02)
    FAA=((FLSZ ) * ( SLSJ<=3 ) * ( XDSJ<=3)*0.5)
    a=(FAA>0)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
