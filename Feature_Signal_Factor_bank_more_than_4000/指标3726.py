...
#开始
    ...
    GH1=(np.abs(((3.48*CLOSE+HIGH+LOW)/4-EMA(CLOSE,20))/EMA(CLOSE,20)))
    ...
    GH5=((MA(C,8)))
    GH6=((MA(C,13)))
    GH7=((((((MA(C,3)) > (REF(MA(C,3),1))) ) * ( ((MA(C,5)) > (REF(MA(C,5),1)))) ) * ( (C > (MA(C,3)))) ) * ( ((MA(C,3)) > (MA(C,5)))))
    GH8=((((np.arctan(((GH3 / (REF(GH3,1))) - 1) * 100)) * 180) / (3.1416)))
    GH9=((((np.arctan(((GH4 / (REF(GH4,1))) - 1) * 100)) * 180) / (3.1416)))
    GH10=((((np.arctan(((GH5 / (REF(GH5,1))) - 1) * 100)) * 180) / (3.1416)))
    GH11=((((np.arctan(((GH6 / (REF(GH6,1))) - 1) * 100)) * 180) / (3.1416)))
    GH12=((GH7  ) * (   GH8>45 ) * ( GH9>45 ) * ( GH10>60 ) * ( GH11>45 ) * ( MA(C,250)>REF(MA(C,250),1)))
    a=(GH12 ) * ( count(GH12,2)==1 ) * (  C/GHH>1.03 ) * (  C/GHH<1.20  ) * ( C/REF(C,1)>1.097 )
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
