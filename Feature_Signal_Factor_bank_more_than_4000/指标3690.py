...
#开始
    ...
    VAR2=((MA(C,10)))
    VAR3=((MA(C,20)))
    VAR4=((((((MA(C,3)) > (REF(MA(C,3),1))) ) * ( ((MA(C,5)) > (REF(MA(C,5),1)))) ) * ( (C > (MA(C,3)))) ) * ( ((MA(C,3)) > (MA(C,5)))))
    VAR5=((((np.arctan(((VAR0 / (REF(VAR0,1))) - 1) * 100)) * 180) / (3.1416)))
    VAR6=((((np.arctan(((VAR1 / (REF(VAR1,1))) - 1) * 100)) * 180) / (3.1416)))
    VAR7=((((np.arctan(((VAR2 / (REF(VAR2,1))) - 1) * 100)) * 180) / (3.1416)))
    VAR8=((((np.arctan(((VAR3 / (REF(VAR3,1))) - 1) * 100)) * 180) / (3.1416)))
    a=(VAR5>45 ) * ( VAR6>45 ) * ( VAR7>60 ) * ( VAR8>45 ) * ( MA(C,250)>REF(MA(C,250),1))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
