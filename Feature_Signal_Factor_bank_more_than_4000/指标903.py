...
#开始
    ...
    牛线=(((20*MID1+19*REF(MID1,1)+18*REF(MID1,2)+17*REF(MID1,3)+16*REF(MID1,4)+15*REF(MID1,5)+14*REF(MID1,6)+13*REF(MID1,7)+12*REF(MID1,8)+11*REF(MID1,9)+10*REF(MID1,10)+9*REF(MID1,11)+8*REF(MID1,12)+7*REF(MID1,13)+6*REF(MID1,14)+5*REF(MID1,15)+4*REF(MID1,16)+3*REF(MID1,17)+2*REF(MID1,18)+REF(MID1,20))/210#CO)L) + (R(ED}
))
    马线=((MA(牛线,3)#CO)L) + (Y(ELLOW}
))
    GB=((C-牛线))
    换手=HSL
    主=(MA(换手,5))
    ...
    GBB=((主-散))
    GB1=(GB+GBB)
    AA=(np.maximum(MA(C,60),MA(C,120)))
    BB=(np.minimum(MA(C,60),MA(C,120)))
    成交量=(V)
    Q=(EMA(V,5))
    Q1=(EMA(V,50))
    OBV=(count(np.where(CLOSE>REF(CLOSE,1),VOL,np.where(CLOSE<REF(CLOSE,1),-VOL,0)),0))
    OBV1=(EMA(OBV,5))
    OBV2=(EMA(OBV,50))
    W=((Q-Q1)*0.00001)
    ...
    W2=(W+W1)
    #R=(0})
    E=(np.maximum(MA(C,45),MA(C,90)))
    FF=(np.minimum(MA(C,45),MA(C,90)))



    VA=(np.where(CLOSE>REF(CLOSE,1),VOL,-VOL))
    OBV11=(count(np.where(CLOSE==REF(CLOSE,1),0,VA),0))
    MAOBV=(MA(OBV11,30))
    MTR=(EMA(EMA(EMA(CLOSE,12),12),12))
    TRIX=((MTR-REF(MTR,1))/REF(MTR,1)*100)
    MATRIX=(MA(TRIX,9))
    MIDA=(CLOSE-np.minimum(REF(CLOSE,1),LOW))
    MIDB=(np.where(CLOSE<REF(CLOSE,1),CLOSE-np.maximum(REF(CLOSE,1),HIGH),0))
    WAD=(count(np.where(CLOSE>REF(CLOSE,1),MIDA,MIDB),0))
    MAWAD=(MA(WAD,30))


    J=(np.maximum(MA(C,30),MA(C,60)))
    K=(np.minimum(MA(C,30),MA(C,60)))
    ...
    ZL=(CROSS(XEV,0))
    a=(SH ) * ( BQIUSV>=0 )  * (( XEV>=0) ) + ( (YZ )) * ( BQIUV>=0 )  * ( XEV>=0 ) * ( BQIUSV>=0 ) * ( BQIUV>=0 ) * ( XEV>=0 ) + ( (ZL )) * ( BQIUSV>=0 ) * ( BQIUV>=0)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
