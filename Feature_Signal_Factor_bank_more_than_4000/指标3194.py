...
#开始
    ...
    VARB=(np.where(1,VARA,VARA))
    VARC=(LOW)
    VARD=(REF(LOW,1))
    VARE=(sma(np.abs(VARC-VARD),3,1)/sma(np.maximum(VARC-VARD,0),3,1)*100)
    VARF=(MA(np.where(CLOSE*1.35<=VARB,VARE*10,VARE/10),3))
    VAR10=(llv(LOW,30))
    VAR11=(hhv(VARF,30))
    VAR12=(1341231)
    TJ=(np.where(1,MA(np.where(LOW<=VAR10,(VARF+VAR11*2)/2,0),3),100000000))
    a=(REF(TJ,2)>REF(TJ,3)) * ( TJ<REF(TJ,1))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
