...
#开始
    ...
    上通道=(MA(L+(H-L)/2,25)*1.152)
    下通道=(MA(L+(H-L)/2,25)*0.952)
    中轨=((上通道+下通道)/2)
    次上通道=((MA(C,5)+MA(O,5))/2*1.062)
    次下通道=((MA(C,5)+MA(O,5))/2*0.982)
    强弱线=((次上通道+次下通道)/2)
    countC=(count(C,4))
    countO=(count(O,4))
    明天C=(C*(1+3/100))
    明天O=(C*(1+3/100))
    count上=(count(L+(H-L)/2,24))
    count下=(count(L+(H-L)/2,24))
    明天值=(C*(1+3/100))
    次上虚拟=(((countC+明天C)/5+(countO+明天O)/5)/2*1.06)
    次下虚拟=(((countC+明天C)/5+(countO+明天O)/5)/2*0.98)
    强弱虚拟=((次上虚拟+次下虚拟)/2)
    上通道虚拟=((count上+明天值)/25*1.15)
    下通道虚拟=((count上+明天值)/25*0.95)
    中轨虚拟=((上通道虚拟+下通道虚拟)/2)
    虚拟金叉=(强弱虚拟>中轨虚拟 ) * ( 强弱线<中轨)
    实体金叉=(CROSS(强弱线,中轨))
    虚拟强势金叉=(强弱虚拟>上通道虚拟 ) * ( 强弱线<上通道虚拟)
    实体强势金叉=(CROSS(强弱线,上通道))
    极端强势情况=((((收盘 ) * ( count(实体强势金叉,2)>0)) ) + ( ((收盘 ) * ( 虚拟强势金叉))) ) * ( count(强弱线>上通道,34)>=17)
    普通情况=(((收盘 ) * ( count(实体金叉,2)>0)) ) + ( ((收盘 ) * ( 虚拟金叉)
))
    通道线大破=((次下通道-下通道)/下通道*100<-1)
    通道线无瑕疵=(count(通道线大破,21)==0)
    K线瑕疵=((np.minimum(C,O)-下通道)/下通道*100<-1)
    不严重瑕疵=((np.minimum(C,O)-下通道)/下通道*100<-1 ) * ( C>O ) * ( C-(C-O)*0.67>下通道)
    K线无瑕疵=(((count(K线瑕疵,21)==0)) ) + ( ((count(K线瑕疵,21)==1 ) * ( count(不严重瑕疵,21)==1)
))
    无瑕疵=(通道线无瑕疵 ) * ( K线无瑕疵)
    强势通道线大破=((次下通道-中轨)/中轨*100<-1)
    强势通道无瑕疵=(count(强势通道线大破,21)==0)
    虚拟=((普通情况) ) + ( ((极端强势情况 ) * ( 强势通道无瑕疵)
))
    XG=(虚拟 ) * ( 无瑕疵)
    a=((((实体金叉 ) * ( 无瑕疵)) ) + ( ((实体强势金叉 ) * ( 强势通道无瑕疵))))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
