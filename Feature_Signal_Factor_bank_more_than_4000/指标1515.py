...
#开始
    ...
    MA20=(MA(C,20))
    MA30=(MA(C,30))
    BB05=(np.arctan((MA05/REF(MA05,1)-1)*100)*180/3.1416)
    速度5=(sma(EMA((MA05-REF(MA05,1))/REF(MA05,1),3)*100,3,1))
    加速度5=(EMA((速度5-REF(速度5,1)),3))
    BB10=(np.arctan((MA10/REF(MA10,1)-1)*100)*180/3.1416)
    三拾日乖离率=((C-MA30)/MA30*100)
    a=((count(CROSS(BB05,30),5)>=1 ) * ( MA05>REF(MA05,1) ) * ( 三拾日乖离率>REF(三拾日乖离率,1) ) * ( MA10>REF(MA10,1) ) * ( 加速度5>REF(加速度5,1) ) * ( 速度5>REF(速度5,1)))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
