...
#开始
    ...
    MA3=(EMA(CLOSE,N31))
    MA4=(EMA(CLOSE,N41))
    TJ1A=(CROSS(MA1,MA2) ) * ( C>O ) * ( C>REF(C,1) ) * (( C/REF(C,1)>=1.0)) + ( (CROSS(MA1,MA4) )) * ( C>O ) * ( C>REF(C,1) ) * ( C/REF(C,1)>=1.01) + ( (CROSS(MA2,MA3) )) * ( C>O ) * ( C>REF(C,1) ) * ( C/REF(C,1)>=1.018) + ( (CROSS(MA1,MA3) )) * ( C>O ) * ( C>REF(C,1) ) * ( C/REF(C,1)>=1.018)
    TJ2A=((np.maximum(MA1,np.maximum(MA2,MA3))-np.minimum(MA1,np.minimum(MA2,MA3)))/np.minimum(MA1,np.minimum(MA2,MA3))*100<=P1)
    a=(TJ1A ) * ( TJ2A)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
