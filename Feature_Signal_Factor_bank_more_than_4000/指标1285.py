...
#开始
    ...
    VAR14=(np.power(MA(CLOSE, 5), 2)+MA(CLOSE, 5))
    VAR15=(np.power(MA(LOW, 5), 2)+MA(LOW, 5))
    VAR16=(np.power(MA(HIGH, 5), 2)+MA(HIGH, 5))
    ...
    VAR30=(VAR6+VAR11+VAR18+VAR27)
    VAR31=(VAR7+VAR12+VAR19+VAR28)
    VAR32=(VAR8+VAR13+VAR20+VAR29)
    VAR33=((HIGH+LOW+CLOSE)/3)
    VAR34=(AMOUNT/VOL/100)
    VAR35=(count(AMOUNT, 5)/count(VOL, 5)/100)
    ...
    VAR47=((EMA(EMA(EMA(VAR46, 15), 10), 5) - 25)* 2+DMA(AMOUNT/VOL, 15))
    VAR48=((VAR30+VAR31+VAR32*30)/32/5-15)
    VAR49=(EMA(VAR48, 13))
    VAR50=((VAR46+VAR48)/2/1.100)
    VAR51=((VAR47+VAR49)/2)
    VAR52=(EMA(VAR51, 1))
    AA=(EMA(VAR52, 2))
    a=(CROSS(AA, VAR50) ) * ( AA>60)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
