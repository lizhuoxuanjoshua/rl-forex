...
#开始
    ...
    RF1=(EMA(CLOSE,3))
    RF2=(((sma(np.maximum((CLOSE - RF1),0),9,1) / sma(np.abs((CLOSE - RF1)),9,1)) * 100))
    ...
    K0=((RF2< 14) ) * ( L<REF(L,1) ) * ( V1<8.8 ) * ( RF7<-65)
    K1=(K0>1 ) * ( C>O)
    K2=(K0>1 ) * ( C/L>1.045)
    K3=((REF(K0,1)>0 ) * ( K0==0))
    准备1=(K0>3 ) * ( RG5==0)
    K4=(np.maximum(np.maximum(K1,K2),K3) )
    进场1=(K4>0 )
    准备2=(K0>3 )
    K5=(np.maximum(np.maximum(K1,K2),K3) ) * ( REF(准备2,1)>0)
    进场2=(K5>1 )
    a=((进场1) ) + ( (进场2))
#结束
    买入 = a
