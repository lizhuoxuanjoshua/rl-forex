...
#开始
    ...
    VARA=(np.where(VA9>100,VA9-100,0))
    VARB=(hhv(HIGH,34))
    ...
    VAR10=(np.where((VAR9>100),VAR9-100,0))
    VAR11=(hhv(HIGH,34))
    ...
    安全=(20)
    关注=(np.where(CROSS(A5,安全),50,0))
    收钱=((CROSS(VARC,牛市) ) * ( VARC>VARD ) * ( VARC<55 ) * ( C>O)*50)
    A6=(llv(C,500)==llv(C,5) ) * ( C/REF(C,1)>(1+9.8/100))
    SEU=(np.where(A6,牛市,0))
    a=(((收钱>0 ) * ( 收钱<400)) ) + ( ((SEU>0 ) * ( SEU<400)) ) + ( ((关注>0 ) * ( 关注<400)) )
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
