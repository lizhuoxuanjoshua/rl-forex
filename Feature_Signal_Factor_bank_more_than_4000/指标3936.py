...
#开始
    ...
    去=((H-np.maximum(C,O)) *1.5<(np.maximum(C,O)-np.minimum(C,O)))
    一字=(C/REF(C,1)>1.0975 ) * ( H==O)
    GB=(去)
    ...
    VAR4AA=(((CLOSE)/(REF(CLOSE,1))>1.045) ) * ( ((HIGH)/(CLOSE)<1.01) ) * ( (VAR3AA>0))
    博弈=(FILTER((VAR4AA>0),45))
    a=(GB ) * ( XG )
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
