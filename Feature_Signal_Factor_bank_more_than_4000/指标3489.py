...
#开始
    ...
    VAR77=(hhv(HIGH,5))
    VAR88=(llv(LOW,5))
    AA=(EMA((CLOSE-VAR88)/(VAR77-VAR88)*7,5))
    VAR99=(EMA(CLOSE,6)-EMA(CLOSE,12))
    VAR100=(5*EMA(VAR99,9))
    VAR110=(1*(VAR99-VAR100))
    VAR120=(np.where(VAR110>0,6*VAR110,0))
    a=(AA<VAR120 ) * ( AA<REF(AA,3) ) * ( AA>REF(AA,1) ) * ( AA<3)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
