...
#开始
    ...
    连阴=(count(((C<REF(C,1)) ) + ( (L<REF(L,1))),10)>=5)
    卧龙柱=(REF(连阴,1) )
    距今=(barslast(卧龙柱))
    再龙根=(count(CROSS(C,REF((C+O)/2,距今)),距今)==2 ) * ( CROSS(C,REF((C+O)/2,距今)))
    连阴2=(count(((C<REF(C,1)) ) + ( (L<REF(L,1))),10)>=5)
    卧龙柱5=(REF(连阴2,1) )
    距今2=(barslast(卧龙柱5))
    卧龙湾=(hhv(C,距今2)<REF((C+O)/2,距今2) ) * ( 距今2<15)
    假阳=(REF(C<=O,1) ) * ( H<REF(O,1) ) * ( C<(REF(O-C,1))/3+REF(C,1) ) * ( AMOUNT/V/100<REF(C,1) ) * ( C>O)
    黄金阳=( 假阳)
    阳距=(barslast(黄金阳))
    黄金坑=(hhv(C,阳距)<REF((C+O)/2,阳距+1) ) * ( 阳距<15)
    卧龙潜=((黄金坑) ) + ( (卧龙湾
))
    SY=(H/np.maximum(C,O))
    XY=(np.minimum(C,O)/L)
    卧龙深潜=(卧龙潜 ) * ( C>O ) * ( XY>1.02 ) * ( SY<1.015)
    牙齿=(REF(MA(CLOSE,8),5))
    R2=(牙齿)
    KU1=(np.where(HIGH==hhv(HIGH,3),1,0))
    UL=((REF(KU1,2)==1 ) * ( REF(KU1,1)==0 ) * ( REF(KU1,1)==0 ) * ( KU1==0))
    上分形=(np.where(HIGH>=R2,UL,REF(UL,barslast(HIGH>=R2))))
    红箭头=(HIGH>上分形 ) * ( REF(HIGH,1)<REF(上分形,1))
    RSV14=((CLOSE - llv(LOW,14))/(hhv(HIGH,14) - llv(LOW,14))-0.5)
    K14=(EMA(RSV14,3))
    D14=(EMA(K14,3))
    J14=(3*K14-2*D14)
    KDJXG=(REF(count(J14<REF(J14,1),5)>=3,1) ) * ( REF(J14,1)<-0.4 ) * ( J14>REF(J14,1))
    海德=(FILTER(KDJXG,3))
    V5=(EMA(CLOSE,1))
    AE=(EMA(V5,5))
    AB=(EMA(hhv(AE,5),1))
    ...
    AF=(EMA(hhv(WJ,17),3))
    三线=(AF)
    HHD=((CROSS(AJ,三线)) ) + ( ((EVERY(REF(三线,1)==REF(AJ,1),2) ) * ( count(AJ>三线,2) ) 
))
    牛起步=(HHD==1)
    AD=(sma((((CLOSE-llv(LOW,33))/(hhv(HIGH,33)-llv(LOW,33)))*100),7,1))
    铁三角=(CROSS(AD,23))
    主线=(18)
    主差=(2)
    起初=(1)
    ...
    YUI=(REF(CLOSE,A1+1)>CLOSE ) * ( J>REF(J,A1+1) ) * ( CROSS(J,J1) ) * ( J<20)
    底背离=(YUI>0)

    信号数=(再龙根+卧龙深潜+红箭头+海德+牛起步+铁三角+底背离)
    XG=(信号数>=3)
