...
#开始
    ...
    XA_1=(MA(sma((LOW+HIGH+2*CLOSE)/4-REF((LOW+HIGH+2*CLOSE)/4,1),5,1)/sma(np.abs((LOW+HIGH+2*CLOSE)/4-REF((LOW+HIGH+2*CLOSE)/4,1)),5,1)*100,1)+50)
    XA_2=(np.where((-XA_1)>95,0,20))
    XA_4=(1)
    XA_5=((CLOSE-EMA(CLOSE,13))/EMA(CLOSE,13)*(0-100))
    XA_6=(np.where(BARScount(CLOSE)==1,CLOSE/10000,(1-CLOSE/sma(CLOSE,13,1))*100))
    XA_7=(EMA(XA_5/10+EMA(CLOSE,10),3))
    XA_8=((-100)*(XA_7-CLOSE)/XA_7)
    XA_9=(EMA(XA_8,5))
    XA_10=(EMA(CLOSE,3))
    XA_11=(EMA(SLOPE(CLOSE,22)*20+CLOSE,55))
    XA_12=(XA_11)
    XA_13=(XA_12*0.985*XA_4)
    XA_14=(MA(CLOSE,5))
    XA_15=(MA(CLOSE,13))
    XA_16=(MA(CLOSE,21)*XA_4)
    XA_17=1
    XA_18=((CLOSE-REF(CLOSE,1))/REF(CLOSE,1)*100*XA_17)
    XA_19=((CLOSE-REF(OPEN,29))/REF(OPEN,29)*100*XA_17)
    XA_20=(REF(CLOSE,1))
    XA_21=(sma(np.maximum(CLOSE-XA_20,0),6,1)/sma(np.abs(CLOSE-XA_20),6,1)*100*XA_17)
    XA_22=(sma(np.maximum(CLOSE-XA_20,0),12,1)/sma(np.abs(CLOSE-XA_20),12,1)*100)
    XA_23=(sma(np.maximum(CLOSE-XA_20,0),24,1)/sma(np.abs(CLOSE-XA_20),24,1)*100*XA_17)
    XA_24=((CLOSE-llv(LOW,45))/(hhv(HIGH,45)-llv(LOW,45))*100)
    XA_25=(sma(XA_24,3,1))
    XA_26=(sma(XA_25,3,1)*XA_4)
    XA_27=(3*XA_25-2*XA_26)
    XA_28=((CROSS(XA_21,10) ) * ( XA_19<0-28*20)*XA_4)
    XA_29=(100-CROSS(76,XA_21) ) * ( XA_19>5*20)
    XA_30=((CROSS(XA_25,XA_26) ) * ( XA_26<10 ) * ( XA_19<0-30 ) * ( XA_18>3)*30)
    XA_31=(-((XA_26<11 ) * ( XA_21<15 ) * ( XA_19<0-26)*10))
    a=(CROSS(XA_31,(0-9)*XA_17))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
