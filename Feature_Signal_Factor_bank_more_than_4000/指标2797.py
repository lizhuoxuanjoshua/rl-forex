...
#开始
    ...
    AA=(100-3*sma((CLOSE-llv(LOW,75))/(hhv(HIGH,75)-llv(LOW,75))*100,20,1)+2*sma(sma((CLOSE-llv(LOW,75))/(hhv(HIGH,75)-llv(LOW,75))*100,20,1),15,1)<REF(100-3*sma((OPEN-llv(LOW,75))/(hhv(HIGH,75)-llv(LOW,75))*100,20,1)+2*sma(sma((OPEN-llv(LOW,75))/(hhv(HIGH,75)-llv(LOW,75))*100,20,1),15,1),1) ) * ( VOL>REF(VOL,1) ) * ( CLOSE>REF(CLOSE,1))
    a=(AA ) * ( count(AA,30)==1)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
