...
#开始
    ...
    X_1=(np.where((CLOSE-REF(CLOSE,1))/REF(CLOSE,1)/((REF(CLOSE,1)-REF(CLOSE,2))/REF(CLOSE,2))>2,1,0))
    X_2=(np.where(CLOSE/OPEN>1.05,1,0))
    X_3=(np.where(VOL/REF(VOL,1)>2,1,0))
    X_4=(np.where(VOL/REF(MA(VOL,5),1)>2,1,0))
    X_5=(np.where(CLOSE/REF(CLOSE,1)>1.05,1,0))
    a=(X_1+X_2+X_3+X_4+X_5)==2
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
