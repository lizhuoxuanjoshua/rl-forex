...
#开始
    ...
    B=(np.arctan((MA(C,5)/REF(MA(C,5),1)-1)*100)*180/3.1416)
    B1=(np.arctan((MA(C,10)/REF(MA(C,10),1)-1)*100)*180/3.1416)
    B2=(np.arctan((MA(C,20)/REF(MA(C,20),1)-1)*100)*180/3.1416)
    AA=((B>45 ) * ( B1>55 ) * ( B2>30))
    B3=(100*(C-llv(L,34))/(hhv(C,34)-llv(L,34)))
    B4=((AMOUNT/C)/(hhv(AMOUNT,34)/hhv(C,34)))
    B5=(np.where(B4>1,1,B4)*100)
    B6=(3*sma((CLOSE-llv(LOW,27))/(hhv(HIGH,27)-llv(LOW,27))*100,5,1)-2*sma(sma((CLOSE-llv(LOW,27))/(hhv(HIGH,27)-llv(LOW,27))*100,5,1),3,1))
    B7=(EMA((CLOSE-llv(LOW,10))/(hhv(HIGH,25)-llv(LOW,10))*4,4)*30)
    B8=(B5>=90 ) * ( B3>80 ) * ( MA(B6,12)<90)
    B9=(barslast(CROSS(B7,MA(B6,12)))<40)
    B10=(C/REF(C,1)>1+0.01*2)
    AA1=(B8 ) * ( B9 ) * ( ((C>O) ) + ( (C/REF(C,1)>1.05)) ) * ( B10)
    BB1=(C/REF(C,1)>1+0.01*9)
    BB2=(count(C/REF(C,1)>1+0.01*9,10))
    BB3=(C/REF(C,1)>1+0.01*9)
    AA2=(np.where(BB1+BB2+BB3==3,1,0))
    BB4=(EMA(C,50)>EMA(C,240))
    BB5=(barslast(CROSS(EMA(C,240),EMA(C,50))))
    BB6=(np.where(BB4>0,BB5,50))
    BB7=(C/llv(C,BB6)*100-100)
    AA3=(hhv(BB7,283))
    AA4=(((AA>0) ) + ( (AA1>0) ) + ( (AA2>0)) ) * ( count(AA>0,2)>=1 ) * ( count(AA1>0,3)>=1   ) * ( count(AA2>0,5)>=1)
    a=(AA4>0 ) * ( count(AA4>0,10)==1  ) * ( AA3<100  ) * ( (H-C)/C*100<1 ) * ( REF(C<O,1))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
