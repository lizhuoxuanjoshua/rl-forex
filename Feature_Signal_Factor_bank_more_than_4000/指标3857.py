...
#开始
    ...
    X_1=(REF(HIGH,2)/REF(CLOSE,3)>=1.004)
    X_2=(REF(HIGH,1)/REF(CLOSE,2)>=1.015)
    X_3=(REF(CLOSE,1)<REF(EMA(CLOSE,12),1))
    X_4=(OPEN<REF(np.maximum(OPEN,CLOSE),1)*1.03)
    X_5=(LOW<REF(LOW,1))
    X_6=(HIGH<REF(HIGH,3)*0.965)
    X_7=(OPEN<REF(np.minimum(OPEN,CLOSE),1))
    X_8=(HIGH/LOW>=1.03)
    X_9=(5)
    X_10=(10)
    X_11=(20)
    ...
    X_15=(MA(VOL,X_11))
    X_16=(MA(VOL,X_12))
    X_17=(np.minimum(REF(VOL,1),REF(VOL,2)))
    X_18=(np.minimum(REF(VOL,3),REF(VOL,4)))
    X_19=(np.minimum(REF(VOL,5),REF(VOL,6)))
    X_20=(np.minimum(REF(VOL,7),REF(VOL,8)))
    X_21=(np.minimum(REF(VOL,9),REF(VOL,10)))
    X_22=(np.minimum(REF(VOL,11),REF(VOL,12)))
    X_23=(np.minimum(REF(VOL,13),REF(VOL,14)))
    X_24=(np.minimum(REF(VOL,14),REF(VOL,14)))
    X_25=(np.minimum(REF(VOL,17),REF(VOL,18)))
    X_26=(np.minimum(REF(VOL,19),REF(VOL,20)))
    X_27=(np.minimum(np.minimum(X_17,X_18),np.minimum(X_19,X_20)))
    X_28=(np.minimum(np.minimum(X_21,X_22),np.minimum(X_23,X_24)))
    X_29=(np.minimum(X_25,X_26))
    X_30=(np.minimum(X_27,X_28))
    X_31=(np.minimum(X_30,X_25))
    X_32=(VOL<=llv(VOL,16)*2.5)
    X_33=(VOL<=X_31*2.5)
    X_34=(VOL>REF(VOL,1))
    a=(X_1 ) * ( X_2 ) * ( X_3 ) * ( X_4 ) * ( X_5 ) * ( X_6 ) * ( X_7 ) * ( X_8 ) * ( X_33 ) * ( X_34)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
