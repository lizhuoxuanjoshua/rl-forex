...
#开始
    ...
    N=8
    ZF=(5)
    放量上涨=(CLOSE/REF(CLOSE,1)>=1.01 ) * ( VOL/REF(VOL,1)>=1.5 ) * ( VOL>MA(VOL,5) ) * ( REF(VOL,1)/REF(MA(VOL,5),1)<=1.8)
    箱体形成=(((hhv(C,3)/llv(C,N)-1)*100<=ZF ) * ( (hhv(C,N)/llv(C,3)-1)*100<=ZF)*0.3)
    接近箱顶=((CLOSE/REF(hhv(CLOSE,N),2)<=1.02 ) * ( HIGH/REF(hhv(CLOSE,N),2)>=0.98)*0.5)
    P1=(barslast(箱体形成))
    P2=(barslast(接近箱顶))
    AA=((放量上涨 ) * ( 接近箱顶 ) * ( P1<=3 ) * ( P2<=1)*2)
    a=(AA>0)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
