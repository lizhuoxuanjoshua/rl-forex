...
#开始
    ...
    V3=(EMA(C,34))
    V4=(EMA(C,55))
    M1=(1000*np.minimum(V1,V4)/np.maximum(V1,V4)>=975)
    M2=(1000*np.minimum(V2,V4)/np.maximum(V2,V4)>=985)
    M3=(1000*np.minimum(V3,V4)/np.maximum(V3,V4)>=990)
    M4=(M1 ) * ( M2 ) * ( M3 ) * ( CROSS(V0,V1))
    M5=(M1 ) * ( M2 ) * ( M3 ) * ( CROSS(V0,V3))
    a=((M4) ) + ( (M5))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
