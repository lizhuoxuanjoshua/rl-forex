...
#开始
    ...
    X_1=VOL/((HIGH-LOW)*2-np.abs(CLOSE-OPEN))

    

    X_2=np.where(CLOSE>OPEN,X_1*(HIGH-LOW),np.where(CLOSE<OPEN,X_1*(HIGH-OPEN+(CLOSE-LOW)),VOL/2))
    ...
    X_9=(CLOSE>X_7) * (CLOSE>=OPEN)

    

    a=(X_2>X_3) * CROSS(CLOSE,X_5) * X_9 
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
