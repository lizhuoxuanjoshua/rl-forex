...
#开始
    ...
    LC=(REF(CLOSE,1))
    RSI1=(sma(np.maximum(CLOSE-LC,0),6,1)/sma(np.abs(CLOSE-LC),6,1)*100)
    A=(REF(RSI1<45,1) ) * ( RSI1>45)
    TYP=((HIGH+LOW+CLOSE)/3)
    CCI=((TYP-MA(TYP,14))/(0.015*AVEDEV(TYP,14)))
    B=(REF(CCI<-100,1) ) * ( CCI>=-100)
    RSI2=(sma(np.maximum(CLOSE-LC,0),12,1)/sma(np.abs(CLOSE-LC),12,1)*100)
    RSI3=(sma(np.maximum(CLOSE-LC,0),18,1)/sma(np.abs(CLOSE-LC),18,1)*100)
    MMS=(MA(3*RSI2-2*sma(np.maximum(CLOSE-LC,0),16,1)/sma(np.abs(CLOSE-LC),16,1)*100,3))
    W=(REF(MMS<39,1) ) * ( MMS>=39)
