...
#开始
    ...
    Ak=(DMA((H+L+C*2)/4,0.9))
    Bk=(VOL/((H-L)*2-np.abs(C-O)))
    Ckk=(np.where(C>O,Bk*(H-L),np.where(C<O,Bk*(H-O+(C-L)),VOL/2))+np.where(C>O,0-Bk*(H-C+(O-L)),np.where(C<O,0-Bk*(H-L),0-VOL/2)))
    Akk=(Ckk/20/1.15)
    攻击流量=(Akk*0.618+REF(Akk,1)*0.382+REF(HSL,2)*0.168)
    Bkk=(EMA(攻击流量,3))
    流量=(np.where(Bkk>REF(Bkk,1),1,0))
    AAk=((np.maximum(C,O)-np.minimum(C,O))/np.minimum(C,O)<0.00618)
    BBk=((H-np.maximum(C,O))/np.maximum(C,O)>0.00382)
    DDk=((np.minimum(C,O)-L)/L>0.00382)

    星k星=(AAk ) * ( BBk ) * ( DDk )
    a=(星k星==1 ) * ( 流量==1 ) * ( C>O ) * ( np.abs((C-MA(C,20))/MA(C,20))>0.0382 ) * ( H-np.maximum(C,O)<=np.minimum(C,O)-L)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
