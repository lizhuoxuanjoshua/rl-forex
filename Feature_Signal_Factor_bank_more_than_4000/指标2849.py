...
#开始
    ...
    MA30=(MA(C,30))
    MA60=(MA(C,60))
    D1=((MAN-MA10)/np.minimum(MAN,MA10))
    D2=((MAN-MA20)/np.minimum(MAN,MA20))
    D3=((MAN-MA30)/np.minimum(MAN,MA30))
    D5=((MA20-MA10)/np.minimum(MA20,MA10))
    D6=((MA30-MA10)/np.minimum(MA30,MA10))
    D8=((MA20-MA30)/np.minimum(MA20,MA30))
    DA=(MA(np.maximum(np.maximum(np.maximum(D1,D2),np.maximum(D3,D5)),np.maximum(D6,D8))*100,2))
    DB=(MA(np.minimum(np.minimum(np.minimum(D1,D2),np.minimum(D3,D5)),np.minimum(D6,D8))*100,2))
    GJ=((2*C+H+L)/4)
    底=(DB<-10 ) * ( GJ<MA30 ) * ( REF(DB,1)<np.minimum(REF(DB,2),DB))
    a=(底 ) * ( count(底,barslast((DB>=-10 ) * ( GJ<MA30)))==1)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
