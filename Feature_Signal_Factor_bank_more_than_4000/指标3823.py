...
#开始
    ...
    XA_1=(EMA((CLOSE-MA((2*CLOSE+HIGH+LOW)/4,30))/MA((2*CLOSE+HIGH+LOW)/4,30)*100,3))
    XA_2=(np.where(CROSS(XA_1,0),40,0))
    XA_3=(1)
    XA_4=((2*CLOSE+HIGH+LOW)/4)
    XA_5=(llv(LOW,94)*XA_3)
    XA_6=(hhv(HIGH,89)*XA_3)
    XA_7=(EMA((CLOSE-XA_5)/(XA_6-XA_5)*100,8))
    XA_8=(EMA(0.667*REF(XA_7,1)+0.333*XA_7,10))
    XA_9=(count(XA_2>=40,5)>0)
    XA_10=(1)
    XA_11=((CROSS(XA_7,XA_8)*XA_10 ) * ( XA_9))*60
    XA_12=(XA_2+XA_11>=1)
    XA_13=(MA(CLOSE,5))
    XA_14=(MA(CLOSE,30))
    XA_15=(MA(CLOSE,5))
    XA_16=(MA(CLOSE,13))
    XA_17=(MA(CLOSE,34))
    XA_18=(np.arctan((XA_15/REF(XA_15,1)-1)*100)*180/3.1416)
    XA_19=(np.arctan((XA_16/REF(XA_16,1)-1)*100)*180/3.1416)
    XA_20=(np.arctan((XA_17/REF(XA_17,1)-1)*100)*180/3.1416)
    XA_21=(sma(EMA((XA_15-REF(XA_15,1))/REF(XA_15,1),3)*100,3,1)*XA_10)
    XA_22=(EMA(XA_21-REF(XA_21,1),3)*XA_10)
    XA_23=((CLOSE-XA_13)/XA_13*100)
    XA_24=((CLOSE-XA_17)/XA_17*100)
    XA_38= ( CLOSE/REF(CLOSE,1)>1.05)
    XA_39= ( CLOSE/REF(CLOSE,1)>1.08)
    XA_40=(XA_38+XA_39>=1)
    XA_41=((CLOSE-REF(CLOSE,1))/REF(CLOSE,1)*100>9)
    XA_42=(MA(CLOSE,5))
    XA_43=(REF(CLOSE,1))
    XA_44=(sma(np.maximum(CLOSE-XA_43,0),6,1)/sma(np.abs(CLOSE-XA_43),6,1)*100)
    XA_45=(CROSS(XA_44,25) ) * ( CLOSE>XA_42 ) * ( XA_41)
    XA_46=(EMA(CLOSE,5))
    XA_47=(EMA(CLOSE,10))
    XA_48=(EMA(CLOSE,14)*XA_10)
    XA_49=(EMA(CLOSE,18)*XA_10)
    XA_50=(EMA(CLOSE,40)*XA_10)
    XA_51=(EMA(CLOSE,60)*XA_10)
    XA_52=(np.maximum(np.maximum(np.maximum(np.maximum(np.maximum(XA_46,XA_47),XA_48),XA_49),XA_50),XA_51)*XA_3)
    XA_53=(np.minimum(np.minimum(np.minimum(np.minimum(np.minimum(XA_46,XA_47),XA_48),XA_49),XA_50),XA_51)*XA_3)
    XA_54=(CLOSE>XA_52 ) * ( LOW<XA_53 ) * ( (CLOSE>REF(hhv(HIGH,3),1) ) * ( (CLOSE-REF(CLOSE,1))/REF(CLOSE,1)>0.09)*XA_3)
    XA_55=(FILTER(XA_54>0,0)==1)
    XA_56=((XA_45) ) + ( (XA_55
))
    XA_57=((EMA(CLOSE,4)+MA(CLOSE,8)+MA(CLOSE,16)*XA_10)/3)
    XA_58=((EMA(CLOSE,6)+MA(CLOSE,12)+MA(CLOSE,24)*XA_10)/3)
    XA_59=((EMA(CLOSE,9)+MA(CLOSE,18)+MA(CLOSE,36)*XA_10)/3)
    XA_60=((EMA(CLOSE,13)+MA(CLOSE,26)+MA(CLOSE,52))/3)
    XA_61=((EMA(CLOSE,18)+MA(CLOSE,36)+MA(CLOSE,72)*XA_3)/3)
    XA_62=((EMA(CLOSE,24)+MA(CLOSE,48)+MA(CLOSE,96)*XA_3)/3)
    XA_63=(OPEN<XA_57 ) * ( OPEN<XA_58 ) * ( OPEN<XA_59 ) * ( OPEN<XA_60 ) * ( OPEN<XA_61 ) * ( OPEN<XA_62 ) * ( CLOSE>XA_57 ) * ( CLOSE>XA_58 ) * ( CLOSE>XA_59 ) * ( CLOSE>XA_60 ) * ( CLOSE>XA_61 ) * ( CLOSE>XA_62)
    XA_64=(REF(CLOSE,1))
    XA_65=(VOL/MA(REF(VOL,1),5)*XA_3)

    XA_68=(OPEN/XA_64)
    XA_69=(np.arctan((EMA(CLOSE,7)/REF(EMA(CLOSE,7),1)-1)*100)*XA_10*57.299999)
    XA_70=(np.arctan((EMA(CLOSE,13)/REF(EMA(CLOSE,13),1)-1)*100)*XA_10*57.299999)
    XA_71=(np.arctan((EMA(CLOSE,34)/REF(EMA(CLOSE,34),1)-1)*100)*XA_10*57.299999)
    XA_72=(XA_69>53 ) * ( XA_70>35.560001 ) * ( XA_71>19 )  * ( XA_68<1.05 ) * ( XA_68>1)
    XA_73=(XA_63+XA_72>=1)
    a=(XA_12+XA_40+XA_56+XA_73>=2)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
