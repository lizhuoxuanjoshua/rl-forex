...
#开始
    ...
    短主=(sma(RSV,3,1))
    短空=(sma(短主,3,1))
    RSV12=((((CLOSE - llv(LOW,15)) / (hhv(HIGH,15) - llv(LOW,15))) * 100))
    中主=(sma(RSV12,3,1))
    中空=(sma(中主,3,1))
    RSV24=((((CLOSE - llv(LOW,21)) / (hhv(HIGH,21) - llv(LOW,21))) * 100))
    长主=(sma(RSV24,3,1))
    长空=(sma(长主,3,1))
    a=(((((CROSS(REF(短主,1),REF(短空,1))) ) + ( (CROSS(短主,短空))) ) * ( CROSS(长主,长空)) ) * ( ((CROSS(REF(中主,1),REF(中空,1)) ) + ( (CROSS(中主,中空))) ) * ( CROSS(长主,长空))))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
