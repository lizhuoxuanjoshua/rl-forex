...
#开始
    ...
    DEA=( EMA(DIFF,9))
    MACD=( 2*(DIFF-DEA))
    TS=( barslast(C/REF(C,1)>1.085))
    A2=( np.where(REF(L,TS)>REF(H,TS+1),REF(C,TS+1),REF(O,TS)))
    A5=( V<REF(V,1) ) * ( V<MA(V,5))
    A6=( np.abs(C/O-1)*100<5)
    A7=( llv(C,TS)>=A2)
    A8=( MACD>-1)
    A9=(MA(C,5)>=A2 ) * ( C>A2 ) * ( C>=MA(C,5) ) * ( REF(C,1)<=REF(MA(C,5),1)) * ( llv(L,TS)<A2*1.045)
    a=  ( A9)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
