...
#开始
    ...
    MA62=(MA(CLOSE,62))
    VAR4=(REF(MA4,4)>MA20)
    VAR5=(REF(MA20<MA62,15)) * ( MA20>=MA62)
    np.absVOL=((MA4-MA20))
    CON1=( VAR4 ) * ( VAR5)
    VAR41=(REF(MA4>MA20,3))
    np.absVOL1=(np.abs(MA4-MA10))
    VAR61=(REF(np.absVOL1,2)>REF(np.absVOL1,1) ) * ( np.absVOL1>REF(np.absVOL1,1))
    CON2=  ( VAR41 ) * ( VAR5 ) * ( VAR61)
    CON=((CON1) ) + ( (CON2
))
    a=(REF(CON,4)==0 ) * ( (REF(CON,3))==0 ) * ( (REF(CON,1))==0 ) * ( (REF(CON,2))==0 ) * ( CON)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
