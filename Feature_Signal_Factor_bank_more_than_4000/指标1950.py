...
#开始
    ...
    VAR1=(hhv(HIGH,7)-llv(LOW,7))
    VAR2=(hhv(HIGH,7)-CLOSE)
    VAR3=(CLOSE-llv(LOW,7))
    VAR4=(VAR2/VAR1*92-72)
    VAR5=((CLOSE-llv(LOW,55))/(hhv(HIGH,55)-llv(LOW,55))*100)
    VAR6=((2*CLOSE+HIGH+LOW)/4)
    VAR7=(sma(VAR3/VAR1*92,3,1))
    VAR8=(llv(LOW,34))
    VAR9=(sma(VAR7,3,1)-sma(VAR4,9,1))
    VARA=(np.where(VAR9>92,VAR9-92,0))
    VARB=(hhv(HIGH,34))
    AA=( VARA*2)
    AB=( EMA((VAR6-VAR8)/(VARB-VAR8)*92,13))
    VARC=(EMA(0.665*REF(AB,1)+0.333*AB,2))
    CC=(sma(VAR5,20,1))
    CC1=(sma(CC,5,1))
    a=((CROSS(CC,CC1) ) * ( (CC1 < 92) ))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
