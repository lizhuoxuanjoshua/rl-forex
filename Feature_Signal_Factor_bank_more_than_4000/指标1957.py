...
#开始
    ...
    VAR2=(REF(CLOSE,1)/CLOSE)
    VAR3=((1-MA(LOW,3)/MA((HIGH+LOW+2*CLOSE)/4,20))*100-5)
    VAR4=((1-sma(VAR2*LOW,3,1)/EMA((HIGH+LOW+2*CLOSE)/4,10))*100-5)
    VAR5=((MA(HIGH,3)/MA((HIGH+LOW+2*CLOSE)/4,20)-1)*100-5)
    VAA=(np.where(VAR5>0,VAR5*CLOSE/REF((HIGH+LOW+2*CLOSE)/4,3),0))
    a=(CROSS(VAA,0.01))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
