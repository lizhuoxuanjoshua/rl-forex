...
#开始
    ...
    VARG=(100-3*sma((CLOSE-llv(LOW,75))/(hhv(HIGH,75)-llv(LOW,75))*100,20,1)+2*sma(sma((CLOSE-llv(LOW,75))/(hhv(HIGH,75)-llv(LOW,75))*100,20,1),15,1))
    VAR80=(100-3*sma((OPEN-llv(LOW,75))/(hhv(HIGH,75)-llv(LOW,75))*100,20,1)+2*sma(sma((OPEN-llv(LOW,75))/(hhv(HIGH,75)-llv(LOW,75))*100,20,1),15,1))
    VAR81=(VARG<REF(VAR80,1) ) * ( VOL>REF(VOL,1) ) * ( CLOSE>REF(CLOSE,1))
    a=(VAR81 ) * ( count(VAR81,30)==1)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
