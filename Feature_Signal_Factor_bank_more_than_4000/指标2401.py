...
#开始
    ...
    BB=(np.abs((CLOSE+2*HIGH+LOW)/4-MA(CLOSE,17))/MA(CLOSE,17))

    EE=(EMA(CLOSE,np.round(BB)))
    AA=(REF(EE,2)*0.02)
    AB=(EMA(AA,7))
    AC=((C-REF(C,1))/REF(C,1)*100)
    EMA3=(EMA(C,3))
    EMA13=(EMA(C,13))
    a=(barslast(REF(CROSS(EMA13,EMA3),1)) ) * ( CROSS(EMA3,EMA13) )
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
