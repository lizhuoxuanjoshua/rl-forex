...
#开始
    ...
    XA_6=(14)
    XA_7=(1)
    XA_8=(MA((2*CLOSE+HIGH+LOW)/4,5))
    XA_9=(1)
    XA_10=(XA_8*XA_1/100)
    XA_11=(XA_8*(200-XA_1)/100)
    XA_12=(np.abs((2*CLOSE+HIGH+LOW)/4-MA(CLOSE,20))/MA(CLOSE,20)*XA_7)
    XA_13=(DMA(CLOSE,XA_12)*XA_9)
    XA_14=((1+XA_2/100)*XA_13)
    XA_15=((1-XA_2/100)*XA_13)
    XA_16=(LOW<np.minimum(XA_11,XA_15)*XA_7)
    XA_17=(CLOSE*VOL)
    XA_18=(EMA((EMA(XA_17,3)*XA_9/EMA(VOL,3)+EMA(XA_17,6)/EMA(VOL,6)+EMA(XA_17,12)/EMA(VOL,12)+EMA(XA_17,24)/EMA(VOL,24))/4,XA_3)*XA_7)
    XA_19=(1.06*XA_18)
    XA_20=(XA_18*0.94)
    XA_21=(EMA(CLOSE,9))
    XA_22=(EMA(XA_21*1.14,5))
    XA_23=(EMA(XA_21*0.86,5))
    XA_24=(LOW<XA_23*1.01)
    XA_25=(EMA(np.maximum(np.maximum(HIGH-LOW,np.abs(HIGH-REF(CLOSE,1))),np.abs(REF(CLOSE,1)-LOW)),XA_4)*XA_7)
    XA_26=(HIGH-REF(HIGH,1))
    XA_27=(REF(LOW,1)-LOW)
    XA_28=(EMA((XA_26>0 ) * ( XA_26>XA_27),XA_4)*XA_9)
    XA_29=(EMA((XA_27>0 ) * ( XA_27>XA_26),XA_4)*XA_9)
    XA_30=(XA_28*100/XA_25)
    XA_31=(XA_29*100/XA_25)
    XA_32=(EMA(np.abs(XA_31-XA_30)/(XA_31+XA_30)*100,XA_5))
    XA_33=(EMA(XA_32,XA_5))
    XA_34=(XA_32>59 ) * ( XA_33>49)
    XA_35=((HIGH+LOW+CLOSE)/3)
    XA_36=((XA_35-MA(XA_35,XA_6))/(0.015*AVEDEV(XA_35,XA_6)))
    XA_37=(XA_36<(-110) ) * ( XA_36>(-200))
    XA_38=(XA_16 ) * ( XA_24 ) * ( XA_34 ) * ( XA_37 ) * ( (CLOSE-OPEN)/OPEN>(-0.06))
    XA_39=(FILTER(XA_38,2)*XA_9)
    a=(XA_38 ) * ( XA_39)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
