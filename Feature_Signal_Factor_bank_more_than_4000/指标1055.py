...
#开始
    ...
    VAR41=((C-EMA(C,8))/EMA(C,8))
    VAR42=(EMA(EMA(VAR41,4),3))
    VAR43=(EMA(EMA(VAR41,4),2))
    VAR44=(100*( VAR42+ VAR43))
    VAR45=(EMA(VAR44,2.5))
    RISE=( VAR44>=REF(VAR44,1))
    FELL=( VAR44<REF(VAR44,1))
    VAR02=(EMA(EMA(VAR41,3),3))
    VAR03=(EMA(EMA(VAR41,3),2))
    VAR04=(100*( VAR02+ VAR03))
    BI=(100*(H+L)/2/EMA(C,17)-100)
    BUY1=(VAR44>REF(VAR44,1) ) * ( REF(VAR44,1)<REF(VAR44,2) ) * ( REF(VAR44,2)<REF(VAR44,3))
    BUY2=(REF(BI,1)>0 ) * ( BUY1 ) * ( VAR44<-2)
    BUY3=(REF(BI,1)<0 ) * ( BUY1 ) * ( REF(VAR44,1)<-3)
    BUY4=(VAR44<0 ) * ( CROSS(VAR44,VAR45) ) * ( BUY1)
    a=(((BUY2) ) + ( (BUY3) ) + ( (BUY4)))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
