...
#开始
    ...
    AMV1=(count(AMOV,5)/count(VOL,5))
    AMV2=(count(AMOV,13)/count(VOL,13))
    AMV3=(count(AMOV,34)/count(VOL,34))
    AMV4=(count(AMOV,60)/count(VOL,60))
    a=(C>O ) * ( CROSS(C,AMV1)) * ( CROSS(C,AMV2)) * ( CROSS(C,AMV3)) * ( CROSS(AMV1,AMV2))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
