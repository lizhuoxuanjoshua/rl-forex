...
#开始
    ...
    HH=(BB>=0 ) * ( REF(BB<0,1) ) * ( count(BB>=0,6)<=1)
    TYP =( AMOUNT/VOL)
    GJW1=(count(np.where(TYP>REF(TYP,1),TYP*VOL,0),21)/count(np.where(TYP<REF(TYP,1),TYP*VOL,0),21))
    GJW2=(100-(100/(1+GJW1)))
    CC=(EMA(GJW2,5))
    CC1=(CC<50 ) * ( CC>=40)
    CC2=(CC>=50 ) * ( CC<60)
    ...
    FF=((((PL1 == 1) ) * ( (PL2 == 1)) * 0.2))
    DD=(((((FF == 0) ) * ( (REF(FF,1) == 0.2)) ) * ( (REF((count((FF == 0.2),10) == 10),1) == 1)) * 1))
    DIFF=(EMA(CLOSE,12) - EMA(CLOSE,26))
    DEA=(EMA(DIFF,9))
    MACD=(2*(DIFF-DEA))
    a=(HH ) * ( count(DD,10)>=1  ) * ( REF(MACD,1)<REF(MACD,2) ) * ( MACD>=0 ) * ( MACD>REF(MACD,1))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
