...
#开始
    ...
    MV3=(EMA(V,3))
    MA3=(EMA(C,3))
    MV3角度=(np.arctan((MV3/REF(MV3,1)-1)*100)*180/3.1416)
    MA3角度=(np.arctan((MA3/REF(MA3,1)-1)*100)*180/3.1416)
    MV5=(EMA(V,5))
    MA5=(EMA(C,5))
    MV5角度=(np.arctan((MV5/REF(MV5,1)-1)*100)*180/3.1416)
    MA5角度=(np.arctan((MA5/REF(MA5,1)-1)*100)*180/3.1416)
    MV10=(EMA(V,10))
    MA10=(EMA(C,10))
    MV10角度=(np.arctan((MV10/REF(MV10,1)-1)*100)*180/3.1416)
    MA10角度=(np.arctan((MA10/REF(MA10,1)-1)*100)*180/3.1416)
    AA=((MV3角度+MA3角度+MV5角度+MA5角度+MV10角度+MA10角度)/6)
    a=(AA>=80)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
