...
#开始
    ...
    JZF=(C-REF(MA(JJ,3),1))/REF(MA(JJ,3),1)

    

    回头=((L<MA(JJ,5)) + (L<MA(C,5))) * C>1.008*L * CROSS(C,REF(MA(JJ,2),2))
    ...
    涨跌比率=np.abs((C-REF(MA(JJ,3),1))/REF(MA(JJ,3),1))

    

    量柱实体大=count(量柱实体,5)>5*0.0188

    

    a=量柱实体大 * (count(涨跌比率>0.06,15)>=3 )
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
