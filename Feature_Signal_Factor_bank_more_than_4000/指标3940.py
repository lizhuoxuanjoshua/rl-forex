...
#开始
    ...
    X_3=(MA(CLOSE,26))
    X_4=(MA(CLOSE,60))
    X_5=(X_1 ) * ( X_1>X_2 ) * ( X_2>X_3 ) * ( X_3>X_4 )
    X_6=(CLOSE/REF(CLOSE,1)>1.098)
    X_7=(count(X_6,6)>=2)
    X_8=(np.where(REF(CLOSE,1)*1.1-CLOSE<0.01,1,0))
    X_9=(REF(CLOSE,1))
    X_10=(sma(np.maximum(CLOSE-X_9,0),2,1)/sma(np.abs(CLOSE-X_9),2,1)*100)
    X_11=(45)
    X_12=(20)
    X_13=(X_10<X_11 ) * ( REF(X_10,1)>X_11)
    X_14=(X_10<X_12 ) * ( REF(X_10,1)>X_12)
    X_15=(count(X_8,13)>=1)
    X_16=(sma(LOW,4,3))
    a=(X_16 ) * ( REF(X_13,1) ) * (( X_15) ) + ( ((X_16 ) * ( REF(X_14,1) ) * ( X_15 ) * ( X_7 ) * ( X_5
)))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
