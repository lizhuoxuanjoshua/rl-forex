...
#开始
    ...
    ZJX2=(EMA(CLOSE,2)-EMA(CLOSE,89))
    ZJX3=(2*(ZJX2-EMA(ZJX2,30))*10)
    ZJX4=(EMA(CLOSE,2)-EMA(CLOSE,150))
    ZJX5=(2*(ZJX4-EMA(ZJX4,100)))
    ZAA=((np.power(ZJX3,3)*0.1+np.power(ZJX3,2))/45+(np.power(ZJX5,3)*0.1+np.power(ZJX5,1))*5)
    ZAB=(EMA(ZAA,5),)
    a=(CROSS(ZAA,ZAB))
#结束
    买入 = a
