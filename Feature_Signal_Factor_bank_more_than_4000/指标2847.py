...
#开始
    ...
    EMA120=(EMA(C,120))
    EMA250=(EMA(C,250))
    a=(CROSS(EMA30,EMA60) ) * ( C>O ) * ( C>REF(C,1) ) * (( C/REF(C,1)>=1.01)) + ( (CROSS(EMA30,EMA120) )) * ( C>O ) * ( C>REF(C,1) ) * ( C/REF(C,1)>=1.018) + ( (CROSS(EMA60,EMA90) )) * ( C>O ) * ( C>REF(C,1) ) * ( C/REF(C,1)>=1.018) + ( (CROSS(EMA30,EMA90) )) * ( C>O ) * ( C>REF(C,1) ) * ( C/REF(C,1)>=1.018)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
