...
#开始
    ...
    Z1=(sma(np.maximum(CLOSE-REF(C,1),0),2,1)*C*102)
    Z2=(sma(np.abs(CLOSE-REF(C,1)),2,1)*C*100)
    Z=(Z1/Z2*100)
    A=(Z)
    B=(MA(C,20)>REF(MA(C,20),1) ) * ( MA(C,55)>REF(MA(C,55),1))
    XG=(A<10 ) * ( B  ) * ( MA(C,20)>MA(C,55) ) * ( V>0)
    关注=(A<10 ) * ( B  ) * ( MA(C,20)>MA(C,55) ) * ( C>MA(C,20) ) * ( V>0 ) * ( MA(C,10)>REF(MA(C,10),1))
    a=(count(关注==1,3)>=1 ) * ( MA(C,10)>REF(MA(C,10),1) ) * ( C>O ) * ( REF(C,1)<REF(O,1))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
