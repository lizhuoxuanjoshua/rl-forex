...
#开始
    ...
    A=(L<MA(C,15) ) * ( L<REF(hhv(C,20),1)*9 ) * ( MA(C,15)<REF(hhv(C,20),1)*0.9)
    B=(CROSS(CYS,0))
    小妖线=(EMA(CLOSE,20))
    MID=((3*CLOSE+LOW+OPEN+HIGH)/6)
    QD=((20*MID+19*REF(MID,1)+18*REF(MID,2)+17*REF(MID,3)+ 16*REF(MID,4)+15*REF(MID,5)+14*REF(MID,6)+ 13*REF(MID,7)+12*REF(MID,8)+11*REF(MID,9)+ 10*REF(MID,10)+9*REF(MID,11)+8*REF(MID,12)+ 7*REF(MID,13)+6*REF(MID,14)+5*REF(MID,15)+ 4*REF(MID,16)+3*REF(MID,17)+2*REF(MID,18)+REF(MID,20))/210)
    小妖=(CROSS(C,小妖线) ) * ( CROSS(C,QD) ) * ( QD<小妖线)
    小妖2=(CROSS(小妖线,QD))
    小妖3=(CROSS(C,小妖线) ) * ( CROSS(C,QD) ) * ( QD<小妖线 )

    小妖4=(((小妖) ) + ( (小妖2) ) + ( (小妖3)) ) * ( B)

    a=(((小妖 ) * ( 小妖2)) ) + ( ((小妖 ) * ( 小妖3)) ) + ( ((小妖2 ) * ( 小妖3)) ) + ( ((小妖 ) * ( 小妖2 ) * ( 小妖3)))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
