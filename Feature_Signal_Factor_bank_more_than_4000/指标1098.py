...
#开始
    ...
    VAR11=(np.where(((VAR8-VAR10)*5>0),(np.power((VAR8-VAR10)*5,2))/10,(-1*(np.power((VAR8-VAR10)*(5),2)))/(10)))
    VAR12=(np.where((VAR11>0),VAR11-0,12))
    AA=((sma(VAR5,13,8)))
    a=(CROSS(AA,VAR12)) * ( AA<REF(AA,2)) * (( C>REF(C,1)) ) + ( (AA>REF(AA,1))) * ( AA<35 ) * ( C>REF(C,1))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
