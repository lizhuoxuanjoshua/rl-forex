...
#开始
    ...
    RSVQD=((((CLOSE - llv(LOW,9)) / (hhv(HIGH,9) - llv(LOW,9))) * 100))
    KQD=(sma(RSVQD,3,1))
    DQD=(sma(KQD,3,1))
    JQD=(((3 * KQD) - (2 * DQD)))
    VARBQD=((((CLOSE - MA(CLOSE,6)) / MA(CLOSE,6)) * 100))
    VARCQD=((((CLOSE - MA(CLOSE,12)) / MA(CLOSE,12)) * 100))
    VARDQD=((((CLOSE - MA(CLOSE,24)) / MA(CLOSE,24)) * 100))
    VAREQD=((((VARBQD + (2 * VARCQD)) + (3 * VARDQD)) / 6))
    VARFQD=(MA(VAREQD,3))
    a=((CROSS(JQD,0) ) * ( (VARFQD <= (0 - 7))))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
