...
#开始
    ...
    HHH=((20*H+19*REF(H,1)+18*REF(H,2)+17*REF(H,3)+16*REF(H,4)+15*REF(H,5)+14*REF(H,6)+13*REF(H,7)+12*REF(H,8)+11*REF(H,9)+10*REF(H,10)+9*REF(H,11)+8*REF(H,12)+7*REF(H,13)+6*REF(H,14)+5*REF(H,15)+4*REF(H,16)+3*REF(H,17)+2*REF(H,18)+REF(H,20))/210)
    QSX=(EMA(HHH,90))
    TJZB1=((C-llv(L,5))/(hhv(C,5)-llv(L,5))*100)
    TJZB2=((C-llv(L,10))/(hhv(C,10)-llv(L,10))*100)
    TJZB3=((C-llv(L,20))/(hhv(C,20)-llv(L,20))*100)
    TJZB4=((C-llv(L,30))/(hhv(C,30)-llv(L,30))*100)
    BIAS1=((CLOSE-MA(CLOSE,6))/MA(CLOSE,6.5)*100)
    BIAS2=((CLOSE-MA(CLOSE,12))/MA(CLOSE,13)*100)
    BIAS3=((CLOSE-MA(CLOSE,24))/MA(CLOSE,26)*100)
    X=(BIAS1+BIAS2+BIAS3)
    VAR1=(3*sma((C-llv(L,60))/(hhv(H,60)-llv(L,60))*100,5,1)-2*sma(sma((C-llv(L,60))/(hhv(H,60)-llv(L,60))*100,5,1),3,1))
    QS=(EMA(VAR1,3))
    a=  ( QS<67 ) * ( TJZB4>84.5  ) * ( TJZB1>76 )  * ( C/O>1.02 )
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
