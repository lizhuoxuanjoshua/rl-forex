...
#开始
    ...
    a=(REF(CLOSE,4)/REF(OPEN,4)>1.03 ) * ( REF(CLOSE,3)<REF(OPEN,3) ) * ( REF(CLOSE,2)<REF(OPEN,2) ) * ( REF(CLOSE,1)<REF(OPEN,1) ) * ( REF(LOW,4)<REF(LOW,3) ) * ( REF(LOW,4)<REF(LOW,2) ) * ( REF(LOW,4) <REF(LOW,1) ) * ( REF(HIGH,4)>REF(HIGH,3) ) * ( REF(HIGH,4)>REF(HIGH,2) ) * ( REF(HIGH,4)>REF(HIGH,1) ) * ( CLOSE/OPEN>1.03 ) * ( CLOSE>REF(CLOSE,4))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
