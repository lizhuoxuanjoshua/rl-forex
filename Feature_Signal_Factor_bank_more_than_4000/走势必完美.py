...
def 策略(合成周期,ktime,high,o,low,c,volume,amount,均线,开盘,收盘,切片位置,subindex,turn):
    ...
    a = (REF(C, 4) > REF(C, 3)) * (REF(C, 3) > REF(C, 2)) * (REF(C, 2) > REF(C, 1)) * (REF(C, 1) < C) * (
                H > REF(H, 1)) * (L > REF(L, 1)) * (REF(H, 1) < REF(H, 2)) * (REF(L, 1) < REF(L, 2)) * (
                    REF(H, 2) < REF(H, 3)) * (REF(L, 2) < REF(L, 3)) * (REF(H, 3) < REF(H, 4)) * (REF(L, 3) < REF(L, 4))
