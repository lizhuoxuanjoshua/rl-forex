...
#开始
    ...
    N=(3)
    XG1=1
    HH=(np.where(C/REF(C,1)>1.098  ,2*C-REF(C,1)-H,2*C-H-L))
    DK=(count(HH,0))
    MDK10=(MA(DK,P2))
    AA=(MA(CLOSE,1)/MA(CLOSE,2)>1.02)
    BB=(MA(CLOSE,1)>=hhv(MA(CLOSE,1),3))
    CC=(MA(VOL,2)/MA(VOL,40)>3)
    DD=(MA(VOL,2)/hhv(MA(VOL,2),30))
    EE=(VOL>REF(VOL,1))
    FF=(CLOSE>REF(CLOSE,1))
    XG2=(AA ) * ( BB ) * ( CC ) * ( DD ) * ( EE ) * ( FF)
    MA10=(MA(C,10))
    MA30=(MA(C,30))
    MA60=(MA(C,55))
    LL=(np.minimum(np.minimum(MA10,MA30),MA60))
    XG3=(count(MA30>REF(MA30,1),2)==2 ) * ( MA60>REF(MA60,1) ) * ( MA10==LL ) * ( CROSS(C,MA10))
    a=(((XG1 ) * ( XG2)) ) + ( ((XG1 ) * ( XG3)) ) + ( ((XG2 ) * ( XG3) 
))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
