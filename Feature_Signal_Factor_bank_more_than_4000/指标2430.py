...
#开始
    ...
    M01=(8)
    M02=(4)
    RSV0=((hhv(HIGH,N0)-CLOSE)/(hhv(HIGH,N0)-llv(LOW,N0))*100)
    LWR1=(sma(RSV0,M01,1))
    LWR2=(sma(LWR1,M02,1))
    ...
    VARA=(hhv(H,34))
    AA=(EMA((VAR6-VAR8)/(VARA-VAR8)*100,8))
    BB=(EMA(AA,5))
    a=(REF(AA-BB<0,5) ) * ( REF(AA-BB<0,4) ) * ( REF(AA-BB<0,3) ) * ( REF(AA-BB<0,2) ) * ( REF(AA-BB<0,1)  ) * ( CROSS(AA>BB,0.5))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
