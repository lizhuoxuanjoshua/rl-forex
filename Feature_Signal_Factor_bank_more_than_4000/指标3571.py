...
#开始
    ...
    WR1=(100-100*(hhv(HIGH,10)-CLOSE)/(hhv(HIGH,10)-llv(LOW,10)))
    WR2=(100-100*(hhv(HIGH,25)-CLOSE)/(hhv(HIGH,25)-llv(LOW,25)))
    WR3=(100-100*(hhv(HIGH,55)-CLOSE)/(hhv(HIGH,55)-llv(LOW,55)))
    WR=((WR1+2*WR2+3*WR3)/6)
    AA=(MA(WR,3))
    a=(CROSS(WR, AA) ) * ( CROSS(WR3, AA) ) * ( AA<10 ) * ( WR3>AA ) * ( WR<10)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
