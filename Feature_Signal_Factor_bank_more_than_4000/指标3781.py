...
#开始
    ...
    X_7=(np.maximum(np.maximum(HIGH-LOW,np.abs(REF(CLOSE,1)-HIGH)),np.abs(REF(CLOSE,1)-LOW)))
    X_8=(MA(X_7,14))
    X_9=(MA((HIGH+LOW)/2,1))
    X_10=(MA(X_7,2))
    X_11=(X_9+X_10)
    X_12=(X_9-X_10)
    X_13=(CLOSE<hhv(X_12,3))
    X_14=(CLOSE<REF(CLOSE,1) ) * ( CLOSE<OPEN ) * ( CLOSE!=LOW ) * ( CLOSE>MA(CLOSE,18))
    X_15=(MA(CLOSE,20)>REF(MA(CLOSE,20),1))
    X_16=(CLOSE<REF(llv(LOW,3),1))
    X_17=(X_8>REF(X_8,1) ) * ( X_7>REF(X_7,1))
    X_18=((CLOSE-LOW)/(HIGH-LOW))
    X_19=(X_18<0.3)
    X_20=((HIGH-OPEN)/(HIGH-LOW))
    X_21=(X_20<0.4)
    a= ( X_14 ) * ( X_15 ) * ( X_16 ) * ( X_17 ) * ( X_19 ) * ( X_13 ) * ( X_4 ) * ( X_5 ) * ( X_6)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
