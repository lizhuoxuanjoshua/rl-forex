...
#开始
    ...
    LONG=(26)
    MID=(9)
    RJJ=((HIGH + LOW + CLOSE)/3)
    RS=(count(np.where(RJJ>REF(RJJ,1),RJJ*VOL,0),N)/count(np.where(RJJ<=REF(RJJ,1),RJJ*VOL,0),N))
    BL=(100-100/(1+RS))
    A1=(count(np.where(BL>REF(BL,1),VOL,np.where(BL<REF(BL,1),-VOL,0)),0))
    DIF=(EMA(A1,short)-EMA(A1,LONG))
    DEA=(EMA(DIF,MID))
    a=(CROSS(DIF,DEA)) * ( DEA<0)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
