...
#开始
    ...
    上线Y=(底结构Y*1.01)
    下线Y=(底结构Y*0.99)
    JJ=((O+CLOSE+HIGH+LOW)/4)
    中线=((MA(JJ,120)+MA(JJ,140)+MA(JJ,160)+MA(JJ,180))/4)
    上线=(中线*1.1)
    下线=(中线*0.92)
    判定=(上线*1.06)
    再判定=(判定*1.06)
    ...
    中上=(中线+(上线-中线)/2)
    中下=(下线+(中线-下线)/2)
    发散=(下线T>上线YX ) * ( 下线YX>上线X ) * ( 下线X>上线Y)
    最低标准线=(中下+(中线-中下)*0.9)
    发散结束=(NOT(发散)  ) * ( REF(发散,1) ) * ( count(CROSS(H,判定),233)>0)
    第二次结束=(发散结束 ) * ( count(发散结束,610)>1)
    a=(L<最低标准线 ) * ( count(第二次结束,377)>0)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
