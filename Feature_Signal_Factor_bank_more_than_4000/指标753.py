...
#开始
    ...
    MA10=(MA(CLOSE,10))
    MA20=(MA(CLOSE,20))
    A1=(np.where(C>=MA5,1,-1))
    A2=(np.where(C>=MA10,1,-1))
    A3=(np.where(C>=MA20,1,-1))
    A4=(np.where(MA5>=REF(MA5,1),1,-1))
    A5=(np.where(MA10>=REF(MA10,1),1,-1))
    A6=(np.where(MA20>=REF(MA20,1),1,-1))
    趋势=((A1+A2+A3+A4+A5+A6)/6*100)
    天佑=(趋势>=50)
    AA=(np.where(天佑,barslast(CROSS(天佑,0.1))+1,0))
    a=(AA==1)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
