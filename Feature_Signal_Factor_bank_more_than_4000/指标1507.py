...
#开始
    ...
    VARD=(np.abs(VAR8-VARA)/VAR8*100)
    VARE=(REF(VAR9,1))
    VARF=(REF(VARD,1))
    VAR10=(REF(M5,1))
    VAR11=(REF(M10,1))
    ...
    VAR23=(sma(VAR16,2,1))
    VAR24=(REF(VAR23,1))
    VAR25=(np.where(CLOSE>OPEN,1,0))
    VAR26=(np.where(M5>VAR10,1,0))
    VAR27=(np.where(M10>VAR11,1,0))
    VAR28=(np.where(VARD>VARF,1,0))
    VAR29=(np.where(VAR17>VAR1A,5,0))
    VAR2A=(np.where(VAR19>VAR15,5,0))

    VAR2C=(np.where(VARC>2.25,10,0))
    VAR2D=(np.where(VAR17>60,6,0))
    VAR2E=(np.where(VAR17>80,8,0))
    VAR2F=(np.where(VAR17>VAR15,10,0))
    VAR30=(np.where(VAR17>VAR15,VAR1D-VAR1E,0))
    VAR31=(5*(VAR17>VAR15 ) * ( VAR15<20))
    VAR32=(5*(CLOSE>VAR8 ) * ( VAR8>VAR9))
    VAR33=(np.where(VAR8>VAR9,5,0))
    VAR34=(np.where(VAR9>VARE,5,0))
    VAR35=(np.where(VAR15<18,VAR1C-VAR15,0))
    VAR36=(8*(VAR15>VAR17 ) * ( VAR15>50))
    VAR37=((VAR25+VAR26+VAR27+VAR28+VAR29+VAR2A+VAR2C+VAR2D+VAR2E+VAR2F+VAR30+VAR31+VAR32+VAR33+VAR34+VAR35-VAR36))

    VAR3A=(MA(VAR37,12))
    a= ( VAR3A<25 ) * ( VAR17>VAR15)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
