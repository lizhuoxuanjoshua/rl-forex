...
#开始
    ...
    VAR5=(sma(np.maximum(CLOSE-REF(CLOSE,1),0),5,1)/sma(np.abs(CLOSE-REF(CLOSE,1)),5,1)*100)
    AA=(EMA(VAR5,3))
    LC=(REF(CLOSE,1))
    ...
    D=(sma(K,15,1))
    J=(3*K-2*D)
    S1=(np.where((J<11 ) * ( BIAS>-11 ) * ( CROSS( S,F) ) * ( (S-F)>0.010),2,0))
    a= ( AA>=REF(AA,1) )  * ( AA>=49.5 ) * ( MA(C,3)>=REF(MA(C,3),1)) * ( MA(C,7)>=REF(MA(C,7),1) ) * (( MA(C,60)>REF(MA(C,60),3)) ) + ( (S1
))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
