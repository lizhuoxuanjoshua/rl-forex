...
#开始
    ...
    AA=(EMA(C,3))
    GD=((C-llv(L,9))/(hhv(H,9)-llv(L,9))*100)
    MY01=(3*sma(GD,3,1)-2*sma(sma(GD,3,1),3,1))
    MY02=(((H+L+C)/3-MA((H+L+C)/3,14))/(0.015*AVEDEV((H+L+C)/3,14))/2+50)
    MY03=(EMA(np.maximum(np.maximum(H-L,np.abs(H-REF(C,1))),np.abs(REF(C,1)-L)),14))
    MY04=(H-REF(H,1))
    MY04A=(REF(L,1)-L)
    条件1=(MY04>0 ) * ( MY04>MY04A)
    条件2=(MY04A>0 ) * ( MY04A>MY04)

    MY05=(EMA(np.where(条件1,MY04,0),14)*100/MY03)
    MY06=(EMA(np.where(条件2,MY04A,0),14)*100/MY03)
    MY07=(EMA(EMA(np.abs(MY06-MY05)/(MY06+MY05)*100,6),6))
    a=(MY01>REF(MY01,1) ) * ( REF(MY01,1)<10 ) * ( MY02>REF(MY02,1) ) * ( REF(MY02,1)<0 ) * ( MY07>REF(MY07,1) ) * ( AA>REF(AA,1))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
