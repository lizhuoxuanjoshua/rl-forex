...
#开始
    ...
    TCRSI=sma(np.maximum(CLOSE-LC1,0),6,1)/sma(np.abs(CLOSE-LC1),6,1)*100
    ...
    THRSI=sma(np.maximum(H-LC2,0),6,1)/sma(np.abs(H-LC2),6,1)*100
    ...
    TRRSI=sma(np.maximum(O-LC3,0),6,1)/sma(np.abs(O-LC3),6,1)*100
    ...
    TLRSI=sma(np.maximum(L-LC4,0),6,1)/sma(np.abs(L-LC4),6,1)*100

    

    TTRSI=(TCRSI+TRRSI+THRSI+TLRSI)/4

    

    参考线=EMA(EMA(TTRSI,6),5)
