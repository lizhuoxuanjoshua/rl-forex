...
#开始
    ...
    工作=(EMA(CLOSE,14))
    波段=(EMA(CLOSE,25))
    趋势=((MA(CLOSE,24)+MA(CLOSE,24*2)+MA(CLOSE,24*4))/3)
    长牛=(EMA(CLOSE,318))
    生命=(EMA(CLOSE,453))
    DIFF=(( EMA(CLOSE,7) - EMA(CLOSE,19)))
    DEA=(EMA(DIFF,9))
    MACD=(0.90*(DIFF-DEA))
    TJ=((DIFF>=DEA) ) * ( C>MA((C+O+L)/3,5))
    TJ1=((DIFF>=0))
    涨幅=((C-REF(C,1))/REF(C,1)*100)
    VAR1=((CLOSE*2+HIGH+LOW)/4)
    SK=( EMA(VAR1,13)-EMA(VAR1,73))
    SD=( EMA(SK,2))
    SJ=(( (CROSS(SK,SD) ) * ( SK<-0.04 ) * ( (C-REF(C,1))/REF(C,1)>=0.03))) + (((CROSS(SK,SD) )) * ( SK<=-0.14 )) + (((CROSS(SK,SD) )) * ( SK<=0.05 ) * ( (V/MA(V,5)>2 ) + ( (C/REF(C,1)>0.035))) ) + ( ((CROSS(C,工作) ) * ( 工作>波段 ) * ( L<趋势 ) * ( C>趋势)) ) + ( ((CROSS(C,工作) ) * ( CROSS(C,波段) ) * ( 涨幅>5)) ) + ( ((TJ ) * ( 涨幅>7.5) 
))
    a=(SJ ) * ( (C-O)>(H-C) ) * ( C<工作*1.25 ) * ( C>趋势 ) * ( C<趋势*1.27)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
