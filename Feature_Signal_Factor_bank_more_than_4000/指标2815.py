...
#开始
    ...
    WJ=((H+L+C)/3)
    V1=(np.where(H==L,1,H-np.maximum(O,C)))
    V2=(np.where(H==L,1,np.maximum(C,O)-WJ))
    V3=(np.where(H==L,1,np.minimum(O,C)-L))
    V4=(np.where(H==L,1,WJ-np.minimum(C,O)))
    V5=(VOL/np.where(H==L,4,H-L))
    V6=((V1*V5))
    V7=((V2*V5))
    V8=((V3*V5))
    V9=((V4*V5))
