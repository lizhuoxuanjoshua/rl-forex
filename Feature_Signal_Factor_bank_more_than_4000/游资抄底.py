...
#开始
    ...
    AA=CROSS(MA(C,5),MA(C,20)) * CROSS(MA(C,5),MA(C,30)) * CROSS(MA(C,5),MA(C,60))

    

    BBB=(MA(C,60)>MA(C,10)) * CROSS(MA(C,10),MA(C,30)) * (MA(C,30)>MA(C,20)) * (np.abs(MA(C,60)-MA(C,20))/MA(C,20)<0.05)

    

    CCC=(MA(C,30)>MA(C,5)) * CROSS(MA(C,5),MA(C,20)) * (MA(C,20)>MA(C,10))

    

    DD=(np.abs((MA(C,5)-MA(C,10))+(MA(C,10)-MA(C,20))+(MA(C,20)-MA(C,30)))<0.02) * (MA(C,5)>REF(MA(C,5),1)) * (MA(C,10)>REF(MA(C,10),1))

    

    EE=(MA(C,20)>MA(C,5)) * CROSS(MA(C,5),MA(C,10)) * (MA(C,10)>MA(C,60)) * (np.abs(MA(C,20)-MA(C,60))/MA(C,60)<0.05)
