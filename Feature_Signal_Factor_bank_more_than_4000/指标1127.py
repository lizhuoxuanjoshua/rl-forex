...
#开始
    ...
    VAR12=((HIGH - LOW))
    VAR13=(np.abs((OPEN - CLOSE)))
    VAR14=((HIGH - np.where((CLOSE > OPEN),CLOSE,OPEN)))
    VAR15=((np.where((CLOSE > OPEN),OPEN,CLOSE) - LOW))
    ...
    a=(((((((VAR16 > (MA(VOL,5) * 2)) ) * ( (VAR16 >= VAR17)) ) * ( (VAR16 >= VAR18)) ) * ( (CLOSE > OPEN))) ) + ( (((VOL > (MA(VOL,5) * 3)) ) )))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
