...
#开始
    ...
    VARA=(((EMA(EMA(VAR9,5),5))/(EMA(EMA(np.abs(VAR9),5),5)))*(100))
    VARB=(((EMA(EMA(VAR9,10),10))/(EMA(EMA(np.abs(VAR9),10),10)))*(100))
    VARC=(((EMA(EMA(VAR9,20),20))/(EMA(EMA(np.abs(VAR9),20),20)))*(100))
    CCI1=(((P - MA(P,14))*(100))/((1.5)*(AVEDEV(P,14))))
    a=((CCI1 < (-100)))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
