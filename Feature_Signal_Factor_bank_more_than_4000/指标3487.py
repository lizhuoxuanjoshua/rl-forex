...
#开始
    ...
    N=(5)
    M=(13)
    V1=(np.abs((2*CLOSE+HIGH+LOW)/4-MA(CLOSE,20))/MA(CLOSE,20))
    V5=(MA((LOW+HIGH+CLOSE)/3,5))
    MA1=(EMA(CLOSE,5))
    MA2=(np.arctan((MA1/REF(MA1,1)-1)*100)*180/3.1416)
    MA3=(np.where(MA2>0,MA2,MA2))
    MA5=(MA(CLOSE,N))
    MA13=(MA(CLOSE,M))
    VAR1=(CLOSE==hhv(C,4) ) * ( ((REF(C,1)>=REF(C,2)) ) + ( (REF(C,1)>=REF(C,3))) ) + ( (REF(C,1)==hhv(C,4) )) * ( CLOSE>=REF(C,2) ) + ( (REF(C,2)==hhv(C,4) )) * ( REF(C,3)==llv(C,4) ) * ( C>=REF(C,1) ) + ( (REF(C,3)==hhv(C,4) )) * ( C>=REF(C,1) ) * ( C>=REF(C,2))
    VAR2=(CLOSE==hhv(C,4) ) * ( REF(C,1)==llv(C,4))
    VAR3=(CLOSE==llv(C,4) ) * ( ((REF(C,1)<REF(C,2)) ) + ( (REF(C,1)<REF(C,3))) ) + ( (REF(C,1)==llv(C,4) )) * ( C<REF(C,2) ) + ( (REF(C,2)==llv(C,4) )) * ( REF(C,1)==hhv(C,4) ) * ( C<REF(C,1) ) + ( (REF(C,3)==llv(C,4) )) * ( C<REF(C,1) ) * ( C<REF(C,2))
    VAR4=(REF(C,1)==llv(C,4) ) * (( C>=REF(C,2)) ) + ( (REF(C,2)==llv(C,4) )) * ( REF(C,1)<=REF(C,3) ) * ( REF(C,2)<REF(C,4) ) * ( C>=REF(C,1))
    VAR5=(REF(C,1)==hhv(C,4) ) * (( C<REF(C,2)) ) + ( (REF(C,2)==hhv(C,4) ) * ( REF(C,1)>REF(C,3) ) + ( REF(C,3)==hhv(C,4) )) * ( REF(C,1)>REF(C,2))
    VAR6=(V5>REF(V5,1) ) * ( REF(V5,1)<REF(V5,2))
    VAR7=(LOW<hhv(V5,13) ) * ( HIGH>hhv(V5,13) ) * ( CLOSE>REF(C,1) ) * ( CLOSE>OPEN)
    VAR8=(VAR7 ) * ( count(VAR6,2))
    VAR9=(100*EMA(EMA(C-REF(C,1),6),6)/EMA(EMA(np.abs(C-REF(C,1)),6),6))
    VAR10=(EMA(MA(C,13),13))
    VAR11=((llv(VAR9,2)==llv(VAR9,7) ) * ( count(VAR9<0,2) ) * ( CROSS(VAR9,MA(VAR9,2)))==1)
    VAR12=((hhv(VAR9,2)==hhv(VAR9,7) ) * ( count(VAR9>50,2) ) * ( CROSS(MA(VAR9,2),VAR9))==1)
    VAR13=(sma(EMA((C-REF(C,1))/REF(C,1),5)*100,3,1)*10)
    VAR14=((((CROSS(VAR13,0)) ) + ( (CROSS(C,VAR10))) ) * ( VAR11)==1)
    VAR15=(MA(C,18)>=REF(MA(C,18),3) ) * ( CROSS(C,MA(C,18)))
    VAR16=(VAR8 ) * ( (REF(VAR8,1))==0)
    RSV=((CLOSE-llv(LOW,9))/(hhv(HIGH,9)-llv(LOW,9))*100)
    K=(sma(RSV,3,1))
    D=(sma(K,3,1))
    J=(3*K-2*D)
    金叉=(CROSS(J,D))
    买1=(((VAR14) ) + ( (VAR15)) ) * ( VAR11)
    买2=(((VAR14) ) + ( (VAR15)) ) * ( VAR4)
    买3=(((VAR14) ) + ( (VAR15)) ) * ( VAR2)
    买4=(((VAR2 ) * ( VAR11)) ) + ( ((VAR4 ) * ( VAR11)
))
    买5=(((金叉 ) * ( VAR4)) ) + ( ((金叉 ) * ( VAR2)) ) + ( ((金叉 ) * ( VAR11)
))
    a=((买1) ) + ( (买2) ) + ( (买3) ) + ( (买4) ) + ( (买5))
#结束
    买入 = a
