...
#开始
    ...
    X_3=(sma(X_2,3,1))
    X_4=(3*X_2-2*X_3)
    a=(OPEN>MA(CLOSE,5) ) * ( OPEN>MA(CLOSE,10) ) * ( OPEN>MA(CLOSE,20) ) * ( OPEN>MA(CLOSE,60) ) * ( OPEN>REF(CLOSE,1)*1.02 ) * ( LOW>REF(CLOSE,1) ) * ( LOW>REF(LOW,1) ) * ( MA(CLOSE,5)>REF(MA(CLOSE,5),1) ) * ( MA(CLOSE,10)>REF(MA(CLOSE,10),1) ) * ( MA(CLOSE,20)>REF(MA(CLOSE,20),1) ) * ( MA(CLOSE,60)>REF(MA(CLOSE,60),1) ) * ( MA(CLOSE,5)>MA(CLOSE,10) ) * ( MA(CLOSE,10)>MA(CLOSE,20) ) * ( X_4>X_2 ) * ( X_2>X_3 ) * ( CLOSE<OPEN ) * ( CLOSE>REF(CLOSE,1)*1.02 ) * ( OPEN/CLOSE<1.03)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
