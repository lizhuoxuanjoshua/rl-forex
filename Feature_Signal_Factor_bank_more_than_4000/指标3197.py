...
#开始
    ...
    VAR7=(count(CLOSE*VOL*100,4)/count(VOL*100,4))
    VAR8=((VAR7*100)/100)
    VAR9=(count(CLOSE*VOL*100,7)/count(VOL*100,7))
    VARA=((VAR9*100)/100)
    VARB=(count(CLOSE*VOL*100,28)/count(VOL*100,28))
    VARC=((VARB*100)/100)
    VARD=(EMA(CLOSE,5)-EMA(CLOSE,10))
    VARE=(EMA(CLOSE,5)-EMA(CLOSE,10))
    VARF=(EMA(VARD,9))
    ...
    VAR1A=(OPEN)
    VAR1B=(CLOSE)
    a=(VAR1A<=VAR1 ) * ( VAR1A<=VAR2 ) * ( VAR1A<=VAR3 ) * ( VAR1B>=VAR1 ) * ( VAR1B>=VARC ) * ( VAR16>0 ) * ( VAR19>0)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
