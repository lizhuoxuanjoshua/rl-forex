...
#开始
    ...
    QAVW1=((C-llv(L,55))/(hhv(H,55)-llv(L,55))*100)
    QAVW2=(100*(hhv(H,55)-C)/(hhv(H,55)-llv(L,55)))
    QAV21=(sma(QAVW2,55,1)+100)
    HH=((hhv(V,135)))
    RSV=((CLOSE - llv(L,21))/(hhv(H,21) - llv(L,21)))
    K=(EMA(RSV,3))
    D=(EMA(K,3))
    J=(3*K-2*D)
    KDJ1=(REF(count(J<REF(J,1),5)>=3,1) ) * ( REF(J,1)<0.03)
    KDJ2=(J>REF(J,1))
    QAV11=(sma(QAVW1,9,1))
    QAV51=(sma(QAV11,3,1)+100)
    KDJ3=(REF(count(J>REF(J,1),5)>=3,1) ) * ( REF(J,1)>0.99)
    KDJ4=(J<REF(J,1))
    VAR22=((CLOSE-llv(LOW,34))/(hhv(HIGH,34)-llv(LOW,34))*200*10)
    能量=(sma(VAR22,5,1))
    WRH=(QAV51-QAV21+50)
    T=((HIGH+LOW+CLOSE)/3)
    CCII=((T-MA(T,9))/(0.016*AVEDEV(T,9)))
    过热=(CROSS(180,CCII))
    JJ=((H+L+C)/3)
    QJ0=(VOL/np.where(H==L,4,H-L))
    QJ1=(np.where(0,QJ0*(JJ-np.minimum(C,O)),QJ0*(np.minimum(O,C)-L)))
    QJ2=(np.where(0,QJ0*(np.minimum(O,C)-L),QJ0*(JJ-np.minimum(C,O))))
    QJ3=(QJ0*(H-np.maximum(O,C)))
    QJ4=(QJ0*(np.maximum(C,O)-JJ))
    QJ5=(QJ1+QJ2)
    QJ6=(QJ3+QJ4)
    DD=((QJ5-QJ6)/(QJ5+QJ6)*100)
    DDX=(((QJ1+QJ2)-(QJ3+QJ4))/count(VOL,10)*100)
    DDZ=(DD*17)
    DDY=(((QJ2+QJ4)-(QJ1+QJ3))/count(VOL,10)*100)
    捡钱1=(CROSS(CCII,-150))
    捡钱2=(CROSS(CCII,-150) ) * ( (((DDX>0 ) * ( DDY>0)) ) + ( (DDZ>400)))
    BIAS=((CLOSE-MA(CLOSE,34))/MA(CLOSE,34)*100)
    捡钱3=(np.where(BIAS<=-16,1,0))
    量1=(MA(VOL,250))
    F=(VOL<量1)
    CCI=((T-MA(T,14))/(0.015*AVEDEV(T,14)))
    a=(F + (捡钱1) + (捡钱2) + (捡钱3) ) * ((能量<550)  + (KDJ1 ) * ( KDJ2)  * ( 能量<650) * ( CCI<-180))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
