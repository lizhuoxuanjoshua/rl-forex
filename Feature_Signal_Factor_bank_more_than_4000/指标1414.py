...
#开始
    ...
    AA=(EMA(CLOSE,2)-EMA(CLOSE,89))
    AA1=(EMA(AA,30))
    AA2=(2*(AA-AA1)*10)
    AA3=(np.power(AA2,3)*0.1+np.power(AA2,2))
    AA4=(np.where(AA2>0.015,AA3,0)/45)
    AA5=(EMA(CLOSE,2)-EMA(CLOSE,150))
    AA6=(EMA(AA5,100))
    AA7=(2*(AA5-AA6))
    AA8=(np.power(AA7,3)*0.1+np.power(AA7,1))
    GZ=(AA4+np.where(AA7>0.1,AA8,0)*5)
    VAR1D=(EMA(CLOSE,2)-EMA(CLOSE,89))
    VAR1F=(2*(VAR1D-EMA(VAR1D,30))*10)
    VAR20=(np.power(VAR1F,3)*0.1+np.power(VAR1F,2))
    VAR25=(1)
    YZ=(VAR25/10)
    VAR26=(EMA(CLOSE,2)-EMA(CLOSE,150))
    VAR27=(EMA(VAR26,100))
    VAR28=(2*(VAR26-VAR27))
    VAR29=(np.power(VAR28,3)*0.1+np.power(VAR28,1))

    a=( C>O ) * ( GZ>0 ) * ( REF(GZ,1)==0)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
