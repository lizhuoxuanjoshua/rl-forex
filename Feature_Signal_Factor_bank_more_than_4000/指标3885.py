...
#开始
    ...
    D=(2*(EMA(CLOSE,15)-EMA(CLOSE,30)-EMA(EMA(CLOSE,15)-EMA(CLOSE,30),12)))
    D1=((CLOSE-llv(LOW,12))/(hhv(HIGH,12)-llv(LOW,12))*100)
    D2=((CLOSE-llv(LOW,45))/(hhv(HIGH,45)-llv(LOW,45))*100)
    D3=((CLOSE-llv(LOW,225))/(hhv(HIGH,225)-llv(LOW,225))*100)
    B3=(REF(sma(D2,15,1),1)-REF(sma(D3,45,1),1))
    B5=(sma(D1,3,1)-REF(sma(D1,3,1),1))
    D9=(EMA((hhv(EMA(VOL/5,4),240)-EMA(VOL/5,4))/EMA(VOL/5,4)*MA(CLOSE,35),13))
    B7=(D>=-0.035 ) * ( D<=0.045 ) * ( CLOSE>MA(CLOSE,5) ) * ( CLOSE>MA(CLOSE,10) ) * ( D9>-100)
    RSV=((CLOSE-llv(LOW,9))/(hhv(HIGH,9)-llv(LOW,9))*100)
    K=(sma(RSV,3,1))
    DD=(sma(K,3,1))
    AA=(CROSS(K,DD))
    a=((B7 ) * ( B5>7 ) * ( REF(B5,1)<0 ) * ( B5-REF(B5,1)>9) )  * ( AA)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
