...
#开始
    ...
    VAR2=((np.power(MA(CLOSE,5),2)+MA(CLOSE,5)))
    VAR3=((np.power(MA(LOW,5),2)+MA(LOW,5)))
    VAR4=((np.power(MA(HIGH,5),2)+MA(HIGH,5)))
    VAR5=((((((VAR2-llv(VAR3,64))/(hhv(VAR4,64)-llv(VAR3,64)))*150)+65)-10))
    VAR6=(((sma(VAR5,3,1)*1.5)-100))
    VAR7=(sma(VAR6,3,1))
    VAR8=(((3*VAR6)-(2*VAR7)))
    AA=(100-3*sma((OPEN-llv(LOW,75))/(hhv(HIGH,75)-llv(LOW,75))*100,20,1)+2*sma(sma((OPEN-llv(LOW,75))/(hhv(HIGH,75)-llv(LOW,75))*100,20,1),15,1))
    AB=(VAR8<REF(AA,1) ) * ( VOL>REF(VOL,1) ) * ( CLOSE>REF(CLOSE,1))
    a=((AB ) * ( count(AB,30)==1))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
