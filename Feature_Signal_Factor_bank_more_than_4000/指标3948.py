...
#开始
    ...
    A3=(RSI1>RSI2)
    #RSI金叉=(np.where(A3,3,0)})
    RSV=(-(hhv(HIGH,13)-CLOSE)/(hhv(HIGH,13)-llv(LOW,13))*100)
    LWR1=(sma(RSV,3,1))
    LWR2=(sma(LWR1,3,1))
    A4=(LWR1>LWR2)
    #LWR金叉=(np.where(A4,4,0)})
    BBI=((MA(CLOSE,3)+MA(CLOSE,6)+MA(CLOSE,12)+MA(CLOSE,24))/4)
    A5=(CLOSE>BBI)
    #BBI金叉=(np.where(A5,5,0)})
    ...
    M3=(MA(C,20))
    M4=(MA(C,60))
    AAA=((C>M1)) * ((M1>M2)) * ((M2>M3) ) * ( (M1/M4<1.45))
    #) * ((M1>REF(M1,1))) * ((M2>REF(M2,1))) * ((M3>REF(M3,1))})
    DIFF1=(EMA(CLOSE,12) - EMA(CLOSE,26))
    DEA1  =( EMA(DIFF1,9))
    MACD1 =( 2*(DIFF1-DEA1))
    a=(CROSS(MACD1,0) ) * ( AAA )  * ( C>=HM)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
