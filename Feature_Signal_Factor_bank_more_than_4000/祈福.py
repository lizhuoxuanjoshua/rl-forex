...
#开始
    ...
    Y1=(sma(np.abs(L-REF(L,1)),34,1))
    Y2=(sma(np.maximum(L-REF(L,1),0),34,2))
    W=(-(EMA(np.where(L<= llv(L,34),Y1/Y2,-3),9)))
    A=(-(EMA(np.where(L<= llv(L,8),Y1/Y2,-3),9)))
    Y3=(sma(np.abs(H-REF(H,1)),34,1))
    Y4=(-sma(np.minimum(H-REF(H,1),0),34,2))
    X=(3-(EMA(np.where(H>= hhv(H,34),-Y3/Y4,3),9)))
    B=(3-(EMA(np.where(H>= hhv(H,8),-Y3/Y4,3),9)))
    DIFL=(EMA(V,12)-EMA(V,26))
    DIF=(EMA(C,12)-EMA(C,26))
    底BL=(DIFL>REF(DIFL,1) ) * ( DIF<REF(DIF,1))
    顶BL=(DIFL<REF(DIFL,1) ) * ( DIF>REF(DIF,1))
    a=((((W<0 ) * ( W<REF(W,1))) ) + ( ((A<0 ) * ( A<REF(A,1)))) ) * ( 底BL)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
