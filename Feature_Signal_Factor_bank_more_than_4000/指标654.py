...
#开始
    ...
    均价=(AMOUNT/(VOL*100))
    A3=(np.where(1.25*MA(hhv(MA(均价,4),10),3)-0.25*hhv(MA(均价,4),10)>hhv(MA(均价,4),10),hhv(MA(均价,4),10),1.25*MA(hhv(MA(均价,4),10),3)-0.25*hhv(MA(均价,4),10)))
    A4=(np.where(1.25*MA(llv(MA(均价,4),10),3)-0.25*llv(MA(均价,4),10)<llv(MA(均价,4),10),llv(MA(均价,4),10),1.25*MA(llv(MA(均价,4),10),3)-0.25*llv(MA(均价,4),10)))
    A5=(np.where(CROSS(CLOSE,A4)+CROSS(CLOSE,MA(CLOSE,13))==2,1,0))
    A6=(np.where(CROSS(CLOSE,MA(CLOSE,13))+CROSS(CLOSE,A3)==2,1,0))
    a=(A5+A6>0)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
