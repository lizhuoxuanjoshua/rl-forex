...
#开始
    ...
    A2=EMA((EMA(A1,3)/EMA(VOL,3)+EMA(A1,6)/EMA(VOL,6)+EMA(A1,12)/EMA(VOL,12)+EMA(A1,24)/EMA(VOL,24))/4,13)

    A3=1.06*A2

    A4=EMA(A3-(EMA(CLOSE,3)-A3),5)

    AAK=np.where(A4<=A3,A4,A3)

    A5=2*A3-AAK

    AAA=np.where(A5>=A3,A5,A3)

    观望=np.where(AAK==AAA,AAK,0)
