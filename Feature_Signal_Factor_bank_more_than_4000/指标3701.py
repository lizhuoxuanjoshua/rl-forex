...
#开始
    ...
    阳涨幅=(4)
    阳倍量=(2)
    单阳=(C>O*(1+阳涨幅/100)) * ( VOL>REF(VOL,1)*阳倍量)
    高不破价=(REF(C,barslast(单阳)))
    中不破价=(REF((C+L)/2,barslast(单阳)))
    低不破价=(REF(L,barslast(单阳)))
    单阳不破 =(barslast( 单阳 )<11 ) * ( ((llv(L,barslast(单阳))>低不破价) ) + ( (llv(L,barslast(单阳))==低不破价)))
    两天=((llv(L,barslast(单阳))>低不破价) ) + ( (llv(L,barslast(单阳))==低不破价
))
    a=(llv(L,barslast(单阳)-2)>中不破价 ) + ( llv(L,barslast(单阳)-2)==中不破价)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
