...
#开始
    ...
    R4=(np.power(MA(CLOSE,5),2)+MA(CLOSE,5))
    R5=(np.power(MA(LOW,5),2)+MA(LOW,5))
    R6=(np.power(MA(HIGH,5),2)+MA(HIGH,5))
    ...
    AA=((20*MID+19*REF(MID,1)+18*REF(MID,2)+17*REF(MID,3)+16*REF(MID,4)+15*REF(MID,5)+14*REF(MID,6)+13*REF(MID,7)+12*REF(MID,8)+11*REF(MID,9)+10*REF(MID,10)+9*REF(MID,11)+8*REF(MID,12)+7*REF(MID,13)+6*REF(MID,14)+5*REF(MID,15)+4*REF(MID,16)+3*REF(MID,17)+2*REF(MID,18)+REF(MID,20))/210)
    MA5=(MA(CLOSE,5))
    MA10=(MA(CLOSE,10))
    AB=(EMA(((R21+R22)/2/1.1),1))
    a=(C>O ) * ( O<=AA ) * ( C>=AA ) * (  AA>REF(AA,1) ) * ( MA5>MA10 ) * ( MA5>REF(MA5,1) ) * ( MA10>REF(MA10,1) ) * ( AB>REF(AB,1))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
