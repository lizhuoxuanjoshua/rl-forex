...
#开始
    ...
    VAR1=(REF(LOW,1))
    VAR2=(sma(np.abs(LOW-VAR1),3,1)/sma(np.maximum(LOW-VAR1,0),3,1)*100)
    VAR3=(EMA(np.where(CLOSE*1.2,VAR2*5,VAR2/10),3))
    VAR4=(llv(LOW,38))
    VAR5=(hhv(VAR3,38))
    VAR6=(np.where(llv(LOW,90),1,0))
    VAR7=(EMA(np.where(LOW<=VAR4,(VAR3+VAR5*2)/2,0),3)/618*VAR6)
    VAR8=(((C-llv(L,20))/(hhv(H,20)-llv(L,20)))*100)
    VAR9=(sma(VAR8,13,8))
    吸筹线=(VAR7)
    趋势=((sma(VAR9,13,8)))
    反趋势=(100*(hhv(HIGH,60)-CLOSE)/(hhv(HIGH,60)-llv(LOW,60)))
    MB1=(hhv(反趋势,20))
    MB2=(llv(反趋势,30))
    顶线=(REF(MB1,1))
    MB4=(REF(MB2,1))
