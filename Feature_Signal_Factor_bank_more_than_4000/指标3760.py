...
#开始
    ...
    ZT=((REF(C,1)*1.095-C)<0.01 ) * ( C==H)
    LC=(REF(CLOSE,1))
    CZX=(sma(np.maximum(CLOSE-LC,0),2,1)/sma(np.abs(CLOSE-LC),2,1)*1002)
    ZCX2=(45)
    CDX=(25)
    PP2=(CZX<ZCX2  ) * ( REF(CZX,1)>ZCX2  ) * ( C>MA(C,21))
    PP3=(CZX<CDX  ) * ( REF(CZX,1)>CDX  ) * ( C>MA(C,21))
    a=(count(ZT,8)>=1 ) * ( REF(((PP2) ) + ( (PP3)) ,0))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
