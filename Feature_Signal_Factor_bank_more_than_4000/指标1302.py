...
#开始
    ...
    红K=(np.where(O<C,(C-O)/REF(C,1),0))
    绿K=(np.where(O>C,(O-C)/REF(C,1),0))
    AA=(count(红K-绿K,5))
    KC=(0.382*(H+L-O-C)+0.382*O+0.618*C+0.618*np.where(C<200,AMOUNT/VOL/100-C,(H+L-O-C)))
    KC1=(MA(KC-REF(KC,1),5))
    DIF=(EMA(KC1,12)-EMA(KC1,26))
    DEA=(EMA(DIF,9))
    AAKC=(KC+AA)
    KKC1=(MA(AAKC-REF(AAKC,1),5))
