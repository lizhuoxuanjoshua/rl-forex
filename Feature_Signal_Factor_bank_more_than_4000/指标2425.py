...
#开始
    ...
    TH=(count(np.where(CLOSE>REF(CLOSE,1),VOL,0),21))
    TL=(count(np.where(CLOSE<REF(CLOSE,1),VOL,0),21))
    TQ=(count(np.where(CLOSE==REF(CLOSE,1),VOL,0),21))
    VAR0=(100*(TH*2+TQ)/(TL*2+TQ))
    VAR1=(MA(VAR0,5))
    W=(CROSS(VAR0,VAR1))
    WL1=(VAR1<100)
    WL2=(VAR1<150 ) * ( VAR1>REF(VAR1,1))
    WLT1=(W ) * ( WL1)
    WLT2=(W ) * ( WL2)
    TJ1=(FILTER(WLT1,5))
    TJ2=(FILTER(WLT2,8))
