...
#开始
    ...
    多方能量=7*sma((CLOSE-llv(LOW,27))/(hhv(HIGH,27)-llv(LOW,27))*30,4,1)-3*sma(sma((CLOSE-llv(LOW,27))/(hhv(HIGH,27)-llv(LOW,27))*30,4,1),3,1)-sma(sma(sma((CLOSE-llv(LOW,27))/(hhv(HIGH,27)-llv(LOW,27))*30,4,1),3,1),2,1)

    

    空方能量=EMA(MA(多方能量,5),7)
    ...
    (CROSS(多方能量, 空方能量)) * (REF(多方能量, 1) < REF(空方能量, 1)) * (CROSS(AA, VAR2)) * (多方能量 > 空方能量);
