...
#开始
    ...
    X_2=((CLOSE-MA(CLOSE,34))/MA(CLOSE,34)*100>(-15) ) * ( (CLOSE-MA(CLOSE,34))/MA(CLOSE,34)*100<(-13) ) * ( CLOSE/REF(CLOSE,1)<0.95 ) * ( CLOSE>LOW ) * ( LOW<=llv(LOW,5))
    X_4=(X_2 ) * ( (CLOSE-LOW)/CLOSE<0.035 ) * ( REF(CLOSE,1)/REF(CLOSE,2)<1.03 )
    a=(X_4 ) * ( (CLOSE-MA(CLOSE,34))/MA(CLOSE,34)*100>(-14.2) ) * ( (CLOSE-MA(CLOSE,34))/MA(CLOSE,34)*100<(-13.5)  ) * ( AMOUNT>100000000)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
