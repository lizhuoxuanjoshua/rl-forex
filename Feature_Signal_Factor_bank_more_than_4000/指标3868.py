...
#开始
    ...
    M5=(MA(C,5))
    M10=(MA(C,10))
    TJ1=(EVERY(V<REF(V,3),2)) * (( np.minimum(C,O)<REF(np.minimum(C,O),1))) + ( (L<REF(L,1)
))
    GL=((M5/M10-1)*100)
    TJ2=((GL<18 ) * ( M5>M10 ) * ( M10>=REF(M10,1)))
    TJ3=(((RANGE(REF(C,1),REF(M10,1),REF(M5,1))) ) + ( (RANGE(REF(O,1),REF(M10,1),REF(M5,1)))))
    a=(REF(TJ1,1) ) * ( TJ3 ) * ( TJ2 ) * ( CROSS(C+0.01>=M5,0.5))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
