...
#开始
    ...
    X_1=(REF(OPEN<CLOSE,3) ) * ( REF(HIGH,3)/REF(CLOSE,4)>=1.005)
    X_2=(REF(OPEN<CLOSE,2) ) * ( REF(HIGH,2)/REF(CLOSE,3)>=1.001)
    X_3=(REF(OPEN>CLOSE,1))
    X_4=(OPEN<CLOSE)
    X_5=(VOL<=REF(VOL,2))
    X_6=(HIGH-CLOSE<CLOSE-OPEN)
    X_7=(CLOSE-OPEN>OPEN-LOW)
    X_8=(VOL<MA(VOL,10))
    X_9=(HIGH/REF(CLOSE,1)>1.005)
    X_10=(HIGH/LOW<1.12)
    X_11=(HIGH/OPEN>1.036)
    X_12=(5)
    X_13=(10)
    X_14=(20)
    X_15=(60)
    ...
    X_18=(MA(VOL,X_14))
    X_19=(MA(VOL,X_15))
    X_20=(np.minimum(REF(VOL,1),REF(VOL,2)))
    X_21=(np.minimum(REF(VOL,3),REF(VOL,4)))
    X_22=(np.minimum(REF(VOL,5),REF(VOL,6)))
    X_23=(np.minimum(REF(VOL,7),REF(VOL,8)))
    X_24=(np.minimum(REF(VOL,9),REF(VOL,10)))
    X_25=(np.minimum(REF(VOL,11),REF(VOL,12)))
    X_26=(np.minimum(REF(VOL,13),REF(VOL,14)))
    X_27=(np.minimum(REF(VOL,15),REF(VOL,16)))
    X_28=(np.minimum(REF(VOL,17),REF(VOL,18)))
    X_29=(np.minimum(REF(VOL,19),REF(VOL,20)))
    X_30=(np.minimum(np.minimum(X_20,X_21),np.minimum(X_22,X_23)))
    X_31=(np.minimum(np.minimum(X_24,X_25),np.minimum(X_26,X_27)))
    X_32=(np.minimum(X_28,X_29))
    X_33=(np.minimum(X_30,X_31))
    X_34=(np.minimum(X_33,X_28))
    X_35=(VOL<=X_34*3.5)
    X_36=(REF(HIGH/CLOSE,1)>=1.02)
    X_37=(REF(HIGH/OPEN,3)<=1.053)
    a=(X_2 ) * ( X_3 ) * ( X_4 ) * ( X_5 ) * ( X_6 ) * ( X_7 ) * ( X_8 ) * ( X_10 ) * ( X_11 ) * ( X_35 ) * ( X_36 ) * ( X_37)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
