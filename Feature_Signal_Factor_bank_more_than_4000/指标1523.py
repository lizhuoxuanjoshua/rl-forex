...
#开始
    ...
    OINC=(np.round(C))
    OIN1C=(C*100-OINC*100)
    OLR0C=(np.mod(OINC,6))
    OLRC=(np.where(OLR0C==0,6,OLR0C))
    OLR01C=(np.mod(OIN1C,6))
    OLR1C=(np.where(OLR01C==0,6,OLR01C))
    OXL1C=(np.mod(OLRC+OLR1C-1,6))
    XLRC=(np.where(OXL1C==0,6,OXL1C))
    M5=(MA(C,5))
    M10=(MA(C,10))
    MKD=(M5>M10 ) * ( L<M5)
    MK=(L<M5 ) * ( L<M10 ) * ( H>M5 ) * ( H>M10)
    MKK=(M5<M10 ) * ( H<M5)
    MKK1=(C<O ) * ( REF(C,1)<REF(O,1) ) * ( REF(C,2)<REF(O,2))
    XD=(C>O ) * ( C>M10)
    XK=(C<O ) * ( C<M10)
    a=(XLRC==1 ) * ( MK ) * ( XD)
#结束
