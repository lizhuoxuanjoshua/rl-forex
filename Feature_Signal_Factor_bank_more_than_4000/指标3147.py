...
#开始
    ...
    X_2=(MA(CLOSE,13))
    X_3=(MA(CLOSE,34))
    X_4=(CLOSE-REF(CLOSE,1))
    X_5=(100*EMA(EMA(X_4,6),6)/EMA(EMA(np.abs(X_4),6),6))
    X_6=(CLOSE<X_1 ) * ( X_1<X_2 ) * ( X_2<X_3)
    a=(CLOSE>REF(hhv(HIGH,2),1) ) * ( (llv(X_5,2)==llv(X_5,7) ) * ( count(X_5<0,2) ) * ( CROSS(X_5,MA(X_5,2))) )
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
