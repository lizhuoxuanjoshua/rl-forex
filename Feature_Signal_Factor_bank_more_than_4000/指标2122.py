...
#开始
    ...
    三日收盘=((C+REF(C,2)+REF(C,3))/3)
    强势线=(EMA((三日均高+三日收盘)/2,7))
    支撑线=EMA((LL+REF(LL,2)+REF(LL,3))/3,7)
    a=(((CROSS(C,强势线) ) * ( CROSS(C,支撑线))) ) + ( ((支撑线>REF(支撑线,1) ) * ( L<支撑线 ) * ( C>强势线)) ) + ( (REF(CROSS(C,支撑线),1)==1 )) * ( CROSS(C,强势线))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
