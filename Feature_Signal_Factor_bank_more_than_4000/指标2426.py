...
#开始
    ...
    VAR1=(llv(LOW,10))
    VAR2=(hhv(HIGH,25))
    天机1=((EMA((CLOSE-VAR1)/(VAR2-VAR1)*4,4)*27.5))
    VAR3=(REF(CLOSE,2))
    天机2=(sma(np.maximum(CLOSE-VAR3,0),7,1)/sma(np.abs(CLOSE-VAR3),7,1)*100)
    T2=((CLOSE-llv(LOW, 18))/(hhv(HIGH, 18)-llv(LOW, 18))*100)
    T3=(sma(T2, 9, 1))
    T4=(sma(T3, 3, 1))
    天机3=(3*T3-2*T4)
    天机线=(EMA(T3, 3))
    逃顶=(天机2< REF(天机2,1) ) * ( 天机2>79)
    a=(REF(天机1,1)>天机1 ) * ( REF(天机3,1)<天机3 ) * ( HSL>=1 ) * ( 天机线<天机2 ) * ( REF(天机3,1)>REF(天机3,2) ) * ( 天机3>40 ) * ( 天机3<77 ) * ( 天机1<86 ) * ( count(逃顶,3)<=0)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
