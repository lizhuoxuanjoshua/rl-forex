...
#开始
    ...
    VARY13=(hhv(HIGH,21))
    VARY14=(llv(LOW,21))
    D=( EMA((CLOSE-VARY14)/(VARY13-VARY14)*100,5))
    K=( EMA((CLOSE-VARY14)/(VARY13-VARY14)*100,20))
    JM=(1)
    ...
    VAR9=(3*VAR7-2*VAR8)
    VARA=(np.power(MA(CLOSE,5),2)+MA(CLOSE,5))
    VARB=(np.power(MA(LOW,5),2)+MA(LOW,5))
    VARC=(np.power(MA(HIGH,5),2)+MA(HIGH,5))
    ...
    VAR1F=(count(AMOUNT,5)/count(VOL,5)/100)
    ...
    MN=(MA(MM,3))
    J=(CROSS(DX1,0) ) * (( count(MN<-12,2)>=1) ) + ( (CROSS(DX1,ZX) )) * ( count(MN<-12,2)>=1)
    a=(CROSS(DX1,ZX) ) * (( count(MN<GLL,2)>=1) ) + ( ((CROSS(DX1,0) ) * ( count(MN<GLL,1)>=1)) ) + ( ((CROSS(DX1,DX2) ) * ( count(MN<GLL,1)>=1)))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
