...
#开始
    ...
    M10=(MA(C,10))
    M30=(MA(C,30))
    HH=(np.maximum(np.maximum(M5,M10),M30))
    LL=(np.minimum(np.minimum(M5,M10),M30))
    NH=((HH/LL-1)*100)
    ZF=((H-L)/REF(C,1)*100)
    A=((((H-np.maximum(C,O))>(np.maximum(C,O)-np.minimum(C,O))*2 ) * ( L<np.minimum(C,O))) ) + ( (((np.minimum(C,O)-L)>(np.maximum(C,O)-np.minimum(C,O))*2 ) * ( H>np.maximum(C,O))
))
    B=((REF(C,1)-C)/C*100)
    D=(O>LL)
    E=(REF(np.minimum(C,O),1)/O)
    K1=(ZF<5 ) * ( A)
    K2=(C<O ) * ( B>2 ) * ( REF(np.minimum(C,O),1)>(O-C)/2+C ) * ( D ) * ( E<1.02)
    K3=(ZF<5 ) * ( A ) * ( np.maximum(C,O)<REF((O-C)/2+C,1))
    a=(REF(K1,2) ) * ( REF(K2,1) ) * ( K3 ) * ( NH<9.8)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
