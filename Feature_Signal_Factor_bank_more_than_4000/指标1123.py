...
#开始
    ...
    R1=((OPEN+2*CLOSE)/3)
    A1=((R1*6+REF(R1,1)*5+REF(R1,2)*4+REF(R1,3)*3+REF(R1,4)*2+REF(R1,5)*1)/21)
    BA1=((A1-REF(A1,1))/REF(A1,1)*100)
    AB=(HSL/REF(HSL,2)*100)
    AB2=(HSL/REF(HSL,1)*100)
    ZF1=((CLOSE-REF(CLOSE,1))/REF(CLOSE,1)*100)
    a=(AB<55) * (REF(AB2,1)>50) * (REF(HSL,2)>2.5) * (HSL<6) * (AB<40) * (ZF1<3) * (BA1<-0.3)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
...
if __name__=='__main__':
    ...
    ktime = np.array(合成k线[0])
    high = np.array(合成k线[1])
    o = np.array(合成k线[2])
    low = np.array(合成k线[3])
    c = np.array(合成k线[4])
    volume = np.array(合成k线[5])
    amount = np.array(合成k线[6])
    均线 = np.array(合成k线[7])
    收盘 = np.array(合成k线[8])
    开盘 = np.array(合成k线[9])
    切片位置 = np.array(合成k线[10])
    subindex = np.array(合成k线[11])
    turn=np.array(合成k线[12])
    isST=np.array(合成k线[13])
