...
#开始
    ...
    LOWV=(llv(LOW,21))
    HIGHV=(hhv(HIGH,21))
    RSV=(EMA((CLOSE-LOWV)/(HIGHV-LOWV)*100,5))
    K=(EMA(RSV,5))
    D=(MA(K,5))
    A1=(REF(C,1))
    A2=(sma(np.maximum(C-A1,0),5,1)/sma(np.abs(C-A1),5,1)*1000)
    A4=(A2-llv(A2,10))
    A5=((MA(A4,2)*3+A4*13)/16)
    A6=(np.where(A5>13,MA(A5,2),A5)/6)
    RSI=(sma(np.maximum(C-A1,0),12,1)/sma(np.abs(C-A1),12,1)*100)
    LIN=(llv(MA(RSI,3),12))
    YAN=((hhv(MA(RSI,3),12)+llv(MA(RSI,3),12))/2)
    FANG=((CROSS(A6,1) ) * ( (A6<30)) * ( RSI<35 ) * ( RSI>=LIN ) * ( RSI<YAN))
    ...
    VAR5=((VAR2+2*VAR3+3*VAR4)/7)
    VAR6=(MA(VAR5,1))
    AA=( np.where(VAR6<-8,1,0))
    a=( FANG==30 ) * ( D<20 )
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
