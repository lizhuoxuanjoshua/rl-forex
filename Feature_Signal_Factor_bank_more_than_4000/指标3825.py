...
#开始
    ...
    A6=(MMS>MMM)
    涨买入=((A1 ) * ( A2 ) * ( A3 ) * ( A4 ) * ( A5 ) * ( A6) ) * ( REF(A1 ) * ( A2 ) * ( A3 ) * ( A4 ) * ( A5 ) * ( A6))
    上涨=(A1 ) * ( A2 ) * ( A3 ) * ( A4 ) * ( A5 ) * ( A6)
    HM=(MA(HIGH,30)*(1+15/100))
    M1=(MA(C,5))
    M2=(MA(C,10))
    M3=(MA(C,20))
    M4=(MA(C,60))
    AAA=((C>M1)) * ((M1>M2)) * ((M2>M3))
    #) * ((M1/M4<1.35)) * ((M1>REF(M1,1))) * ((M2>REF(M2,1))) * ((M3>REF(M3,1))})
    DIFF1=(EMA(CLOSE,12) - EMA(CLOSE,26))
    DEA1  =( EMA(DIFF1,9))
    MACD1 =( 2*(DIFF1-DEA1))
    MACDH=(CROSS(MACD1,0))
    DIFF22=(EMA(CLOSE,12) - EMA(CLOSE,26))
    DEA22=(EMA(DIFF22,9))
    JJ =(C*3+H+L+O)/6
    SK =( EMA(JJ,13) - EMA(JJ,21))
    SD =( EMA(SK,3))
    SP =( EMA(SK,10))
    AAA1=(EMA(C,7)-EMA(C,21))
    B1=(EMA((0.668*REF(AAA1,1)+0.333*AAA1),1))
    X1=(V/count(V,13))
    X2=(DMA(C,X1))
    X3=((C-X2)/X2*40)
    L2=(MA(AMOUNT/(100*V),13))
    CYS13=( (C-L2)/L2*100)
    XDFHTH=(REF(AAA1,1)<=REF(B1,1))
    点火=(CYS13>4.5 ) * ( DIFF22>DEA22 ) * ( C>=EMA(C,5)  ) * ( AAA1-B1>0)
    MA5=(MA(C,5))
    压力=(hhv(MA5,60))
    小牛涨大=(O>REF(C,1)*1.02 ) * ( C>O ) * ( C>=压力 ) * ( 压力>=REF(C,1)  ) * ( MA5>REF(MA5,1) )
    VAR3AA=(np.where((CLOSE>REF(CLOSE,1)),88,0))
    VAR4AA=(((CLOSE)/(REF(CLOSE,1))>1.05) ) * ( ((HIGH)/(CLOSE)<1.01) ) * ( (VAR3AA>0))
    大胆搏=(FILTER((VAR4AA>90),45))
    最强火=(CROSS(MACD1,0) ) * ( AAA ) * ( 涨买入 ) * ( C>=HM)
    a=((大胆搏 ) * ( 点火 ) * ( 涨买入) ) + ( ((大胆搏 )) * ( 点火 ) * ( 小牛涨大) ) * ( AAA)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
