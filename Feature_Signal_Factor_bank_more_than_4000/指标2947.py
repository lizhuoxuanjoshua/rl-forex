...
#开始
    ...
    VOLUME=(VOL)
    MAVOL1=(MA(VOLUME,1)*1.2)
    MAVOL2=(MA(VOLUME,2)*1.2)
    MAVOL13=(MA(VOLUME,13)*1.2)
    MAVOL21=(MA(VOLUME,21)*1.2)
    MAVOL34=((MA(VOLUME,34)*1.2
))
    MAVOL43=(MA(VOLUME,43)*1.2)
    MAVOL55=(MA(VOLUME,55)*1.2)
    铭180=(MA(3*sma((CLOSE-llv(LOW,27))/(hhv(HIGH,27)-llv(LOW,27))*100,5,1)-2*sma(sma((CLOSE-llv(LOW,27))/(hhv(HIGH,27)-llv(LOW,27))*100,5,1),3,1),180))
    MAVOL120=(MA(VOLUME,120)*1.2)
    MAVOL180=(MA(VOLUME,180)*1.2)
    a=(count(MAVOL13>REF(MAVOL13,1),3)==3 ) * ( count(MAVOL21>REF(MAVOL21,1),2)==2) * ( count(MAVOL34>REF(MAVOL34,1),2)==2 ) * ( count(MAVOL43>REF(MAVOL43,1),1)==1 ) * ( count(MAVOL55>REF(MAVOL55,1),1) ) * ( count(MAVOL120>REF(MAVOL120,1),1) ) * ( count(铭180>REF(铭180,1),3)==3)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
