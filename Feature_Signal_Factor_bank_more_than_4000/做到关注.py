...
#开始
    ...
    VAR01=sma(np.maximum(np.maximum(HIGH-LOW,np.abs(HIGH-REF(CLOSE,1))),np.abs(LOW-REF(CLOSE,1))),15,1)
    ...
    VAR04=sma(np.where((VAR02>0) * (VAR02>VAR03),VAR02,0),15,1)
    

    VAR05=sma(np.where((VAR03>0) * (VAR03>VAR02),VAR03,0),15,1)
    ...
    VAR09=4*sma((CLOSE-llv(LOW,5))/(hhv(HIGH,5)-llv(LOW,5))*100,5,1)-3*sma(sma((CLOSE-llv(LOW,5))/(hhv(HIGH,5)-llv(LOW,5))*100,5,1),3.2,1)
    ...
    提示买入=np.where(VAR09<=8,8,0)

    

    买入=np.where(CROSS(VAR09,8),110,0)
