...
#开始
    ...
    B1=((CLOSE-MA(CLOSE,6))/MA(CLOSE,6)*100)
    B2=((CLOSE-MA(CLOSE,13))/MA(CLOSE,13)*100)
    B3=((CLOSE-MA(CLOSE,20))/MA(CLOSE,20)*100)
    B4=((CLOSE-MA(CLOSE,30))/MA(CLOSE,30)*100)
    a=(B1<-7 ) * ( B2<-9 ) * ( B3<-11 ) * ( B4<-13)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
