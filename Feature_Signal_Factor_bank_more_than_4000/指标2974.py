...
#开始
    ...
    MTR=(EMA(np.maximum(np.maximum(HIGH-LOW,np.abs(HIGH-REF(CLOSE,1))),np.abs(REF(CLOSE,1)-LOW)),12))
    HD =(HIGH-REF(HIGH,1))
    LD =(REF(LOW,1)-LOW)
    条件1=(((HD > 0) ) * ( (HD > LD)))
    条件2=(((LD > 0) ) * ( (LD > HD)))
    DMP=(EMA(np.where(条件1,HD,0),12))
    DMM=(EMA(np.where(条件2,LD,0),12))
    PDI=( DMP*100/MTR)
    MDI=( DMM*100/MTR)
    ADX=( EMA(np.abs(MDI-PDI)/(MDI+PDI)*100,2))
    ADXR=(EMA(ADX,2))
    ADD=(ADX>REF(ADX,1) ) * ( REF(ADX,1)<=REF(ADX,2))
    ...
    BB= (BBI1 + BBI2) / 2
    TBB = np.multiply(BARSLAST(BB < REF(BB, 1)),REF(BB, 1) >= REF(BB, 2))
    a= ( CROSS(C,BBI1))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
