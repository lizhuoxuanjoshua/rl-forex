...
#开始
    ...
    VARA=(count(CLOSE*VOL*100,4)/count(VOL*100,4))
    VARB=((VARA*100)/100)
    VARC=(count(CLOSE*VOL*100,7)/count(VOL*100,7))
    VARD=((VARC*100)/100)
    VARE=(count(CLOSE*VOL*100,28)/count(VOL*100,28))
    VARF=((VARE*100)/100)
    ...
    VAR1A=(OPEN)
    VAR1B=(CLOSE)
    a=(VAR1A<=VAR4 ) * ( VAR1A<=VAR5 ) * ( VAR1A<=VAR6 ) * ( VAR1B>=VAR4 ) * ( VAR1B>=VARF ) * ( VAR16>0 ) * ( VAR19>0 ) * ( VAR3==1)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
