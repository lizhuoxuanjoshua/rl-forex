...
#开始
    ...
    X_1=(OPEN>CLOSE)
    X_2=(LAST(X_1,4,3))
    X_3=(REF(CLOSE<OPEN,1))
    X_4=(REF(CLOSE,1)<CLOSE)
    X_5=(REF(OPEN/CLOSE,1)<1.06)
    X_6=((HIGH+LOW+CLOSE)/3)
    X_7=((X_6-MA(X_6,14))/(0.013*AVEDEV(X_6,14)))
    X_8=(X_7>(-250))
    X_9=(X_7>REF(X_7,1)*1.06)
    X_10=(CLOSE/REF(CLOSE,1)>1.005)
    X_11=(VOL<llv(VOL,18)*3.5)
    X_12=(CLOSE>REF(OPEN,1)*0.998)
    X_13=(REF(OPEN/CLOSE,1)<1.065)
    X_14=(REF(VOL,1)<REF(VOL,2)*2.5)
    X_15=(REF(LOW,2)>REF(LOW,3)*0.99)
    X_16=(HIGH<REF(HIGH,8)*0.995)
    X_17=(MA(CLOSE,60)/CLOSE>1.035)
    X_18=(LOW>REF(LOW,2)*0.995)
    X_19=(OPEN/CLOSE>1.07)
    X_20=(count(X_19,4)==0)
    X_21=(OPEN>REF(CLOSE,3)*0.998)
    X_22=(HIGH/LOW<1.07)
    X_23=(REF(HIGH,1)>REF(LOW,2))
    X_24=(OPEN>REF(CLOSE,1))
    X_25=(count(X_24,8)>=1)
    X_26=(REF(VOL<=EMA(VOL,110)*0.7,1))
    X_27=(HIGH/REF(CLOSE,1)>1.015 ) * ( CLOSE>REF(HIGH,1))
    a=(X_2 ) * ( X_3 ) * ( X_4 ) * ( X_5 ) * ( X_8 ) * ( X_10 ) * ( X_11 ) * ( X_12 ) * ( X_12 ) * ( X_14 ) * ( X_15 ) * ( X_16 ) * ( X_9 ) * ( X_17 ) * ( X_18 ) * ( X_20 ) * ( X_21 ) * ( X_22 ) * ( X_23 ) * ( X_25 ) * ( X_26 ) * ( X_27)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
