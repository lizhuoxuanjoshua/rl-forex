...
#开始
    ...
    A5=(MA(C,60))
    A6=(MA(C,120))
    CC=(np.where(A1>A2,6,0)+np.where(A2>A3,6,0)+np.where(A3>A4,6,0)+np.where(A4>A5,6,0)+np.where(A5>A6,6,0))
    DD=(np.where(C<A2,0,30-(C/A2-1)*150))
    DIFF=(EMA(CLOSE,12)-EMA(CLOSE,26))
    DEA=EMA(DIFF,9)
    MACD=(2*(DIFF-DEA))
    AA1=(REF(MACD,1)<REF(MACD,2) ) * ( REF(MACD,2)<REF(MACD,3) ) * ( REF(MACD,3)<REF(MACD,4))
    AA2=(MACD>=0 ) * ( count(MACD>REF(MACD,1),1)==1)
    AA3=(EMA(C,30)>=REF(EMA(C,30),1) ) * (EMA(C,5)>EMA(C,10) ) * ( EMA(C,10)>EMA(C,30))
    AA4=CROSS(C,EMA(C,30)) +  (C>EMA(C,30))
    AA5=(AA1 ) * ( AA2 ) * ( AA3 ) * ( AA4)
    AA6=(AA1 ) * ( AA2 ) * ( AA3 ) * ( AA4 ) * ( np.abs((DIFF-DEA)/C)<0.018)
    AA7=(AA1 ) * ( AA2 ) * ( AA3 ) * ( AA4 ) * ( MACD<0.10)
    a=(AA+CC+DD ) * (( AA5) ) + ( (AA6) ) + ( (AA7  )) * ( C>REF(H,1) ) * ( C>REF(H,2) ) * ( REF(O,1)>REF(C,1)) * ( REF(O,2)>REF(C,2) ) * ( V>REF(V,1)*2)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
