...
#开始
    ...
    牛熊线=(MA(CLOSE,26))
    生命线=(EMA(CLOSE,55))
    JD1=(np.maximum(np.maximum(趋势线,牛熊线),生命线))
    JX1=(np.minimum(np.minimum(趋势线,牛熊线),生命线))
    JD2=(np.maximum(np.maximum(MA(C,5),MA(C,10)),MA(C,20)))
    JX2=(np.minimum(np.minimum(MA(C,5),MA(C,10)),MA(C,20)))
    a=((JX2/JD2*100>98.5 ) * ( C>JD2 ) * ( O<JX2 ) * ( REF(MA(C,20),1)/MA(C,20)<1) ) * ( CROSS(J,D))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
