...
#开始
    ...
    N2=(8)
    LC=(REF(CLOSE,1))
    RSI1=(sma(np.maximum(CLOSE-LC,0),N1,1)/sma(np.abs(CLOSE-LC),N1,1)*100)
    RSI2=(sma(np.maximum(CLOSE-LC,0),N2,1)/sma(np.abs(CLOSE-LC),N2,1)*100)
    a=(CROSS(RSI1,20) ) * ( (CROSS(RSI1,RSI2) ) * ( RSI1<50))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
