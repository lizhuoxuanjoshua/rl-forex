...
#开始
    ...
    N1=(10)
    LC=(REF(CLOSE,1))
    RSI5=(sma(np.maximum(CLOSE-LC,0),N,1)/sma(np.abs(CLOSE-LC),N,1)*100)
    WR10=(100*(hhv(HIGH,N1)-CLOSE)/(hhv(HIGH,N1)-llv(LOW,N1)))
    NEWVOL=(RSI5-WR10)
    a=((NEWVOL>REF(NEWVOL,2)) ) * ( ( REF(NEWVOL,1)<REF(NEWVOL,3) ) * ( NEWVOL<0 ))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
