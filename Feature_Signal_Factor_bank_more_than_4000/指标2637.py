...
#开始
    ...
    VAR1=((C+H+O+L)/4)
    卖出=(MA(VAR1,20)*(1+80/1000))
    买入=(MA(VAR1,32)*(1-100/1000))
    标准=(买入+(卖出-买入)*2/3)
    赢=(MA(C,1))
    ...
    趋势轨1=(EMA(赢在龙头,12))
    趋势轨2=(EMA(赢在龙头,19))
    赢在=((趋势轨1-REF(趋势轨1,1))/REF(趋势轨1,1)*100)
    ...
    散户=(MA(换手,34))
    均量=((主力+大户+中户+散户)/4)
    机构意愿=(np.subtract(主力,均量))
    大户意愿=(np.subtract(中户,均量))
    散户意愿=(np.subtract(散户,均量))
    a=(趋势1拐点 ) * ( (散户意愿>=0 ) * ( 大户意愿<0) ) * ( ((机构意愿>0) ) + ( (机构意愿<0)) ) * ( 止损线A>REF(止损线A,1) ) * ( H<标准)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
