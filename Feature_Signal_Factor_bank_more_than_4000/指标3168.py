...
#开始
    ...
    X_4=(CLOSE==OPEN)
    X_5=(count(VOL,5)/X_1)
    X_6=(np.where(X_5>0.3,1,0))
    X_7=(np.where(X_2 *X_6,VOL,0))
    X_8=(np.where(X_3 *X_6,VOL,0))
    X_9=(np.where(X_4 *X_6,VOL,0))
    X_10=(count(X_8,3))
    X_11=(count(X_7,3))
    X_12=(X_10-X_11)
    X_13=(np.abs(X_10/X_12-1)<0.01)
    a=(X_13 ) * ( REF(X_10,2)>REF(X_12,2) ) * ( REF(X_13,1))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
