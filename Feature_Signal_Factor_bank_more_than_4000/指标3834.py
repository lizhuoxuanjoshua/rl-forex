...
#开始
    ...
    D=(MA(CLOSE,1)-MA(CLOSE,6))
    A1=(D>0)
    B=(A1>0)
    B1=(count(A1,8)-count(B,8))
    BN=(5)
    A2=(D<0)
    C1=(B1>=4)
    D1=(count(A2,10)==0)
    ...
    BVAR2=(80)
    E=(CROSS(BVAR1,BVAR2))
    RSI1DTC=(((sma(np.maximum(CLOSE - REF(CLOSE,1),0),6,1))/(sma(np.abs(CLOSE - REF(CLOSE,1)),6,1)))*(100))
    G=((RSI1DTC < 25))
    VAR1MMQZ=(((CLOSE-MA(CLOSE,6))/MA(CLOSE,6)*100+(CLOSE-MA(CLOSE,24))/MA(CLOSE,24)*100+(CLOSE-MA(CLOSE,32))/MA(CLOSE,32)*100)/3)
    G1=(VAR1MMQZ<-9 ) * ( VAR1MMQZ> REF(VAR1MMQZ,1))
    F=(( G) ) + ( (G1
))
    KTB=(sma((CLOSE-llv(LOW,9))/(hhv(HIGH,9)-llv(LOW,9))*100,3,1))
    DTB=(sma(KTB,3,1))
    JTB=(3*KTB-2*DTB)
    E1=(CROSS(CLOSE,REF(EMA(DMA((((HIGH+LOW)+(CLOSE*2))/4.15),0.9),3),1))) * ( JTB<55)
    a=((C/REF(C,1)>=1.03 ) * ( count(E1,3)>=1 ) * ( count(F,6)>=1 ) * ( count(E,10)>=1 ) * ( count(REF(C1,10),25)>=4 ) * ( count(REF(D1,10),25)>=4))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
