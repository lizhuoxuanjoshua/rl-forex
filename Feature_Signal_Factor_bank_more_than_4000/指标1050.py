...
#开始
    ...
    AA=((7*sma((CLOSE-llv(LOW,27))/(hhv(HIGH,27)-llv(LOW,27))*30,4,1)-3*sma(sma((CLOSE-llv(LOW,27))/(hhv(HIGH,27)-llv(LOW,27))*30,4,1),3,1)-sma(sma(sma((CLOSE-llv(LOW,27))/(hhv(HIGH,27)-llv(LOW,27))*30,4,1),3,1),2,1)))
    AAA=(EMA(MA(AA,5),7))
    a=(CROSS(AA,AAA) ) * ( AAA<45)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
