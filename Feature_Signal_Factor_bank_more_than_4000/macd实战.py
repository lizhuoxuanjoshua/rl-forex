...
#开始
    ...
    A2= barslast(REF((count(DEA<=0,A1+1)==A1+1) * (llv(DIFF,5)==llv(DIFF,A1+1)) * CROSS(DIFF,DEA),1))
    ...
    DIBL1=(llv(C,A3+1)==llv(C,A1+1)) * (llv(DIFF,A3+1)!=llv(DIFF,A1+1)) * (count(DEA<=0,A2+1)==A2+1) * CROSS(DIFF,DEA)

    

    a=CROSS(DIFF,DEA) * (DEA<=0) * (DIBL1==0)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
