...
#开始
    ...
    上通道=(MA(L+(H-L)/2,25)*1.15)
    下通道=(MA(L+(H-L)/2,25)*0.95)
    中轨=((上通道+下通道)/2)
    股价=(C)
    次上通道=((MA(C,5)+MA(O,5))/2*1.06)
    次下通道=((MA(C,5)+MA(O,5))/2*0.98)
    强弱线=((次上通道+次下通道)/2)
    涨停=((C-REF(C,1))/REF(C,1)*100>=9.92)
    不吓人=(C>=O ) * ( REF(C>=O,1))
    涨停数=(count(涨停,8)>2)
    上水=(CROSS(XYS0,0))
    下水=(CROSS(0,XYS0))
    抽脚=(((XYS0>REF(XYS0,1) ) * ( XYS0<0)) ) + ( ((XYS0>REF(XYS0,1) ) * ( REF(XYS0,1)<0)
))
    抽脚一次原始=(抽脚 ) * ( count(抽脚,barslast(下水))==1 ) * ( count(累加>39,13)>0)
    抽脚一次买点=(抽脚 ) * ( count(抽脚,barslast(下水))==1 ) * ( count(累加>39,13)>0 ) * ( ((REF(C>=O,barslast(下水))) ) + ( (REF(C>=O,barslast(下水)-1))))
    抽脚两次买点=( NOT(抽脚一次原始) ) * ( 抽脚 ) * ( REF(XYS0,1)<REF(XYS0,2) ) * ( XYS0>-1 ) * ( count(累加>39,13)>0 ) * ( C>O)
    抽脚买点=(((抽脚一次买点) ) + ( (抽脚两次买点)) ) * ( count(累加>39,13)>0)
