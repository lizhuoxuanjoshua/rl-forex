...
#开始
    ...
    VAR1=(1)
    VAR2=((CLOSE-llv(LOW,100))/(hhv(HIGH,100)-llv(LOW,100))*200)
    VAR3=(sma(VAR2,3,1))
    VAR4=(sma(VAR3,3,1))
    VAR5=(3*VAR3-2*VAR4)
    VAR6=(llv(LOW,5))
    ...
    VARA=(3*VAR8-2*VAR9)
    VARB=(np.power(MA(CLOSE,5),2)+MA(CLOSE,5))
    VARC=(np.power(MA(LOW,5),2)+MA(LOW,5))
    VARD=(np.power(MA(HIGH,5),2)+MA(HIGH,5))
    ...
    VAR20=(count(AMOUNT,5)/count(VOL,5)/100)
    ...
    VAR2B=(((VAR2A+VAR29+VAR28+VAR27+VAR21)/5+165)/4*VAR1)
    VAR2C=((EMA(EMA(EMA(VAR2B,15),10),5)-25)*2+VAR1)
    VAR2D=((VAR1B+VAR1C+VAR1D*30)/32/5-15*VAR1)
    VAR2E=(EMA(VAR2D,13)*VAR1)
    VAR2F=((VAR2B+VAR2D)/2/1.1*VAR1)
    AA=(VAR2F*VAR1)
    a=(CROSS(AA,0))
#结束
    买入 = a
