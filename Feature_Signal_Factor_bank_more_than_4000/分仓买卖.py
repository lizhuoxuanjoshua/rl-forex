...
#开始
    ...
    VAR1=np.maximum(MA1,np.maximum(MA2,np.maximum(MA3,MA4)))

    

    VAR2=np.minimum(MA1,np.minimum(MA2,np.minimum(MA3,MA4)))
    ...
    VAR1A=CROSS(MA(CLOSE,5),MA(CLOSE,30))

    

    a=VAR1A * (CLOSE>MA(CLOSE,250))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
