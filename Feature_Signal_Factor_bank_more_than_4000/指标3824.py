...
#开始
    ...
    MA1=(MA(VAR4,2))
    买1=(np.where(VAR4<=MA1,0,1))
    T1=( MA1<60 ) * ( 买1<1)
    VAR1A=((2*CLOSE+HIGH+LOW)/4)
    VAR2A=(llv(LOW,34))
    VAR3A=(hhv(HIGH,34))
    VAR4A=(EMA((VAR1A-VAR2A)/(VAR3A-VAR2A)*100,8))
    涨=(VAR4A)
    T2=( 涨>50)
    股池=(T1 ) * ( T2)
    T3=(CROSS(C,REF(EMA(DMA((((H+L)+(C*2))/4.15),0.9),3),1)))
    VAR1C=(((llv(LOW,5)*hhv(HIGH,5))/C/C)*100)
    T4=(CROSS(105,VAR1C))
    ...
    D=(sma(K,3,1))
    J=(3*K-2*D)
    T6=(J>REF(J,1) ) * ( REF(J,1)<REF(J,2))
    T7=(K>REF(K,1) ) * ( REF(K,1)<REF(K,2))
    T8=(C>MA(C,5))
    T9=(C>MA(C,10))
    最小=(np.minimum(MA(C,10),MA(C,20)))
    最大=(np.maximum(MA(C,10),MA(C,20)))
    T10=(最小*1.05>最大)
    T11=(MA(C,10)>MA(C,20))
    T12=(O<REF(C,1)*1.04)
    T13=(MA(C,60)*1.1>MA(C,20))
    a=((股池<1) ) * ( (REF(股池,1)>0)) * ( T3 ) * ( T4 ) * ( T6 ) * ( T7 ) * ( T8 ) * ( T9 ) * ( T10 ) * ( T11 ) * ( T12 ) * ( T13)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
