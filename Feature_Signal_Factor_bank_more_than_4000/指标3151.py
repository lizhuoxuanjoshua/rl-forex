...
#开始
    ...
    X_6=(REF(CLOSE,1)>3)
    X_9=( X_5 ) * ( X_6)
    X_10=((hhv(HIGH,9)-CLOSE)/(hhv(HIGH,9)-llv(LOW,9))*100-70)
    X_11=(sma(X_10,9,1)+100)
    X_12=((CLOSE-llv(LOW,9))/(hhv(HIGH,9)-llv(LOW,9))*100)
    X_13=(sma(X_12,3,1))
    X_14=(sma(X_13,3,1)+100)
    X_15=(X_14-X_11)
    X_16=(np.where(X_15>45,X_15-45,0))
    X_17=(np.where(CLOSE>OPEN,CLOSE,OPEN))
    X_18=(np.where(CLOSE>OPEN,OPEN,CLOSE))
    X_19=(llv(X_17,4))
    X_20=(hhv(X_18,4))
    X_21=(X_17>=X_19 ) * ( X_18<=X_19 ) * ( X_17>=X_20 ) * ( X_18<=X_20)
    X_22=(REF(X_17,1)>=X_19 ) * ( REF(X_18,1)<=X_19 ) * ( REF(X_17,1)>=X_20 ) * ( REF(X_18,1)<=X_20)
    X_23=(REF(X_17,2)>=X_19 ) * ( REF(X_18,2)<=X_19 ) * ( REF(X_17,2)>=X_20 ) * ( REF(X_18,2)<=X_20)
    X_24=(REF(X_17,3)>=X_19 ) * ( REF(X_18,3)<=X_19 ) * ( REF(X_17,3)>=X_20 ) * ( REF(X_18,3)<=X_20)
    X_25=(X_21 ) * ( X_22 ) * ( X_23 ) * ( X_24)
    X_26=(REF(X_20,barslast(X_25)))
    X_27=((CLOSE-REF(CLOSE,1))/REF(CLOSE,1)*100)
    X_28=((REF(OPEN,5)+REF(CLOSE,5))/2)
    X_29=((HIGH+LOW+CLOSE+OPEN)/4)
    X_30=(np.maximum(X_28,np.maximum(X_29,hhv(HIGH,5))))
    X_31=(np.minimum(X_28,np.minimum(X_29,llv(LOW,5))))
    X_32=(X_30==X_28)
    X_33=(X_32)
    X_34=(REF(X_30,barslast(X_33)))
    X_35=(CLOSE>X_34)
    X_36=(EMA(EMA(EMA(OPEN,5),5),5))
    X_37=(np.arctan((X_36/REF(X_36,1)-1)*100)*57.299999)
    X_38=((X_37>10 ) * ( X_36>REF(X_36,1)))
    X_39=(EMA(CLOSE,5)-EMA(CLOSE,55))
    X_40=(EMA(X_39,10))
    X_41=(X_39-X_40)
    X_42=(MA(LOW+(HIGH-LOW)/2,26)*1.15)
    X_43=(MA(LOW+(HIGH-LOW)/2,26)*0.95)
    X_44=((X_42+X_43)/2)
    X_45=((MA(CLOSE,5)+MA(OPEN,5))/2*1.06)
    X_46=((MA(CLOSE,5)+MA(OPEN,5))/2*0.98)
    X_47=((X_45+X_46)/2)
    X_48=(X_45>X_43)
    X_49=(X_45>X_44)
    X_50=(X_45>X_42)
    ...
    X_55=(X_46>X_44)
    X_56=(X_46>X_42)
    X_57=(X_48+X_49+X_50+X_51+X_52+X_53+X_54+X_55+X_56)
    X_58=(X_9 ) * ( X_57>0 ) * ( X_16>1 ) * ( count(X_25>0,13) ) * ( count(X_57<2,50) ) * ( X_38 ) * ( CLOSE>X_26 ) * ( X_41>0 ) * ( X_35)
    a=(FILTER(X_58,9))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
