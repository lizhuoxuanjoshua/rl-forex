...
#开始
    ...
    X_1=(REF(CLOSE,2)/REF(CLOSE,1)>1.05 ) * ( CLOSE/REF(CLOSE,1)>1.05 ) * ( CLOSE>REF(CLOSE,2))
    X_2=((2*CLOSE+HIGH+LOW)/4-EMA(CLOSE,20))
    X_3=(np.abs(REF(CLOSE,1)))
    X_4=(X_3/REF(CLOSE,1)>1.02 ) * ( X_3/REF(CLOSE,2)>1.02)
    a=((X_1 ) * ( X_2>(-10) ) * ( X_2<0.9 )  * ( X_4))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
