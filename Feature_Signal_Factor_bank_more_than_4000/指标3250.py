...
#开始
    ...
    AA2=(llv(LOW,3)==llv(LOW,13) ) * (( CLOSE<REF((HIGH+LOW)/2)) *np.maximum(REF(llv(LOW,12),barslast(AA1)+1)-LOW,0))
    AA3=(EMA(AA2,3)*3)
    AA6=(REF(AA3,1)==hhv(AA3,3) )  * ( count(CLOSE/REF(CLOSE,1)<0.9025,8)==0 ) * ( llv(LOW,3)/hhv(HIGH,18)<0.88)
    a=(FILTER(AA6 ,5))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
