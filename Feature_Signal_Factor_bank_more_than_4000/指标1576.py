...
#开始
    ...
    VAR3=(VOL/((HIGH-LOW)*2-np.abs(CLOSE-OPEN)))
    VAR4=((np.where(CLOSE>OPEN,VAR3*(HIGH-LOW),np.where(CLOSE<OPEN,VAR3*(HIGH-OPEN+CLOSE-LOW),VOL/2))/(np.where(CLOSE>OPEN,0-VAR3*(HIGH-CLOSE+OPEN-LOW),np.where(CLOSE<OPEN,0-VAR3*(HIGH-LOW),0-VOL/2))*(-1))-np.where(CLOSE>OPEN,0-VAR3*(HIGH-CLOSE+OPEN-LOW),np.where(CLOSE<OPEN,0-VAR3*(HIGH-LOW),0-VOL/2))*(-1)/np.where(CLOSE>OPEN,VAR3*(HIGH-LOW),np.where(CLOSE<OPEN,VAR3*(HIGH-OPEN+CLOSE-LOW),VOL/2))))
    VAR5=(AMOUNT/(100*VOL))
    VAR6=(MA(VAR5,40))
    VAR7=(100*(CLOSE-VAR6)/VAR6)
    a=(VAR4>0 ) * ( VAR7<-12 ) * ( CLOSE/OPEN>1.05)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
