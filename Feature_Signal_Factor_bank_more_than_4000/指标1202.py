...
#开始
    ...
    AA=(C/REF(C,1)>1.05 ) * ( C==H)
    AAA=(FILTER(AA,28)*100)
    VAR1A=((C/REF(C,1)>1.05 ) * ( H/C<1.01 ) * ( C>REF(C,1)))
    VAR2A=(FILTER((VAR1A>0),45) ) * ( FILTER((VAR1A>0),35) ) * ( AAA>0)
    a=(VAR2A ) * ( count(VAR2A,60)==1)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
