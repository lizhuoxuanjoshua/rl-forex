...
#开始
    ...
    MA10=(MA(C,10))
    A1=(barslast(REF(C==REF(llv(C,120),13),1)))
    L1=(llv(C,A1+1))
    H1=(hhv(C,A1))
    HTS=(barslast(C==hhv(C,A1)))
    HTS1=(HTS>5)
    L2=(llv(C,10))
    L2TS=(barslast(C==llv(C,10))<HTS)
    FT1=(H1/L1>1.08)
    FT2=(H1/L2>1.08)
    DBQY=(C/llv(C,250)<1.2)
    RSV=((CLOSE-llv(LOW,20))/(hhv(HIGH,20)-llv(LOW,20))*100)
    K=(sma(RSV,3,1))
    D=(sma(K,3,1))
    VA1=(np.where(CLOSE>OPEN,AMO,0))
    VA2=(np.where(OPEN>CLOSE,AMO,0))
    LIURU=(count(VA1,A1))
    LIUCHU=(count(VA2,A1))
    ZIJINBI=(np.divide(LIURU,LIUCHU)>1.05)
    a=(HTS1 ) * ( L2TS ) * ( FT1 ) * ( FT2 ) * ( L2/L1>0.95 ) * ( ZIJINBI ) * ( DBQY ) *  ( K<75) * ( MA10>REF(MA10,1))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
