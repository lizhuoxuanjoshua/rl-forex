...
#开始
    ...
    RSI1=sma(np.maximum(CLOSE-OK,0),7,1)/sma(np.abs(CLOSE-OK),7,1)*100

    

    RSI2=sma(np.maximum(CLOSE-OK,0),7,1)/sma(np.abs(CLOSE-OK),7,1)*100

    

    鱼=sma(np.maximum(CLOSE-OK,0),7,1)/sma(np.abs(CLOSE-OK),7,1)*100
    ...
    鱼网蓝柱= CROSS(鱼,好线) * (REF(MA(C,3),1)<=MA(C,3))

    

    镶黄柱=(C>MA(C,3)) * (C>=MA(C,7))  * 鱼网蓝柱
