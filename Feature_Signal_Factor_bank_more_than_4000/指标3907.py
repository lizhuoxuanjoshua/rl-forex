...
#开始
    ...
    DEA=(EMA(DIFF,9))
    AP3=(CROSS(DIFF,DEA))
    AP4=(CROSS(MA(V,5),MA(V,10)))
    GYTJ1=(BETWEEN(C/REF(C,1),1.02,0.95))
    GYTJ2=(C/REF(C,1)>1.02 ) * ( O/REF(C,1)<1.05)
    a=(AP1 ) * ( count(AP2,2) ) * ( count(AP3,5) ) * ( count(AP4,5)) * ( EVERY(REF(GYTJ1,1),5) ) * ( GYTJ2)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
