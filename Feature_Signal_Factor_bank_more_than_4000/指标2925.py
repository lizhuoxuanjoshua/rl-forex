...
#开始
    ...
    圣=(((((np.where(CLOSE>OPEN,VOL/((HIGH-LOW)*2-np.abs(CLOSE-OPEN))*(HIGH-LOW),np.where(CLOSE<OPEN,VOL/((HIGH-LOW)*2-np.abs(CLOSE-OPEN))*(HIGH-OPEN+CLOSE-LOW),VOL/2))+np.where(CLOSE>OPEN,0-VOL/((HIGH-LOW)*2-np.abs(CLOSE-OPEN))*(HIGH-CLOSE+OPEN-LOW),np.where(CLOSE<OPEN,0-VOL/((HIGH-LOW)*2-np.abs(CLOSE-OPEN))*(HIGH-LOW),0-VOL/2)))/20)/1.15*0.55+(REF(((np.where(CLOSE>OPEN,VOL/((HIGH-LOW)*2-np.abs(CLOSE-OPEN))*(HIGH-LOW),np.where(CLOSE<OPEN,VOL/((HIGH-LOW)*2-np.abs(CLOSE-OPEN))*(HIGH-OPEN+CLOSE-LOW),VOL/2))+np.where(CLOSE>OPEN,0-VOL/((HIGH-LOW)*2-np.abs(CLOSE-OPEN))*(HIGH-CLOSE+OPEN-LOW),np.where(CLOSE<OPEN,0-VOL/((HIGH-LOW)*2-np.abs(CLOSE-OPEN))*(HIGH-LOW),0-VOL/2)))/20)/1.15,1)*0.33))+(REF(((np.where(CLOSE>OPEN,VOL/((HIGH-LOW)*2-np.abs(CLOSE-OPEN))*(HIGH-LOW),np.where(CLOSE<OPEN,VOL/((HIGH-LOW)*2-np.abs(CLOSE-OPEN))*(HIGH-OPEN+CLOSE-LOW),VOL/2))+np.where(CLOSE>OPEN,0-VOL/((HIGH-LOW)*2-np.abs(CLOSE-OPEN))*(HIGH-LOW)*(HIGH-CLOSE+OPEN-LOW),np.where(CLOSE<OPEN,0-VOL/((HIGH-LOW)*2-np.abs(CLOSE-OPEN))*(HIGH-LOW),0-VOL/2)))/20)/1.15,2)*0.22)))
    灵=(EMA(圣,3))
    石1=((count(灵,1)
))
    石2=((count(灵,2)
    ...
    圣灵石2=(石1 ) * ( 石2 ) * ( 石3 ) * ( 石4 ) * ( 石5 ) * ( 石6 ) * ( 石7 ) * ( 石8 ) * ( 石9 ) * ( 石10 ) * ( (CLOSE-OPEN)>=((MA(C,3)+MA(C,6)+MA(C,12)+MA(C,24))/4+6*STD((MA(C,3)+MA(C,6)+MA(C,12)+MA(C,24))/4,11)-(MA(C,3)+MA(C,6)+MA(C,12)+MA(C,24))/4) ) * ( V/REF(V,1)>=1.9)
    a=(VOL>REF(V,1)*1.9 ) * ( 圣灵石2)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
