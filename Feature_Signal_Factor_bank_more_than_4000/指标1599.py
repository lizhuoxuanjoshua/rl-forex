...
#开始
    ...
    N=(14)
    MM=(6)
    MTR=(EMA(np.maximum(np.maximum(HIGH-LOW,np.abs(HIGH-REF(CLOSE,1))),np.abs(REF(CLOSE,1)-LOW)),N))
    HD =(HIGH-REF(HIGH,1))
    LD =(REF(LOW,1)-LOW)
    DMP=(EMA(HD,N))
    DMM=(EMA(LD,N))
    多头线=(DMP*100/MTR)
    空头线=(DMM*100/MTR)
    趋势线=(EMA(np.abs(空头线-多头线)/(空头线+多头线)*100,MM))
    BOLL=(MA(CLOSE,20))
    UB=(BOLL+2*STD(CLOSE,20))
    VAR3AA=(np.where((CLOSE>REF(CLOSE,1)),88,0))
    a=(((CLOSE)/(REF(CLOSE,1))>1.025) ) * (((HIGH)/(CLOSE)<1.03) ) * ( ((CLOSE)>(UB)) ) * ( (多头线-空头线)>5 ) * ( 多头线>20 ) * ( (趋势线>REF(趋势线,1)) ) * ( (多头线>REF(多头线,1)) ) * ( (VAR3AA>0))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
