...
#开始
    ...
    BULL30=(REF(MA(CLOSE,30),1)<MA(CLOSE,30))
    BULL5=(REF(MA(CLOSE,5),1)<MA(CLOSE,5))
    BULL1UP10=(CLOSE>MA(CLOSE,10))
    BULL5OLD=(REF(MA(CLOSE,5),2)<REF(MA(CLOSE,5),3))
    BULL5UP10=(CROSS(MA(CLOSE,5),MA(CLOSE,10)))
    BULL5UP30=(MA(CLOSE,5)>MA(CLOSE,30))
    a=((0) ) + ( (BULL30 )) * ( BULL5 ) * ( BULL1UP10 ) * ( BULL5UP10)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
