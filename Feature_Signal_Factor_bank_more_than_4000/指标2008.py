...
#开始
    ...
    VAR4=(llv(LOW,34))
    VAR5=(hhv(HIGH,34))
    AA=((3)*(sma(((CLOSE-llv(LOW,27))/(hhv(HIGH,27)-llv(LOW,27)))*(100),5,1))-(2)*(sma(sma(((CLOSE-llv(LOW,27))/(hhv(HIGH,27)-llv(LOW,27)))*(100),5,1),3,1)))
    AB=(EMA(((VAR3-VAR4)/(VAR5-VAR4))*(100),13))
    AC=(EMA((0.66699999)*(REF(AB,1))+(0.333)*(AB),1))
    a=(CROSS(AA,10) ) * ( (AC<AB))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
