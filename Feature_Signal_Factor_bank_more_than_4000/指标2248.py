...
#开始
    ...
    VAR21=((((((MA(AMOUNT,5) / MA(VOL,5)) ) - ((MA(AMOUNT,110) / MA(VOL,110)))) / ((MA(AMOUNT,5) / MA(VOL,5)) )) < (0 - 0.273)))
    VAR22=((((CLOSE - MA(CLOSE,30)) / MA(CLOSE,30)) < (0 - 0.125)))
    获利线=((VAR21 ) * ( VAR22))
    VAR2Q=(EMA(CLOSE,3))
    神奇线=(((sma(np.maximum((CLOSE - VAR2Q),0),6,1) / sma(np.abs((CLOSE - VAR2Q)),6,1)) * 100))
    a=(FILTER(((神奇线 < 6) ) * ( 获利线),3))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
