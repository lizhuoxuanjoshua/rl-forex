...
#开始
    ...
    跳空跌幅=(np.where(O<REF(C,1),-(O-REF(C,1))/REF(C,1)*100,0))
    实体跌幅=(np.where(C<O,-(C-O)/O*100,0))
    上影线=(np.where(C>=O,-(C-H)/H*100,-(O-H)/H*100))
    跳空涨幅=(np.where(O>REF(C,1),(O-REF(C,1))/REF(C,1)*100,0))
    实体涨幅=(np.where(C>O,(C-O)/O*100,0))
    下影线=(np.where(C>=O,(O-L)/L*100,(C-L)/L*100))
    向下振幅=(跳空跌幅+实体跌幅+上影线)
    向上振幅=(跳空涨幅+实体涨幅+下影线)
    振幅占比=(count(向下振幅,10)/(count(向上振幅,10)+count(向下振幅,10)))
    a=(count(向下振幅,10)>30 ) * ( 振幅占比>0.50)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
