...
#开始
    ...
    XD=((MA(C,5)>MA(C,20)) ) + ( (MA(C,5)==MA(C,20)) ) + ( (CROSS(MA(C,5),MA(C,20))
))
    XG=((MA(C,10)>=MA(C,20)) ) + ( (MA(C,10)==MA(C,20)) ) + ( (CROSS(MA(C,10),MA(C,20))
))
    XF=((MA(C,5)>MA(C,10)) ) + ( (MA(C,5)==MA(C,10)) ) + ( (CROSS(MA(C,5),MA(C,10))
))
    XS=((MA(C,20)>MA(C,60)) ) + ( (MA(C,20)==MA(C,60)) ) + ( (CROSS(MA(C,20),MA(C,60))
))
    XT=((MA(C,5)>MA(C,60)) ) + ( (MA(C,5)==MA(C,60)) ) + ( (CROSS(MA(C,5),MA(C,60))
))
    XY=((MA(C,5)>MA(C,120)) ) + ( (MA(C,5)==MA(C,120)) ) + ( (CROSS(MA(C,5),MA(C,120))
))
    XU=((MA(C,10)>MA(C,60)) ) + ( (MA(C,10)==MA(C,60)) ) + ( (CROSS(MA(C,10),MA(C,60))
))
    XI=((MA(C,60)>MA(C,120)) ) + ( (MA(C,60)==MA(C,120)) ) + ( (CROSS(MA(C,60),MA(C,120))
))
    XO=((MA(C,10)>MA(C,120)) ) + ( (MA(C,10)==MA(C,120)) ) + ( (CROSS(MA(C,10),MA(C,120))
))
    XP=((MA(C,20)>MA(C,120)) ) + ( (MA(C,20)==MA(C,120)) ) + ( (CROSS(MA(C,20),MA(C,120))
))
    MB=((C>MA(C,5)) ) + ( (C==MA(C,5)) ) + ( (CROSS(C,MA(C,5))
))
    BD=(XF ) * (  XD ) * ( XG)
    MC=((C>MA(C,60)) ) + ( (C==MA(C,60)) ) + ( (CROSS(C,MA(C,60))
))
    BF=(BD ) * ( XT ) * ( XY ) * ( XU ) * ( XO ) * ( XS ) * ( XP ) * ( XI)
    VR=EMA(EMA(CLOSE,13),13)
    AS=((VR-REF(VR,1))/REF(VR,1)*1000)
    A10=(CROSS(AS,0))
    AP=(np.where(A10,5,0))
    AL=((AS>REF(AS,1) ) * ( AS>0))
    LL=( CLOSE>REF(CLOSE,1)*1.038)
    VA=(100)
    AJ=(VA>50 ) * ( AS>0)
    AH=((AS<REF(AS,1) ) * ( AS>0))
    MM=MA(V,5)
    MX=(MA(V,35))
    GH=(hhv(np.maximum(C,0),30)/llv(np.minimum(C,O),30)<=1.15)
    MZ=(MA(V,135))
    CM=((MM>MX) ) + ( (MM==MX) ) + ( (CROSS(MM,MX)
))
    CN=((MM>MZ) ) + ( (MM==MZ) ) + ( (CROSS(MM,MZ)
))
    CB=((MX>MZ) ) + ( (MX==MZ) ) + ( (CROSS(MX,MZ)
))
    CV=(CM ) * (   CN  )  * ( CB)
    XG12=((XS) ) + ( (XP
))
    ...
    XG2=(CV ) * ( (AJ ) + ( (AL>3)) ) * ( AH==0)
    XG3=(BF  ) * ( AJ  + (AL>2))
    XG4=((BD ) * ( CV) ) * ( (AJ ) + ( (AL>2)) ) * ( AH==0)
    DO=((CLOSE-REF(CLOSE,1))/REF(CLOSE,1))
    DP=DO>0.04
    DJ=(DO>0.049 ) * ( ((AJ) ) + ( (AL>2)))
    DI=(DO>0.08)
    XG5=(DP ) * (((AJ) ) + ( (AL>3)) ) * ( XG12 )
    XG6=(DO>0.025 ) * ( ((REF(AP,1)) ) + ( (REF(AP,2))) ) * ( BD ) * (  (AJ ) + ( (AL>1.3)))
    XGG=(XD ) * ( XG ) * ( XT ) * ( XY ) * ( XU  ) * ( XO)
    XGH=(XS ) * ( XP  ) * ( XI  )  * ( AJ ) * ( MC ) * ( AH==0)
    XG7=(XGG ) * ( XGH)
    XG9=((((XG1) ) + ( (XG2) ) + ( (XG3) ) + ( (XG4 ) ) + ( (XG5) ) + ( (XG6) ) + ( (XG7)) ) * ( MB) ) + ( (((DJ ) + ( DI) ) * ( AS>0)
))
    DIFF=(EMA(CLOSE,12)-EMA(CLOSE,26))
    DEA=EMA(DIFF,9)
    MACD=(2*(DIFF-DEA))
    FA=(REF(MACD,1)<REF(MACD,2) ) * ( REF(MACD,2)<REF(MACD,3) ) * ( REF(MACD,3)<REF(MACD,4))
    FB=(MACD>=0 ) * ( count(MACD>REF(MACD,1),1)==1)
    FC=(EMA(C,30)>=REF(EMA(C,30),1) ) * ( EMA(C,5)>EMA(C,10) ) * ( EMA(C,10)>EMA(C,30))
    FD=((CROSS(C,EMA(C,30))) ) + ( (C>EMA(C,30)
))
    FE=(FA ) * ( FB ) * ( FC ) * ( FD)
    FF=(FA ) * ( FB ) * ( FC ) * ( FD ) * ( np.abs((DIFF-DEA)/C)<0.018)
    FG=(FA ) * ( FB ) * ( FC ) * ( FD ) * ( MACD<0.10)
    RR=((H-C)/C*100<1)
    RSV=((CLOSE-llv(LOW,9))/(hhv(HIGH,9)-llv(LOW,9))*100)
    K=(sma(RSV,3,1))
    D=(sma(K,3,1))
    OO=(CROSS(K,D) ) * ( LL ) * ( RR)
    a=(XG9  )  * ( REF(XG9,1)==0 ) * ( ((FE) ) + ( (FF) ) + ( (FG)))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
