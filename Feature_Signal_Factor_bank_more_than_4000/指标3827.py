...
#开始
    ...
    M=30
    N=1.04
    LCZ=((((C - MA(C,210)) / MA(C,210)) * 100))
    A1=((count((MA(C,60) > MA(C,210)),50) >= 30))
    A2=((count((LCZ < 35),50) >= 30))
    A3=((count((C > MA(C,210)),50) >= 30))
    跑道乖离=((count(((((MA(C,13) - MA(C,60)) / MA(C,60)) * 100) < 30),60) >= 20))
    跑道乖离率=((((((REF(C,1) - MA(C,60)) / MA(C,60)) * 100) < 35) ) * ( ((((C - MA(C,60)) / MA(C,60)) * 100) > 0)))
    跑道=((((((((A1 ) * ( A2) ) * ( A3) ) * ( (MA(C,13) > MA(C,250))) ) * ( (MA(C,210) > MA(C,250)))) ) + ( ((((跑道乖离 ) * ( 跑道乖离率) ) * ( (MA(C,13) > MA(C,250))) ) * ( (MA(C,210) > MA(C,250))))) ) + ( (((跑道乖离 ) * ( 跑道乖离率) ) * ( (MA(C,60) > MA(C,250))))))
    波动涨幅小=((((((REF(C,1) / REF(L,6)) < 1.15)) ) + ( ((REF(C,1) < REF(C,3)))) ) + ( ((REF(((hhv(H,5) - llv(L,5)) / llv(L,5)),1) < 0.15))))
    早盘涨停板=(((((C / REF(C,1)) > N) ) * ( (C > O)) ))
    去除ST=(count(((((C / REF(C,1)) - 1) * 100) > 5.6),60))
    去除ST和停牌=((去除ST))
    鳄鱼嘴涨停板1=(((((跑道 ) * ( 早盘涨停板) ) * ( (C > REF(hhv(H,30),1))) ) * ( 波动涨幅小) ) * ( 去除ST和停牌))
    a=(鳄鱼嘴涨停板1 ) * ( (((REF(barslast(鳄鱼嘴涨停板1),1) > 6)) ) + ( ((1))))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
