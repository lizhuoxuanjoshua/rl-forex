...
#开始
    ...
    AA=((O+H+L+C)/4)
    BB=(MA(AA,3))
    CC=(count(np.where(AA>REF(AA,1),AA*VOL,0),4)/count(np.where(AA<REF(AA,1),AA*VOL,0),4))
    ...
    中期=(B4)
    长期=(C4)
    D1=(中期>REF(中期,1)) * ( 短期>REF(短期,1)) * ( 长期>REF(长期,1)) * ( 长期<8 ) * ( 中期<10 ) * ( 短期<15)
    D2=(CROSS(短期,中期)) * ( 中期<8)
    D3=(CROSS(短期,长期)) * ( 长期<8 ) * ( 中期<20)
    a=(((D1) ) + ( (D2) ) + ( (D3)) ) * ( DD<15 ) * ( C>REF(C,1))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
