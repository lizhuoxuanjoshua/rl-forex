...
#开始
    ...
    A1=(3*sma((C-llv(L,60))/(hhv(H,60)-llv(L,60))*100,5,1)-2*sma((C-llv(L,60))/(hhv(H,60)-llv(L,60))*100,5,1))
    黑马线=(EMA(A1,5))
    A2=((黑马线-REF(黑马线,1))/REF(黑马线,1)*100+10)
    A3=(EMA(C,5))
    A4=(((C-llv(L,20))/(hhv(H,20)-llv(L,20)))*100)
    K20=(sma(A4,6,1))
    a=(((A2<-3) ) + ( (llv(K20,5)<10)) ) * ( (C==llv(C,10) ) + ( (L==llv(L,10))) ) * ( C>REF(C,1)*0.905)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
