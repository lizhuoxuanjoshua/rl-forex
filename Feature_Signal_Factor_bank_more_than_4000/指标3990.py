...
#开始
    ...
    WY1001X=((2*CLOSE+HIGH+LOW)/4)
    WY1002X=(EMA(WY1001X,2))
    WY1003X=(EMA(WY1002X,2))
    WY1004X=(EMA(WY1003X,2))
    XYS0X=((WY1004X-REF(WY1004X,1))/REF(WY1004X,1)*100)
    刚刚死叉=(CROSS(0,XYS0) ) * ( NOT(REF(CROSS(0,XYS0),1)))
    小周期刚刚死叉=(CROSS(0,XYS0X) ) * ( NOT(REF(CROSS(0,XYS0X),1)))
    死叉反常涨=(刚刚死叉 ) * ( C>O)
    小周期反常涨=(小周期刚刚死叉 ) * ( C>O ) * ( XYS0>0)
    一字板=((C-REF(C,1))/REF(C,1)>0.08 ) * ( np.abs((H-O)/O*100)<3 ) * ( (H-L)/H*100<7)
    低开高走=(C>O ) * ( (O-REF(C,1))/REF(C,1)*100<-2 ) * ( O<REF(L,1) ) * ( (H-REF(L,1))/REF(L,1)>-0.03)
    连续两个情况=( C>O ) * ( REF(死叉反常涨,1) ) * ( XYS0<0)
    预选=((死叉反常涨) ) + ( (小周期反常涨) ) + ( ((一字板 ) * ( REF(刚刚死叉,1))
))
    a=(预选 ) * ( ((低开高走) ) + ( (((一字板 ) * ( REF(刚刚死叉,1))))))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
