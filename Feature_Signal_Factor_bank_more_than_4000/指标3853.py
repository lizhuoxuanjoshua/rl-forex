...
#开始
    ...
    增量=(V>REF(V,1))
    AA=(MA(C,5))
    AB=(np.arctan((AA/REF(AA,1)-1)*100)*57.3)
    A1=(O<MA(C,10) ) * ( C>MA(C,5) ) * ( MA(C,5)>MA(C,10) ) * ( AB>45)
    A2=(L<MA(C,10) ) * ( C>MA(C,5) ) * ( MA(C,5)>MA(C,10) ) * ( AB>45)
    DD=(C/REF(C,1)<1.07 ) * ( C>=REF(C,1))
    VAR1=(VOL/((HIGH-LOW)*2-np.abs(CLOSE-OPEN)))
    AC=(np.where(CLOSE>OPEN,VAR1*(HIGH-LOW),np.where(CLOSE<OPEN,VAR1*((HIGH-OPEN)+(CLOSE-LOW)),VOL/2)))
    AD=(np.where(CLOSE>OPEN,VAR1*((HIGH-CLOSE)+(OPEN-LOW)),np.where(CLOSE<OPEN,VAR1*(HIGH-LOW),VOL/2)))
    X2=(np.abs(MA(C,10)/MA(C,20)-1)<0.04)
    X4=(np.abs(MA(C,5)/MA(C,10)-1)<0.03)
    X3=(np.abs(MA(C,20)/MA(C,30)-1)<0.04)
    B1=(X2 ) * ( X4 ) * ( X3)
    CC1=(MA(C,20)>REF(MA(C,20),1))
    a=(B1 ) * ( ((A1) ) + ( (A2 )) * ( AC>AD*4) ) * ( CC1  ) * ( 增量 ) * ( DD)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
