...
#开始
    ...
    VARE=(CLOSE-REF(C,1))
    VARF=(np.maximum(VARE,0))
    VAR10=(np.abs(VARE))
    VAR11=(sma(VARF,7,1)/sma(VAR10,7,1)*100)
    VAR12=(sma(VARF,13,1)/sma(VAR10,13,1)*100)
    VAR13=(sma(np.maximum(VARE,0),6,1)/sma(np.abs(VARE),6,1)*100)
    VAR14=(-200*(hhv(H,60)-CLOSE)/(hhv(H,60)-llv(L,60))+100)
    VAR15=((CLOSE-llv(L,15))/(hhv(H,15)-llv(L,15))*100)
    VAR16=(sma((sma(VAR15,4,1)-50)*2,3,1))
    VAR1A=((hhv(H,30)-CLOSE)/CLOSE*100)
    a=(VAR13<=25 ) * ( VAR14<-95 ) * ( VAR1A>20 ) * ( VAR16<-30 )  * ( VAR11<22 ) * ( VAR12<28)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
