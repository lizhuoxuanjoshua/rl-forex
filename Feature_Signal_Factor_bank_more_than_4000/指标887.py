...
#开始
    ...
    N=(5)
    M=(3)
    VAR1=((CLOSE-OPEN)/OPEN*100)
    VAR2=(REF(CLOSE,1))
    VAR3=(sma(np.maximum(CLOSE-VAR2,0),6,1)/sma(np.abs(CLOSE-VAR2),6,1)*100)
    VAR4=(count(VAR3<M,20)>=1)
    VAR5=((MA(AMOUNT,5)/MA(VOL,5)/100-MA(AMOUNT,125)/MA(VOL,125)/100)/(MA(AMOUNT,5)/MA(VOL,5)/100))
    VAR6=(MA((llv(LOW,45)-CLOSE)/(hhv(HIGH,45)-llv(LOW,45))*100,N))
    VAR7=(CROSS(-5,VAR6) ) * ( VAR5<-0.4)
    VAR8=(REF(CLOSE,1))
    VAR9=(sma(np.maximum(CLOSE-VAR8,0),5,1)/sma(np.abs(CLOSE-VAR8),5,1)*125)
    VAR10=(sma(np.maximum(CLOSE-VAR8,0),10,1)/sma(np.abs(CLOSE-VAR8),10,1)*125)
    VAR11=(MA(CLOSE,10))
    VAR12=(MA(CLOSE,20))
    VAR13=(VAR9<80 ) * ( VAR10<78 ) * ( count(VOL,10)*125>=10 ) * ( CLOSE/llv(CLOSE,120)<=1.2 ) * ( (CLOSE-llv(CLOSE,15))/llv(CLOSE,15)>=0.12 ) * ( CLOSE/OPEN>1.02 ) * ( VAR11>VAR12)
    a=(VAR1>0 ) * (( VAR4) ) + ( (VAR7) ) + ( (VAR13))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
