...
#开始
    ...
    X_2=(MA(CLOSE,13))
    X_3=(MA(CLOSE,34))
    X_4=(sma(np.maximum(CLOSE-REF(CLOSE,1),0),17,1)/sma(np.abs(CLOSE-REF(CLOSE,1)),17,1)*100)
    X_5=(barslast(CROSS(X_1,X_2)))
    X_6=(barslast(CROSS(X_1,X_3)))
    X_7=(barslast(CROSS(X_2,X_3)))
    X_8=(X_5>=X_6 ) * ( X_6>X_7 ) * ( X_5-X_7<20)
    X_9=(count(CROSS(X_1,X_2),X_5+1)==1 ) * ( count(CROSS(X_1,X_3),X_5+1)==1 ) * ( CROSS(X_2,X_3) ) * ( X_1>X_2 ) * ( X_2>X_3)
    X_10=(X_1>REF(X_1,1) ) * ( X_2>REF(X_2,1) ) * ( X_3>REF(X_3,1))
    a=(X_8 ) * ( X_9 ) * ( X_10)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
