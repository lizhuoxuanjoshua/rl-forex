...
#开始
    ...
    LCX1=(O<MA(C,5) ) * ( O<MA(C,10) ) * ( O<MA(C,20) ) * ( C>MA(C,5) ) * ( C>MA(C,10) ) * ( C>MA(C,20))
    LCX11=(O<MA(C,5) ) * ( O<MA(C,10) ) * ( O<MA(C,30) ) * ( C>MA(C,5) ) * ( C>MA(C,10) ) * ( C>MA(C,30))
    LCX2=(MA(C,5)>MA(C,10) ) * ( MA(C,10)>MA(C,20) ) * ( MA(C,10)>MA(C,30) ) * ( MA(C,30)>MA(C,60))
    LCX3=(NOT((CROSS(MA(C,10),MA(C,5))) ) + ( (CROSS(MA(C,30),MA(C,20))) ) + ( (CROSS(MA(C,50),MA(C,10))) ) + ( (CROSS(MA(C,10),MA(C,20))) ) + ( (CROSS(MA(C,10),MA(C,30)))))
    LCX4=(C>O ) * ( C/REF(C,1)>1.02)
    a=(((LCX1) ) + ( (LCX11)) ) * ( LCX2 ) * ( LCX3 ) * ( LCX4)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
