...
#开始
    ...
    XZ2=((hhv(XZ1,N)))
    XZ3=((llv(XZ1,N)))
    TRTR=(EMA(np.maximum(np.maximum(H-L,np.abs(H-REF(C,1))),np.abs(REF(C,1)-L)),M))
    HD=(H-REF(H,1))
    LD=(REF(L,1)-L)
    条件1=(HD>0 ) * ( HD>LD)
    条件2=(LD>0 ) * ( LD>HD)
    DMP=(EMA(np.where(条件1,HD,0),M))
    DMM=(EMA(np.where(条件2,LD,0),M))
    VAR1=(DMP/TRTR)
    VAR2=(DMM/TRTR)
    VAR3=(EMA(VAR1,M))
    VAR4=(EMA(VAR2,M))
    VAR5=(np.where(VAR3>VAR4,XZ2,XZ3))
    VAR6=(REF(VAR5,2)>0 ) * ( LAST(VAR5<0,1,1))
    VAR7=(REF(VAR5,3)>0 ) * ( LAST(VAR5<0,2,1))
    a=(((VAR6) ) + ( (VAR7)) ) * ( VAR5>0)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
