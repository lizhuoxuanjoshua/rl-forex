...
#开始
    ...
    VAR04=(REF(VAR02,2)<REF(VAR02,1) ) * ( VAR02<REF(VAR02,1))
    VAR05=(count(VAR03,7)>0 ) * ( VAR04)
    VAR06=(REF((LOW+OPEN+CLOSE+HIGH)/4,1))
    VAR07=(sma(np.abs(LOW-VAR06),13,1)/sma(np.maximum(LOW-VAR06,0),10,1))
    VAR08=(EMA(VAR07,10))
    VAR09=(EMA(C,5))
    VAR10=(llv(LOW,33))
    VAR11=(EMA(np.where(LOW<=VAR10,VAR09,0),3)*40)
    VAR12=(np.where(VAR11>100,VAR11*0.312,VAR11))
    a=(VAR05 ) * ( count(VAR05,5)==1)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
