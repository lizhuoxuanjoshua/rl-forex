...
#开始
    ...
    MA34=(MA(C,34))
    MA55=(MA(C,55))
    角度MA13=(np.arctan((MA13/REF(MA13,1)-1)*100)*180/3.1416)
    角度MA34=(np.arctan((MA34/REF(MA34,1)-1)*100)*180/3.1416)
    角度MA55=(np.arctan((MA55/REF(MA55,1)-1)*100)*180/3.1416)
    A11=((OPEN<MA111) ) + ( (OPEN<MA112) ) + ( (OPEN<MA113
))
    A12=(C>np.maximum(MA111,np.maximum(MA112,MA113)))
    A13=(V/MA(V,30)>1.2)
    A14=(C/REF(C,1)>1.049)
    生命=(EMA(EMA(CLOSE,50),1))
    半=(MA(CLOSE,125))
    年=(MA(CLOSE,240))
    涨幅=((C-REF(C,1))/REF(C,1)*100)
    换手率=(HSL)
    条件=(C>生命 ) * ( 生命>半 ) * ( 半>年 ) * ( 涨幅>6.5 ) * ( 换手率>6)
    条件2= ( (hhv( HIGH,43)-llv( LOW,43))/llv( LOW,43)<90/100)
    a=(A11 ) * ( A12 ) * ( A13 ) * ( A14 ) * ( 条件 ) * ( 条件2 ) * ( C>MA13 ) * ( C>MA34 ) * ( MA13>MA34 ) * ( MA34>MA55 ) * ( 角度MA13>0 ) * ( 角度MA34>0 ) * ( 角度MA55>0)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
