...
#开始
    ...
    XJ=((C+0.618*REF(C,1)+0.382*REF(C,2)+0.236*REF(C,3)+0.146*REF(C,4))/2.382)
    GAO=(hhv(H,0)-llv(L,0))
    TIAOJIE=(np.where(GAO>=1,np.where(GAO>=5,2,1),0.618))
    ZDX1=(XJ+(sma(np.maximum(XJ-REF(XJ,1),0),5,1)/sma(np.abs(XJ-REF(XJ,1)),5,1)*TIAOJIE-TIAOJIE*0.5))
    ZDX2=(XJ+(sma(np.maximum(XJ-REF(XJ,1),0),5,1)/sma(np.abs(XJ-REF(XJ,1)),5,1)*100-50))
    AA=(np.where(C>100,ZDX2,ZDX1))
    AB=(EMA(((SLOPE(CLOSE,20) * 20) + CLOSE),60))
    a=(CROSS(AA,AB))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
