...
#开始
    ...
    MMA=(C-REF(C,1))
    MMB=(100*EMA(EMA(MMA,9),9)/EMA(EMA(np.abs(MMA),9),9))
    MMC=(MA(MMB,5))
    ...
    AA=(V8>=1 ) * ( V9>=1 ) * ( CLOSE>OPEN ) * ( REF(CLOSE,1)>REF(OPEN,1) ) * ( (VOL>REF(VOL,1)) ) * ( MMS>MML ) * ( CROSS(ABV,M2))
    AB=(V8>=1 ) * ( V9>=1 ) * ( CLOSE>OPEN ) * ( REF(CLOSE,1)>REF(OPEN,1) ) * ( (VOL>REF(VOL,1)) ) * ( CROSS(MMS,MML) ) * ( ABV>REF(ABV,1))
    AC=(V8>=1 ) * ( V9>=1 ) * ( CLOSE>OPEN ) * ( REF(CLOSE,1)>REF(OPEN,1) ) * ( (VOL>REF(VOL,1))) * ( ABV>REF(ABV,1) ) * ( MMS>MML ) * ( CROSS(MMM,MML) ) * ( MMS>REF(MMS,1) ) * ( MMM>REF(MMM,1) ) * ( MML>REF(MML,1))
    AD=(ABV>M2 ) * ( MMB>MMC ) * ( CROSS(MMS,MML) ) * ( MMS>REF(MMS,1) ) * ( MMM>REF(MMM,1) ) * ( MML>REF(MML,1))
    a=((AA) ) + ( (AB) ) + ( (AC) ) + ( (AD
))
#结束
    买入 = a
