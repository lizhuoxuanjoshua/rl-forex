...
#开始
    ...
    SS1=sma(np.maximum(C-REF(C,1),0),5,1)/sma(np.abs(C-REF(C,1)),5,1)*100

    SS2=sma(np.maximum(C-REF(C,1),0),10,1)/sma(np.abs(C-REF(C,1)),10,1)*100
    ...
    B=(C>X_2 ) * ( C>X_3 ) * ( C>X_4 ) * ( C>X_6 ) * ( C>X_5 ) * ( C>X_7 ) * ( C>X_8)

    买入=(CROSS(XS3,XS4) ) * ( XS3>0.05)
