...
#开始
    ...
    D=(np.arctan((MA(C,5)/REF(MA(C,5),1)-1)*100)*180/3.1416)
    D1=(np.arctan((MA(C,10)/REF(MA(C,10),1)-1)*100)*180/3.1416)
    D2=(np.arctan((MA(C,20)/REF(MA(C,20),1)-1)*100)*180/3.1416)
    D3=(100*(C-llv(L,34))/(hhv(C,34)-llv(L,34)))
    D4=((AMOUNT/C)/(hhv(AMOUNT,34)/hhv(C,34)))
    D5=(np.where(D4>1,1,D4)*100)
    D6=(3*sma((CLOSE-llv(LOW,27))/(hhv(HIGH,27)-llv(LOW,27))*100,5,1)-2*sma(sma((CLOSE-llv(LOW,27))/(hhv(HIGH,27)-llv(LOW,27))*100,5,1),3,1))
    D7=(EMA((CLOSE-llv(LOW,10))/(hhv(HIGH,25)-llv(LOW,10))*4,4)*30)
    D8=(D5>=90 ) * ( D3>80 ) * ( MA(D6,12)<90)
    D9=(barslast(CROSS(D7,MA(D6,12)))<40)
    D10=(C/REF(C,1)>1+0.01*2)
    AA=((D>45 ) * ( D1>55 ) * ( D2>30))
    AA1=(D8 ) * ( D9 ) * ( ((C>O) ) + ( (C/REF(C,1)>1.05)) ) * ( D10)
