...
#开始
    ...
    操作线=(MA(AA,5))
    VAR9=((CLOSE+OPEN)/2)
    黄线=((EMA(VAR9,3)+EMA(VAR9,6)+EMA(VAR9,12)+EMA(VAR9,24))/4)
    青线=EMA(VAR4,45)*1.15
    绿线=(EMA(VAR4,55))
    DK=(EMA((CLOSE-MA((2*CLOSE+HIGH+LOW)/4,30))/MA((2*CLOSE+HIGH+LOW)/4,30)*100,3))
    精品1=(EMA(EMA(EMA(CLOSE,2),2),2))
    精品2=((MA(精品1,13) * 0.999))
    基础0=(DK>REF(DK,1) ) * ( REF(DK,1)>REF(DK,2) ) * ( REF(DK,2)>REF(DK,3))
    基础1=(绿线>REF(绿线,1) ) * ( REF(绿线,1)>REF(绿线,2) ) * ( REF(绿线,2)>REF(绿线,3))
    基础2=(精品2>REF(精品2,1) ) * ( REF(精品2,1)>REF(精品2,2) ) * ( REF(精品2,2)>REF(精品2,3))
    基础3=(黄线>REF(黄线,1) ) * ( REF(黄线,1)>REF(黄线,2) ) * ( REF(黄线,2)>REF(黄线,3))
    基础4=(操作线>REF(操作线,1) ) * ( REF(操作线,1)>REF(操作线,2) ) * ( REF(操作线,2)>REF(操作线,3))
    a=(基础1 ) * ( 基础2 ) * ( 基础3 ) * ( 基础4 ) * ( CROSS(黄线,绿线))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
