...
#开始
    ...
    RSV2=((CLOSE-llv(LOW,33))/(hhv(HIGH,33)-llv(LOW,33))*100)
    RSV3=((CLOSE-llv(LOW,55))/(hhv(HIGH,55)-llv(LOW,55))*100)
    短线=(sma(sma(RSV1,3,1),3,1)+3*STD(CLOSE,21))
    中线=(sma(RSV2,5,1)+2*STD(CLOSE,33))
    长线=(sma(RSV3,5,1))
    中线2=(-100*(hhv(HIGH,55)-CLOSE)/(hhv(HIGH,55)-llv(LOW,55)))
    a=(CROSS(中线2,-90) ) * ( (长线<中线 ) * (( 长线>REF(长线,1))) + ( (短线<20 )) * ( 中线<20 ) * ( 长线<20))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
