...
#开始
    ...
    AA=(VAR1-REF(VAR1,1))/REF(VAR1,1)*1
    ...
    VAR3=np.maximum(np.abs(HIGH-VAR2),np.abs(VAR2-LOW))

    

    VAR4=np.abs((OPEN+CLOSE)/2-VAR2)
    ...
    DD=np.where(CROSS(VAR6,VAR5),0.008,0)

    

    a=(AA>CC) * CROSS(BB,CC) * (DD) * (量比>=1.01)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
