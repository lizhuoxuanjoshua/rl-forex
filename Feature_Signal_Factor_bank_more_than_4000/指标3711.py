...
#开始
    ...
    AAA=( REF(CLOSE,13))
    BBB=(sma(np.maximum(CLOSE-AAA,0),34,1)/sma(np.abs(CLOSE-AAA),34,1)*1000)
    HHH=(BBB-llv(BBB,11))
    DQ=((MA(HHH,21)*3+HHH*13)/16)
    PJZH=((C+H+L)/3)
    DDD=(MA(PJZH,4))
    CZUO=(DDD)
    MMZL=(llv(DDD,8))
    XG1=((CZUO-MMZL)>0 ) * ( REF((CZUO-MMZL),1)==0)
    BZHA=((CLOSE-llv(LOW,9))/(hhv(HIGH,9)-llv(LOW,9))*10)
    BZHB=(sma(BZHA,3,1))
    AAAA=(sma(BZHB,3,1))
    ABB=(np.where(AAAA>=REF(AAAA,1),AAAA,0))
    XG2=(AAAA==ABB ) * ( AAAA<5.5 ) * ( REF(ABB,1)==0 ) * ( AAAA>=REF(AAAA,1))
    XG=(XG2 ) * ( XG1)
    FXH1=((H-C-REF(C,1))/REF(C,1)*100)
    FXH2=((H-REF(C,1)-REF(C,2))/REF(C,2)*100)
    FXH3=((H-REF(C,2)-REF(C,3))/REF(C,3)*100)
    FXH4=((H-REF(C,3)-REF(C,4))/REF(C,4)*100)
    FXH5=((H-REF(C,4)-REF(C,5))/REF(C,5)*100)
    FXH6=((H-REF(C,5)-REF(C,6))/REF(C,6)*100)
    FXH7=((H-REF(C,6)-REF(C,7))/REF(C,7)*100)
    FXH8=((H-REF(C,7)-REF(C,8))/REF(C,8)*100)
    FXH9=((H-REF(C,8)-REF(C,9))/REF(C,9)*100)
    FXH10=((H-REF(C,9)-REF(C,10))/REF(C,10)*100)
    FXD5=((FXH1+FXH2+FXH3+FXH4+FXH5))
    FXD10=((FXH1+FXH2+FXH3+FXH4+FXH5+FXH6+FXH7+FXH8+FXH9+FXH10))
    ZXFA5=((((count(FXD5,3))*30)-MA(count(FXD5,3),45))/10)
    ZXFA10=((FXD10*30-(REF(count(FXD10,3),1)))/10)
    ZXFA2=((FXD5*30-(REF(count(FXD5,3),1)))/10)
    ZXF=((H-C-REF(C,1))/REF(C,1)*100)
    AAX1=(((count(ZXF,3))*30)-MA(count(ZXF,3),45))
    AAX3=((ZXF*30-(REF(count(ZXF,3),1))
))
    XH=(MA((ZXFA10+ZXFA2+ZXFA5+AAX1+AAX3)/5000,13))
    FL1=((L-O-REF(O,1))/REF(O,1)*100)
    FL2=((L-REF(O,1)-REF(O,2))/REF(O,2)*100)
    FL3=((L-REF(O,2)-REF(O,3))/REF(O,3)*100)
    FL4=((L-REF(O,3)-REF(O,4))/REF(O,4)*100)
    FL5=((L-REF(O,4)-REF(O,5))/REF(O,5)*100)
    FL6=((L-REF(O,5)-REF(O,6))/REF(O,6)*100)
    FL7=((L-REF(O,6)-REF(O,7))/REF(O,7)*100)
    FL8=((L-REF(O,7)-REF(O,8))/REF(O,8)*100)
    FL9=((L-REF(O,8)-REF(O,9))/REF(O,9)*100)
    FL10=((L-REF(O,9)-REF(O,10))/REF(O,10)*100)
    FDK5=((FL1+FL2+FL3+FL4+FL5))
    FDK10=((FL1+FL2+FL3+FL4+FL5+FL6+FL7+FL8+FL9+FL10))
    ZFAK5=((((count(FDK5,3))*30)-MA(count(FDK5,3),45))/10)
    ZFAK10=((FDK10*30-(REF(count(FDK10,3),1)))/10)
    ZFAK2=((FDK5*30-(REF(count(FDK5,3),1)))/10)
    ZFK=((L-O-REF(O,1))/REF(O,1)*100)
    AAK1=(((count(ZFK,3))*30)-MA(count(ZFK,3),45))
    AAK3=((ZFK*30-(REF(count(ZFK,3),1))
))
    XL=((MA((ZFAK10+ZFAK2+ZFAK5+AAK1+AAK3)/5000,13)
))
    XHZ=(MA(XH-REF(XH,1),3)*100)
    XLZ=(-MA(XL-REF(XL,1),3)*100)
    波段高点=( hhv(HIGH,9))
    波段低点=( CLOSE)
    坑=(波段高点/波段低点)
    a:(XG==1 ) * (( DQ<5) ) * ( (XLZ<0.2) ) * ( (XHZ>-0.3) ) * ( (坑<1.08))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
