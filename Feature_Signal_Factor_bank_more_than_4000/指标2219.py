...
#开始
    ...
    LA=(REF(CLOSE,1))
    LB=(((sma(np.maximum((CLOSE - LA),0),5,1) / sma(np.abs((CLOSE - LA)),5,1)) * 100))
    LC=((HIGH - REF(HIGH,1)))
    LD=((REF(LOW,1) - LOW))
    LE=(count(np.maximum(np.maximum((HIGH - LOW),np.abs((HIGH - REF(CLOSE,1)))),np.abs((LOW - REF(CLOSE,1)))),10))
    LF=(count(LC,10))
    LG=(count(LD,10))
    LH=(((LF * 100) / LE))
    LI=(((LG * 100) / LE))
    LJ=(MA(((np.abs((LI - LH)) / (LI + LH)) * 100),5))
    AV=((LB + LJ))
