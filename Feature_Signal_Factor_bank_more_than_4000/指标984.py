...
#开始
    ...
    JJ=(REF(EMA(PJJ,3),1))
    QJJ=(VOL / ((H - L) * 2 - np.abs(C - O)))
    XVL=(np.where(C > O,QJJ * (H - L),np.where(C < O,QJJ * (H - O + (C - L)),VOL / 2)) + np.where(C > O,0 - QJJ * (H - C + (O - L)),np.where(C < O,0 - QJJ * (H - L),0 - VOL / 2)))
    ZLL=(VOL )
    HSL1=(np.where(ZLL > 10,10,ZLL))
    HSL=(XVL / 21 / 1.15)
    AA=(HSL * 0.66 + REF(HSL,1) * 0.33 + REF(HSL,2) * 0.22)
    GJJ=(EMA(AA,7))
    LLJX=(EMA(AA,3))
    流量=(LLJX)
    DIF=(EMA(CLOSE,5)-EMA(CLOSE,10))
    DEA=(EMA(DIF,5))
    a=((流量<0)<REF((流量<0),1)/1.15 ) * ( CROSS(DIF,DEA))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
