...
#开始
    ...
    A5=(EMA(C,20))
    a=(CROSS(A1,A2) ) * ( C>O ) * ( C>REF(C,1) ) * (( C/REF(C,1)>=1.01)) + ( (CROSS(A1,A4) )) * ( C>O ) * ( C>REF(C,1) ) * ( C/REF(C,1)>=1.018) + ( (CROSS(A2,A3) )) * ( C>O ) * ( C>REF(C,1) ) * ( C/REF(C,1)>=1.018) + ( (CROSS(A1,A3) )) * ( C>O ) * ( C>REF(C,1) ) * ( C/REF(C,1)>=1.018 ) * ( CROSS(A1,A4))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
