...
#开始
    ...
    VAR1=(AMOUNT/((HIGH-LOW)*2-np.abs(CLOSE-OPEN)))
    流入亿=(np.where(CLOSE>OPEN,VAR1*(HIGH-LOW),np.where(CLOSE<OPEN,VAR1*((HIGH-OPEN)+(CLOSE-LOW)),AMOUNT/2))/100000000)
    流出亿=(np.where(CLOSE>OPEN,0-VAR1*((HIGH-CLOSE)+(OPEN-LOW)),np.where(CLOSE<OPEN,0-VAR1*(HIGH-LOW),0-AMOUNT/2))/100000000)
    差=((流入亿+流出亿))
    强=(EMA((CLOSE-REF(CLOSE,1))/CLOSE*10,2))
    主力=(EMA(差,1))
    差A=(3*EMA(EMA(EMA(EMA(强,2),2),2),2))
    净流=(3*EMA(EMA(EMA(EMA(差A,1),2),2),2))
    a=(CROSS(净流,0))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
