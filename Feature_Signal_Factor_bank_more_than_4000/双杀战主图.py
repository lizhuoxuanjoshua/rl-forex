...
#开始
    ...
    DIR1=(np.abs(CLOSE-REF(CLOSE,5)))
    VIR1=(count(np.abs(CLOSE-REF(CLOSE,1)),5))
    ER1=(DIR1/VIR1)
    CS1=(ER1*(0.8-2/8)+2/8)
    CQ1=(CS1*CS1)
    AMA1=(EMA(DMA(CLOSE,CQ1),2))
    DIR2=(np.abs(CLOSE-REF(CLOSE,10)))
    VIR2=(count(np.abs(CLOSE-REF(CLOSE,1)),10))
    ER2=(DIR2/VIR2)
    CS2=(ER2*(0.8-2/24)+2/24)
    CQ2=(CS2*CS2)
    AMA2=(EMA(DMA(CLOSE,CQ2),2))
    AMA3=(np.where(AMA2/REF(AMA2,1)>0.997,AMA2,0))
    AMA4=(np.where(AMA2/REF(AMA2,1)>1.001,AMA2,0))
    a=(CROSS(VAR2,MA(VAR3,6)))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
