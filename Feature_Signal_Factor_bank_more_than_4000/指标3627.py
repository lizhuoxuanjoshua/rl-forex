...
#开始
    ...
    MA55=(MA(C,55))
    BM1=((C+O)/2)
    BM2=((count(MA30>REF(MA30,1),5)==5) ) + ( (count(MA55>REF(MA55,1),5)==5
))
    BM3=(count(MA5/MA30>REF(MA5/MA30,1),5)==5)
    BM4=((BM1*2-REF(BM1,3)))
    BM5=(C/MA(C,5))
    BM6=(REF(BM1/MA(BM1,5),1))
    BM7=(count(BM6>BM5,3)==3)
    BM8=(C>O)
    AA=(((CROSS(BM4,MA5)) ) + ( (CROSS(BM4,MA30))) ) * ( BM2 ) * ( BM8 ) * ( C>MA(C,60))
    a=(AA ) * ( ((BM7) ) + ( (REF(BM7,1)) ) + ( (REF(BM7,2))))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
