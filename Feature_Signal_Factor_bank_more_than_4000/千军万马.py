...
#开始
    ...
    VARC=(sma(np.maximum(CLOSE-REF(C,1),0),6,1)/sma(np.abs(CLOSE-REF(C,1)),6,1)*100)
    VAR1=((HIGH+LOW+OPEN+2*CLOSE)/5)
    VAR2=(REF(VAR1,1))
    VAR3=(sma(np.maximum(VAR1-VAR2,0),10,1)/sma(np.abs(VAR1-VAR2),10,1)*100)
    VAR4=(count(VAR3<20,5)>=1 ) * ( count(VAR1==llv(VAR1,10),10)>=1 ) * ( CLOSE>=OPEN*1.038 ) * ( VOL>MA(VOL,5)*1.2)
    VAR5=(均衡>REF(均衡,1)) * ( 中轨<REF(中轨,1)) * ( C>REF(C,1))
