...
#开始
    ...
    VL=(REF(LOW,1))
    VH=(REF(H,1))
    VAR1=(sma(np.abs(H-VH),13,1)/sma(np.maximum(H-VH,0),13,1)*100)
    VAR2=(sma(np.abs(L-VL),13,1)/sma(np.maximum(L-VL,0),13,1)*100)
    VAR3=(REF(CLOSE,1))
    VAR4=(MA(VAR2,13))
    VAR5=(llv(L,34))
    VAR6=(sma(np.maximum(CLOSE-VAR3,0),6,1)/sma(np.abs(CLOSE-VAR3),6,1)*100)
    VAR7=(CROSS(93,VAR6))
    VAR8=(EMA(np.where(L<=VAR5,VAR4,0),3))
    VAR9=(H>llv(L,0)*7)
