...
#开始
    ...
    QQ=((CLOSE-llv(LOW,9))/(hhv(HIGH,9)-llv(LOW,9))*100)
    AQ=(3*sma(QQ,3,1)-2*sma(sma(QQ,3,1),3,1))
    BQ=(DMA((((HIGH + LOW) + (CLOSE * 2)) / 4.15),0.9))
    CQ=(CROSS(CLOSE,REF(EMA(BQ,3),1)) ) * ( AQ<50 ) * ( REF(AQ<0 ,1))
    DQ=( AQ>0 ) * ( REF(C<O,1))
    EQ=(C>O ) * ( H/C<1.01)
    FQ=(C>REF(C,3) ) * ( C/REF(C,1)>1.03)
    GG=(REF(H>O,1) ) * ( REF(L<O,1))
    a=(CQ ) + ( DQ ) + ( EQ )
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
