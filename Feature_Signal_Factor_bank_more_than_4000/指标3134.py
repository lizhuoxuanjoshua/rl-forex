...
#开始
    ...
    AA=((C*2+H+L+O)/5)
    A0=(MA(C,5))
    A1=((EMA(AA,4)+MA(AA,8)+MA(AA,16))/3)
    A2=((EMA(AA,9)+MA(AA,18)+MA(AA,36))/3)
    A3=((EMA(AA,13)+MA(AA,26)+MA(AA,52))/3)
    A4=((EMA(AA,24)+MA(AA,48)+MA(AA,96))/3)
    A5=(EMA(100*(C-llv(L,100))/(hhv(H,100)-llv(L,100)),5))
    A6=(A5/4)
    A7=(np.maximum(np.maximum(A1,A2),np.maximum(A3,A4)))
    A8=(np.minimum(np.minimum(A1,A2),np.minimum(A3,A4)))
    A9=(np.minimum(np.minimum(A1,A2),A3))
    A10=(np.maximum(np.maximum(A1,A2),A3))
    a=(A4>REF(A4,2)) * ( REF(A1,3)<REF(A3,3)) * ( C>O ) * ((A10-A9)/A9<0.005)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
