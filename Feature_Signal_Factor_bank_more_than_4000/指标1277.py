...
#开始
    ...
    X31=(SG < REF(SG,1)) +  (X30)
    LC=(REF(CLOSE,1))
    操作线=(sma(np.maximum(CLOSE-LC,0),2,1)/sma(np.abs(CLOSE-LC),2,1)*1002)
    ...
    PP3=(操作线<抄底线 ) * ( REF(操作线,1)>抄底线)
    a=(((((((count((X31 == 1),4) == 3) ) * ( (X31 == 0))) * ( O<C ) * ( (((C-REF(C,1))/REF(C,1))>0.085))*89) >=89))) + ((((C-REF(C,1))/REF(C,1)>0.085 )) * ( (REF(PP3,1) ) + ( (REF(PP,1)))) )
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
