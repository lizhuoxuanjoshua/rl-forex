...
#开始
    ...
    VAR6=(EMA(VAR5,715))
    VAR7=(2*(VAR5-VAR6))
    VAR8=(np.power(VAR7,3)*0.1+np.power(VAR7,2))
    VARA=(EMA(CLOSE,2)-EMA(CLOSE,34))
    VARB=(EMA(VARA,33))
    VARC=(2*(VARA-VARB))
    VARD=(np.power(VARC,3)*0.1+np.power(VARC,3))
    VARF=(EMA(CLOSE,2)-EMA(CLOSE,89))
    VAR10=(EMA(VARF,30))
    VAR11=(2*(VARF-VAR10)*10)
    VAR12=(np.power(VAR11,3)*0.1+np.power(VAR11,2))
    VAR13=(np.sqrt(np.sqrt(LOW*HIGH*OPEN*CLOSE)))
    VAR14=(EMA(VAR13*0.97,3))
    VAR15=((HIGH+LOW+CLOSE)/3)
    VAR16=((VAR15-MA(VAR15,14))/(0.015*AVEDEV(VAR15,14)))
    VAR17=(np.where(VAR11>0.015,VAR12,0)*100)
    VAR18=((CLOSE-llv(CLOSE,95))/(hhv(CLOSE,95)-llv(CLOSE,95))*100)
    VAR19=(VOL/240>MA(VOL,6)/240 ) * ( CLOSE>OPEN*1.003)
    VAR1A=(VAR19 ) * ( CLOSE<MA(CLOSE,10)*1.05 ) * ( CLOSE<MA(CLOSE,10)*1.3)
    VAR1B=(count(np.where(CLOSE>REF(CLOSE,1),VOL,np.where(CLOSE<REF(CLOSE,1),0-VOL,0)),0)/1000)
    VAR1C=(MA(VAR1B,13))
    VAR1D=(VAR1B-VAR1C)
    VAR1E=(EMA(VAR1D,3))
    VAR1F=(CROSS(VAR1D,VAR1E))
    a=(CROSS(EMA(CLOSE,3),EMA(CLOSE,13)) ) * ( VAR17>0 ) * ( VAR4<5 ) * ( VAR8>0 ) * ( VAR18<30 ) * ( VAR1A)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
