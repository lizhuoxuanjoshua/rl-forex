...
#开始
    ...
    VAR1=(REF(CLOSE,1))
    AA=(sma(np.maximum(CLOSE-VAR1,0),5,1)/sma(np.abs(CLOSE-VAR1),5,1)*100)
    AB=(3*sma((CLOSE-llv(LOW,27))/(hhv(HIGH,27)-llv(LOW,27))*100,3,1)-2*sma((CLOSE-llv(LOW,27))/(hhv(HIGH,27)-llv(LOW,27))*100,3,1))
    a=(CROSS(AB,20) ) * ( AA>50)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
