...
#开始
    ...
    A1=(count(np.abs(MA(CLOSE,24)-MA(CLOSE,60))/MA(CLOSE,60)<=0.006,2)==2)
    A2=(MA(CLOSE,24)>REF(MA(CLOSE,24),1)-0.05 ) * ( MA(CLOSE,24)<REF(MA(CLOSE,24),1)+0.05)
    A22=(MA(CLOSE,60)>REF(MA(CLOSE,60),1)-0.05 ) * ( MA(CLOSE,60)<REF(MA(CLOSE,60),1)+0.05)
    A3=(MA(CLOSE,5)>MA(CLOSE,24) ) * ( MA(CLOSE,5)>MA(CLOSE,60))
    A4=(C>MA(C,24)) * ( C>MA(C,60) ) * ( C>O ) * ( C==hhv(C,5) ) * ( H/C<1.03)
    XG1=(A1 ) * ( A2 ) * ( A22 ) * ( A3 ) * ( A4)
    A5=( CROSS(MA(V,5),MA(V,60)))
    A6=(REF(A5,1) ) * ( NOT(REF(XG1,1)))
    A7=(REF(A5,2) ) * ( NOT(REF(XG1,1)) ) * ( NOT(REF(XG1,2)))
    MA1=(MA(AMOUNT,6))
    MA2=(MA(AMOUNT,12))
    MA3=(MA(AMOUNT,24))
    A1=(MA1>MA2 ) * ( MA2>MA3)
    A2=(MA1>REF(MA1,1) ) * ( MA2>REF(MA2,1) ) * ( MA3>REF(MA3,1))
    LNXG=(A1 ) * ( A2)
    XGXG=(LNXG ) * ( NOT((REF(LNXG,1))))
    A8=(CROSS(V,EMA(V,30)*2) ) * ( MA(V,5)>MA(V,60) ) * ( LNXG  ) * ( NOT(REF(XG1,2)))
    A9=(MA(C,5)/MA(C,24)<1.05)
    a=(XG1  ) * ( ((A5) ) + ( (A6) ) + ( (A7) ) + ( (A8)) ) * ( A9)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
