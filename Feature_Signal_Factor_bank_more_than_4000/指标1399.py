...
#开始
    ...
    A1D=(barslast(A1B))
    A1E=(barslast(A1C))
    A1F=(np.where((A1E > 0),(CLOSE - REF(CLOSE, 1)) / REF(CLOSE, 1) * 100,0))
    A20=(count(A1F,0))
    A21=(np.where((A1D > 0),(0 - 2),0))
    A22=(np.where((A1E > 0),count((CLOSE - REF(CLOSE, 1)) / REF(CLOSE, 1) * 100,A1E),0))
    A23=(A22 / A1E)
    A25=(EMA(EMA(EMA(CLOSE, 3), 3), 3))
    AA=(((A25 - REF(A25, 1)) + A25))
    AB=(MA(AA, 15))
    a=(CROSS(AA,AB))
#结束
    买入 = a
