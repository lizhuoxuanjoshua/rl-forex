...
#开始
    ...
    VAR2=(REF(LOW,1)*0.9)
    VAR3=(LOW*0.9)
    VAR4=((VAR3*VOL+VAR2*(VOL)))
    VAR5=(EMA(VAR4,30))
    VAR6=(CLOSE-REF(CLOSE,1))
    VAR7=(np.maximum(VAR6,0))
    VAR8=(np.abs(VAR6))
    VAR9=(sma(VAR7,7,1)/sma(VAR8,7,1)*100)
    VARA=(sma(VAR7,13,1)/sma(VAR8,13,1)*100)
    VARC=(sma(np.maximum(VAR6,0),6,1)/sma(np.abs(VAR6),6,1)*100)
    ...
    AA=(VARC<=25 ) * ( VARD<-95 ) * ( VAR13>20 ) * ( VARF<-30 ) * ( VAR5-CLOSE>=-0.25 ) * ( VAR9<22 ) * ( VARA<28)
    a=(CROSS(AA,0.5) ) * ( count(AA==1,10)==1)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
