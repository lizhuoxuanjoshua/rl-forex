...
#开始
    ...
    MAVOL1=(MA(VOLUME,10))
    MAVOL2=(MA(VOLUME,20))
    天数=(barslast(C/REF(C,1)>=1.095 ) * ( C==H))
    DT=(REF(M1,3)>REF(M2,3)) * ( REF(M2,3)>REF(M3,3) ) * ( REF(M1,2)>REF(M2,2)) * ( REF(M2,2)>REF(M3,2) ) * ( REF(M1,1)>REF(M2,1)) * ( REF(M2,1)>REF(M3,1) ) * ( M1>M2 ) * ( M2>M3)
    V5=(MA(V,5))
    V10=(MA(V,10))
    QQ=(V/REF(V,1)>2.5)
    WW=(REF(C,1)/REF(C,2)<1 ) * ( C/REF(C,1)>1.05 ) * (  C/REF(C,1)<1.15)
    VV=(REF(V5,1)<REF(V10,1) ) * ( REF(V5,2)<REF(V10,2) ) * ( REF(V5,3)<REF(V10,3) ) * ( REF(V5,4)<REF(V10,4)) * ( REF(V5,5)<REF(V10,5)) * ( REF(V5,6)<REF(V10,6)) * ( REF(V5,7)<REF(V10,7)) * ( REF(V5,8)<REF(V10,8))
    KK=(REF(L,1)<=REF(llv(L,5),1)) * (((( REF(V,1)<0.9*REF(V,2)))) + (((REF(C,1)/REF(C,2)<REF(C,2)/REF(C,3))) )) * ( (H-C)/C<0.08)
    GG=((REF(C,1)/REF(C,2)>1.095 ) * ( REF(C,1)==REF(H,1)))
    TT=(V<0.5*REF(V,1) ) * ( V<0.18*hhv(V,15) ) * ( C/REF(C,1)<1.03 ) * ( C>L  ) * ( V<=llv(V,10))
    AA=(REF(C,1)/REF(C,2)>1.095 ) * ( REF(C,1)==REF(H,1))
    BB=(REF(C,2)<REF(H,2) ) * (  V/REF(V,1)<0.92 ) * ( O<=REF(C,1) ) * ( REF(C,1)/REF(C,2)>1.095  ) * ( O>C ) * ( C>L ) * ( (O-C)/C<0.04 ) * ( REF(V,2)/REF(V,3)<0.8 ) * ( ((REF(V,1)/REF(V,2)<2.6) ) + ( (REF(V,1)/REF(V,2)>2.7)))
    M=(V<0.89 *REF(V,1) ) * ( V<0.19*hhv(V,15)) * ( C<O ) * ( L<=llv(L,13 )) * ( H<REF(H,1) ) * ( H>O ) * ( REF(C,1)>REF(L,1)) * ( C/REF(C,1)<0.98)
    a=( (((V/MAVOL2>1.2 ) * ( REF(V,1)/REF(MAVOL2,1)>1.7 ) * (  REF(V,2)/REF(MAVOL2,2)>1.7 ) * ( C/REF(C,1)<0.92 ) * ( V/REF(V,1)<0.75 ) * ( DT ) * ( C>L ) * ( H>O)) ) + ( ((QQ ) * ( WW ) * ( VV ) * ( KK ) * ( REF(C,1)/REF(C,2)<0.99 ) * ( REF(V,1)/REF(V,2)>0.95 ) * ( H<1.198* REF(C,1))) ) + ( ((GG ) * ( TT  ) * ( REF(C,1)/REF(C,2)<0.96) ) ) + ( ( (AA ) * ( BB ) * ( REF(H,1)<hhv(H,60)) * ( H>O ) * ( REF((H-C)/C,1)<0.05 ) * ( ((REF(C,1)/REF(C,2)<1.19 ))))))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
