...
#开始
    ...
    M5=((VOL / (((HIGH - LOW) * 2) - np.abs((CLOSE - OPEN)))))
    M6=((np.where((CLOSE>OPEN),(M4*(HIGH-LOW)),np.where((CLOSE<OPEN),(M4*((HIGH-OPEN)+(CLOSE-LOW))),(VOL/2)))+np.where((CLOSE>OPEN),(0-(M5*((HIGH-CLOSE)+(OPEN-LOW)))),np.where((CLOSE<OPEN),(0-(M5*(HIGH-LOW))),(0-(VOL/2))))))
    AA=(((M6/20)/1.15))
    a=(FILTER(((AA* 0.4) >= 0),13))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
