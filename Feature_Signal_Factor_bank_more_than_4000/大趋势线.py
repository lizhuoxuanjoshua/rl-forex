...
#开始
    ...
    大趋势=EMA(3*sma((C-llv(L,55))/(hhv(H,55)-llv(L,55))*100,5,1)-2*sma(sma((C-llv(L,55))/(hhv(H,55)-llv(L,55))*100,5,1),3,1),3)

    

    ZLXH2=sma(np.abs(L-REF(L,1)),3,1)/sma(np.maximum(L-REF(L,1),0),3,1)*100

    

    ZLXH3=EMA(ZLXH2*10,3)
    ...
    ZLXH7=EMA(np.where(L<=ZLXH4,(ZLXH3+ZLXH5*2)/2,0),3)/618

    

    ZLXH8=np.where(ZLXH7>=100,100,ZLXH7)

    

    FCQK3=sma(np.abs(L-REF(L,1)),3,1)/sma(np.maximum(L-REF(L,1),0),3,1)*100

    

    FCQK4=EMA(FCQK3*10,3)
    ...
    FCQK7=EMA(np.where(LOW<=FCQK5,(FCQK4+FCQK6*2)/2,0),3)/618

    

    FCQK8=np.where(FCQK7>500,500,FCQK7)

    

    主力吸筹=np.where(ZLXH8>0,ZLXH8,0)

    

    发财吸筹=np.where(FCQK8>-120,np.abs(FCQK8),0)

    

    趋势吸筹=np.where(大趋势<=14,大趋势,0)

    

    最大值=np.maximum(趋势吸筹,np.maximum(主力吸筹,发财吸筹))

    

    最小值=np.minimum(趋势吸筹,np.minimum(主力吸筹,发财吸筹))
