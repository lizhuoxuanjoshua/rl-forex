...
#开始
    ...
    SZ=(500)
    ZT=(C/REF(C,1)>=1.098)
    TJ1=(count(ZT,10)>=1 ) * ( count(CLOSE<=REF(CLOSE,1),5)>=3)
    TJ21=((100*np.abs(CLOSE-MA(C,20))/MA(C,20)<=3) ) * ( (100*np.abs(LOW-MA(C,20))/MA(C,20)<=4) ) * ( H>MA(C,20) ) * ( count(CROSS(MA(C,20),CLOSE),20)<=2 ) * ( count(CLOSE<=MA(C,20),3)<=1)
    TJ22=((100*np.abs(CLOSE-MA(C,60))/MA(C,60)<=3) ) * ( (100*np.abs(LOW-MA(C,60))/MA(C,60)<=4) ) * ( H>MA(C,60) ) * ( count(CROSS(MA(C,60),CLOSE),60)<=2 ) * ( count(CLOSE<=MA(C,60),3)<=1 ) * ( MA(CLOSE,20)>REF(MA(CLOSE,20),1))
    TJ2=((TJ21) ) + ( (TJ22
))
    BIAS_V=((CLOSE-MA(CLOSE,L1))/MA(CLOSE,L1)*100)
    TJ3=(L2>BIAS_V ) * ( BIAS_V<REF(BIAS_V,1))
    TJ4=(MA(C,10)>MA(C,20) ) * ( MA(C,20)>MA(C,60) ) * ( MA(C,60)>MA(C,120))
    TJ5=(count(VOL<REF(VOL,1),2)==2 ) * ( VOL<=hhv(VOL,10)/2 ) * ( VOL<MA(VOL,5))
    a=(TJ1 ) * ( TJ2 ) * ( TJ3 ) * ( TJ4 ) * ( TJ5)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
