...
#开始
    ...
    AAA=(REF(C,1)/REF(C,2)<0.999)
    BBB=(C/REF(C,1)>1.001 ) * ( V*1.1<REF(V,1))
    CCC=(REF(C,1)*1.005<REF(O,1) ) * ( C>O*1.005)
    KKK=(C/REF(C,1)>1.0989)
    XIANCXH=(BBB ) * ( AAA ) * ( CCC  ) * ( NOT(KKK))
    RSV3=((C-llv(L,9))/(hhv(H,9)-llv(L,9))*100)
    K=(sma(RSV3,3,1))
    D=(sma(K,3,1))
    J=(3*K-2*D)
    X=(llv(J,2)==llv(J,8))
    进场=(CROSS(J,REF(J+0.01,1)) ) * ( X ) * ( J<20)
    a=(进场 ) * ( XIANCXH)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
