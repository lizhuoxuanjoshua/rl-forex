...
#开始
    ...
    M=(5)
    LC=(REF(CLOSE,1))
    RS1=(sma(np.maximum(CLOSE-LC,0),N1,1)/sma(np.abs(CLOSE-LC),N1,1)*100)
    RS2=(sma(np.maximum(CLOSE-LC,0),N2,1)/sma(np.abs(CLOSE-LC),N2,1)*100)
    RAA=(MA((RS1+RS2)/2,M))
    a=(CROSS(RS1,50) ) * ( RAA>REF(RAA,1))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
