...
#开始
    ...
    LL=((CLOSE-llv(LOW,20))/(hhv(HIGH,20)-llv(LOW,20))*100)
    LL1=sma(sma(LL,3,1),3,1)/28.57
    LL2=EMA(LL1,5)
    LL3=(3*LL1-2*LL2)
    MA5=MA(CLOSE,5)
    MA10=MA(CLOSE,10)
    ...
    MA60=MA(CLOSE,60)
    MA120=(MA(CLOSE,120))
    SS=(MA5>REF(MA5,1) ) * ( MA10>REF(MA10,1) ) * ( MA20>REF(MA20,1)) * ( MA60>REF(MA60,1) ) * ( MA120>REF(MA120,1))
    XG=(CROSS(C,MA(C,10)) ) * ( CROSS(C,MA(C,20)) ) * ( CROSS(C,MA(C,30)) ) * ( V>REF(V,1)*4)
    AA=(CROSS(LL3,LL1) ) * ( LL1<2.1 ) * ( C>O ) * ( SS ) * ( XG)
    QQ=np.abs(MA(C,10)/MA(C,20)-1)<0.01
    DD=(np.abs(MA(C,5)/MA(C,10)-1)<0.01)
    QD=np.abs(MA(C,5)/MA(C,20)-1)<0.01
    DQ=(MA(C,5)>REF(MA(C,5),1) ) * ( QQ ) * ( DD ) * ( QD)
    QQ1=(MA(C,3)+MA(C,6)+MA(C,12)+MA(C,24))/4
    QQ2=(QQ1+6*STD(QQ1,11))
    QQ3=(QQ1-6*STD(QQ1,11))
    DD1=(np.maximum(np.maximum(MA(C,5),MA(C,10)),np.maximum(MA(C,10),MA(C,20))))
    DD2=(np.minimum(np.minimum(MA(C,5),MA(C,10)),np.minimum(MA(C,10),MA(C,20))))
    ZZ=(O<=DD2 ) * ( C>=DD1 ) * (  REF(C<O,1)  ) * ( C>QQ2 ) * (C>QQ1 ) * ( QQ1>O ) * ( O/QQ3<1.005 ) * ( DQ)
    a=((AA) ) + ( (ZZ
))
#结束
    买入 = a
