...
#开始
    ...
    VAR1=(REF(CLOSE,1))
    VAR2=(((sma(np.maximum((CLOSE - VAR1),0),7,1) / sma(np.abs((CLOSE - VAR1)),7,1)) * 100))
    VAR3=(CROSS(79,VAR2))
    VAR4=(FILTER(VAR3,4))
    VAR19=((HIGH+LOW+OPEN+2*CLOSE)/5)
    VAR29=(REF(VAR1,1))
    VAR39=(sma((CLOSE-VAR2),6,6)/sma(np.abs(CLOSE-VAR2),6,6)*100)
    VAR49=((CLOSE-REF(OPEN,29))/REF(OPEN,29)*100)
    VAR5=(EMA(0.667*REF(VAR4,1)+0.333*VAR4,5))
    VAR6=((VOL>=1.3*MA(VOL,5) )  * ( VAR4<-17) ) * ( REF(LOW,1)==llv(LOW,120))
    VAR7=(sma(np.maximum(CLOSE-REF(C,1),0),6,1)/sma(np.abs(CLOSE-REF(C,1)),6,1)*100)
    VAR8=(sma(np.maximum(VAR1-VAR2,0),10,1)/sma(np.abs(VAR1-VAR2),10,1)*100)
    VAR9=((count(VAR8<20,5)>=1 ) * ( count(VAR1==llv(VAR1,10),10)>=1 ) * ( CLOSE>=OPEN*1.038 ) * ( VOL>MA(VOL,5)*1.2))
    a=((VAR9) ) + ( (VAR6))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
