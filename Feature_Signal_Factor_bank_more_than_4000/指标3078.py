...
#开始
    ...
    AB=( (VAR1-REF(VAR1,1))/REF(VAR1,1)*1000)
    AA=(AB>REF(AB,1) ) * ( AB>0)
    a=(AA ) * ( CROSS(均线1,均线2) ) * ( C>O ) * ( C>REF(C,1) ) * (( C/REF(C,1)>=1.01)) + ( (CROSS(均线1,均线4) )) * ( C>O ) * ( C>REF(C,1) ) * ( C/REF(C,1)>=1.018) + ( (CROSS(均线2,均线3) )) * ( C>O ) * ( C>REF(C,1) ) * ( C/REF(C,1)>=1.018) + ( (CROSS(均线1,均线3) )) * ( C>O ) * ( C>REF(C,1) ) * ( C/REF(C,1)>=1.018 ) * ( CROSS(均线1,均线4))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
