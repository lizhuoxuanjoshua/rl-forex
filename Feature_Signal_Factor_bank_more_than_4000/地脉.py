...
#开始
    ...
    VAR3=sma(np.abs(LOW-VAR2),3,1)/sma(np.maximum(LOW-VAR2,0),3,1)*100

    

    VAR4=EMA(np.where(CLOSE*1.2,VAR3*10,VAR3/10),3)
    ...
    VAR7=np.where(llv(LOW,90),1,0)

    

    VAR8=EMA(np.where(LOW<=VAR5,(VAR4+VAR6*2)/2,0),3)/618*VAR7
    ...
    VAR2S=sma(np.abs(LOW-VAR1S),13,1)/sma(np.maximum(LOW-VAR1S,0),10,1)
    ...
    VAR5S=EMA(np.where(LOW<=VAR4S,VAR3S,0),3)

    

    主力进场=np.where(VAR5S>REF(VAR5S,1),VAR5S,0)*1.1

    

    洗盘=np.where(VAR5S<REF(VAR5S,1),VAR5S,0)

    

    a=CROSS(洗盘,主力进场) * (买入==-20)
