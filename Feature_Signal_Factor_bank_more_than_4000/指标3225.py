...
#开始
    ...
    VAR8=(10*(VAR6>=150 ) * ( VAR6<200 ) * ( VAR7>=150 ) * ( VAR7<200))
    VAR9=(-10*(VAR6<=-150 ) * ( VAR6>-200 ) * ( VAR7<=-150 ) * ( VAR7>-200))
    VARA=((CLOSE-MA(CLOSE,13))/MA(CLOSE,13)*100)
    VARB=(100-np.abs(VARA))
    VARE=(REF(LOW,1)*0.9)
    VARF=(LOW*0.9)
    VAR10=(VARF*VOL+VARE)
    VAR11=(EMA(VAR10,30))
    VAR12=(VAR11-CLOSE)
    VAR13=(REF(CLOSE,1))
    VAR14=(sma(np.maximum(CLOSE-VAR13,0),7,1)/sma(np.abs(CLOSE-VAR13),7,1)*100)
    VAR15=(sma(np.maximum(CLOSE-VAR13,0),13,1)/sma(np.abs(CLOSE-VAR13),13,1)*100)
    VAR17=(sma(np.maximum(CLOSE-REF(CLOSE,1),0),6,1)/sma(np.abs(CLOSE-REF(CLOSE,1)),6,1)*100)
    ...
    VAR20=((VAR17<=25 ) * ( VAR18<-95 ) * ( VAR1F>20 ) * ( VAR1B<-30 )  * ( VAR12>=-0.25 ) * ( VAR14<22 ) * ( VAR15<28 ) )
    绿军=(np.where(VAR1<0,VAR2,0))
    导弹=(VAR20)
    条件=(VAR7>=200 ) * ( VAR6>=150)
    条件1=(VAR7<=-200 ) * ( VAR6<=-150)
    预警系统=(np.where(条件,15,np.where(条件1,-15,VAR9))+60)
    VZZ=(VAR3>=REF(VAR4,1) ) * ( CLOSE<REF(CLOSE,1) ) * ( 预警系统==60 ) * ( 导弹==0 ) * ( 绿军>=REF(绿军,1) )
    a=(VZZ ) * ( count(VZZ,30)==1)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
