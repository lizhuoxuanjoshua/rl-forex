...
#开始
    ...
    MA9=(EMA(CLOSE,730))
    MA10=(EMA(CLOSE,888))
    涨停收盘=((C-REF(C,1))*100/REF(C,1)>=4.8 ) * (  H-C<0.04)
    ...
    B55=(MA(VOLUME,55))
    B135=(MA(VOLUME,135))
    CCI=((TYP-MA(TYP,43))/(0.015*AVEDEV(TYP,43)))


    CR1=((REF(C,1)-REF(C,2))/REF(C,2)*100<2.8)
    VAR1X=((CLOSE*2+HIGH+LOW)/4*10)
    VAR2X=(EMA(VAR1X,13)-EMA(VAR1X,34))
    VAR3X=(EMA(VAR2X,5))
    VAR4X=(2*(VAR2X-VAR3X)*5.5)
    主力=(VAR4X)
    高开=(O>REF(C,1)*1.03)
    XG1=(C>MA5 ) * ( C> MA6 ) * (  DIF>=0.01 ) * ( DIF<=0.20 ) * ( MA30>MA60 ) * ( MA120>MA250  ) * ( ((CROSS(C,MA1))) + ( (CROSS(C,MA120))) )  * ( np.abs((MA1-MA250)/MA1*100)<20 ) * ( CCI<351 ) * (  高开  ) * ( MA30>MA60 ) * ( MA120>MA250  ) * ( (CROSS(C,MA1)) + ( (CROSS(C,MA120))) ) *  ( np.abs((MA1-MA250)/MA1*100)<20)
    XG2=(高 )  * ( MA3>MA4 ) * ( MA4>MA5 ) * ( MA5>MA6 ) * ( MA6>MA7 ) * ( MA7>MA8 ) * ( MA8>MA9  ) * ( MA9>MA10 ) * ( np.abs((MA1-MA10)/MA1*100)<25 )  * ( CROSS(C,MA1) ) * ( CCI>70 ) * ( CCI<90 ) * ( MA30>MA60 ) * ( MA120>MA250  ) * ( MA30>MA60 ) * ( MA120>MA250  ) * ( ((CROSS(C,MA1))) + ( (CROSS(C,MA120))) ) *  ( np.abs((MA1-MA250)/MA1*100)<20)
    XG3=(C>MA1  ) * ( C>MA2  ) * ( C>MA3  ) * ( C>MA4  ) * ( C>MA5 ) * ( C>MA5 ) * ( C> MA6  ) * ( C>MA7  ) * ( C>MA8  ) * ( C>MA9  ) * ( C>MA10 )   * (  DIF>=0.01 ) * ( DIF<=0.20 )  * ( MA30>MA60 ) * ( MA120>MA250  ) * ( ((CROSS(C,MA1))) + ( (CROSS(C,MA120))) ) * ( np.abs((MA1-MA250)/MA1*100)<20  ) * ( CCI>60 ) * ( CCI<91  ) * ( MA30>MA60 ) * ( MA120>MA250  ) * ( (CROSS(C,MA1)) + ( (CROSS(C,MA120))) ) * ( np.abs((MA1-MA250)/MA1*100)<20 ) * ( 涨停收盘)
    XG=((XG1) ) + ( (XG2) ) + ( (XG3
))
    收盘涨9=((C-REF(C,1))*100/REF(C,1)>=5.2 ) * ( H-C<0.019)
    STAR=(EVERY(CLOSE<OPEN,2))
    AAX=(HIGH<(REF(LOW,1)-0.001) ) * ( STAR)
    买点3=(REF(REF(OPEN,1),barslast(AAX)))
    买点1=(REF(REF(LOW,1),barslast(AAX)))
    买点2=(REF(HIGH,barslast(AAX)))
    涨停收盘6=((C-REF(C,1))*100/REF(C,1)>=6.3 ) * ( H-C<=0.04)
    停板=( 涨停收盘6 ) * (  (C-REF(C,1))*100/REF(C,1)<9)
    黑马=( 主力>-0.7  ) * ( (MAA3-MAA5)<0.36 ) * ( MAA3>MA120 ) * ( MACD>-0.8  ) * ( MACD<0.18 ) * (  DIF>=-0.19)
    牛股基因=(收盘涨9 ) * ( DIF>=-0.08 ) * (  DEA<=0.06)
    牛股2=(牛股基因 ) * ( CCI>92 ) * ( MA120>MA250 ) * ( CCI<95)
    牛股3=(牛股基因 ) * ( CCI>108 ) * ( MA120>MA250 ) * ( CCI<113)
    牛股4=(牛股基因 ) * ( CCI>90  )  * ( LOW< MA35 ) * ( CCI<99)
    牛股5=(牛股基因 ) * ( CCI>48  )  * ( CROSS(C,买点3) ) * ( CCI<99 ) * ( 高开)
    菊黄K=((( 牛股2) ) + ( (牛股3) ) + ( (牛股4 ) ) + ( (牛股5) )  + ( (XG)))
    粉K=((1 ) * ( MA30>MA60 ) * ( ((CROSS(C,MA30))) + ( (CROSS(C,MA120))) + ( (CROSS(C,MA250))) + ( (CROSS(C,MA500)) ) + ( (LOW< MA30)))  ) * (  MA30- MA60<0.7 ) * (  涨停收盘  )
    a=(菊黄K ) + ( 粉K )
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
