...
#开始
    ...
    EMA38=(EMA(C,38))
    EMA72=(EMA(C,72))
    a=(CROSS(EMA4,EMA7) ) * ( C>O ) * ( C>REF(C,1) ) * (( C/REF(C,1)>=1.01)) + ( (CROSS(EMA4,EMA38) )) * ( C>O ) * ( C>REF(C,1) ) * ( C/REF(C,1)>=1.018) + ( (CROSS(EMA7,EMA18) )) * ( C>O ) * ( C>REF(C,1) ) * ( C/REF(C,1)>=1.018) + ( (CROSS(EMA4,EMA18) )) * ( C>O ) * ( C>REF(C,1) ) * ( C/REF(C,1)>=1.018)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
