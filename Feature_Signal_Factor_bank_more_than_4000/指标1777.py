...
#开始
    ...
    VAR1=((CLOSE-llv(LOW,75))/(hhv(HIGH,75)-llv(LOW,75))*100)
    VAR2=(sma(VAR1,20,1))
    VAR3=(sma(VAR2,15,1))
    VAR4=(100-3*VAR2+2*VAR3)
    VAR5=(np.where(VAR4>REF(VAR4,1),VAR4,VAR4-5))
    VAR6=(hhv(HIGH,25))
    VAR7=(llv(LOW,25))
    VAR8=(EMA((CLOSE-VAR7)/(VAR6-VAR7)*100,5))
    VAR9=(VAR4>VAR5 ) * ( VAR8>=REF(VAR8,1))
    VAR10=(VAR4>VAR5)
    VAR11=(np.maximum(CLOSE,EMA(CLOSE, 13)))
    VAR12=(REF(C,1))
    VAR13=(hhv(VAR12,2))
    VAR14=(REF(VAR13,1))
    VAR15=(llv(VAR12,2))
    VAR16=(REF(VAR15,1))
    VAR17=(barslast(C<VAR15 ) * ( VAR12>=VAR16))
    VAR18=(REF(VAR17,1))
    VAR19=(barslast(C>VAR13 ) * ( VAR12<=VAR14))
    VAR20=(REF(VAR19,1))

    VAR22=(np.maximum(CLOSE, VAR11))
    VAR23=(np.minimum(CLOSE, VAR22))

    VAR25=(VAR23>=REF(VAR23,1) ) * ( VAR23==VAR22)
    a= ( VAR9 ) * ( VAR25)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
