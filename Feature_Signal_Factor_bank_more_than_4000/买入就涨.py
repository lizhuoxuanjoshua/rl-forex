...
#开始
    ...
    VAR1A=np.where((CROSS(VAR6,VAR8)) * (CROSS(VAR7,VAR8)) * (CROSS(VAR7,VAR6)),1,0)
    ...
    VAR1F=np.power(VAR1D,3)*0.1+np.power(VAR1D,2)

    

    拉升资金=np.where(VAR1D>0.015,VAR1F,0)/45
    ...
    主力资金=np.power(VAR11,3)*0.1+np.power(VAR11,1) 

    

    a=(CROSS(主力资金,0)) * (CROSS(拉升资金,0.01)) * (VOL/MA(REF(VOL,1),5)>2)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
