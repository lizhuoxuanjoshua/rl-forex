...
#开始
    ...
    RSV1=((CLOSE-llv(LOW,90))/(hhv(HIGH,90)-llv(LOW,90))*100)
    K1=(sma(RSV1,5,1))
    D1=(sma(K1,3,1))
    J1=(3*K1-2*D1)
    RSV2=((CLOSE-llv(LOW,90))/(hhv(HIGH,90)-llv(LOW,90))*100)
    K2=(sma(RSV2,9,1))
    D2=(sma(K2,3,1))
    J2=(3*K2-2*D2)
    AA=(CROSS(J1,J2) ) * ( J1<20 ) * ( J2<20)
    AB=(100*(hhv(HIGH,90)-CLOSE)/(hhv(HIGH,90)-llv(LOW,90)))
    a=(CROSS(90,AB) ) * ( AA)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
