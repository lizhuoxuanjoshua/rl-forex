...
#开始
    ...
    VARR5=(VARR3>VARR4)
    JX=(REF(CLOSE,1))
    RSI1=(sma(np.maximum(CLOSE-JX,0),12,1)/sma(np.abs(CLOSE-JX),12,1)*100)
    RSI2=(sma(np.maximum(CLOSE-JX,0),56,1)/sma(np.abs(CLOSE-JX),56,1)*100)
    VARR6=(RSI1>RSI2 ) * ( VARR5 ) * ( VARR2)

    AA1=(np.abs(((3.48*CLOSE+HIGH+LOW)/4-EMA(CLOSE,23))/EMA(CLOSE,23)))
    AA2=(DMA(((2.15*CLOSE+LOW+HIGH)/4),AA1))
    S1=(EMA(AA2,200)*1.1)
    S2=(CROSS(C,S1) ) * ( REF(C*1.097,1)<C ) * ( VARR6 )
    S3=(REF(C*1.05,1)<=C)
    a=(REF(S2,1) ) * ( (REF(C,1)-C)/REF(C,1)*S3)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
