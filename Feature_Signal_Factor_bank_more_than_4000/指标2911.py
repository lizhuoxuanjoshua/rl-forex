...
#开始
    ...
    AAA=(sma(np.maximum(C-REF(C,1),0),8,1)/sma(np.abs(C-REF(C,1)),8,1)*100)
    BB=(90-AAA)
    X0=(((C-llv(L,33))/(hhv(H,33)-llv(L,33)))*67)
    X1=(MA(3*sma(X0,33,1)-2*sma(sma(X0,43,1),5,1),2))
    X2=(np.where(X1>REF(X1,1),X1,X1+2*(X1-REF(X1,1))))
    X_L1=((O+C*3+L+H)/6)
    X_L2=(V*X_L1)
    X_L3=(REF(count(X_L2,5)/count(V,5),1))
    AA=((EMA(X2,7))+20)
    a= ( ((CROSS(AAA,BB)) ) + ( (CROSS(AAA,50))) ) * ( C>=X_L3)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
