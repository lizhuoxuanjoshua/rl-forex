...
#开始
    ...
    LC=(EMA(CLOSE,3))
    A=(((sma(np.maximum((CLOSE - LC),0),6,1) / sma(np.abs((CLOSE - LC)),6,1)) * 100))
    B=(MA(A,10))
    QVAR2=(np.where(((HIGH + LOW) <= (REF(HIGH,1) + REF(LOW,1))),0,np.maximum(np.abs((HIGH - REF(HIGH,1))),np.abs((LOW - REF(LOW,1))))))
    QVAR3=(np.where(((HIGH + LOW) >= (REF(HIGH,1) + REF(LOW,1))),0,np.maximum(np.abs((HIGH - REF(HIGH,1))),np.abs((LOW - REF(LOW,1))))))
    QVAR4=((count(QVAR2,13) / (count(QVAR2,13) + count(QVAR3,13))))
    QVAR5=((count(QVAR3,13) / (count(QVAR3,13) + count(QVAR2,13))))
    ...
    QVARB=((((QVAR9 < REF(QVARA,1)) ) * ( (VOL > REF(VOL,1))) ) * ( (CLOSE > REF(CLOSE,1))))
