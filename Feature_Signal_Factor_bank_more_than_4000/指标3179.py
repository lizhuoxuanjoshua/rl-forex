...
#开始
    ...
    RFV=(C/REF(C,1)>1.099 ) * ( H>L ) * ( H==C)
    TGH=(barslast(RFV))
    TYGR=(REF(C,TGH))
    UIOP=(REF(O,TGH))
    QSCF=(np.add(TYGR,UIOP)/2)
    EDCX=(REF(barslast(RFV),1))
    QWSDF=(CROSS(C,QSCF) ) * ( C>O ) * ( C>REF(C,1) ) * ( (C-REF(C,1))/REF(C,1)*100>0 ) * ( (H/L-1)*100>0 ) * ( EDCX>=2 ) * ( EDCX<=15)
    QWSDFA=(FILTER(QWSDF,30))
    SS=(QWSDFA==1)
    TY=(barslast(SS))
    TYTYGR=(REF(C,TY))
    TYUIOP=(REF(O,TY))
    TYQ=(CROSS(H,TYUIOP) ) * ( TY>=5 ) * ( TY<=15)
    TYQ2=(CROSS(H,TYTYGR) ) * ( TY>=5 ) * ( TY<=15)
    a=FILTER(TYQ2,15)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
