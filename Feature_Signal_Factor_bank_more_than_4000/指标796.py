...
#开始
    ...
    SG=(EMA(hhv(HIGH,1),8))
    X1=(EMA(CLOSE,8))
    X30=(((X1 < REF(X1,1)) ) * ( (CLOSE < X1)))
    X31=(((((SG < REF(SG,1))) ) + ( (X30))))
    LC=(REF(CLOSE,1))
    操作线=(sma(np.maximum(CLOSE-LC,0),2,1)/sma(np.abs(CLOSE-LC),2,1)*100)
    趋势线=(sma(np.maximum(CLOSE-LC,0),14,1)/sma(np.abs(CLOSE-LC),14,1)*100)
    震仓线=(45)
    震仓线2=(50)
    逃命线=(80)
    抄底线=(20)
    绝底线=(5)
    PP2=(操作线<震仓线2 ) * ( REF(操作线,1)>震仓线2)
    PP=(操作线<震仓线 ) * ( REF(操作线,1)>震仓线)
    PP3=(操作线<抄底线 ) * ( REF(操作线,1)>抄底线*VAB3)
    a=(((((((count((X31 == 1),4) == 3) ) * ( (X31 == 0))) * ( O<C ) * ( (((C-REF(C,1))/REF(C,1))>0.085))*89) >=89))) + ((((C-REF(C,1))/REF(C,1)>0.085 )) * ( (REF(PP3,1) ) + ( (REF(PP,1)))) ) * ( VAB3==1)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
