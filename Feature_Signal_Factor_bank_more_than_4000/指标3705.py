...
#开始
    ...
    最高位=(barslast((hhv(C,76)==C )))
    最低位=(barslast((llv(L,最高位)==L)))
    最高价=(REF(C,最高位))
    最低价=(llv(L,最高位))
    振幅合理=((最高价-最低价)/最低价>0.1)
    前高条件=(最高位<77 ) * ( 最高位>10 ) * ( 最高位>最低位 ) * ( 振幅合理)
    三日振幅=((hhv(np.maximum(C,O),3)-llv(np.minimum(C,O),3))/llv(np.minimum(C,O),3)*100<5)
    今日大阳=(C/REF(C,1)>1.055)
    上影长度1=( H-np.maximum(C,O))
    下影长度1=(np.minimum(C,O)-L)
    K线长度1=(np.abs(C-O))
    纺锤=((下影长度1 >K线长度1/5 ) * ( 上影长度1>K线长度1/5 ))
    去影线1=(((下影长度1==0) ) + ( (上影长度1==0)))
    TY1=(C)
    SR11=(REF(TY1,10)==hhv(TY1,2*10+1))
    SR21=(FILTER(SR11,10))
    a=(REF(三日振幅,1) ) * ( 今日大阳 ) * ( REF(前高条件,1) ) * ( 去影线1)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
