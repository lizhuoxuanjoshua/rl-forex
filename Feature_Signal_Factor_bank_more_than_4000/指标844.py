...
#开始
    ...
    VAR2=(REF(CLOSE,1))
    VAR3=(sma(np.maximum(CLOSE-VAR2,0),3,1)/sma(np.abs(CLOSE-VAR2),3,1)*100)
    VAR4=(count(VAR3<13,18)>=1)
    VAR6=(O==L ) * ( H==C)
    VAR7=(V < MA(V,3) ) * ( V < REF(V,1))
    P1=((C/REF(C,1)-1))
    P2=(count(P1,3)*count(P1,5)*count(P1,5)*count(P1,8)*count(P1,10)*count(P1,13)*count(P1,18))
    P3=(3*P2)
    P4=(count(P2,18)+0.00005)
    A=(C>REF(C,1))
    B=((C-REF(C,1))/L*100)
    LC=( REF(CLOSE,1))
    RSI=( sma(np.maximum(C-LC,0),5,1)/sma(np.abs(CLOSE-LC),3,1)*100)
    A4 =( count(RSI<13,21)>=1)
    a= ( VAR4 )
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
