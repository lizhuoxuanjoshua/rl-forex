...
#开始
    ...
    VAR1=((C+L+H)/3)
    VAR2=(sma(VAR1,5,1)-sma(VAR1,13,1))
    VAR3=((VAR2-sma(VAR2, 3,1))*100)
    VAR4=(sma(VAR3,5,2))
    VAR5=(np.where(VAR3>=REF(VAR3,1),VAR3,0))
    VAR6=(np.where(VAR3<REF(VAR3,1),VAR3,0))
    VAR7=(EMA(CLOSE,12)-EMA(CLOSE,26)*10)
    VAR8=(EMA(VAR7,9))
    a=(((((hhv(VAR3,4)<0) ) * ( (VAR3>REF(VAR3,1))) ) * ( (REF(VAR3,1)<REF(VAR3,2))) ) * ( (REF(VAR3,2)<REF(VAR3,3))))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
