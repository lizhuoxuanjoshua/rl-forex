...
#开始
    ...
    QQ1=(hhv(2*(EMA(CLOSE,12)-EMA(CLOSE,26)-CC),4)<0) * ( 2*(EMA(CLOSE,12)-EMA(CLOSE,26)-CC)>REF(2*(EMA(CLOSE,12)-EMA(CLOSE,26)-CC),1)) * ( REF(2*(EMA(CLOSE,12)-EMA(CLOSE,26)-CC),1)<REF(2*(EMA(CLOSE,12)-EMA(CLOSE,26)-CC),2)  ) * ( REF(2*(EMA(CLOSE,12)-EMA(CLOSE,26)-CC),2) <REF(2*(EMA(CLOSE,12)-EMA(CLOSE,26)-CC),3))
    QQ2=(EMA(EMA(EMA(EMA(CLOSE,2),2),2),2))
    DD1=(CROSS(QQ2,EMA(REF(QQ2,1),2)) ) * ( C>O ) * ( V>=MA(V,21) ) * ( V> REF(V,1))
    DD2=(DD1+QQ1+DD>=2  ) * ( EMA(CLOSE,12)-EMA(CLOSE,26)>CC ) * ( H/C<1.01 ) * ( REF(MA(C,5),1)<MA(C,5) ) * (  REF(MA(C,5),2)>REF(MA(C,5),1))
    MACD=((EMA(CLOSE,12)-EMA(CLOSE,26)-EMA(EMA(CLOSE,12)-EMA(CLOSE,26),9))*2)
    RSV=((CLOSE-llv(LOW,9))/(hhv(HIGH,9)-llv(LOW,9))*100)
    K=(sma(RSV,3,1))
    D=(sma(K,3,1))
    cc1=(CROSS(K,D) ) * ( MACD>0)
    a=(DD2 ) * ( REF(H/C<1.02,1) ) * ( cc1)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
