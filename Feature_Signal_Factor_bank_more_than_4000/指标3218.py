...
#开始
    ...
    VAR1=((VOL / (((HIGH - LOW) * 2) - np.abs((CLOSE - OPEN)))))
    买盘=(np.where((CLOSE > OPEN),(VAR1 * (HIGH - LOW)),np.where((CLOSE < OPEN),(VAR1 * ((HIGH - OPEN) + (CLOSE - LOW))),(VOL / 2))))
    卖盘=(np.where((CLOSE > OPEN),(0 - (VAR1 * ((HIGH - CLOSE) + (OPEN - LOW)))),np.where((CLOSE < OPEN),(0 - (VAR1 * (HIGH - LOW))),(0 - (VOL / 2)))))
    买卖差=((买盘 + 卖盘))
    量比=((VOL / MA(VOL,5)))

    VAR2=((HIGH - LOW))
    VAR3=(np.abs((OPEN - CLOSE)))
    VAR4=((HIGH - np.where((CLOSE > OPEN),CLOSE,OPEN)))
    VAR5=((np.where((CLOSE > OPEN),OPEN,CLOSE) - LOW))
    VAR6=(((VAR3 / VAR2) * VOL))
    VAR7=(((VAR4 / VAR2) * VOL))
    VAR8=(((VAR5 / VAR2) * VOL))
    VARA=((((CLOSE - MA(CLOSE,5)) / MA(CLOSE,5)) * 100))
    VARB=(((MA(CLOSE,5) > MA(CLOSE,10)) ) * ( (MA(CLOSE,10) > MA(CLOSE,30))) ) * ( (MA(CLOSE,30) > MA(CLOSE,60)))
    VARC=(((MA(CLOSE,5) < MA(CLOSE,10)) ) * ( (MA(CLOSE,10) < MA(CLOSE,30))))
    VARD=(REF(CLOSE,1))
    VARE=(((sma(np.maximum((CLOSE - VARD),0),6,1) / sma(np.abs((CLOSE - VARD)),6,1)) * 100))
    VAR40=(((np.where((((VAR6 >= VAR7) ) * ( (VAR6 >= VAR8)) ) * ( (CLOSE > OPEN)),VAR6,买盘) + 买盘) / 2))
    VAR41=(((np.where(((VAR7 > VAR6) ) * ( (VAR7 > VAR8)),VAR7,买盘) + 买盘) / 2))
    VAR42=(((np.where(((VAR8 > VAR6) ) * ( (VAR8 > VAR7)),VAR8,买盘) + 买盘) / 2))
    VAR50=(((np.where((((VAR6 >= VAR7) ) * ( (VAR6 >= VAR8)) ) * ( (CLOSE < OPEN)),VAR6,卖盘) + 卖盘) / 2))
    三日上攻=(MA((((VAR40 + VAR41) + VAR42) / 3),3))
    三日下攻=(MA(VAR50,3))
    上攻均量=(MA(买盘,3))
    a=((((((量比 > 1) ) * ( ((买盘 / 三日上攻) > 1.58)) ) * ( np.where((三日上攻 > 上攻均量),(三日上攻 > 三日下攻),(三日上攻 > REF(三日上攻,1)))) ) * ( (三日下攻 < REF(三日下攻,1))) ) * ( ((((CLOSE - REF(CLOSE,3)) / REF(CLOSE,3)) * 100) < 14)))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
