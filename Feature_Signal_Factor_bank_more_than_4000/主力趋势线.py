...
#开始
    ...
    K1=((hhv(HIGH,9)-CLOSE)/(hhv(HIGH,9)-llv(LOW,9))*100-70)
    K2=(sma(K1,9,1)+100)
    K3=((CLOSE-llv(LOW,9))/(hhv(HIGH,9)-llv(LOW,9))*100)
    K4=(sma(K3,3,1))
    K5=(sma(K4,3,1)+100)
    K6=(K5-K2)
    趋势=np.where(K6>45,K6-45,0)
    主力线=(3*sma((CLOSE-llv(LOW,27))/(hhv(HIGH,27)-llv(LOW,27))*100,5,1)-2*sma(sma((CLOSE-llv(LOW,27))/(hhv(HIGH,27)-llv(LOW,27))*100,5,1),3,1))
    QA=(主力线)
    QB=(趋势)
    QAA=(((CROSS(QA,8)) ) + ( (CROSS(QA,8.4)) ) + ( (CROSS(QA,8.8)) ) + ( (CROSS(QA,9.2)) ) + ( (CROSS(QA,9.6)) ) + ( (CROSS(QA,10)) ) + ( (CROSS(QA,10.2)) ) + ( (CROSS(QA,10.6)) ) + ( (CROSS(QA,11))))
    QBB=(((CROSS(QB,2)) ) + ( (CROSS(QB,3)) ) + ( (CROSS(QB,4)) ) + ( (CROSS(QB,5)) ) + ( (CROSS(QB,6)) ) + ( (CROSS(QB,7)) ) + ( (CROSS(QB,8)) ) + ( (CROSS(QB,9)) ) + ( (CROSS(QB,10)) ) + ( (CROSS(QB,11))))
    条件=(((QAA) ) + ( (QBB)))
    a=(条件 ) * ( C>REF(C,1) ) * ( C>MA(C,3))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
