...
#开始
    ...
    X1=(np.abs(MA(C,10)/MA(C,20)-1)<0.01)
    X2=(np.abs(MA(C,5)/MA(C,10)-1)<0.01)
    X3=(np.abs(MA(C,5)/MA(C,20)-1)<0.01)
    X4=(MA(C,5)>REF(MA(C,5),1) ) * ( X1 ) * ( X2 ) * ( X3)
    BBIBOLL=((MA(C,3)+MA(C,6)+MA(C,12)+MA(C,24))/4)
    UPR=(BBIBOLL+6*STD(BBIBOLL,11))
    DWN=(BBIBOLL-6*STD(BBIBOLL,11))
    X5=(np.maximum(np.maximum(MA(C,5),MA(C,10)),np.maximum(MA(C,10),MA(C,20))))
    X6=(np.minimum(np.minimum(MA(C,5),MA(C,10)),np.minimum(MA(C,10),MA(C,20))))
    X7=(O<=X6 ) * ( C>=X5  ) * ( REF(C>O,1) ) * (  MA(C,60)>REF(MA(C,60),1)) * ( MA(C,20)>REF(MA(C,20),1) ) * ( C>O ) * (C>UPR ) * ( C>BBIBOLL ) * ( BBIBOLL>O )  * ( X4)
    X8=(O<=X6 ) * ( C>=X5  ) * (  REF(C<O,1)  ) * ( C>UPR ) * ( C>BBIBOLL ) * ( BBIBOLL>O ) * ( O/DWN<1.005 ) * ( X4)
    a=((X7) ) + ( (X8))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
