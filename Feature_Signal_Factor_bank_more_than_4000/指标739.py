...
#开始
    ...
    A2=((((H + L) + O) + C) / 4)
    A17=((count((A2 * VOL),16)) / (count(VOL,16)))
    A18=((count((A2 * VOL),11)) / (count(VOL,11)))
    A19=((count((A2 * VOL),6)) / (count(VOL,6)))
    A20=(((count((A2 * VOL),7)) - (A2 * VOL)) / ((count(VOL,7)) - VOL))
    A21=(((A18 - A17) / A2) * 1000)
    A22=(np.where((A21 > 50),50,(np.where((A21 < -50),-50,A21))))
    A23=(((A19 - A18) / A2) * 1000)
    A24=(np.where((A23 > 50),50,(np.where((A23 < -50),-50,A23))))
    A25=(((((A19 - A20) / A20) * 5) / 4) * 1000)
    A26=(np.where((A25 > 50),50,(np.where((A25 < -50),-50,A25))))
    A27=((A22 + A24) + A26)
    红柱线=(np.where((A27 > 0),A27,0))
    AA=((((红柱线 > (REF(红柱线,1))) ) * ( (红柱线 > 49)) ) * ( (CLOSE < (REF(CLOSE,1)))) ) * ( (((OPEN < (REF(CLOSE,1)))) ) + ( (((OPEN == (REF(CLOSE,1))) ) * ( ((HIGH / (REF(CLOSE,1))) < 1.02)))))
    a=(AA ==1)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
