...
#开始
    ...
    命LCC=(EMA(命LC,5))
    空上H=(hhv(命HCC,10))
    空下H=(np.where(命HCC<空上H,(命HCC-(空上H-命HCC)),空上H))
    多下H=(llv(空下H,10))
    空上L=(hhv(命LCC,10))
    空下L=(np.where(命LCC<空上L,(命LCC-(空上L-命LCC)),空上L))
    多下L=(llv(空下L,10))
    弱势线=(np.where(空下L>多下H,(空下L+(空下L-多下H)),空下L))
    强势线=(np.where(空下H>多下L,(空下H+(空下H-多下L)),空下H))
    命=(EMA(C,5))
    命C=(EMA(命,5))
    命CC=(EMA(命C,5))
    CG=(命CC)
    空上=(hhv(CG,10))
    空下=(np.where(CG<空上,(CG-(空上-CG)),空上))
    多下=(llv(空下,10))
    多上=(np.where(空下>多下,(空下+(空下-多下)),空下))
    多1=(llv(多上,1))
    多5=(llv(多上,5))
    a=(多1>多5 ) * ( CROSS(强势线,C))
#结束
