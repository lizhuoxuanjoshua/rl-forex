...
#开始
    ...
    X_8=(MA(CLOSE,120))
    X_9=(MA(CLOSE,250))
    X_10=(100*(CLOSE-REF(llv(CLOSE,240),1))/(REF(hhv(CLOSE,240),1)-REF(llv(CLOSE,240),1)))
    X_11=(np.maximum(np.maximum(X_1,X_2),np.maximum(X_3,X_4)))
    X_12=(np.maximum(np.maximum(X_11,X_6),np.maximum(X_7,X_8)))
    X_13=(np.minimum(np.minimum(X_1,X_2),np.minimum(X_3,X_4)))
    X_14=(np.minimum(np.minimum(X_13,X_6),np.minimum(X_7,X_8)))
    X_15=((X_12/X_14-1)*100<10 ) * ( X_10<60)
    X_16=(CROSS(CLOSE,X_1))
    X_17=(CROSS(CLOSE,X_2))
    X_18=(CROSS(CLOSE,X_3))
    X_19=(CROSS(CLOSE,X_4))
    X_20=((CLOSE/REF(CLOSE,1)-1)*100>=2 ) * ( VOL>REF(VOL,1))
    X_21=(X_16+X_17+X_18+X_19>=2)
    X_22=(X_1>REF(X_1,1) ) * ( X_2>REF(X_2,1) ) * ( X_3>REF(X_3,1) ) * ( X_4>REF(X_4,1) ) * ( X_6>REF(X_6,1))
    X_23=(count(CROSS(X_3,X_4),20)+count(CROSS(X_1,X_2),20)+count(CROSS(X_2,X_3),20)+count(CROSS(X_4,X_6),20)+count(CROSS(X_6,X_8),20)>=2)
