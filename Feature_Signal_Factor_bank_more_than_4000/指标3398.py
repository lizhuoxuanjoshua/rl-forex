...
#开始
    ...
    N=(40)
    N1=(20)
    AA=(((3 * sma((((CLOSE - llv(LOW,N)) / (hhv(HIGH,N) - llv(LOW,N))) * 100),5,1)) - (2 * sma(sma((((CLOSE - llv(LOW,N)) / (hhv(HIGH,N) - llv(LOW,N))) * 100),5,1),3,1))))
    AB=(((3 * sma((((CLOSE - llv(LOW,N1)) / (hhv(HIGH,N1) - llv(LOW,N1))) * 100),5,1)) - (2 * sma(sma((((CLOSE - llv(LOW,N1)) / (hhv(HIGH,N1) - llv(LOW,N1))) * 100),5,1),3,1))))
    a=(CROSS(AB,AA))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
