...
#开始
    ...
    TT=(MA(C,30)>MA(C,60) ) * ( MA(C,60)>MA(C,120))
    DD=(C>REF(C,1))
    QQ=(V>REF(V,1))
    TT1=(count(L<MA(C,13),5)==1)
    TT2=((C-REF(C,1))/REF(C,1)*100>3)
    DD1=(V>REF(V,1)*2 ) * ( C>REF(C,1))
    DD2=(TT1 ) * ( 0<MA(C,13)) * ( TT2) * ( count(C/REF(C,1)>1.095,15)>0)
    DD3=(REF((C>O) ) + ( (O>C)))
    a=(DD1 )* ( DD3 ) * ( QQ )
#结束
    买入 = a
