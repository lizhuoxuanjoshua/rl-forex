...
#开始
    ...
    TRTR=(EMA(np.maximum(np.maximum(HIGH-LOW,np.abs(HIGH-REF(CLOSE,1))),np.abs(REF(CLOSE,1)-LOW)),7))
    HD =(HIGH-REF(HIGH,1))
    LD =(REF(LOW,1)-LOW)
    DMP=(EMA(HD,7))
    DMM=(EMA(LD,7))
    PDI=(DMP*100/TRTR)
    MDI=(DMM*100/TRTR)
    ADX=(EMA(np.abs(MDI-PDI)/(MDI+PDI)*100,7))
    ADXR=(EMA(ADX,7))
    a=(count(PDI<MDI,3)==3 ) * ( REF(ADX>50,1) ) * ( REF(ADX,1)>=REF(ADX,2) ) * (ADX<REF(ADX,1))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
