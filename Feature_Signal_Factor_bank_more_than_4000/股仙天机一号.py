...
#开始
    ...
    VAR1=(llv(LOW,30))
    VAR2=(hhv(HIGH,20))
    VAR3=(EMA((CLOSE-VAR1)/(VAR2-VAR1)*100,13)*0.998)
    价仙=(EMA(0.667*REF(VAR3,1)+0.3333*VAR3,1))
    中轨=(sma(sma(价仙,20,1),20,1))
    下轨=(中轨-25)
    AA=(count(np.where(CLOSE>REF(CLOSE,1),VOL,np.where(CLOSE<REF(CLOSE,1),-VOL,0)),0))
    VARA=(llv(AA,30))
    VARB=(hhv(AA,20))
    VARC=(EMA((AA-VARA)/(VARB-VARA)*100,13)*0.998)
    量仙=(EMA(0.667*REF(VARC,1)+0.3333*VARC,1))
    a=(价仙<下轨 )
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
