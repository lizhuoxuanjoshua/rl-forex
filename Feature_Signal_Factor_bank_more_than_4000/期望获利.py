...
#开始
    ...
    M2=(6)
    M3=(8)
    EE=((H+np.maximum(C,O))/2)
    EE1=(MA(EE,3))
    EE2=((L+np.minimum(C,O))/2)
    EE3=(MA(EE2,3))
    超短生命线=(MA(EE,3))
    FF=((H+L+C+O)/4)
    FF1=(MA(FF,3))
    FF3=(REF((C+O)/2,1)>REF((C+O)/2,2) ) * ( REF(np.minimum(C,O),1)>REF(np.maximum(C,O),2))
    实际乖离=((np.maximum(FF,FF1)-np.minimum(FF,FF1))/np.minimum(FF,FF1)*100)
    FF4=((((FF1-(REF(FF1,1)-FF1))-L)/L)*100)
    FF5=((((EE1+(EE1-REF(EE1,1)))-L)/L)*100)
    FF6=(FF1-(REF(FF1,1)-FF1))
    FF7=(EE1+(EE1-REF(EE1,1)))
    期望获利率=(np.where(FF3,FF5,FF4))
    YY1=(REF(EVERY(C>=O,4),1) ) * ( REF(EVERY((C+O)/2>FF1,4),1) ) * ( (C+O)/2<FF1)
    YY2=  ( (FF1-O)/O*1000>5 ) * ( O<REF(np.minimum(C,O),1))
    A=(sma(L,4,3))
    B=(sma(H,4,3))
    BB=(sma(np.minimum(C,O),4,3))
    BB1=(REF(L,1)-hhv((np.maximum(REF(L,2),REF(L,1))-np.minimum(REF(L,2),REF(L,1))),2))
    BB2=(REF((C+O)/2,1)>REF((C+O)/2,2))
    BB3=(L<=BB1+BB1*0.01 ) * ( O>REF(O,2) ) * ( O>=REF(L,3) ) * ( count(C>O,7)>3)
    BB6=((BB1-L)/L*100)
    BB7=((REF(L,1)-BB1)/BB1*100)
    A1=((A-L)/L>N/1000)
    A8=(A1)
    A9=((REF(H,1)-O)/O*100-(REF(np.maximum(C,O),2)-REF(np.maximum(C,O),1))/REF(np.maximum(C,O),1)*100)
    A10=(np.where(BB2,BB6,A9))
    BB4=(BB6+A10>2)
    BB5=(np.where(BB2,BB3,BB4) ) * ( 期望获利率>2)
    a= ( A8 ) * ( BB5)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
