...
#开始
    ...
    VAR2=(1)
    VAR3=((MA(AMOUNT,5)/MA(VOL,5)/100-MA(AMOUNT,125)/MA(VOL,125)/100)/(MA(AMOUNT,5)/MA(VOL,5)/100)<-0.4)
    VAR5=((CLOSE-llv(LOW,100))/(hhv(HIGH,100)-llv(LOW,100))*200)
    VAR6=(sma(VAR5,3,1))
    VAR7=(sma(VAR6,3,1))
    VAR8=(3*VAR6-2*VAR7)
    VAR9=(llv(LOW,5))
    VARA=(hhv(HIGH,5))
    VARB=(EMA((CLOSE-VAR9)/(VARA-VAR9)*100,4)*2)
    VARC=(EMA(0.667*REF(VARB,1)+0.333*VARB,2))
    VARD=(3*VARB-2*VARC)
    VARE=(np.power(MA(CLOSE,5),2)+MA(CLOSE,5))
    VARF=(np.power(MA(LOW,5),2)+MA(LOW,5))
    VAR10=(np.power(MA(HIGH,5),2)+MA(HIGH,5))
    VAR11=((VARE-llv(VARF,64))/(hhv(VAR10,64)-llv(VARF,64))*150+65-10)
    ...
    VAR16=(EMA(VAR15,13))
    VAR17=(STD(VAR15,13))
    VAR18=((VAR15-VAR16)*100/VAR17)
    VAR19=(EMA(VAR18,5))
    VAR1A=(EMA(VAR19,13))
    VAR1B=(EMA(VAR19,13)+200-100)
    VAR1C=(EMA(VAR1B,10))
    VAR1D=(3*VAR1B-2*VAR1C)
    VAR1E=(VAR6+VARB+VAR12+VAR1B)
    VAR1F=(VAR7+VARC+VAR13+VAR1C)
    VAR20=(VAR8+VARD+VAR14+VAR1D)
    VAR21=((HIGH+LOW+CLOSE)/3)
    VAR22=(AMOUNT/VOL)
    VAR23=(count(AMOUNT,5)/count(VOL,5))
    VAR24=((VAR21-MA(VAR21,34))/(0.015*AVEDEV(VAR21,34)))
    VAR25=(EMA(AMOUNT/VOL,VOL))
    VAR26=((HIGH+LOW+CLOSE+VAR21)/4)
    VAR27=(EMA(VAR26,24))
    VAR28=(STD(VAR26,24))
    VAR29=((VAR26-VAR27)/VAR28*100)
    VAR2A=((VAR21-MA(VAR21,24))/(0.015*AVEDEV(VAR21,24)))
    VAR2B=(EMA(VAR29,5))
    VAR2C=((VAR22-MA(VAR22,21))/(0.015*AVEDEV(VAR22,21)))
    VAR2D=((VAR23-MA(VAR23,90))/(0.015*AVEDEV(VAR23,90)))
    VAR2E=(((VAR2D+VAR2C+VAR2B+VAR2A+VAR24)/5+165)/4)
    VAR2F=((EMA(EMA(EMA(VAR2E,15),10),5)-25)*2+VAR25)
    VAR30=((VAR1E+VAR1F+VAR20*30)/32/5-15)
    VAR31=(EMA(VAR30,13))
    VAR32=((VAR2E+VAR30)/2/1.1)
    DX=VAR32*VAR2
    VAR33=((VAR2F+VAR31)/2*VAR2)
    VAR34=(EMA(VAR33,1))
    ZX=EMA(VAR34,2)
    VAR35=(CLOSE*50/100)
    VAR36=(MA(VAR35,30)*0.83)
    VAR37=(MA(VAR35/VAR36,3))
    a=(CROSS(DX,ZX) ) * (( count(VAR37<1.04,3)>=1) ) + ( (CROSS(DX,0) )) * ( count(VAR37<1.04,3)>=1)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
