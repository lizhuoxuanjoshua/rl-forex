...
#开始
    ...
    AA=(((3*sma((CLOSE-llv(LOW,27))/(hhv(HIGH,27)-llv(LOW,27))*100,5,1)-2*sma(sma((CLOSE-llv(LOW,27))/(hhv(HIGH,27)-llv(LOW,27))*100,5,1),3,1)-50)*1.032+50))
    VAR2=((2*CLOSE+HIGH+LOW+OPEN)/5)
    VAR3=(llv(LOW,34))
    VAR4=(hhv(HIGH,34))
    SK=(EMA((VAR2-VAR3)/(VAR4-VAR3)*100,13)*1)
    SD=(EMA(SK,2))
    买入=(np.where(CROSS(AA,21),50,18.5))
    VAR2E=(np.where(CROSS(SK,SD),25,18.5))
    a=(买入>VAR2E)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
