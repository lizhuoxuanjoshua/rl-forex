...
#开始
    ...
    MA120=(MA(CLOSE,120))
    MA250=(MA(CLOSE,250))
    DG=(np.maximum(np.maximum(MA5,MA10),np.maximum(MA20,MA30)))
    ZG=(np.maximum(np.maximum(DG,MA60),np.maximum(MA90,MA120)))
    DD=(np.minimum(np.minimum(MA5,MA10),np.minimum(MA20,MA30)))
    ZD=(np.minimum(np.minimum(DD,MA60),np.minimum(MA90,MA120)))
    AA=((ZG/ZD-1)*100<N)
    a=(FILTER(AA,10))
#结束
    买入 = a
