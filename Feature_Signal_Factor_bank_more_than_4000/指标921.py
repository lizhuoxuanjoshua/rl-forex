...
#开始
    ...
    TJ1=(100*(C-llv(L,5))/(hhv(C,5)-llv(L,5)))
    TJ2=(100*(C-llv(L,10))/(hhv(C,10)-llv(L,10)))
    TJ3=(100*(C-llv(L,20))/(hhv(C,20)-llv(L,20)))
    TJ4=(100*(C-llv(L,30))/(hhv(C,30)-llv(L,30)))
    a=(TJ1<8 ) * ( TJ2<7 ) * ( TJ3<6 ) * ( TJ4<5)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
