...
#开始
    ...
    TH=(count(np.where(CLOSE>REF(CLOSE,1),VOL,0),26))
    TL=(count(np.where(CLOSE<REF(CLOSE,1),VOL,0),26))
    TQ=(count(np.where(CLOSE==REF(CLOSE,1),VOL,0),26))
    AVR=(100*(TH*2+TQ)/(TL*2+TQ))
    MAVR=(MA(AVR,10))
    a=(CROSS(AVR,MAVR) ) * ( AVR<=100)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
