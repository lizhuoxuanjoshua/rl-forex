...
#开始
    ...
    A3=(MA(C,150)>=REF(MA(C,150),1))
    VS1=(EVERY(A3,2))
    短=(((3*sma((CLOSE-llv(LOW,27))/(hhv(HIGH,27)-llv(LOW,27))*100,5,1)-2*sma(sma((CLOSE-llv(LOW,27))/(hhv(HIGH,27)-llv(LOW,27))*100,5,1),3,1)-50)*1.032+50))
    VAR2=((2*CLOSE+HIGH+LOW+OPEN)/5)
    VAR3=(llv(LOW,34))
    VAR4=(hhv(HIGH,34))
    长=(EMA((VAR2-VAR3)/(VAR4-VAR3)*100,13))
    VAR5=(CROSS(短,长)) * ( 长<25)
    a=(VAR5 ) * ( VS1)
#结束
    买入 = a
