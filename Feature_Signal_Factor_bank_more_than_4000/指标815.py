...
#开始
    ...
    主力=((EMA(100*(C-llv(LOW,30))/(hhv(H,30)-llv(LOW,30)),3)))
    TMD=(C-REF(C,1))
    金线=(100*EMA(EMA(TMD,9),9)/EMA(EMA(np.abs(TMD),9),9))
    银线=(MA(金线,5))
    平均=((主力+金线)/2)
    次强=(主力>50 ) * ( 平均>30 ) * ( 金线>0)
    强=(主力>80 ) * ( 平均>50 ) * ( 金线>0)
    极强=(强 ) * ( 金线>50)
    AA=((((CROSS(主力,80) ) * ( CROSS(平均,50))) ) + ( ((CROSS(主力,50) ) * ( CROSS(平均,30))) ) + ( ((CROSS(主力,50) ) * ( CROSS(平均,20))) ) + ( ((CROSS(主力,30) ) * ( CROSS(平均,20))) ) + ( ((CROSS(主力,75) ) * ( CROSS(平均,40))) ) + ( ((CROSS(主力,50) ) * ( CROSS(平均,8))) ) + ( ((CROSS(主力,50) ) * ( CROSS(平均,13))) ) + ( ((CROSS(主力,80) ) * ( CROSS(平均,75))) ) + ( ((CROSS(主力,30) ) * ( CROSS(平均,8))) ) + ( ((CROSS(主力,30) ) * ( CROSS(平均,13))) ) + ( ((CROSS(主力,34) ) * ( CROSS(金线,0) ) * ( CROSS(平均,21))) ) + ( ((CROSS(主力,34) ) * ( CROSS(平均,13) ) * ( CROSS(金线,0))) ) + ( ((CROSS(主力,60) ) * ( CROSS(平均,30) ) * ( 次强)) ) + ( ((CROSS(主力,67) ) * ( CROSS(平均,37) ) * ( 次强))) ) * ( V/REF(V,1)>1.18)
