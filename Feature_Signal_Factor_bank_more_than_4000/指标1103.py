...
#开始
    ...
    OBV=(count(np.where(CLOSE==REF(CLOSE,1),0,VA),300))
    MAOBV=(MA(OBV,30))
    MAOBV1=(MA(OBV,13))
    MAOBV2=(MA(OBV,5))
    TJ1=(CROSS(OBV,MAOBV2) ) * ( MAOBV2>REF(MAOBV2,1) )
    TJ11=(FILTER(TJ1,5))
    TJ2=(FILTER(CROSS(MAOBV2,MAOBV1),5))
    TJ3=((CROSS(OBV,MAOBV2))  +  (CROSS(OBV,MAOBV1) ))
    TJ33=(FILTER(TJ3,5))
    a=((TJ11) ) + ( (TJ33) ) +TJ1 +TJ2 +TJ3
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
