...
#开始
    ...
    高=(EMA(C,5)>EMA(C,15))
    底点=(barslast(CROSS(EMA(C,15),EMA(C,5))))
    时间=(np.where(高>0,底点,15))
    山顶=(C/llv(C,时间)*100-100)
    低=(EMA(C,5)<EMA(C,15))
    高点=(barslast(CROSS(EMA(C,5),EMA(C,15))))
    时间A=(np.where(低>0,高点,15))
    山底=(100-C/hhv(C,时间A)*100)
    a=(CROSS(山顶,山底))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
