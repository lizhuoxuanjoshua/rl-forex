...
#开始
    ...
    AP1=(CROSS(MA(C,5),MA(C,24)))
    AP2=((CROSS(MA(V,5),MA(V,60))) ) + ( (CROSS(V,MA(V,60))
))
    DIFF=(EMA(CLOSE,12) - EMA(CLOSE,26))
    DEA=(EMA(DIFF,9))
    AP3=(DIFF>DEA)
    AP4=(EVERY(REF(C,1)/REF(C,2)<1.03,6) ) * ( H/np.maximum(C,O)<1.01 ) * ( EVERY(REF(np.maximum(C,O),1)/REF(np.minimum(C,O),1)<1.02,5))
    AP5=(O/REF(np.maximum(C,O),1)<1.01)
    AP6=((REF(H,1)/REF(np.maximum(C,O),2)<1.02) ) * ( np.maximum(C,O)/L<1.02)
