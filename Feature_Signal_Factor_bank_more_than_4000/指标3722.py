...
#开始
    ...
    BB=(REF(AA,1)>13 ) * ( REF(AA,1)/AA>1.23 ) * ( CLOSE/REF(CLOSE,1)>1.03)

    VAR4=(EMA(CLOSE,24))
    LDN=(EMA(VAR4*80/100,5))
    TJ1=((REF(LDN,1)>REF(C,1) ) * ( C>LDN ) * ( C>REF(C,1)*1.02))
    BHP=(C/sma(C,13,1)-1)
    DF1=(EMA(CLOSE,3)-EMA(CLOSE,89))
    DF2=(EMA(DF1,21))
    GKCV=((DF1-DF2)*10)
    CFOL=(np.power(GKCV,3)*0.1+np.power(GKCV,2))
    YET=(np.where(GKCV>0.015,CFOL,0)/45)
    HVR=(YET/REF(YET,3)>1.5 ) * ( YET/REF(YET,1)<1.196)
    a=(YET>8 ) * ( FILTER(HVR,3))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
