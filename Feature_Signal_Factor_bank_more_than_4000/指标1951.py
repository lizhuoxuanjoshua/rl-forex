...
#开始
    ...
    底信号=(3)
    AB=(np.where(CROSS(趋势,底信号),80,0))
    VAR2X=((CLOSE-llv(LOW,9))/(hhv(HIGH,9)-llv(LOW,9))*100)
    VAR3X=(sma(VAR2X,4,1))
    VAR4X=(sma(VAR3X,2,1))
    VAR5X=(sma(VAR4X,2,1))
    VAR7=(CROSS(VAR4X,VAR5X) ) * ( VAR4X<35)
    VAR8=(VAR7 ) * ( VOL>REF(VOL,1)*2 ) * ( VOL>REF(MA(VOL,5),1)*2)
    VAR01=((CLOSE-llv(LOW,36))/(hhv(HIGH,36)-llv(LOW,36))*100)
    VAR02=(sma(VAR01,3,1))
    VAR03=(sma(VAR02,3,1))
    VAR04=(sma(VAR03,3,1))
    VAR06=(CROSS(VAR03,VAR04) ) * ( VAR03<25)
    VAR07=(VAR06 ) * ( VOL>REF(VOL,1)*2 ) * ( VOL>REF(MA(VOL,5),1)*2)
    ASD=((VAR8) ) + ( (VAR07
))
    AA=((ASD,15))
    a=(AB ) * ( AB )
