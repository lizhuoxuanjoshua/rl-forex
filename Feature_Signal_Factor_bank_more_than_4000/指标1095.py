...
#开始
    ...
    HOLD=(np.where(C>0,EMA(np.maximum(C-REF(C,1),0),83)/EMA(np.abs(C-REF(C,1)),83)*101,0))
    ACT=(np.where(C>0,EMA(np.maximum(C-REF(C,1),0),233)/EMA(np.abs(C-REF(C,1)),233)*101,0))

    ANS=  ( ACT>70.5 ) * ( REF(ACT,1)>70)
    a=(ANS ) * ( count(ANS,12)==1 ) * ( REF(count(HOLD>70,55)>0,20))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
