...
#开始
    ...
    MA3=(EMA(CLOSE,20))
    MA4=(EMA(CLOSE,30))
    a=(CROSS(MA1,MA2) ) * ( C>O ) * ( C>REF(C,1) ) * (( C/REF(C,1)>=1.018) ) + ( (CROSS(MA1,MA4) )) * ( C>O ) * ( C>REF(C,1) ) * ( C/REF(C,1)>=1.018 ) + ( (CROSS(MA2,MA3) )) * ( C>O ) * ( C>REF(C,1) ) * ( C/REF(C,1)>=1.018 ) + ( (CROSS(MA1,MA3) )) * ( C>O ) * ( C>REF(C,1) ) * ( C/REF(C,1)>=1.018)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
