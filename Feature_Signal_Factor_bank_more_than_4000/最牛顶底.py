...
#开始
    ...
    HJ_1=(REF(CLOSE,1))
    HJ_2=(sma(np.maximum(CLOSE-HJ_1,0),2,1)/sma(np.abs(CLOSE-HJ_1),2,1)*100)
    HJ_3=((CLOSE-llv(LOW,9))/(hhv(HIGH,9)-llv(LOW,9))*100)
    HJ_4=(sma(HJ_3,3,1)*HJ_2/100)
    HJ_5=(sma(np.maximum(CLOSE-HJ_1,0),275,1)/sma(np.abs(CLOSE-HJ_1),275,1)*100)
    HJ_6=((CLOSE-llv(LOW,26))/(hhv(HIGH,26)-llv(LOW,26))*100)
    HJ_7=(sma(HJ_6,2,1)*HJ_5/100)
    HJ_8=(sma(np.maximum(CLOSE-HJ_1,0),139,1)/sma(np.abs(CLOSE-HJ_1),139,1)*100)
    HJ_9=((CLOSE-llv(LOW,33))/(hhv(HIGH,33)-llv(LOW,33))*100)
    HJ_10=(sma(HJ_9,2,1)*HJ_8/100)
    a=CLOSE<MA(llv(LOW,8),20) * ( HJ_7>REF(HJ_7,1))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
