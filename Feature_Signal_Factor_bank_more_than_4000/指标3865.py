...
#开始
    ...
    MA20=(MA(C,20))
    MA30=(MA(C,30))
    MA5A=((C-MA05)/MA05*100)
    MA30A=((C-MA30)/MA30*100)
    JC=(CROSS(0,MA5A) ) * ( MA30A<REF(MA30A,1) ) * ( MA10>REF(MA10,1))
    卖出=(FILTER(JC,10))
    MA1A=(MA(CLOSE,M1))
    MA1=(EMA(CLOSE,1))
    ...
    M1A=(MA(VAR4,1))
    买=(M1A>REF(M1A,1) ) * ( REF(M1A,1)<REF(M1A,2))
    T1=(REF(卖出,2) ) * ( 买 ) * ( CROSS(C,M6))
    日线=(sma(C,6.5,1))
    T2=(日线>REF(日线,1) ) * ( REF(日线,1)<REF(日线,2))
    a=(T1 ) * ( T2 ) * ( C>REF(C,3))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
