...
#开始
    ...
    M=(6)
    M1=(20)
    TR=(EMA(np.maximum(np.maximum(HIGH-LOW,np.abs(HIGH-REF(CLOSE,1))),np.abs(REF(CLOSE,1)-LOW)),N))
    HD =(HIGH-REF(HIGH,1))
    LD =(REF(LOW,1)-LOW)

    条件=(HD > 0) * (HD > LD)
    条件1=(LD>0) * (LD>HD)
    DMP=EMA(np.where(条件,HD,0),14)
    DMM=EMA(np.where(条件1,LD,0),14)
    PDI=( DMP*100/TR)
    MDI=(DMM*100/TR)
    DM=(PDI-MDI)
    ADX=(EMA(np.abs(MDI-PDI)/(MDI+PDI)*100,6))
    ADXR=(EMA(ADX,6))
    a=(CROSS(ADXR,ADX) ) * ( ADXR<20 ) * ( DM>0)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
