...
#开始
    ...
    HH=(np.maximum(np.maximum(np.maximum(M3,M8),M21),M55))
    LL=(np.minimum(np.minimum(np.minimum(M3,M8),M21),M55))
    SXZH=(HH/LL<=1.03 ) * ( M3>REF(M3,1) ) * ( M8>REF(M8,1) ) * ( M21>REF(M21,1) ) * ( M55>REF(M55,1))
    a=(SXZH ) * ( L<HH ) * ( C>HH ) * ( 月涨幅>月跌幅)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
