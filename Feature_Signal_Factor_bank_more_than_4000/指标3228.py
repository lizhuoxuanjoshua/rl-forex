...
#开始
    ...
    VAR2=(CROSS(VAR1,压力线))
    VAR3=(REF(CLOSE,1))
    VAR4=(sma(np.maximum(CLOSE-VAR3,0),7,1)/sma(np.abs(CLOSE-VAR3),7,1)*100)
    VAR5=(CROSS(79,VAR4))
    VAR6=(FILTER(VAR5,4))
    VAR7=(MA5>REF(MA5,1)) * ( MA10>REF(MA10,1)) * ( MA20>REF(MA20,1)) * ( MA5<MA10)
    VAR8=( MA5>REF(MA5,1)) * ( MA5<MA10)
    VAR9=(REF(L,2)<支撑线 ) * ( L>支撑线 ) * ( C>O ) * ( MA5>REF(MA5,1))
    a=(((VAR7) ) + ( (VAR8) ) + ( (VAR9)) ) * ( VAR4<45)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
