...
#开始
    ...
    上轨=(MA(EMA(HIGH,11)*1.025,5))
    下轨=(MA(EMA(LOW,11)*0.975,5))
    中轨=((上轨+下轨)/2)
    顶线=(上轨+中轨-下轨)
    底线=(下轨+下轨-中轨)
    VAR1=(llv(LOW,4))
    VAR2=(hhv(HIGH,4))
    AK=(EMA((((CLOSE-VAR1)/(VAR2-VAR1))*25),34))
    买入=(CROSS(EMA((((CLOSE-VAR1)/(VAR2-VAR1))*50),3),EMA(AK,8)-1.6))
    卖出=(CROSS(EMA(AK,8)+27.3,EMA((((CLOSE-VAR1)/(VAR2-VAR1))*50),3)))
    A3=((C-llv(L,9))/(hhv(H,9)-llv(L,9))*100)
    A4=(sma(A3,3,1))
    A5=(sma(A4,3,1))
    A6=(3*A4-2*A5)
    A7=(C-REF(C,1))
    A8=(100*EMA(EMA(A7,6),6)/EMA(EMA(np.abs(A7),6),6))
    买=(llv(A8,2)==llv(A8,7) ) * ( count(A8<0,2) ) * ( CROSS(A8,MA(A8,2)))
    VAR5=((CLOSE*2+HIGH+LOW)/4)
    SK=( EMA(VAR5,13)-EMA(VAR5,21))
    SD=( EMA(SK,2))
    MAC3=((np.arctan((MA(C,3)/REF(MA(C,3),1)-1)*100)*180/3.14115926))
    M3=(MA(MAC3,3))
    a=((CROSS(MAC3,M3) ) * ( L<底线))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
