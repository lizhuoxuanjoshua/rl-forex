...
#开始
    ...
    MA20=(MA(C,20))
    MA30=(MA(C,30))
    DTPL=(MA5>MA10 ) * ( MA10>MA20 ) * ( MA20>MA30)
    DTSJ=(barslast(DTPL ) * ( (REF(DTPL,1)==0)))
    ZXDT=(MA10>MA20 ) * ( MA20>MA30 ) * ( MA10>REF(MA10,1) ) * ( MA20>REF(MA20,1)) * ( MA30>REF(MA30,1))
    BMBD=(DTSJ<20 ) * ( ZXDT ) * ( count(ZXDT,5)>=3)
    TDDF=((L/REF(C,1)-1)*100<=-5 ) * ( (REF(hhv(H,5),1)/L-1)*100>=DDF)
    DRZC=(count(H/REF(C,1)>=1.08,7) ) * ( L/REF(C,1)<=0.93 ) * ( BMBD)
    BAA=((((DRZC) ) + ( (TDDF)) ) * ( BMBD ) * ( L==llv(L,5))*-20)
    a=((DRZC) ) + ( (BAA==-20
))
#结束
    买入 = a
