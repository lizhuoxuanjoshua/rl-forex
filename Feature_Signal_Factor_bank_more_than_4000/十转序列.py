...
#开始
    ...
    N1=(REF(N,1)==1) * (C>=REF(C,4)) * (A1
)
    

    N2=(REF(N1,1)==1) * (C>=REF(C,4)) * (A1
)
    

    N3=(REF(N2,1)==1) * (C>=REF(C,4)) * (A1
)
    

    N4=(REF(N3,1)==1) * (C>=REF(C,4)) * (A1
)
    

    N5=(REF(N4,1)==1) * (C>=REF(C,4)) * (A1
)
    

    N6=(REF(N5,1)==1) * (C>=REF(C,4)) * (A1
)
    

    N7=(REF(N6,1)==1) * (C>=REF(C,4)) * (A1
)
    

    N8=(REF(N7,1)==1) * (C>=REF(C,4)) * (A1
)
    

    a=(REF(N8,1)==1 )* (C>=REF(C,4)) * (A1)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
