...
#开始
    ...
    MM=(5)
    NN=(30)
    成交量阻力=(((VOL+REF(VOL,1))/2-llv(VOL,N))/(hhv(VOL,N)-llv(VOL,N))*100)
    股价阻力=(((C+REF(C,1))/2-llv(C,M))/(hhv(C,M)-llv(C,M))*100)
    成本=((L+C+C+O+O+H)/6)
    成本均线=(count(成本*VOL,MM)/count(VOL,MM))
    成本阻力=((成本均线-llv(成本均线,NN))/(hhv(成本均线,NN)-llv(成本均线,NN))*100)
    a=( 成交量阻力<10 ) * ( 股价阻力<10 ) * ( 成本阻力<5)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
