...
#开始
    ...
    AA=(np.where(C>REF(C,1),(3*H+4*C+3*OPEN+2*L)/12,(2*H+4*C+3*OPEN+3*L)/12))
    AB=(EMA(AA,3))
    AC=(EMA(AA,8))
    a=(CROSS(AB,AC) ) * ( ((CROSS(REF(AC,4),REF(AB,4))) ) + ( (CROSS(REF(AC,3),REF(AB,3))) ) + ( (CROSS(REF(AC,2),REF(AB,2))) ) + ( (CROSS(REF(AC,1),REF(AB,1)))))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
