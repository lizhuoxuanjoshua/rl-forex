...
#开始
    ...
    XA_1=(EMA(CLOSE,89))
    XA_2=(EMA(EMA(EMA(HIGH,13),13),13))
    XA_3=(EMA(EMA(EMA(HIGH,5),5),5))
    XA_4=((CLOSE-REF(CLOSE,1))/REF(CLOSE,1)*100)
    XA_8=(REF(CLOSE<89,1) ) * ( REF(CLOSE>2,1))
    XA_10=(CLOSE>XA_3 ) * ( CLOSE>XA_2 ) * ( CLOSE>XA_1)
    XA_11=(HIGH/XA_3)
    XA_12=(XA_4>=9.84 ) * ( REF(XA_4<9.84,1) ) * ( REF(XA_4<9.84,2))
    XA_13=(XA_8 ) * ( XA_10 ) * ( XA_11<1.16 ) * ( XA_12)
    XA_14=(np.where(CLOSE>OPEN,CLOSE,OPEN))
    XA_15=(np.where(CLOSE>OPEN,OPEN,CLOSE))
    XA_16=(llv(XA_14,4))
    XA_17=(hhv(XA_15,4))
    XA_18=(XA_14>=XA_16 ) * ( XA_15<=XA_16 ) * ( XA_14>=XA_17 ) * ( XA_15<=XA_17)
    XA_19=(REF(XA_14,1)>=XA_16 ) * ( REF(XA_15,1)<=XA_16 ) * ( REF(XA_14,1)>=XA_17 ) * ( REF(XA_15,1)<=XA_17)
    XA_20=(REF(XA_14,2)>=XA_16 ) * ( REF(XA_15,2)<=XA_16 ) * ( REF(XA_14,2)>=XA_17 ) * ( REF(XA_15,2)<=XA_17)
    XA_21=(REF(XA_14,3)>=XA_16 ) * ( REF(XA_15,3)<=XA_16 ) * ( REF(XA_14,3)>=XA_17 ) * ( REF(XA_15,3)<=XA_17)
    XA_22=(XA_18 ) * ( XA_19 ) * ( XA_20 ) * ( XA_21)
    XA_23=(count(XA_22>0,36))
    XA_24=((REF(OPEN,5)+REF(CLOSE,5))/2)
    XA_25=((HIGH+LOW+CLOSE+OPEN)/4)
    XA_26=(np.maximum(XA_24,np.maximum(XA_25,hhv(HIGH,5))))
    XA_27=(np.minimum(XA_24,np.minimum(XA_25,llv(LOW,5))))
    XA_28=(XA_26==XA_24)
    XA_29=(XA_28)
    XA_30=(REF(XA_26,barslast(XA_29)))
    XA_31=(XA_4>9.85 ) * ( (OPEN!=CLOSE))
    XA_32=(count(XA_31,21))
    XA_33=(XA_32==1 ) * ( XA_31)
    XA_34=(REF(XA_17,barslast(XA_22)))
    XA_35=(REF(XA_26,barslast(XA_29)))
    XA_36=(XA_27/XA_35)
    XA_37=(XA_27/XA_34)
    XA_38=(XA_36<1.3 ) * ( XA_37<1.3 ) * ( XA_31)

    XA_41=(XA_13 ) * ( XA_38 ) * ( XA_23)
    a=FILTER(XA_41,13)  * ( XA_33)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
