...
#开始
    ...
    突击队1=(REF(LOW,1))
    突击队2=(sma(np.abs(LOW-突击队1),3,1)/sma(np.maximum(LOW-突击队1,0),3,1)*100)
    突击队3=(EMA(np.where(CLOSE*1.2,突击队2*10,突击队2/10),3))
    突击队4=(llv(LOW,38))
    突击队5=(hhv(突击队3,38))
    突击队6=(np.where(llv(LOW,90),1,0))
    突击队7=(EMA(np.where(LOW<=突击队4,(突击队3+突击队5*2)/2,0),3)/618*突击队6)
    突击队8=(MA(LOW,2)*0.98)
    突击队9=(MA(LOW,26)*0.84)
    a=(REF(突击队8,1)<突击队9 ) * ( REF(突击队8,1)<突击队8 ) * ( REF(突击队8,1)<REF(突击队8,2))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
