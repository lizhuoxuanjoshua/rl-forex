...
#开始
    ...
    A1=(hhv(EMA(VOL/5,4),240))
    A2=(llv(EMA(VOL/5,4),240))
    A3=(MA(CLOSE,35))
    A4=(EMA(CLOSE,15)-EMA(CLOSE,30))
    A5=(EMA(A4,12))
    A6=(2*(A4-A5))
    A7=((CLOSE-llv(LOW,12))/(hhv(HIGH,12)-llv(LOW,12))*100)
    A8=((CLOSE-llv(LOW,45))/(hhv(HIGH,45)-llv(LOW,45))*100)
    BA=((CLOSE-llv(LOW,225))/(hhv(HIGH,225)-llv(LOW,225))*100)
    B1=(sma(A7,3,1))
    B2=(sma(A8,15,1))
    B3=(sma(BA,45,1))
    B4=((B3-REF(B3,1))*50)
    B5=((B2-REF(B2,1))*10)
    B6=(REF(B2,1)-REF(B3,1))
    B7=((B2-B6-B3)*30)
    B8=(B1-REF(B1,1))
    B10=( EMA((EMA(VOL/5,4)-A2)/A2*A3,13))
    A9=(EMA((A1-EMA(VOL/5,4))/EMA(VOL/5,4)*A3,13))
    B9=(A6>=-0.035 ) * ( A6<=0.045 ) * ( CLOSE>MA(CLOSE,5)) * ( CLOSE>MA(CLOSE,10) ) * ( A9>-100)
    DD=(count(C/REF(C,1)>1.055,60))
    a=  ( B8>7 ) * ( REF(B8,1)<0 ) * ( B8-REF(B8,1)>9)  * ( DD )
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
