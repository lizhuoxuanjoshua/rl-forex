...
#开始
    ...
    WJ=((HIGH+LOW+CLOSE)/3)
    V1=(np.where(HIGH==LOW,1,HIGH-np.maximum(OPEN,CLOSE)))
    V2=(np.where(HIGH==LOW,1,np.maximum(CLOSE,OPEN)-WJ))
    V3=(np.where(HIGH==LOW,1,np.minimum(OPEN,CLOSE)-LOW))
    V4=(np.where(HIGH==LOW,1,WJ-np.minimum(CLOSE,OPEN)))
    V5=(VOL/np.where(HIGH==LOW,4,HIGH-LOW))
    V6=((V1*V5))
    V7=((V2*V5))
    V8=((V3*V5))
    V9=((V4*V5))
    AA=(count((V9+V8)-(V6+V7),60))
    a=(CROSS(AA,0))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
