...
#开始
    ...
    SS=(MA((LOW+HIGH+CLOSE)/3,5)>REF(MA((LOW+HIGH+CLOSE)/3,5),1) ) * ( REF(MA((LOW+HIGH+CLOSE)/3,5),1)<REF(MA((LOW+HIGH+CLOSE)/3,5),2))
    SC=(L<hhv(MA((LOW+HIGH+CLOSE)/3,5),13) ) * ( H>hhv(MA((LOW+HIGH+CLOSE)/3,5),13) ) * ( C>REF(C,1) ) * ( C>O)
    MR=(SC ) * ( count(SS,2))
    a=(MR)
#结束
    买入 = a
