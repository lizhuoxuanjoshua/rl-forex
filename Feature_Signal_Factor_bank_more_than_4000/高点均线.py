...
#开始
    ...
    UP=(HH)
    DN=(LL)
    BIAS=(np.where(C>HH,(C-HH)/HH*100,np.where(C<LL,(C-LL)/LL*100,0)))
    TP=(CROSS(C,HH) ) * ( EVERY(REF(C,1)<REF(HH,1),3))
    TC=(barslast(TP))
    TPUP=(TP ) * ( CROSS(C,REF(H,TC+1)))
    a=(CROSS(C,REF(H*1.004,TC)) ) * ( C>HH)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
