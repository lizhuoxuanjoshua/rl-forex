...
#开始
    ...
    XP1=(10)
    XP2=(AAA1-REF(AAA1,1))
    SS1=(CROSS(XP2,XP1) ) * ( CLOSE>=MA(CLOSE,60))
    S1=((CLOSE-REF(CLOSE,3))/REF(CLOSE,3))
    A=(MA(C,26))
    TJ1=(L/A<1.005 ) * ( C>=A)
    LC=(REF(CLOSE,1))
    RSI1=(sma(np.maximum(CLOSE-LC,0),3,1)/sma(np.abs(CLOSE-LC),3,1)*100)
    RSI2=(sma(np.maximum(CLOSE-LC,0),6,1)/sma(np.abs(CLOSE-LC),6,1)*100)
    RSI3=(sma(np.maximum(CLOSE-LC,0),12,1)/sma(np.abs(CLOSE-LC),12,1)*100)
    RSI4=(sma(np.maximum(CLOSE-LC,0),24,1)/sma(np.abs(CLOSE-LC),24,1)*100)
    ZJ1=(RSI1-REF(RSI1,1)>35)
    ZJ2=(RSI2-REF(RSI2,1)>25)
    ZJ3=(RSI3-REF(RSI3,1)>13)
    ZJ4=(RSI4-REF(RSI4,1)>8)
    a=(SS1 ) * ( ZJ1 ) * ( ZJ2 ) * ( ZJ3 ) * ( ZJ4 ) * ( TJ1)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
