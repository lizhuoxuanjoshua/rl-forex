...
#开始
    ...
    AA=(5*sma((CLOSE-llv(LOW,27))/(hhv(HIGH,27)-llv(LOW,27))*100,5,1)-3*sma(sma((CLOSE-llv(LOW,27))/(hhv(HIGH,27)-llv(LOW,27))*100,5,1),3,1)-sma(sma(sma((CLOSE-llv(LOW,27))/(hhv(HIGH,27)-llv(LOW,27))*100,5,1),3,1),2,1))
    BBC=(llv(AA,2)==llv(AA,8))
    a=(CROSS(AA,REF(AA+0.01,1)) ) * ( BBC ) * ( AA<0)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
