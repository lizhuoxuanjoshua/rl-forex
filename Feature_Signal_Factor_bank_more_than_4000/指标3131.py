...
#开始
    ...
    A2=(barslast(CROSS(97,GJTJ1))<=1 ) * ( barslast(CROSS(GJTJ1,85))<=1)
    B2=(barslast(CROSS(97,GJTJ2))<=1 ) * ( barslast(CROSS(GJTJ2,85))<=1)
    A3=(A2 ) * ( B2)
    AA=(barslast(CROSS(97,GJTJ1))<=2 ) * ( barslast(CROSS(GJTJ1,85))<=1)
    BA=(barslast(CROSS(97,GJTJ2))<=2 ) * ( barslast(CROSS(GJTJ2,85))<=1)
    A4=(AA ) * ( BA)
    AAA=(barslast(CROSS(97,GJTJ1))<=3 ) * ( barslast(CROSS(GJTJ1,85))<=1)
    BAA=(barslast(CROSS(97,GJTJ2))<=3 ) * ( barslast(CROSS(GJTJ2,85))<=1)
    A8=(AAA ) * ( BAA)
    B21=(REF(A3,1))
    B3=(REF(A4,1))
    B4=(REF(A8,1))
    D2=(GJTJ1==100 ) * ( GJTJ2==100 ) * ( A3)
    D3=(GJTJ1==100 ) * ( GJTJ2==100 ) * ( A4)
    D4=(GJTJ1==100 ) * ( GJTJ2==100 ) * ( A8)
    a=((D2) ) + ( (D3) ) + ( (D3))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
