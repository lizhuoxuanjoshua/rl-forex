...
#开始
    ...
    N3=(26)
    MTM=(C-REF(C,1))
    DX=(100*EMA(EMA(MTM,N1),N1)/EMA(EMA(np.abs(MTM),N1),N1))
    ZX=(100*EMA(EMA(MTM,N2),N2)/EMA(EMA(np.abs(MTM),N2),N2))
    CX=(100*EMA(EMA(MTM,N3),N3)/EMA(EMA(np.abs(MTM),N3),N3))
    DXGD=(llv(DX,2)==llv(DX,5) ) * ( CROSS(DX,MA(DX,2)))
    ZXGD=(llv(ZX,2)==llv(ZX,5) ) * ( CROSS(ZX,MA(ZX,2)) ) * ( DX>ZX)
    DTPL=(DX>ZX ) * ( ZX>CX ) * ( CX>0)
    QSXS=(DX>REF(DX,1) ) * ( ZX>REF(ZX,1))
    TSJC=(CROSS(DX,CX) ) * ( CROSS(ZX,CX))
    a=(CROSS(DTPL,0.8) ) * ( ((TSJC) ) + ( (count(QSXS,3)==3)) ) * ( CX<40)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
