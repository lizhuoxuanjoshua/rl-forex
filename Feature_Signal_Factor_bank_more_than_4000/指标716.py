...
#开始
    ...
    VL=(REF(LOW,1))
    VAR12=((sma(np.abs(LOW-VL),14,1))/(sma(np.maximum(LOW-VL,0),14,1))*100)
    VAR14=(MA(VAR12,13))
    VAR15=(llv(LOW,28))
    VAR18=(EMA(np.where(LOW<=VAR15,VAR14,0),3))
    VAR110=(sma(sma((CLOSE-llv(LOW,60))/(hhv(HIGH,60)-llv(LOW,60))*100,7,1),5,1))
    VAR111=(sma((CLOSE-llv(LOW,60))/(hhv(HIGH,60)-llv(LOW,60))*100,7,1)*16.18-VAR110*10.9)
    a=(VAR111<=VAR110 ) * ( VAR18>REF(VAR18,1))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
