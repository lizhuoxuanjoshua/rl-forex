...
#开始
    ...
    G1=(8)
    股票=(0)
    理=(np.where(CLOSE>MA(CLOSE,5),股票+1,股票))
    想=(np.where(CLOSE>MA(CLOSE,10),理+1,理))
    论=(np.where(CLOSE>MA(CLOSE,20),想+1,想))
    坛=(np.where(CLOSE>MA(CLOSE,60),论+1,论))
    估=(np.where(MA(CLOSE,5)>MA(CLOSE,10),坛+1,坛))
    界=(np.where(MA(CLOSE,10)>MA(CLOSE,20),估+1,估))
    文=(np.where(MA(CLOSE,20)>MA(CLOSE,60),界+1,界))
    估算=(理+想+论+坛+估+界+文)
    a=(REF(估算,1)<=2 ) * ( 估算>=G1)
#结束
    买入 = a
