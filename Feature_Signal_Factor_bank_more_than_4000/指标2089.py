...
#开始
    ...
    FF=(MA(C,30))
    FG1=(MA(C,60))
    FH=(np.abs(FC-FD)/np.minimum(FC,FD))
    FI=(np.abs(FC-FE)/np.minimum(FC,FE))
    FJ=(np.abs(FC-FF)/np.minimum(FC,FF))
    FK=(np.abs(FE-FD)/np.minimum(FE,FD))
    FL=(np.abs(FF-FD)/np.minimum(FF,FD))
    FM=(np.abs(FE-FF)/np.minimum(FE,FF))
    FN=(np.maximum(np.maximum(np.maximum(FH,FI),np.maximum(FJ,FK)),np.maximum(FL,FM))*1002)
    FO=(EMA(FN,3))
    FS=((CLOSE-llv(LOW,9))/(hhv(HIGH,9)-llv(LOW,9))*100)
    FT=((sma(FS,3,1)-18)*1.55)
    FV=((CLOSE-llv(LOW,100))/(hhv(HIGH,100)-llv(LOW,100))*100)
    FW=(sma(FV,10,1))
    GJ=((hhv(H,5)-C)/(hhv(H,5)-llv(L,5))*(-1)+0.9)
    GK=((hhv(H,10)-C)/(hhv(H,10)-llv(L,10))*(-1)+0.92)
    GL=((hhv(H,15)-C)/(hhv(H,15)-llv(L,15))*(-1)+0.93)
    GM=((hhv(H,55)-C)/(hhv(H,55)-llv(L,55))*(-1)+0.94)
    GN=((hhv(H,89)-C)/(hhv(H,89)-llv(L,89))*(-1)+0.95)
    GO=((hhv(H,120)-C)/(hhv(H,120)-llv(L,120))*(-1)+0.91)
    SP=((hhv(H,180)-C)/(hhv(H,180)-llv(L,180))*(-1)+0.96)
    SQ=((((GJ+GK+GL)*8+GM+GN+SP+GO)/28-0.1)*185)
    GR=(sma(SQ,3,1))
    GS=((FT+FW+GR*2)/4)
    a=(FO<8 ) * ( GS>95)
#结束
