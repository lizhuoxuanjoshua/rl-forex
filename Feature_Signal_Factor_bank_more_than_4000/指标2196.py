...
#开始
    ...
    VVAR2=(REF(VVAR1,1))
    VVAR3=(sma(np.maximum(VVAR1-VVAR2,0),10,1)/sma(np.abs(VVAR1-VVAR2),10,1)*100)
    a=(count(VVAR3<20,5)>=1 ) * ( count(VVAR1==llv(VVAR1,10),10)>=1 ) * ( CLOSE>=OPEN*1.038 ) * ( VOL>MA(VOL,5)*1.2)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
