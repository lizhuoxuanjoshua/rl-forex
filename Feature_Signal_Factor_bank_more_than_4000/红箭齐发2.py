...
def 策略(合成周期,ktime,high,o,low,c,volume,amount,均线,开盘,收盘,切片位置,subindex,turn):
    ...
    np.where(C > DEA2, DEA2, 0)

    np.where(DEA2 <= REF(DEA2, 1), DEA2, 0)

    a = (MACD < 0) * (MACD > REF(MACD, 1)) * (REF(MACD, 1) < REF(MACD, 2)) * (REF(MACD, 2) < REF(MACD, 3)) * (
