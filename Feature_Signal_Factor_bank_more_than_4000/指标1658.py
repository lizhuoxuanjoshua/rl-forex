...
#开始
    ...
    VAR10A=(CLOSE/MA(CLOSE,20)*100<80)
    VAR20A=(CLOSE/MA(CLOSE,40)*100<78)
    VAR30A=(CLOSE/MA(CLOSE,60)*100<72)
    VAR40A=(HIGH>LOW*1.051)
    VAR50A=(VAR40A ) * ( count(VAR40A,5)>1)
    AA=(VAR50A ) * ( ((VAR10A) ) + ( (VAR20A) ) + ( (VAR30A)))
    LC=(REF(CLOSE,1))
    RSI=(sma(np.maximum(CLOSE-LC,0),6,1)/sma(np.abs(CLOSE-LC),6,1)*100)
    RSI底=(RSI<20)
    MA5=(MA(C,5))
    五日乖离率=(((C-MA5)/MA5*100))
    乖离底=(五日乖离率<-10)
    RSV1=((hhv(H,10)-C)/(hhv(H,10)-llv(L,10))*101)
    KK=(sma(RSV1,3,1))
    DD=(sma(KK,3,1))
    JJ=(3*KK-2*DD)
    WWQQ=(np.where(JJ>=100,100,np.where(JJ<=0,0,JJ)))
    AB=(WWQQ==100)
    a= ( 乖离底 )
#结束
    买入 = a
