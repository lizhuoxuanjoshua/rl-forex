...
def 策略(合成周期,ktime,high,o,low,c,volume,amount,均线,开盘,收盘,切片位置,subindex,turn):
    ...
    a=volume<REF(volume)
#开始
    var1 = np.logical_and(c > REF(c,1), c > REF(c,2))
    refVar1 = np.array([0.0])
    refVar1 = np.append(refVar1, var1)[:-1]

    var2 = np.logical_and(refVar1, c <= REF(c,1))
    var2 = np.logical_and(var2, c >= REF(c,2))
    refVar2 = np.array([0.0])
    refVar2 = np.append(refVar2, var2)[:-1]

    var3 = np.logical_and(refVar2, c >= REF(c,1))
    var3 = np.logical_and(var3, c <= REF(c,2))
    refVar3 = np.array([0.0])
    refVar3 = np.append(refVar3, var3)[:-1]

    var4 = np.logical_and(refVar3, c <= REF(c,1))
    var4 = np.logical_and(var4, c >= REF(c,2))
    refVar4 = np.array([0.0])
    refVar4 = np.append(refVar4, var4)[:-1]

    var5 = np.logical_and(refVar4, c >= REF(c,1))
    var5 = np.logical_and(var5, c <= REF(c,2))
    refVar5 = np.array([0.0])
    refVar5 = np.append(refVar5, var5)[:-1]

    var6 = np.logical_and(refVar5, c <= REF(c,1))
    var6 = np.logical_and(var6, c >= REF(c,2))
    refVar6 = np.array([0.0])
    refVar6 = np.append(refVar6, var6)[:-1]

    var7 = np.logical_and(refVar6, c >= REF(c,1))
    var7 = np.logical_and(var7, c <= REF(c,2))
    refVar7 = np.array([0.0])
    refVar7 = np.append(refVar7, var7)[:-1]

    var8 = np.logical_and(refVar7, c <= REF(c,1))
    var8 = np.logical_and(var8, c >= REF(c,2))
    refVar8 = np.array([0.0])
    refVar8 = np.append(refVar8, var8)[:-1]

    var9 = np.logical_and(refVar8, c >= REF(c,1))
    var9 = np.logical_and(var9, c <= REF(c,2))
    refVar9 = np.array([0.0])
    refVar9 = np.append(refVar9, var9)[:-1]

    vara = np.logical_and(refVar9, c <= REF(c,1))
    vara = np.logical_and(vara, c >= REF(c,2))
    refVara = np.array([0.0])
    refVara = np.append(refVara, vara)[:-1]

    varb = np.logical_and(refVara, c >= REF(c,1))
    varb = np.logical_and(varb, c <= REF(c,2))
    refVarb = np.array([0.0])
    refVarb = np.append(refVarb, varb)[:-1]

    varc = np.logical_and(refVarb, c <= REF(c,1))
    varc = np.logical_and(varc, c >= REF(c,2))

    y = np.logical_or(var1, var2)
    y = np.logical_or(y, var3)
    y = np.logical_or(y, var4)
    y = np.logical_or(y, var5)
    y = np.logical_or(y, var6)
    y = np.logical_or(y, var7)
    y = np.logical_or(y, var8)
    y = np.logical_or(y, var9)
    y = np.logical_or(y, vara)
    y = np.logical_or(y, varb)
    y = np.logical_or(y, varc)
    y = np.multiply(y, c)

    z = np.logical_or(var1, var2)
    z = np.logical_or(z, var3)
    z = np.logical_or(z, var4)
    z = np.logical_or(z, var5)
    z = np.logical_or(z, var6)
    z = np.logical_or(z, var7)
    z = np.logical_or(z, var8)
    z = np.logical_or(z, var9)
    z = np.logical_or(z, vara)
    z = np.logical_or(z, varb)
    z = np.logical_or(z, varc)
    z = np.multiply(z, REF(c,1))

    vard = np.logical_and(c < REF(c,1), c < REF(c,2))
    refVard = np.array([0.0])
    refVard = np.append(refVard, vard)[:-1]

    vare = np.logical_and(refVard, c >= REF(c,1))
    vare = np.logical_and(vare, c <= REF(c,2))
    refVare = np.array([0.0])
    refVare = np.append(refVare, vare)[:-1]

    varf = np.logical_and(refVare, c <= REF(c,1))
    varf = np.logical_and(varf, c >= REF(c,2))
    refVarf = np.array([0.0])
    refVarf = np.append(refVarf, varf)[:-1]

    var10 = np.logical_and(refVarf, c >= REF(c,1))
    var10 = np.logical_and(var10, c <= REF(c,2))
    refVar10 = np.array([0.0])
    refVar10 = np.append(refVar10, var10)[:-1]

    var11 = np.logical_and(refVar10, c <= REF(c,1))
    var11 = np.logical_and(var11, c >= REF(c,2))
    refVar11 = np.array([0.0])
    refVar11 = np.append(refVar11, var11)[:-1]

    var12 = np.logical_and(refVar11, c >= REF(c,1))
    var12 = np.logical_and(var12, c <= REF(c,2))
    refVar12 = np.array([0.0])
    refVar12 = np.append(refVar12, var12)[:-1]

    var13 = np.logical_and(refVar12, c <= REF(c,1))
    var13 = np.logical_and(var13, c >= REF(c,2))
    refVar13 = np.array([0.0])
    refVar13 = np.append(refVar13, var13)[:-1]

    var14 = np.logical_and(refVar13, c >= REF(c,1))
    var14 = np.logical_and(var14, c <= REF(c,2))
    refVar14 = np.array([0.0])
    refVar14 = np.append(refVar14, var14)[:-1]

    var15 = np.logical_and(refVar14, c <= REF(c,1))
    var15 = np.logical_and(var15, c >= REF(c,2))
    refVar15 = np.array([0.0])
    refVar15 = np.append(refVar15, var15)[:-1]

    var16 = np.logical_and(refVar15, c >= REF(c,1))
    var16 = np.logical_and(var16, c <= REF(c,2))
    refVar16 = np.array([0.0])
    refVar16 = np.append(refVar16, var16)[:-1]

    var17 = np.logical_and(refVar16, c <= REF(c,1))
    var17 = np.logical_and(var17, c >= REF(c,2))
    refVar17 = np.array([0.0])
    refVar17 = np.append(refVar17, var17)[:-1]

    var18 = np.logical_and(refVar17, c >= REF(c,1))
    var18 = np.logical_and(var18, c <= REF(c,2))

    w = np.logical_or(vard, vare)
    w = np.logical_or(w, varf)
    w = np.logical_or(w, var10)
    w = np.logical_or(w, var11)
    w = np.logical_or(w, var12)
    w = np.logical_or(w, var13)
    w = np.logical_or(w, var14)
    w = np.logical_or(w, var15)
    w = np.logical_or(w, var16)
    w = np.logical_or(w, var17)
    w = np.logical_or(w, var18)
    w = np.multiply(w, c)

    x = np.logical_or(vard, vare)
    x = np.logical_or(x, varf)
    x = np.logical_or(x, var10)
    x = np.logical_or(x, var11)
    x = np.logical_or(x, var12)
    x = np.logical_or(x, var13)
    x = np.logical_or(x, var14)
    x = np.logical_or(x, var15)
    x = np.logical_or(x, var16)
    x = np.logical_or(x, var17)
    x = np.logical_or(x, var18)
    x = np.multiply(x, REF(c,1))

    refx = np.array([0.0])
    refx = np.append(refx, x)[:-1]
    var19 = np.logical_and(refx, var1)

    u = np.multiply(var19, REF(c,1))
    v = np.multiply(var19, REF(c,2))

    refz = np.array([0.0])
    refz = np.append(refz, z)[:-1]
    var1a = np.logical_and(refz, vard)
#结束
    买入 = var1a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
