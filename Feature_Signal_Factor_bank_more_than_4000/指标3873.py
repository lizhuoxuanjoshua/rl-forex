...
#开始
    ...
    VAR1=((EMA(C,12)-EMA(C,26))*100)
    VAR2=(EMA(VAR1,9))
    VAR3=(2*(VAR1-VAR2))
    VAR4=(CROSS(VAR1,VAR2) ) * ( VAR1<0 ) * ( VAR2<0 ) * ( VAR3>0)
    V5=(MA(V,5))
    V10=(MA(V,10))
    VAR5=(count(CROSS(V5,V10),4)==1 ) * ( V>V5 ) * ( V>V10 ) * ( V>REF(V,1))
    VAR6=(count(C>O,5)>=3)
    VAR7=(H/C<1.03 ) * ( C>MA(C,20))
    VAR8=1
    VAR9=(C>O ) * ( C/llv(L,4)<1.15)
    RSV=((CLOSE-llv(LOW,9))/(hhv(HIGH,9)-llv(LOW,9))*100)
    K=(sma(RSV,3,1))
    D=(sma(K,3,1))
    J=(3*K-2*D)
    VAR10=(J<100)
    VARA=(VAR3>VAR1 ) * ( VAR3>VAR2 ) * ( VAR3>0)
    VARE=(MA(C,5)>REF(MA(C,5),1))
    MB=(VARA  ) * ( VARE)
    a=(VAR4 ) * ( VAR5 ) * ( VAR6 ) * ( VAR7 ) * ( VAR8 ) * ( VAR9 ) * ( VAR10 ) * ( MB)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
