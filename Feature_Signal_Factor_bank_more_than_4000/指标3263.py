...
#开始
    ...
    CHECK1=(-1)
    VAR1=(np.where(CHECK1<0,(HIGH+LOW+CLOSE)/3,0))
    VAR2=(np.where(CHECK1<0,(VAR1-MA(VAR1,14))/(0.015*AVEDEV(VAR1,14)),0))
    VAR3=(np.where(CHECK1<0,(VAR1-MA(VAR1,70))/(0.015*AVEDEV(VAR1,70)),0))
    VAR4=(np.where(CHECK1<0,(VAR2>=150 ) * ( VAR2<200 ) * ( VAR3>=150 ) * ( VAR3<200),0))
    VAR5=(VAR2<=-150 ) * ( VAR2>-200 ) * ( VAR3<=-150 ) * ( VAR3>-200)
    VAR6=(np.where(CHECK1<0,(CLOSE-MA(CLOSE,13))/MA(CLOSE,13)*100,0))
    VAR7=(np.where(CHECK1<0,100-np.abs(VAR6),0))
    VAR8=(np.where(CHECK1<0,np.where(VAR7<90,VAR7,100),0))
    PJX=(np.where(CHECK1<0,MA(100*(CLOSE-llv(CLOSE,34))/(hhv(HIGH,34)-llv(LOW,34)),5)-20,0))
    VAR1A=(np.where(PJX>0,PJX,0))
    条件1=(VAR3>=200 ) * ( VAR2>=150)
    条件2=(VAR3<=-200 ) * ( VAR2<=-150)
    VAR1B=(np.where(条件1,15,np.where(条件2,-15,VAR5))+60)
    VAR1C=(VAR1A>48 ) * ( VAR1B>60 ) * ( VAR8<100)
    VAR1D=(VAR1A<48 ) * ( VAR1B<60 ) * ( VAR8<100)
    a=(VAR1D ) * ( count(VAR1D,30)==1)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
