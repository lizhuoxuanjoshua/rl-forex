...
#开始
    ...
    ZKP=np.where(C>REF(C,1),REF(C,1),0)

    

    ZSP=np.where(C>REF(C,1),C,0)

    

    DKP=np.where(C<=REF(C,1),REF(C,1),0)

    

    DSP=np.where(C<=REF(C,1),C,0)
    ...
    a=(count(CROSS(五日乖离率,0),3)>=1) * (三拾日乖离率>REF(三拾日乖离率,1)) * (MA5>REF(MA5,1)) * ((C/REF(C,1)-1)*100>5) * (REF(C,1)<MA30)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
