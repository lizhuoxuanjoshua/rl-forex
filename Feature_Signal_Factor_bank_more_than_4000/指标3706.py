...
#开始
    ...
    MA30=(MA(C,30))
    MA120=(sma(C,120,1))
    ZH=(np.abs(MAA5-MA20)/MA20*100<=10)
    GG=(np.abs(MAA5-MA30)/MA30*100<=10)
    HH=(MAA5>REF(MAA5,1))
    QS=(MA20>REF(MA20,1))
    ZZ=(MA30>REF(MA30,1))
    DTPL=(MA20>MA30 ) * ( MA30>MA120)
    QQ=(DTPL ) * ( QS ) * ( ZZ ) * ( count(HH,4)<=1)
    HS=(C>MAA5 ) * ( C>REF(C,3))
    TT=(HS ) * ( ((ZH) ) + ( (GG)) ) * ( QQ)
    WW=(TT )
    WW1= ( C>MA(C,5))
    DT=(MA(AMOUNT,6)>MA(AMOUNT,12) ) * ( MA(AMOUNT,12)>MA(AMOUNT,24))
    a=(WW ) * ( count((MA10>MA20),5)==5 ) * ( WW1 ) * ( DT)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
