...
#开始
    ...
    K1=(np.where(CLOSE>=REF(CLOSE,1),HIGH-OPEN+CLOSE-LOW+CLOSE-REF(CLOSE,1),HIGH-OPEN+CLOSE-LOW))
    K2=(np.where(CLOSE<=REF(CLOSE,1),OPEN-LOW+HIGH-CLOSE+REF(CLOSE,1)-CLOSE,OPEN-LOW+HIGH-CLOSE))
    DT=(VOL*K1/(K1+K2))
    KT=(VOL-DT)
    买量=(count(DT,1))
    卖量=(count(KT,1))
    DDX=(EMA(买量-卖量,5)*VOL/1*100)
    DDX1=(sma(DDX,3,1))
    DDX2=(sma(DDX1,3,1))
    DDX3=(EMA(DDX2,5))
