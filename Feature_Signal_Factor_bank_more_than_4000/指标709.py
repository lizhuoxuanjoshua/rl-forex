...
#开始
    ...
    D=(sma(K,3,1))
    J=(((3 * K) - (2 * D)))
    JS=(100*(CLOSE-REF(CLOSE,5))/(5*REF(CLOSE,5)))
    MAJS1=MA(JS,5)
    MAJS2=MA(JS,10)
    振10=((hhv(H,10)/llv(L,10)-1)*100)
    振幅前10=(REF(振10,1))
    a=(np.abs(REF(OPEN,1)-REF(CLOSE,1))/REF(CLOSE,1)<0.03 ) * ( CLOSE/OPEN>1.03 ) * ( CLOSE>REF(CLOSE,2) ) * ( C/REF(C,1)>1.071 ) * ( BIAS-REF(BIAS,1)>6 ) * ( MA5>MA10 ) * ( MA10>MA20 ) * ( MA10>MA60 ) * ( CROSS(J,D) ) * ( MAJS1>REF(MAJS1,1) ) * ( MAJS2>REF(MAJS2,1) ) * ( 振幅前10<15)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
