...
#开始
    ...
    DEFUL=(np.where((MACD2 >= 0),hhv(MACD2,(DEFUT + 1)),REF(hhv(MACD2,(DEFUT + 1)),(DEFDT + 1))))
    DEFDL=(np.where((MACD2 < 0),llv(MACD2,(DEFDT + 1)),REF(llv(MACD2,(DEFDT + 1)),(DEFUT + 1))))
    DEFU2L=(REF(DEFUL,(DEFUT + 1)))
    DEFD2L=(REF(DEFDL,(DEFDT + 1)))
    a=(((((llv(MACD2,4) > 0) ) * ( (MACD2 < REF(MACD2,1))) ) * ( (REF(MACD2,1) > REF(MACD2,2))) ) * ( (REF(MACD2,2) > REF(MACD2,3))))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
