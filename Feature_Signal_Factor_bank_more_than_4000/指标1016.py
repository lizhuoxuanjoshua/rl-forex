...
#开始
    ...
    AAAA=(3*sma((C-llv(L,55))/(hhv(H,55)-llv(L,55))*100,5,1)-2*sma(sma((C-llv(L,55))/(hhv(H,55)-llv(L,55))*100,5,1),3,1))
    AAA=(EMA(AAAA,3))
    AA=((AAA-REF(AAA,1))/REF(AAA,1)*100)
    a=((AAA<=13 ) * ( AA>13) ) * ( FILTER((AAA<=13 ) * ( AA>13),10))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
