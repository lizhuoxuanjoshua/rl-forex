...
#开始
    ...
    B=(REF(MA(CLOSE,7),1))
    VAR101=(1)
    VAR102=((2*CLOSE+HIGH+LOW)/4)
    VAR103=1
    VAR104=(llv(LOW,5))
    VAR105=(hhv(HIGH,4))
    散户=(EMA((VAR102-VAR104)/(VAR105-VAR104)*100,4)*VAR103*VAR101)
    VAR107=((VAR102-llv(LOW,4))/(hhv(HIGH,4)-llv(LOW,4))*100)
    VAR108=(sma(VAR107,2,1))
    DDDD=(sma(VAR108,2,1)*VAR101)
    AAAA=( EMA(0.667*REF(散户,1)+0.333*散户,2)*VAR103*VAR101)
    BBBB=( EMA((VAR102-VAR104)/(VAR105-VAR104)*100,4)*VAR103*VAR101)
    EEEE=(DDDD>REF(DDDD,1))
    FFFF=(AAAA>REF(AAAA,1))
    RRRR=(BBBB>REF(BBBB,1))
    QQQQ=(REF(BBBB,1)>REF(AAAA,1))

    a=(BBBB>AAAA ) * ( BBBB<90 ) * ( EEEE ) * ( FFFF ) * ( QQQQ ) * ( RRRR ) * ( A>B)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
