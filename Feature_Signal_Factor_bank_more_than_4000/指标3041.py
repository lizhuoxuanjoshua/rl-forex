...
#开始
    ...
    VAR2=(sma(np.abs(VAR1-H),13,1)/sma(np.maximum(VAR1-H,0),10,1))
    VAR3=(EMA(VAR2,10))
    VAR4=(hhv(H,33))
    VAR5=(EMA(np.where(H>=VAR4,VAR3,0),3)*-1)
    VAR6=(sma(np.abs(VAR1-L),10,1)/sma(np.minimum(VAR1-L,0),5,1))
    VAR7=(EMA(VAR6,5))
    VAR8=(llv(L,3))
    VAR9=(EMA(np.where(L>=VAR8,VAR7,0),3)*-1)
    AA=(-(np.where(VAR9>REF(VAR9,1),VAR9,0)))
    a=(CROSS(AA,-1))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
