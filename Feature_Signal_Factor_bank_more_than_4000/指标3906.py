...
#开始
    ...
    UB1=(C>O ) * ( C/REF(C,1)>1.041 ) * ( C>MA(C,5) ) * ( MA(C,5)>MA(C,10) ) * ( MA(C,10)>MA(C,20) ) * ( MA(C,10)>MA(C,30) ) * ( C==hhv(C,5))
    US=((REF(H,1)-np.maximum(REF(O,1),REF(C,1)))/np.maximum(O,C)*100>1.2 ) * ( (np.minimum(REF(O,1),REF(C,1))-REF(L,1))/L*100<1.2)
    UX=((np.minimum(O,C)-L)/L*100>1.2 ) * ( (H-np.maximum(O,C))/np.maximum(O,C)*100<1.2)
    UK=(V>REF(V,1)/2)
    a=(US ) * ( UX ) * ( UK  ) * ( UB1)
#结束
    买入 = a
