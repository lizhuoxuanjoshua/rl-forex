...
#开始
    ...
    HJ_14=(np.maximum(HJ_11,HJ_12))
    HJ_15=(np.maximum(HJ_14,HJ_13))
    ...
    HJ_25=(HJ_16 ) * ( HJ_17 ) * ( HJ_18 ) * ( HJ_19 ) * ( HJ_20 ) * ( HJ_21 ) * ( HJ_22)
    暴利=(FILTER(HJ_25,15))
    PL1=((((CLOSE-MA54)/MA54)<0.1))
    PL2=((((CLOSE-MA10)/MA10)<0.3))
    偏离率=((((PL1==1) ) * ( (PL2==1))*0.2))
    平台突破=(((((偏离率==0) ) * ( (REF(偏离率,1)==0.2)) ) * ( (REF((count((偏离率==0.2),10)==10),1)==1))*-0.1))
    妖股突破=(平台突破==-0.1)
    XDF=(((暴利) ) + ( (妖股突破)))
    VARA=(DMA(AMOUNT/VOL/100,VOL))
    VARAA=(C/(REF(CLOSE,1))>1.05 ) * ( (C/O>1.05))
    FLIGA=(((VOL/REF(VOL,1)>1.2 ) * ( count(CLOSE>OPEN,1))) ) + ( ((LOW>REF(HIGH,1) ) * ( count(OPEN>CLOSE,1) ) * ( VOL/REF(VOL,1)>1.2)
))
    强妖=(XDF ) * ( C/REF(C,1)>1.095 ) * ( FLIGA ) * ( VARAA)
    XA_1=(VOL/REF(MA(VOL,5),1))
    XA_2=((CLOSE-REF(CLOSE,1))/REF(CLOSE,1)*100)
    XA_3=(HIGH)
    XA_4=(REF(HIGH,1))
    XA_5=(CLOSE)
    XA_6=(LOW)
    ...
    XA_17=(XA_5>XA_11)
    XA_18=(XA_5>XA_12)
    XA_19=(XA_5>MA(XA_5,88))
    太妖=((CLOSE-REF(CLOSE,1))/REF(CLOSE,1)*100>=9.93)
    妖中妖=(太妖 ) * ( 强妖)
    X_1=((3*CLOSE+LOW+OPEN+HIGH)/6)
    B=((20*X_1+19*REF(X_1,1)+18*REF(X_1,2)+17*REF(X_1,3)+16*REF(X_1,4)+15*REF(X_1,5)+14*REF(X_1,6)+13*REF(X_1,7)+12*REF(X_1,8)+11*REF(X_1,9)+10*REF(X_1,10)+9*REF(X_1,11)+8*REF(X_1,12)+7*REF(X_1,13)+6*REF(X_1,14)+5*REF(X_1,15)+4*REF(X_1,16)+3*REF(X_1,17)+2*REF(X_1,18)+REF(X_1,20))/210)
    D=(MA(B,15))
    ZD=(hhv(H,250)/llv(H,250)<=1.4)
    a=(妖中妖 ) * ( REF(ZD,1))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
