...
#开始
    ...
    涨停=((C-REF(C,1))/REF(C,1)*100>9.1)
    特征=(count((H-L)/L*100>18,3)>0 ) * ( count(涨停,8)>2)
    条件=(V<REF(V,1) ) * ( REF(V,1)>REF(V,2) ) * ( (C-REF(C,1))/REF(C,1)*100>9.1)
    XG=(count(特征,8)>0 ) * ( 条件)
    条件2=(V<REF(V,1) ) * ( REF(V,1)>REF(V,2) ) * ( C<O)
    a=(XG ) * ( count(XG,9)<=2)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
