...
#开始
    ...
    LC=(REF(C,1))
    RSI6=(sma(np.maximum(C-LC,0),6,1)/sma(np.abs(C-LC),6,1)*100)
    RS20=((C-llv(L,20))/(hhv(H,20)-llv(L,20))*100)
    BI20=((C-MA(C,20))/MA(C,20)*200+25)
    BI40=((C-MA(C,40))/MA(C,40)*100+14)
    XG=(BI20<0 ) * ( BI40<0 ) * ( ((RS20<5) ) + ( (L==llv(L,36))))
    a=(hhv(XG,8)==1 ) * ( RSI6<16  ) * ( (C<O) ) * ( C==llv(C,30) ) * ( L==llv(L,30) ) * ( C<O ) * ( C>LC*0.9014)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
