...
#开始
    ...
    M2=(MA(C,20))
    M3=(MA(C,60))
    A1=(M1>M2 ) * ( M2>M3 ) * ( EVERY(M3>REF(M3,1),2))
    ZT=(C/REF(C,1)>1.095 ) * ( C==H)
    A2=(ZT ) * ( CROSS(C,M1) ) * ( CROSS(C,M2))
    A3=(REF(C,1)<M2)
    A4=(REF(hhv(C,19),1)<=H ) * ( REF(llv(C,19),1)>=L)
    A6=(count(ZT,60)<=1)
    a=(REF(A6,1)  ) * ( A4 ) * ( A1 ) * ( A2 ) * ( A3 ) * ( H==hhv(H,20) ) * ( hhv(C,20)/llv(L,20)<=1.22)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
