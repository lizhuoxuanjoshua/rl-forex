...
#开始
    ...
    D2=sma(np.abs(L-REF(L,1)),34,1)

    D3=sma(np.maximum(L-REF(L,1),0),34,2)

    DD=-(EMA(np.where(L<=D1,D2/D3,-3),9))

    F1=llv(L,8)

    F2=sma(np.abs(L-REF(L,1)),34,1)

    F3=sma(np.maximum(L-REF(L,1),0),34,2)

    A=-(EMA(np.where(L<=F1,F2/F3,-3),9))

    a=(REF(A,1)<0) * (REF(A,2)>REF(A,1)) * (A>REF(A,1))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
