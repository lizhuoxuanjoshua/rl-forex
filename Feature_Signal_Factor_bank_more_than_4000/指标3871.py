...
#开始
    ...
    XA_12=(REF(XA_5,1))
    XA_13=(XA_5>XA_12)
    XA_14=(np.where(XA_7,10,0-10)+np.where(XA_8,10,0-10)+np.where(XA_9,10,0-10)+np.where(XA_10,10,0-10)+np.where(XA_11,10,0-10)+np.where(XA_13,10,0-10))
    XA_15=(np.arctan((XA_1/REF(XA_1,1)-1)*100)*180/3.1416)
    XA_16=((CLOSE-llv(LOW,21))/(hhv(HIGH,21)-llv(LOW,21))*100)
    XA_17=(CEILING(sma(XA_16,13,8)))
    XA_18=(XA_17<10 ) * ( XA_14==(-60) ) * ( XA_15<(-60))
    XA_19=(np.arctan((XA_2/REF(XA_2,1)-1)*100)*180/3.1416)
    XA_20=(CLOSE>XA_6 ) * ( OPEN>XA_6 ) * ( CLOSE>OPEN ) * ( XA_15>50 ) * ( XA_19>70)
    XA_21=(FILTER(XA_20,13))
    XA_22=(MA(LOW+(HIGH-LOW)/2,26)*1.15)
    XA_23=(MA(LOW+(HIGH-LOW)/2,26)*0.95)
    XA_24=((XA_22+XA_23)/2)
    XA_25=((MA(CLOSE,5)+MA(OPEN,5))/2*1.06)
    XA_26=((MA(CLOSE,5)+MA(OPEN,5))/2*0.98)
    XA_27=((XA_25+XA_26)/2)
    XA_28=(XA_25>XA_23)
    XA_29=(XA_25>XA_24)
    XA_30=(XA_25>XA_22)
    ...
    XA_35=(XA_26>XA_24)
    XA_36=(XA_26>XA_22)
    XA_37=(np.where(XA_28,10,0-10)+np.where(XA_29,10,0-10)+np.where(XA_30,10,0-10)+np.where(XA_31,10,0-10)+np.where(XA_32,10,0-10)+np.where(XA_33,10,0-10)+np.where(XA_34,10,0-10)+np.where(XA_35,10,0-10)+np.where(XA_36,10,0-10))
    XA_38=(EMA(EMA(EMA(CLOSE,13),13),13))
    XA_39=(np.arctan((XA_38/REF(XA_38,1)-1)*100)*180/3.1416)
    a=(count(XA_18,30) ) * ( XA_21 ) * ( XA_37<10 ) * ( XA_39<=0)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
