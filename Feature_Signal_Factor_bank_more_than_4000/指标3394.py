...
#开始
    ...
    BWVAR15=(EMA(EMA(C,5),count(V,8)/2.8))
    ...
    BWVAR14=(H>L*1.053 ) * ( count(H>L*1.053,5)>2 ) * ( ((C/MA(C,30)*100<75) ) + ( (C/MA(C,50)*100<78)))
    天王买4=(BWVAR16 ) * ( BWVAR11 ) * ( BWVAR12 ) * (( BWVAR13) ) + ( (BWVAR14#-34,1.76}
))
    JBVAR1=((C-MA(C,48))/MA(C,48)*100<-28)
    JBVAR2=(sma(np.maximum(C-REF(C,1),0),12,1)/sma(np.abs(C-REF(C,1)),12,1)*100>23)
    天王买5=(JBVAR1 ) * ( JBVAR2 ) * ( VOL>MA(VOL,18) ) * ( count(C==0,6)==0)
    XRVAR2=((C-MA(C,50))/MA(C,50)*100<-23)
    XRVAR3=(sma(np.maximum(C-REF(C,1),0),3,1)/sma(np.abs(C-REF(C,1)),3,1)*100>16)
    XRVAR4=(CROSS(100*(hhv(H,27)-C)/(hhv(H,27)-llv(L,27)),79))
    XRVAR5=((0-REF(C,1))/REF(C,1)<0.05 ) * ( V>MA(V,23))
    天王买6=(XRVAR2 ) * ( XRVAR3 ) * ( XRVAR4 ) * ( ((0>C) ) + ( (0<C)) ) * ( XRVAR5)
    a=((天王买4+天王买5+天王买6)>=2)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
