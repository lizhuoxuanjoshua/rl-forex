...
#开始
    ...
    V1=3*sma((C-llv(L,55))/(hhv(H,55)-llv(L,55))*100,5,1)-2*sma(sma((C-llv(L,55))/(hhv(H,55)-llv(L,55))*100,5,1),3,1)

    趋势线=EMA(V1,3)

    V2=(趋势线-REF(趋势线,1))/REF(趋势线,1)*100

    a=(趋势线<=13) * (V2>13) * (REF(V2,1)<13)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
