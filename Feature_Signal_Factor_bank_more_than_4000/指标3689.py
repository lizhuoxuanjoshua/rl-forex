...
#开始
    ...
    VARR5=(VARR3>VARR4)
    LC=(REF(CLOSE,1))
    RSI1=(sma(np.maximum(CLOSE-LC,0),12,1)/sma(np.abs(CLOSE-LC),12,1)*100)
    RSI2=(sma(np.maximum(CLOSE-LC,0),56,1)/sma(np.abs(CLOSE-LC),56,1)*100)
    VARR6=(RSI1>RSI2 ) * ( VARR5 ) * ( VARR2)
    VASS1=(hhv(H,30))
    VASS2=(llv(L,30))
    VASS3=(REF((VASS1/VASS2-1)*100<=30,1))
    A1=(np.abs(((3.48*CLOSE+HIGH+LOW)/4-EMA(CLOSE,23))/EMA(CLOSE,23)))
    A2=(EMA(((2.15*CLOSE+LOW+HIGH)/4),A1))
    游资=(EMA(A2,200)*1.1)
    条件=((C-REF(C,1))/REF(C,1)*100>5)
    a=(CROSS(C,游资) ) * ( REF(C*1.097,1)<C ) * ( VARR6 ) * ( VASS3)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
