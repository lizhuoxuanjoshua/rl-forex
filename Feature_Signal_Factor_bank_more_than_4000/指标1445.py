...
#开始
    ...
    MA62=(MA(C,62))
    MA133=(MA(C,133))
    W=(np.where(MA5>REF(MA5,1),MA5,0))
    WW=(np.where(MA5<REF(MA5,1),MA5,0))
    AA=( EMA(EMA(EMA(100*(CLOSE-llv(LOW,33))/(hhv(HIGH,33)-llv(LOW,33)),1),2),2))
    BB=( EMA(AA,5))
    EE=((AA>50) ) * ( CROSS(AA,BB) ) * ( BB>REF(BB,1))
    a=(FILTER(EE==1,1))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
