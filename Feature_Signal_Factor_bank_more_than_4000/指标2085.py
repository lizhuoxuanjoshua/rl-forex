...
#开始
    ...
    VAR2=(((CLOSE>OPEN) ) + ( (CLOSE>REF(CLOSE,1))) ) * ( VOL>REF(VOL,1))
    VAR3=(count(VAR2,3)==3)
    VAR4=(llv(LOW,5)==llv(LOW,120))
    VAR5=(VAR3 ) * ( VAR4)
    VAR6=(count(CROSS(MA(CLOSE,10),MA(CLOSE,5)),2)==1 ) * ( count(CROSS(MA(CLOSE,5),MA(CLOSE,20)),2)==1)
    VAR7=(count(VAR5,3)>0 ) * ( count(VAR6,3)>0)
    VAR8=(VAR7 ) * ( count(VAR7,20)==1)
    VARD=((CLOSE-llv(LOW,89))/(hhv(HIGH,89)-llv(LOW,89))*100)
    VARE=(sma(VARD,8,1))
    VARF=(sma(VARE,10,1))
    VAR10=(VARE>REF(VARE,1) ) * ( VARE>VARF ) * ( barslast(CROSS(VARE,VARF))<=34)
    a=(VAR10)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
