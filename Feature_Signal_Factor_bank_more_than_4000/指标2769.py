...
#开始
    ...
    VAR33=(CLOSE-llv(LOW,33))/(hhv(HIGH,33)-llv(LOW,33))
    K33=sma(VAR33,2,1)
    VOLUME=(VOL)
    VAR1VV=(VOL/((H-L)*2-np.abs(C-O)))
    AA=(np.where(C>O,VAR1VV*(H-L),np.where(C<O,VAR1VV*((H-O)+(C-L)),VOL/2)))
    AB=(np.where(C>O,0-VAR1VV*((H-C)+(O-L)),np.where(C<O,0-VAR1VV*(H-L),0+V/2)))
    vb=((AA+AB))
    vbk=(np.where(vb>0,vb,-vb))
    a=(V>=hhv(V,30) ) * ( vbk>V*0.6 ) * ( V>K33 )
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
