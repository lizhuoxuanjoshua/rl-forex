...
#开始
    ...
    SVM1=(MA(CLOSE,20)-MA(CLOSE,60))/MA(CLOSE,60)*200

    SVM2=-(MA(CLOSE,20)-MA(CLOSE,60))/MA(CLOSE,60)*200

    KM=np.where(SVM1>0,SVM1,0)

    KKM=-np.where(SVM2>0,SVM2,0)

    DNN=hhv(KM,9)

    DXX=hhv(-KKM,9)

    SV1=(CLOSE-MA(CLOSE,20))/MA(CLOSE,20)*200

    SV2=-(CLOSE-MA(CLOSE,20))/MA(CLOSE,20)*200

    K=np.where(SV1>0,SV1,0)

    KK=-np.where(SV2>0,SV2,0)
    ...
    a=CROSS(XNN,XXX) * (DXX==0) * (MA20>REF(MA20,1))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
