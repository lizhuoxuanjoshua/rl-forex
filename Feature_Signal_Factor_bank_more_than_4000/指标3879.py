...
#开始
    ...
    XA_2=(REF(LOW,1))
    XA_3=(1)
    XA_4=(sma(np.abs(LOW-XA_2),3,1)/sma(np.maximum(LOW-XA_2,0),3,1)*100)
    XA_5=(EMA(np.where(CLOSE*1.2,XA_4*10,XA_4/10),3)*XA_1)
    XA_6=(llv(LOW,38))
    XA_7=(hhv(XA_5,38)*XA_1)
    XA_8=(np.where(llv(LOW,90),1,0)*XA_3)
    XA_9=(EMA(np.where(LOW<=XA_6,(XA_5+XA_7*2)/2,0),3)/618*XA_8*XA_3)
    XA_10=((CLOSE-MA(CLOSE,40))/MA(CLOSE,40)*100)
    a=(CROSS(XA_10,(-24)))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
