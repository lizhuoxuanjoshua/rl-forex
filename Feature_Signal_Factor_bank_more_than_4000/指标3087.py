...
#开始
    ...
    JJZ=((HIGH+LOW+CLOSE)/3)
    QJ0Z=(AMOUNT/np.where(HIGH==LOW,4,HIGH-LOW))
    买1=(QJ0Z*(np.minimum(OPEN,CLOSE)-LOW))
    买2=(QJ0Z*(JJZ-np.minimum(CLOSE,OPEN)))
    卖1=(QJ0Z*(np.maximum(CLOSE,OPEN)-JJZ))
    卖2=(QJ0Z*(HIGH-np.maximum(OPEN,CLOSE)))
    DTZ=(((买1+买2)-(卖1+卖2))/10000)
    AC=(count(DTZ,66))
    AA=(sma(AC,15,1))
    AB=(sma(AA,6,1))
    a=(CROSS(AA,AB))
