...
#开始
    ...
    EMA55=(EMA(C,55))
    EMA120=(EMA(C,120))
    BR2=(np.abs((EMA13-EMA34)+(EMA34-EMA55))/C<0.2)
    BR3=(C>EMA55 ) * ( ((REF(C,1)<REF(EMA55,1)) ) + ( (REF(C,2)<REF(EMA55,2)) ) + ( (REF(C,3)<REF(EMA55,3)) ) + ( (REF(C,4)<REF(EMA55,4)) ) + ( (REF(C,5)<REF(EMA55,5))))
    BR4=( BR2 ) * ( BR3)
    a=(BR4 ) * ( FILTER(BR4,10))
#结束
    买入 = a
