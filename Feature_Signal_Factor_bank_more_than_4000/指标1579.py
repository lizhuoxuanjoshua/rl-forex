...
#开始
    ...
    RAA=(sma(sma(RSV1,3,1),3,1)+3*STD(CLOSE,21))
    RAB=(sma(RSV2,5,1)+2*STD(CLOSE,37))
    RSV3=((CLOSE-llv(LOW,55))/(hhv(HIGH,55)-llv(LOW,55))*100)
    RAC=(sma(RSV3,5,1))
    AA2=(-100*(hhv(HIGH,40)-CLOSE)/(hhv(HIGH,40)-llv(LOW,40)))
    a=(CROSS(AA2,-90) ) * ( (RAC<RAB ) * (( RAC>REF(RAC,1))) + ( (RAA<20 )) * ( RAB<20 ) * ( RAC<20))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
