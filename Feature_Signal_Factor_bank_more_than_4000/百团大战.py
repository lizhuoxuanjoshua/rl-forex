...
#开始
    ...
    MTM1=(MA((CLOSE-REF(CLOSE,13))*31,5)
)
    ...
    VARFB=np.where(C==C,VARFA,VARFA)
    ...
    VARFE=sma(np.abs(VARFC-VARFD),3,1)/sma(np.maximum(VARFC-VARFD,0),3,1)*100

    

    VARFF=MA(np.where(CLOSE*1.35<=VARFB,VARFE*10,VARFE/10),3)
    ...
    绝佳买点=np.where(C==C,MA(np.where(LOW<=VARF10,(VARFF+VARF11*2)/2,0),3),100000000)/(200)

    

    a=CROSS(绝佳买点,1.20000)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
