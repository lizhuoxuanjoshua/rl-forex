...
#开始
    ...
    AB=(MA(C,24))
    穿头破脚=(REF(CLOSE,1)/REF(OPEN,1)<0.97 ) * ( CLOSE/OPEN>1.04 ) * ( CLOSE>REF(OPEN,1) ) * ( OPEN< REF(CLOSE,1))
    早晨之星=(REF(CLOSE,2)/REF(OPEN,2) < 0.95 ) * ( REF(OPEN,1) < REF(CLOSE,2) ) * ( np.abs(REF(OPEN,1)-REF(CLOSE,1))/REF(CLOSE,1)<0.03 ) * (CLOSE/OPEN>1.05 ) * ( CLOSE>REF(CLOSE,2))
    早晨十字星=(REF(CLOSE,2)/REF(OPEN,2) < 0.95 ) * ( REF(OPEN,1) < REF(CLOSE,2) ) * ( REF(OPEN,1)==REF(CLOSE,1) ) * ( CLOSE/OPEN>1.05 ) * ( CLOSE>REF(CLOSE,2))
    平底=((np.abs(LOW-REF(LOW,1))/LOW<0.001 ) * ( np.abs(REF(LOW,1)-REF(LOW,2))/REF(LOW,1)<=0.001))
    曙光初现=(REF(CLOSE,1)/REF(OPEN,1)<0.97 ) * ( CLOSE/OPEN>1.03 ) * ( OPEN< REF(CLOSE,1) ) * ( CLOSE>REF(CLOSE,1))
    跳空缺口=( LOW>REF(HIGH,1))
    AA=(count(((穿头破脚) ) + ( (早晨之星) ) + ( (早晨十字星) ) + ( (平底) ) + ( (曙光初现) ) + ( (跳空缺口)) ,5) >=1)
    a=(CROSS(C,AB))  * ( AA)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
