...
#开始
    ...
    较量=(EMA((CLOSE-MA((2*CLOSE+HIGH+LOW)/4,30))/MA((2*CLOSE+HIGH+LOW)/4,30)*100,3))
    买点1=(np.where(CROSS(较量,0),40,0))
    A5=(llv(L,94))
    A6=(hhv(H,89))
    A7=(EMA((C-A5)/(A6-A5)*100,8))
    A8=(EMA(0.667*REF(A7,1)+0.333*A7,10))
    AA=(count(买点1>=40,5)>0)
    a=(CROSS(A7,A8) ) * ( AA)
#结束
    买入 = a
