...
#开始
    ...
    VAR17=(REF(CLOSE,1))
    VAR18=(sma(np.maximum(CLOSE-VAR17,0),7,1)/sma(np.abs(CLOSE-VAR17),7,1)*100)
    VAR19=(sma(np.maximum(CLOSE-VAR17,0),13,1)/sma(np.abs(CLOSE-VAR17),13,1)*100)
    VAR1A=(barslast(VAR19<33)<=3 ) * ( CROSS(VAR18,VAR19))
    ...
    a= ( VAR1B ) * ( CROSS(VAR1E,VAR1F) ) * ( VAR1E<=25)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
