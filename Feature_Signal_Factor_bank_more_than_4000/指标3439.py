...
#开始
    ...
    VARE=(3*VARC-2*VARD)
    VARF=(np.power(MA(CLOSE,5),2)+MA(CLOSE,5))
    VAR10=(np.power(MA(LOW,5),2)+MA(LOW,5))
    VAR11=(np.power(MA(HIGH,5),2)+MA(HIGH,5))
    ...
    VAR24=(count(AMOUNT,5)/count(VOL,5)/100)
    VAR25=((VAR22-MA(VAR22,34))/(0.015*AVEDEV(VAR22,34)))
    VAR26=(DMA(AMOUNT/VOL/100,VOL))
    ...
    VAR3B=(MA(VAR3A,3))
    a=(CROSS(价位线,0) ) * (( count(VAR3B<-12,2)>=1) ) + ( (CROSS(价位线,趋势线) )) * ( count(VAR3B<-12,2)>=1)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
