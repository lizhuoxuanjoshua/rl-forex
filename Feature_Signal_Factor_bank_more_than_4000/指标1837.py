...
#开始
    ...
    BC=((np.where(C>(H+L+C)/3,(H*0.618+L*0.382)*0.382,(H*0.382+L*0.618)*0.382)+(O*0.382+C*0.618)*0.618))
    市价=(AMOUNT/(VOL))
    DEM3=(VOL/(count(AMOUNT,3)/(市价)))

    DEM7=(VOL/(count(AMOUNT,7)/(市价)))
    DEM13=(VOL/(count(AMOUNT,13)/(市价)))
    CB=(VOL/(count(AMOUNT,38)/(市价)))
    GLC=(((BC/DEM3-1)*100+(BC/DEM7-1)*100+(BC/DEM13-1)*100+(BC/CB-1)*100)/4)
    GL3=(((DEM3/DEM7-1)*100+(DEM3/DEM13-1)*100+(DEM3/REF(DEM3,1)-1)*100)/3)
    GL7=(((DEM7/DEM13-1)*100+(DEM7/REF(DEM7,1)-1)*100)/2)
    GL13=((DEM13/REF(DEM13,1)-1)*100)
    GL21=(((DEM3/CB-1)*100+(DEM7/CB-1)*100+(DEM13/CB-1)*100+(CB/REF(CB,1)-1)*100)/4)
    乖=((GL3+GL7+GL13))
    合=((GLC+GL3+GL7+GL13+GL21)/5)
    离=(((EMA(合,12)+EMA(合,14))/2+EMA(合,13))/2)
    a=(CROSS(乖,0)) * ( (离>REF(离,1)))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
