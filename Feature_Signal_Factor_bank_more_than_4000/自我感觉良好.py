...
#开始
    ...
    VAR5=EMA(np.where(LOW<=VAR4,VAR3,0),3)*5

    VAR21=sma(np.abs(HIGH-VAR1),13,1)/sma(np.minimum(HIGH-VAR1,0),10,1)*2

    VAR31=EMA(VAR21,10)*2

    VAR41=hhv(HIGH,33)

    VAR51=EMA(np.where(HIGH>=VAR41,VAR31,0),3)*3

    a=(VAR51<REF(VAR51,1)) * (REF(VAR51,1)>REF(VAR51,2))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
