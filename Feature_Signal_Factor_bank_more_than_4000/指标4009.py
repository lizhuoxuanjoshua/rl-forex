...
#开始
    ...
    均价=(AMOUNT/VOL)
    X_15=((CLOSE-REF(CLOSE,1))/REF(CLOSE,1)*100)
    X_16=((HIGH-REF(CLOSE,1))/REF(CLOSE,1)*100)
    X_17=((AMOUNT/VOL/100-REF(CLOSE,1))/REF(CLOSE,1)*100)
    真C=((np.where(CLOSE<均价,均价+(HIGH-均价)/3,CLOSE+(HIGH-CLOSE)/3-(CLOSE-均价)/3)))
    真C涨幅=np.where(X_15<=X_17,X_17+(X_16-X_17)/3,X_15+(X_16-X_15)/3)
    X_21=(CLOSE>=REF(CLOSE,1)*1.093 ) * ( CLOSE==HIGH)
    X_31=((VOL>=REF(VOL,1)*1.75 ) * ( X_15>=1.99 ) * ( 真C涨幅>=2.45 ) * ( X_16>3)*0.2)
    X_32=((VOL>=REF(VOL,1)*1.65 ) * ( X_15>=1.99 ) * ( 真C涨幅>=3 ) * ( X_16>3.3)*0.2)
    X_33=((VOL>=REF(VOL,1)*1.55 ) * ( X_15>=2.25 ) * ( 真C涨幅>=3.3 ) * ( X_16>3.5)*0.2)
    X_34=((VOL>=REF(VOL,1)*1.5 ) * ( X_15>=2.5 ) * ( 真C涨幅>=3.5 ) * ( X_16>4)*0.2)
    X_35=((VOL>=REF(VOL,1)*1.45 ) * ( X_15>=2.68 ) * ( 真C涨幅>=4 ) * ( X_16>5)*0.2)
    X_36=(((X_31) ) + ( (X_32) ) + ( (X_33) ) + ( (X_34) ) + ( (X_35))*0.2)
    X_37=(VOL>REF(VOL,1) ) * ( CLOSE>=REF(CLOSE,1) ) * ( REF(CLOSE,1)>=REF(CLOSE,2))
    X_38=((CLOSE-REF(CLOSE,2))/REF(CLOSE,2)*100)
    X_39=(X_37 ) * ( X_38>=4.5 ) * ( VOL+REF(VOL,1)>=REF(VOL,2)*4)
    X_40=(VOL>REF(VOL,1) ) * ( REF(VOL,1)>REF(VOL,2)*0.9 ) * ( VOL>REF(VOL,2) ) * ( CLOSE>REF(CLOSE,1) ) * ( REF(CLOSE,1)>=REF(CLOSE,2) ) * ( REF(CLOSE,2)>=REF(CLOSE,3) ) * ( VOL+REF(VOL,1)+REF(VOL,2)>=REF(VOL,3)*5)
    X_41=((CLOSE-REF(CLOSE,3))/REF(CLOSE,3)*100)
    X_42=(X_40 ) * ( X_41>=5)
    X_43=(((X_39) ) + ( (X_42))*0.4)
    X_44=(CLOSE<=REF(CLOSE,1) ) * ( CLOSE>REF(CLOSE,1)*0.97)
    X_45=(CLOSE<=REF(CLOSE,1)*0.97)
    X_46=(REF(X_44,1) ) * ( CLOSE>=REF(OPEN,1) ) * ( CLOSE>=REF(CLOSE,1)*1.04)
    X_47=(REF(X_45,1) ) * ( CLOSE>=(REF(OPEN,1)+REF(CLOSE,1))*0.5 ) * ( CLOSE>=REF(CLOSE,1)*1.045)
    X_48=((X_46) ) + ( (X_47
))
    X_49=(CLOSE>=REF(CLOSE,2)*1.045 ) * ( CLOSE>=REF(CLOSE,1)*1.04 ) * ( REF(X_44,2) ) * ( CLOSE>=REF(OPEN,2))
    X_50=(CLOSE>=REF(CLOSE,2)*1.05 ) * ( CLOSE>=REF(CLOSE,1)*1.045 ) * ( REF(X_45,2) ) * ( CLOSE>=(REF(OPEN,1)+REF(CLOSE,1))*0.5)
    X_51=((X_49) ) + ( (X_50
))
    X_52=(((X_48) ) + ( (X_51))*0.6)
    X_53=(CLOSE>=REF(CLOSE,1)*1.05)
    X_54=(CLOSE>=REF(CLOSE,1)*1.045 ) * ( HIGH>=REF(CLOSE,1)*1.05 ) * ( 真C涨幅>=5)
    X_55=((X_53) ) + ( (X_54
))
    美柱=((X_55) ) + ( (X_52) ) + ( (X_36) ) + ( (X_43) )
