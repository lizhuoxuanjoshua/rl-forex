...
#开始
    ...
    主力追踪=(3*sma((C-llv(L,13))/(hhv(H,13)-llv(L,13))*100,5,1)-2*sma(sma((C-llv(L,13))/(hhv(H,13)-llv(L,13))*100,5,1),3,1))
    LOWV=(llv(C,15))
    HIGHV=(hhv(C,15))
    RSV=(EMA((C-LOWV)/(HIGHV-LOWV)*100,3))
    K=(EMA(RSV,3))
    牛股1=(REF(主力追踪<=10,1) ) * ( K>REF(K,1))
    VAR1=(4*sma((CLOSE-llv(LOW,5))/(hhv(HIGH,5)-llv(LOW,5))*100,5,1)- 3*sma(sma((CLOSE-llv(LOW,5))/(hhv(HIGH,5)-llv(LOW,5))*100,5,1),3.2,1))
    牛股2=(np.where(CROSS(VAR1,8),80,0))
    快线=((CLOSE-llv(LOW,9))/(hhv(HIGH,9)-llv(LOW,9))*100)
    慢线=(sma(快线,3,1))
    空=(sma(慢线,3,1))
    牛股3=(barslast(CROSS(慢线,空))<9 ) * ( CROSS(慢线,空) ) * ( 慢线<20)
    a= ( (牛股2) ) + ( (牛股3
))
#结束
    买入 = a
