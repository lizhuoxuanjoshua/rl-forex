...
#开始
    ...
    TTR=count(np.maximum(np.maximum((HIGH - LOW),np.abs((HIGH - REF(CLOSE,1)))),np.abs((LOW - REF(CLOSE,1)))),5)

    HD=(HIGH - REF(HIGH,1))

    LD=(REF(LOW,1) - LOW)

    DMP=(count(np.where(((HD > 0) * (HD > LD)),HD,0),5) )

    DMM=count(np.where(((LD > 0) * (LD > HD)),LD,0),5)

    神偷线=(((DMP ) * 100) / TTR)

    辅助线=((DMM * 100) / TTR)

    XXX=(MA(((np.abs((辅助线 - 神偷线)) / (辅助线 + 神偷线)) * 100),3))
