...
#开始
    ...
    AA=(MA(3*sma((C-llv(L,21))/(hhv(H,21)-llv(L,21))*100,5,1)-2*sma(sma((C-llv(L,21))/(hhv(H,21)-llv(L,21))*100,5,1),3,1),3))
    AB=(MA(3*sma((C-llv(L,27))/(hhv(H,27)-llv(L,27))*100,5,1)-2*sma(sma((C-llv(L,27))/(hhv(H,27)-llv(L,27))*100,5,1),3,1),5))
    a=(CROSS(AA,AB)) * ( AB<10)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
