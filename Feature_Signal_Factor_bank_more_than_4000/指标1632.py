...
#开始
    ...
    VARC=(np.maximum(VARB,0))
    VARD=(np.abs(VARB))
    VARE=(sma(VARC,7,1)/sma(VARD,7,1)*100)
    VARF=(sma(VARC,13,1)/sma(VARD,13,1)*100)
    VAR10=(BARScount(CLOSE))
    VAR11=(sma(np.maximum(VARB,0),6,1)/sma(np.abs(VARB),6,1)*100)
    VAR12=((-200)*(hhv(HIGH,60)-CLOSE)/(hhv(HIGH,60)-llv(LOW,60))+100)
    VAR13=((CLOSE-llv(LOW,15))/(hhv(HIGH,15)-llv(LOW,15))*100)
    VAR14=(sma((sma(VAR13,4,1)-50)*2,3,1))
    VAR18=((hhv(HIGH,30)-CLOSE)/CLOSE*100)
    VAR19=(VAR11<=25 ) * ( VAR12<-95 ) * ( VAR18>20 ) * ( VAR14<-30  )  * ( VARE<22 ) * ( VARF<28 ) * ( VAR10>50)
    a=(CROSS(VAR19,0.5) ) * ( count(VAR19==1,10)==1)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
