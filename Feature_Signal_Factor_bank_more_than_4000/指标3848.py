...
#开始
    ...
    CX1=(O<MA(C,5) ) * ( O<MA(C,10) ) * ( O<MA(C,20) ) * ( C>MA(C,5) ) * ( C>MA(C,10) ) * ( C>MA(C,20))
    CX11=(O<MA(C,5) ) * ( O<MA(C,10) ) * ( O<MA(C,30) ) * ( C>MA(C,5) ) * ( C>MA(C,10) ) * ( C>MA(C,30))
    CX2=(MA(C,5)>MA(C,10) ) * ( MA(C,10)>MA(C,20) ) * ( MA(C,10)>MA(C,30) ) * ( MA(C,30)>MA(C,60))
    CX3=(NOT((CROSS(MA(C,10),MA(C,5))) ) + ( (CROSS(MA(C,30),MA(C,20)))))
    CX4=(C>O ) * ( C/REF(C,1)>1.02)
    a=(((CX1) ) + ( (CX11)) ) * ( CX2 ) * ( CX3 ) * ( CX4)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
