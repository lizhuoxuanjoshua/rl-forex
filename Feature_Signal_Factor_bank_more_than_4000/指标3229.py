...
#开始
    ...
    AA=(sma(sma(RSV1,3,1),3,1)+3*STD(CLOSE,21))
    AB=(sma(RSV2,5,1)+2*STD(CLOSE,37))
    RSV3=((CLOSE-llv(LOW,55))/(hhv(HIGH,55)-llv(LOW,55))*100)
    AC=(sma(RSV3,5,1))
    A2=(-100*(hhv(HIGH,40)-CLOSE)/(hhv(HIGH,40)-llv(LOW,40)))
    a=(CROSS(A2,-90) ) * ( (AC<AB ) * (( AC>REF(AC,1))) + ( (AA<20 )) * ( AB<20 ) * ( AC<20))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
