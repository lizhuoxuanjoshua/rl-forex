...
#开始
    ...
    强弱线=((次上通道+次下通道)/2)
    次上通道1=((MA(C,20)+MA(O,20))/2*1.122)
    次下通道1=((MA(C,20)+MA(O,20))/2*0.962)
    强弱线1=((次上通道1+次下通道1)/2)
    次上通道2=(((MA(C,80)+MA(O,80))/2*1.24
))
    次下通道2=(((MA(C,80)+MA(O,80))/2*0.92
))
    强弱线2=(((次上通道2+次下通道2)/2
))
    次上通道3=((MA(C,160)+MA(O,160))/2*1.482)
    次下通道3=((MA(C,160)+MA(O,160))/2*0.842)
    强弱线3=((次上通道3+次下通道3)/2)
    死叉=((CROSS(上通道,强弱线)) ) + ( (CROSS(中轨,强弱线)
))
    强势死叉=(CROSS(上通道,次下通道))
    涨幅=((C-REF(C,1))/REF(C,1)*100)
    实体涨幅=((C-O)/O*100)
    涨幅抵抗=(((涨幅>7) ) + ( (实体涨幅>7)) ) * ( barslast(死叉)<=4)
    死叉冲程=(-np.where(死叉,(强弱线-REF(强弱线,1))/REF(强弱线,1),0))

    a=(count(死叉,3)>0 ) * ( count(涨幅抵抗,3)>0)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
