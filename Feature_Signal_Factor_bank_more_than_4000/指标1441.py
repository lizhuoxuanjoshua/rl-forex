...
#开始
    ...
    A1=(REF(C,2)<REF(C,3) ) * ( REF(C,3)<REF(C,4))
    SST=(C>REF(O,3))
    ST1=(REF(C,1)<=REF(O,2))
    ST2=(REF(C,2)/REF(C,3)<1 ) * ( REF(L,2)<REF(C,3))
    ST3=(REF(C,2)/REF(C,3)>1 ) * ( REF(O,2)<REF(C,3))
    ST4=((ST2) ) + ( (ST3
))
    ST5=(C/REF(C,1)>1.03)
    AA1=(A1 ) * ( SST ) * ( ST1 ) * ( ST4 ) * ( V/REF(V,1)>1 ) * ( ST5 ) * ( V/REF(V,1)>1.5 ) * ( K30<50)
    AA2=(REF(C,5)/REF(L,1)>1.09 ) * ( C>REF(O,2) ) * ( V/REF(V,1)>1.2 ) * ( K30<50)
    a=((AA1) ) + ( (AA2))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
