...
#开始
    ...
    M1=(REF(CLOSE,1))
    M2=(sma(np.maximum(CLOSE-M1,0),7,1)/sma(np.abs(CLOSE-M1),7,1)*100)
    G1=((REF(M2,1)<20 ) * ( M2>REF(M2,1)))
    TU=(C/MA(C,40)<0.74)
    PER1=(5)
    MMA=(EMA(C,PER1))
    SMMA=(EMA(MMA,PER1))
    ...
    TDF=( np.power(DIVMA,1)*np.power(AVERIMPET,3))
    NTDF=(TDF/hhv(np.abs(TDF),PER1*3))
    YUL=(count(TDJ,5)>1)
    启动=(TU ) * ( TDJ ) * ( YUL)
    确定=(CROSS(NTDF,-0.9))
    波段=((G1 ) * (( C1>20) ) + ( (C>REF(C,1))) ) * ( REF(启动,1))
    选股=(REF(启动,1) ) * ( ((确定) ) + ( (C>REF(C,1))))
    a=(count(选股,13)>=1 ) * ( 波段)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
