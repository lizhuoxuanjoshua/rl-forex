...
#开始
    ...
    Z_J2=(EMA(Z_J1,100))
    Z_J3=(2*(Z_J1-Z_J2))
    Z_J4=(np.power(Z_J3,3)*0.1+np.power(Z_J3,1))
    G_J1=(EMA(C,2)-EMA(C,89))
    G_J2=(EMA(G_J1,30))
    G_J3=(2*(G_J1-G_J2)*9.8)
    G_J4=(np.power(G_J3,3)*0.1+np.power(G_J3,2))
    AA=(np.where(G_J3>0.015,G_J4,0)/28)
    AB=(REF((AA/REF(AA,1)>4.5)  * ( AA/REF(AA,1))>3) ) * ( ((C-REF(C,1))/REF(C,1)>0.098
))
    a=(FILTER(AB,5))
#结束
    买入 = a
