...
#开始
    ...
    VAR0 =((C-REF(C,1))/REF(C,1)*100)
    VAR1 =((C-REF(C,2))/REF(C,2)*100)
    VAR2 =((C-REF(C,3))/REF(C,3)*100)
    VAR3 =((C-REF(C,4))/REF(C,4)*100)
    VAR4 =((C-REF(C,5))/REF(C,5)*100)
    VAR5 =((C-REF(C,6))/REF(C,6)*100)
    VAR6 =((C-REF(C,7))/REF(C,7)*100)
    VAR7 =((C-REF(C,8))/REF(C,8)*100)
    VAR8 =((C-REF(C,9))/REF(C,9)*100)
    VAR9 =((C-REF(C,10))/REF(C,10)*100)
    VAR20=(VAR0+VAR1+VAR2+VAR3+VAR4)
    VAR21=(VAR0+VAR1+VAR2+VAR3+VAR4+VAR5+VAR6+VAR7+VAR8+VAR9)
    VAR23=(((count(VAR20,3)*30)-MA(count(VAR20,3),45))/10)
    VAR24=(((VAR21*30)-REF(count(VAR21,3),1))/10)
    VAR25=(((VAR20*30)-REF(count(VAR20,3),1))/10)
    VAR26=((C-REF(C,1))/REF(C,1)*100)
    VAR27=((count(VAR26,3)*30)-MA(count(VAR26,3),45))
    VAR28=((VAR26*30)-REF(count(VAR26,3),1))
    XCII1=((VAR24+VAR25+VAR23+VAR27+VAR28)/5)
    a=(CROSS(XCII1,400) ) * ( OPEN>REF(C,1) ) * ( REF(XCII1,1)>100)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
