...
#开始
    ...
    可能底=np.where((操盘线>REF(操盘线,1)) * (REF(操盘线,1)<REF(操盘线,2)) * (操盘线<23) ,38,0)

    

    底后追买=np.where((操盘线>REF(操盘线,1)) * CROSS(操盘线,V5) * (V5<50),60,0)
    ...
    VARA1=(((CLOSE - MA(CLOSE,5)) / MA(CLOSE,5)) * 100)

    

    VARA2=(((CLOSE - MA(CLOSE,10)) / MA(CLOSE,10)) * 100)

    

    VARA3=(((CLOSE - MA(CLOSE,20)) / MA(CLOSE,20)) * 100)
