...
#开始
    ...
    AA=(100*(C-llv(L,5))/(hhv(C,5)-llv(L,5)))
    AAA=(100*(C-llv(L,10))/(hhv(C,10)-llv(L,10)))
    AAB=(100*(C-llv(L,20))/(hhv(C,20)-llv(L,20)))
    AAD=(100*(C-llv(L,30))/(hhv(C,30)-llv(L,30)))
    a=((AA<=6 ) * ( AAA<=6 ) * ( AAB<=6 ) * ( AAD<=6))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
