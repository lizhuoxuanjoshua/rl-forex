...
#开始
    ...
    前炮=((C-REF(C,1))*100/REF(C,1)>7)
    后炮=((C-REF(C,1))*100/REF(C,1)<7)
    T1=(V<REF(V,barslast(前炮)))
    调整天数=(barslast(前炮)<10)
    不破前炮开盘=(llv(C,barslast(前炮))>=REF(O,barslast(前炮)))
    小实体=(hhv(np.abs(C-REF(C,1))/REF(C,1)*100,barslast(前炮))<=6)
    回调=(调整天数 ) * ( 不破前炮开盘 ) * ( 小实体)
    选股=(后炮 ) * ( REF(回调,1) ) * ( T1)
    a=(选股 )
#结束
