...
#开始
    ...
    MA5=(MA(C,40))
    MA6=(MA(C,60))
    P3=(np.abs(MA1-MA2)+np.abs(MA2-MA5)+np.abs(MA1-MA5))
    P31=(np.abs(MA1-MA2)+np.abs(MA2-MA3)+np.abs(MA1-MA3))
    月三线粘合=(np.where(P31/C<5/100,2222,0))
    牛嘴=((月三线粘合 ) * ( MA1>REF(MA1,1) ) * ( C>MA1 ) * ( C>MA2) * ( C>MA6 ) * ( MA1>MA6 ) * ( MA2>MA6 ) * ( DIFF>REF(DIFF,1)) * ( MA5>REF(MA5,1) ) * ( MA6>REF(MA6,1) ) * ( MA4>REF(MA5,2)) * ( MA6>REF(MA6,2) ) * ( MA6>REF(MA6,3)))
    a=(牛嘴 ) * ( MACD>-0.32 ) * ( (CLOSE-REF(CLOSE,1))/REF(CLOSE,1)>1.5/100 ) * ( VOL/REF(MA(VOL,5),1)>1.5)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
