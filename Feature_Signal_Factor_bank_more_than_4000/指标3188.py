...
#开始
    ...
    VARD=(REF(LOW,1))
    VARE=(sma(np.abs(L-VARD),3,1)/sma(np.maximum(L-VARD,0),3,1)*100)
    VARF=(MA(np.where(CLOSE*1.35<=VARA,VARE*10,VARE/10),3))
    VAR10=(llv(LOW,30))
    VAR11=(hhv(VARF,30))
    VZZ=(MA(np.where(LOW<=VAR10,(VARF+VAR11*2)/2,0),3))
    a=(VZZ>500 ) * ( VZZ>2500)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
