...
#开始
    ...
    A1=(np.where(C>REF(C,1)*1.015,1,0))
    A2=(np.where(C>REF(C,1)*1.02,2,0))
    A3=(np.where(C>REF(C,1)*1.03,3,0))
    A4=(np.where(C>REF(C,1)*1.04,4,0))
    A5=(np.where(C>REF(C,1)*1.05,5,0))
    A6=(np.where(C>REF(C,1)*1.06,6,0))
    A7=(np.where(C>REF(C,1)*1.07,7,0))
    A8=(np.where(C>REF(C,1)*1.08,8,0))
    A9=(np.where(C>REF(C,1)*1.09,9,0))
    A10=(np.where(C>REF(C,1)*1.099,10,0))
    X1=(np.maximum(np.maximum(np.maximum(np.maximum(np.maximum(np.maximum(np.maximum(np.maximum(np.maximum(A1,A2),A3),A4),A5),A6),A7),A8),A9),A10))
    X2=((MA(X1,5)+MA(C,3))*0.66)
    X3=(MA(X2,5))
    a=(((CROSS(X1,2)) ) + ( (CROSS(X1,2)) ) + (CROSS(X1,1,) )) * ( CROSS(X2,X3))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
