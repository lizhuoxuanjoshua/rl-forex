...
#开始
    ...
    X_6=(EMA(CLOSE,300)-3*STD(CLOSE,256))
    X_7=(np.where(X_1>X_5,X_1,X_5)*0.94)
    X_10=(CLOSE<REF(llv(LOW,4),1) ) * ( count(HIGH>X_7,8)>4 ) * ( count(CLOSE>X_7,100)<30 ) * ( count(LOW<X_7,10)>2)
    a=(REF(X_10==1,1)  * ( CLOSE>OPEN ) ) * ( count(CLOSE/REF(CLOSE,1)>1.09,15)<3)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
