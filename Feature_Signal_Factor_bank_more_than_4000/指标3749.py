...
#开始
    ...
    无钝1=(EMA(EMA(EMA(CLOSE,5),5),5))
    无钝2=(EMA(EMA(EMA(CLOSE,7),7),7))
    无钝3=(EMA(EMA(EMA(CLOSE,12),12),12))
    峰谷1线=((无钝1-REF(无钝1,1))/REF(无钝1,1)*200)
    峰谷2线=((无钝2-REF(无钝2,1))/REF(无钝2,1)*250)
    筹划线=((无钝3-REF(无钝3,1))/REF(无钝3,1)*300)
    决断线=(MA(筹划线,9))
    红蓝带=((无钝1-无钝2)*5)
    a=(CROSS(筹划线,决断线) ) * ( 峰谷1线>峰谷2线 ) * ( 筹划线<0 ) * ( 决断线<0)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
