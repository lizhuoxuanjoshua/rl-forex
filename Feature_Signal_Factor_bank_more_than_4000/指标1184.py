...
#开始
    ...
    VAR2=(EMA(VAR1,30))
    VAR3=(2*(VAR1-VAR2)*10)
    VAR4=(np.power(VAR3,3)*0.1+np.power(VAR3,2))
    VAR5=(np.where(VAR3>0.015,VAR4,0)/45)
    AA=(VAR5)
    VAR6=(EMA(CLOSE,2)-EMA(CLOSE,150))
    VAR7=(EMA(VAR6,100))
    VAR8=(2*(VAR6-VAR7))
    VAR9=(np.power(VAR8,3)*0.1+np.power(VAR8,1))
    AAB=(np.where(VAR8>0.1,VAR9,0)*5)
    GZ=(AA+AAB)
    a=(GZ>0 ) * ( REF(GZ,1)==0)
#结束
    买入 = a
