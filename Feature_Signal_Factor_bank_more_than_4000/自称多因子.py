...
#开始
    ...
    XZ_9=EMA((EMA(XZ_8,3)/EMA(VOL,3)+EMA(XZ_8,6)/EMA(VOL,6)+EMA(XZ_8,12)/EMA(VOL,12) + EMA (XZ_8,24)/ EMA( VOL,24 ) )/4 , 13 ) 
    ...
    XZ_16=(CLOSE/REF(CLOSE,1)>1.03) * (CLOSE==HIGH)


    a=CROSS(CLOSE,XZ_11) * XZ_16
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
