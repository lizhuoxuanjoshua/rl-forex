...
#开始
    ...
    TDU3=(barslast(DU3))
    TUD3=(barslast(UD3))
    UDGLINE=(np.where(TDU3<TUD3,REF(hhv(H,2),TDU3),REF(llv(L,2),TUD3)))
    JDU0=(REF(C,1)<=REF(UDGLINE,1) ) * ( C>UDGLINE ) * ( TDU0<TUD0)
    JUD0=(REF(C,1)>=REF(UDGLINE,1) ) * ( C<UDGLINE ) * ( TUD0<TDU0)
    JDU1=(JDU0 ) * ( count(JDU0,TDU0)==1)
    JUD1=(JUD0 ) * ( count(JUD0,TUD0)==1)
    a=(JDU1 ) * ( REF(barslast(JUD1),1)<REF(barslast(JDU1),1))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
