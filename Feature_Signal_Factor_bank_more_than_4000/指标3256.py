...
#开始
    ...
    VAR2=(np.where(((EMA((CLOSE + LOW)/(2),3) - VAR1)*(5) > 0),(np.power((EMA((CLOSE + LOW)/(2),3) - VAR1)*(5),2))/(10),(((-1))*(np.power((EMA((CLOSE + LOW)/(2),3) - VAR1)*(5),2)))/(10)))
    VAR3=(np.where((VAR2 > 0),VAR2 - 0,(-1)))
    a=(CROSS(VAR3,0) ) * ( ((CLOSE + LOW)/(2) > MA((CLOSE + LOW)/(2),8)))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
