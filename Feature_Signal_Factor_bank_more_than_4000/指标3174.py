...
#开始
    ...
    BD30=(C>BD2 ) * ( BD9>0 ) * ( BD16>0 ) * ( BD23>0)
    BD31=(C>MA(C,5) ) * ( BD24 ) * ( BD25  ) * ( BD26 ) * ( BD27 ) * ( BD28 ) * ( BD29)
    BD32=((count(BD31,5)==2) ) + ( (count(BD31,5)==3) ) + ( (count(BD31,5)==4) ) + ( (count(BD31,9)==2
))
    a=(BD31 ) * ( BD32 ) * ( REF(BD31==0,1) ) * ( BD30)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
