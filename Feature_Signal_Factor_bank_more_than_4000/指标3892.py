...
#开始
    ...
    X_1=(REF(CLOSE,1))
    X_2=(sma(np.maximum(CLOSE-X_1,0),2,1)/sma(np.abs(CLOSE-X_1),2,1)*100-50)
    X_3=(X_2<(-45))
    X_4=(CROSS((-40),X_2))
    X_5=((X_3) ) + ( (X_4
))
    ...
    X_7=(10)
    X_8=(6)
    X_9=(100*(hhv(HIGH,X_7)-CLOSE)/(hhv(HIGH,X_7)-llv(LOW,X_7)))
    X_10=(100*(hhv(HIGH,6)-CLOSE)/(hhv(HIGH,6)-llv(LOW,6)))
    X_11=((X_9>90) ) + ( (X_10>90
))
    X_12=(5)
    X_13=(6)
    ...
    X_15=(8)
    X_16=(1)
    X_17=((HIGH+np.maximum(CLOSE,OPEN))/2)
    X_18=(MA(X_17,3))
    X_19=((LOW+np.minimum(CLOSE,OPEN))/2)
    X_20=(MA(X_19,3))
    X_21=(MA(X_17,3))
    X_22=((HIGH+LOW+CLOSE+OPEN)/4)
    X_23=(MA(X_22,3))
    X_24=(REF((CLOSE+OPEN)/2,1)>REF((CLOSE+OPEN)/2,2) ) * ( REF(np.minimum(CLOSE,OPEN),1)>REF(np.maximum(CLOSE,OPEN),2))
    X_25=((np.maximum(X_22,X_23)-np.minimum(X_22,X_23))/np.minimum(X_22,X_23)*100)
    X_26=((X_23-(REF(X_23,1)-X_23)-LOW)/LOW*100)
    X_27=((X_18+(X_18-REF(X_18,1))-LOW)/LOW*100)
    X_28=(X_23-(REF(X_23,1)-X_23))
    X_29=(X_18+(X_18-REF(X_18,1)))
    X_30=(np.where(X_24,X_27,X_26))
    X_31=(REF(EVERY(CLOSE>=OPEN,4),1) ) * ( REF(EVERY((CLOSE+OPEN)/2>X_23,4),1) ) * ( (CLOSE+OPEN)/2<X_23)
    X_32=(NOT(X_31) ) * ( (X_23-OPEN)/OPEN*1000>5 ) * ( OPEN<REF(np.minimum(CLOSE,OPEN),1))
    X_33=(sma(LOW,4,3))
    X_34=(sma(HIGH,4,3))
    X_35=(sma(np.minimum(CLOSE,OPEN),4,3))
    X_36=(REF(LOW,1)-hhv(np.maximum(REF(LOW,2),REF(LOW,1))-np.minimum(REF(LOW,2),REF(LOW,1)),2))
    X_37=(REF((CLOSE+OPEN)/2,1)>REF((CLOSE+OPEN)/2,2))
    X_38=(LOW<=X_36+X_36*0.01 ) * ( OPEN>REF(OPEN,2) ) * ( OPEN>=REF(LOW,3) ) * ( count(CLOSE>OPEN,7)>3)
    X_39=((X_36-LOW)/LOW*100)
    X_40=((REF(LOW,1)-X_36)/X_36*100)
    X_41=((X_33-LOW)/LOW>X_12/1000)
    X_42=(X_41)
    X_43=((REF(HIGH,1)-OPEN)/OPEN*100-(REF(np.maximum(CLOSE,OPEN),2)-REF(np.maximum(CLOSE,OPEN),1))/REF(np.maximum(CLOSE,OPEN),1)*100)
    X_44=(np.where(X_37,X_39,X_43))
    X_45=(X_39+X_44>2)
    X_46=(np.where(X_37,X_38,X_45) ) * ( X_30>2)
    X_47=(X_16 ) * ( X_42 ) * ( X_46)
    a=(REF(X_47,1) ) * ( LOW<REF(LOW/0.995,1) ) * ( X_6 ) * ( X_11 )
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
