...
#开始
    ...
    J2=((count(CLOSE*VOL,16)/count(VOL,16)
))
    J3=(count(CLOSE*VOL,128)/count(VOL,128))
    AB1T=(EMA(J2,3))
    AB2T=(EMA(J2,26))
    AB3T=(AB1T-AB2T)
    准势T=(EMA(EMA(EMA(J2,3),3),3)+AB3T)
    MAC1T=(EMA(准势T,50))
    底结构T=(EMA(MAC1T,20))
    上线T=(底结构T*1.01)
    下线T=(底结构T*0.99)
    ...
    AB2=(EMA(J3,26))
    AB3=(AB1-AB2)
    准势=(EMA(EMA(EMA(J3,3),3),3)+AB3)
    MAC1=(EMA(准势,50))
    底结构=(EMA(MAC1,20))
    上线X=(底结构*1.01)
    下线X=(底结构*0.99)
    J3X=(count(CLOSE*VOL,128/5)/count(VOL,128/5))
    AB1YX=(EMA(J3X,3*10/5))
    AB2YX=(EMA(J3X,26*10/5))
    AB3YX=(AB1YX-AB2YX)
    准势YX=(EMA(EMA(EMA(J3X,3*10/5),3*10/5),3*10/5)+AB3YX)
    MAC1YX=(EMA(准势YX,50*10/5))
    底结构YX=(EMA(MAC1YX,20*10/5))
    上线YX=(底结构YX*1.01)
    下线YX=(底结构YX*0.99)
    AB1Y=(EMA(J3,3*3))
    AB2Y=(EMA(J3,26*3))
    AB3Y=(AB1Y-AB2Y)
    准势Y=(EMA(EMA(EMA(J3,3*3),3*3),3*3)+AB3Y)
    MAC1Y=(EMA(准势Y,50*3))
    底结构Y=(EMA(MAC1Y,20*3))
    上线Y=(底结构Y*1.01)
    下线Y=(底结构Y*0.99)
    最高线=(np.maximum(上线T,np.maximum(上线X,np.maximum(上线YX,上线Y))))
    最低线=(np.minimum(下线T,np.minimum(下线X,np.minimum(下线YX,下线Y))))

    涨停=((C-REF(C,barslast(开盘)+1))/REF(C,barslast(开盘)+1)>0.097)

    水下=( C<最低线 ) * ( O<最高线)
    收盘水上=(收盘 ) * ( C*1.01>最高线 ) * ( O*1.01>最高线 ) * ( 涨停)
    轨道发散=(下线YX>上线T ) * ( 下线X>上线YX ) * ( 下线Y>上线X)
    a=(收盘水上 ) * ( count(水下,48*2) ) * ( count(轨道发散,48*2)>0 )
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
