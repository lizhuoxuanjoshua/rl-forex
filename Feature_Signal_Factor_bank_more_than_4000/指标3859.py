...
#开始
    ...
    X_14=(MA(CLOSE,125))
    X_15=(MA(CLOSE,250))
    X_16=((CLOSE-llv(LOW,18))/(hhv(HIGH,18)-llv(LOW,18))*100)
    X_17=(sma(X_16,9,1))
    X_18=(sma(X_17,3,1))

    X_22=(X_10>X_11 ) * ( X_11>X_12 ) * ( X_12>X_13 ) * ( X_13>X_15 ) * ( X_13>X_14)

    a=((CLOSE<X_10 ) * ( CLOSE>OPEN) ) * ( CLOSE>X_11 ) * ( EVERY(X_22,3) ) * ( CLOSE>OPEN )
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
