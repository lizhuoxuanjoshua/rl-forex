...
#开始
    ...
    MA5=(MA(CLOSE,5))
    MA10=(MA(CLOSE,10))
    MA5JD=(np.arctan((MA5/REF(MA5,1)-1)*100)*180/3.1416)
    VAR01=((REF(MA5,1)>MA5 ) * ( REF(MA5,1)>REF(MA5,2))==0)
    VAR02=(REF(LOW,2)==REF(CLOSE,2) ) * ( REF(LOW,1)==REF(CLOSE,1) ) * ( REF(LOW,1)/LOW>=1.06)
    VAR03=(np.where(VAR02,1,0))
    VAR04=(CROSS(MA10,MA5))
    VAR05=(CROSS(MA5,MA10))
    VAR06=(VAR05 ) * ( REF(VAR04,1))
    VAR07=((HIGH/REF(CLOSE,1)>=1.096 ) * ( CLOSE!=HIGH))
    VAR08=(MA5JD>(-35))
    VAR09=((REF(CLOSE,2)/REF(CLOSE,1)>1.07 ) * ( REF(HIGH,2)/REF(CLOSE,3)>=1.095))==0
    VAR10=(CLOSE>REF(np.maximum(CLOSE,OPEN),1))
    VAR12=(CLOSE/REF(CLOSE,1)>1.06)
    VAR13=((CLOSE-REF(CLOSE,1))/REF(CLOSE,1)<(-0.06))
    VAR15=(REF(CLOSE,1)<(REF(CLOSE,barslast(VAR13))+REF(CLOSE,barslast(VAR13)+1))/2 ) * ( CLOSE>(REF(CLOSE,barslast(VAR13))+REF(CLOSE,barslast(VAR13)+1))/2)
    a=(VAR01 ) * ( VAR03==0 ) * ( VAR07==2 ) * ( VAR08 ) * ( VAR09 ) * ( VAR10 ) * ( barslast(VAR13)<=7 ) * ( VAR12 ) * ( VAR15)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
