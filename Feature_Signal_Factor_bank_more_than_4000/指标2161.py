...
#开始
    ...
    H1=-100*(hhv(C,3)-C)/(hhv(C,3)-llv(L,3))+100
    H2=(-100*(hhv(C,10)-C)/(hhv(C,10)-llv(L,10))+100)
    H3=(((count(C*V*100,21)/count(V*100,22))*100)/100)
    H4=(EMA(C,5)-EMA(C,10))
    H5=(EMA(H4,9))
    H6=(H5-REF(H5,1))
    H7=(H4-REF(H4,1))
    a=(O<=MA(C,5) ) * ( O<=MA(C,10) ) * ( O<=MA(C,30) ) * ( C>=MA(C,5) ) * ( C>=H3 ) * ( H6>0 ) * ( H7>0)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
