...
#开始
    ...
    AA=((CLOSE-MA(CLOSE,50))/MA(CLOSE,50)*100<-23)
    AB=(sma(np.maximum(CLOSE-REF(CLOSE,1),0),3,1)/sma(np.abs(CLOSE-REF(CLOSE,1)),3,1)*100>16)
    AC=(100*(hhv(HIGH,27)-CLOSE)/(hhv(HIGH,27)-llv(LOW,27)))
    a=(AA ) * ( AB ) * ( CROSS(AC,79) ) * ( ((OPEN>CLOSE) ) + ( (OPEN<CLOSE)) ) * ( (OPEN-REF(CLOSE,1))/REF(CLOSE,1)<0.05 ) * ( VOL>MA(VOL,23))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
