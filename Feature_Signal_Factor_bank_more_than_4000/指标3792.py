...
#开始
    ...
    X_7=(MA(VOL,X_3))
    X_8=(MA(VOL,X_4))
    X_9=(np.minimum(REF(VOL,1),REF(VOL,2)))
    X_10=(np.minimum(REF(VOL,3),REF(VOL,4)))
    X_11=(np.minimum(REF(VOL,5),REF(VOL,6)))
    X_12=(np.minimum(REF(VOL,7),REF(VOL,8)))
    X_13=(np.minimum(REF(VOL,9),REF(VOL,10)))
    X_14=(np.minimum(REF(VOL,11),REF(VOL,12)))
    X_15=(np.minimum(REF(VOL,13),REF(VOL,14)))
    X_16=(np.minimum(REF(VOL,15),REF(VOL,16)))
    X_17=(np.minimum(REF(VOL,17),REF(VOL,18)))
    X_18=(np.minimum(REF(VOL,19),REF(VOL,20)))
    X_19=(np.minimum(np.minimum(X_9,X_10),np.minimum(X_11,X_12)))
    X_20=(np.minimum(np.minimum(X_13,X_14),np.minimum(X_15,X_16)))
    X_21=(np.minimum(X_17,X_18))
    X_22=(np.minimum(X_19,X_20))
    X_23=(np.minimum(X_22,X_17))
    X_24=(REF(VOL>X_23*1.8,2))
    X_25=(CLOSE/REF(CLOSE,1)>=1.0995)
    X_26=(barslast(X_25))
    X_27=(X_26<=15 ) * ( X_26>=0)
    X_28=(HIGH>EMA(CLOSE,6))


    X_31=(MA(CLOSE,5)>REF(MA(CLOSE,5),1))


    X_34=(REF(CLOSE<OPEN,1) ) * ( REF(HIGH,1)<REF(MA(HIGH,5),1)*1.05)
    X_35=(CLOSE/REF(CLOSE,1)>=1.03)
    X_36=(REF(VOL,2)>=REF(VOL,3))
    X_37=(OPEN>REF(LOW,1))
    X_38=(REF(EMA(CLOSE,17)/CLOSE,1)>0.993)
    X_39=(REF(HIGH/OPEN,2)<1.05)
    a=(X_24 ) * ( X_27 )   * ( X_34 ) * ( X_35 ) * ( X_37 ) * ( X_38)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
