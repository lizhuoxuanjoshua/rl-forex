...
#开始
    ...
    X_1=(HIGH>EMA(CLOSE,6))
    X_2=(count(X_1,15)>=1)
    X_3=(CLOSE<MA(HIGH,5))
    X_4=(LAST(X_3,3,1))
    X_5=(EMA(CLOSE,110)/CLOSE>1.01)
    X_6=(CLOSE>=MA(CLOSE,4)*0.997 ) * ( CLOSE<=MA(CLOSE,4)*1.04)
    X_7=(HIGH<REF(CLOSE,4)*1.03)
    X_8=(CLOSE>REF(CLOSE,1))
    X_9=(REF(LOW,3)<=REF(LOW,5)*0.995)
    X_10=(VOL<=MA(VOL,20)*1.05)
    X_11=(VOL<REF(VOL,4)*1.6)
    X_12=(MA(HIGH,20)/CLOSE>1.01)
    X_13=(HIGH<REF(HIGH,4)*0.97 ) * ( LOW<REF(LOW,4)*0.97)
    X_14=(llv(LOW,11))
    X_15=(hhv(HIGH,11))
    X_16=(EMA((CLOSE-X_14)/(X_15-X_14)*100,2))
    X_17=(EMA(X_16,2))
    X_18=(EMA(X_17,2))
    X_19=(X_17>X_18*1.04)
    X_20=(CLOSE/REF(CLOSE,1)>1.015 ) * ( CLOSE/REF(CLOSE,1)<1.065)
    X_21=(HIGH/REF(LOW,1)<1.065)
    X_22=(np.arctan((MA(CLOSE,15)/REF(MA(CLOSE,15),1)-1)*100)*180/3.141159<10)
    X_23=(HIGH/CLOSE<1.01 ) * ( OPEN/LOW<1.02)
    X_24=(REF(HIGH/CLOSE,1)>1.013 ) * ( REF(OPEN/LOW,1)<1.05)
    X_25=(MA(CLOSE,6)/REF(CLOSE,1)>1.004)
    X_26=(HIGH<EMA(CLOSE,5))
    X_27=(count(X_26,8)<6)
    X_28=(OPEN/CLOSE>1.045)
    X_29=(count(X_28,13)<2)
    X_30=(CLOSE>REF(OPEN,1))
    a=(X_2 ) * ( X_4 ) * ( X_5 ) * ( X_6 ) * ( X_7 ) * ( X_8 ) * ( X_9 ) * ( X_10 ) * ( X_11 ) * ( X_12 ) * ( X_13 ) * ( X_19 ) * ( X_20 ) * ( X_22 ) * ( X_23 ) * ( X_24 ) * ( X_27 ) * ( X_27 ) * ( X_29)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
