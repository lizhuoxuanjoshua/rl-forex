...
#开始
    ...
    X_1=(1)
    X_2=((HIGH+np.maximum(CLOSE,OPEN))/2)
    X_3=(MA(X_2,3))
    X_4=((LOW+np.minimum(CLOSE,OPEN))/2)
    X_5=(MA(X_4,3))
    X_6=((HIGH+LOW+CLOSE+OPEN)/4)
    X_7=(MA(X_6,3))
    X_8=(REF((CLOSE+OPEN)/2,1)>REF((CLOSE+OPEN)/2,2) ) * ( REF(np.minimum(CLOSE,OPEN),1)>REF(np.maximum(CLOSE,OPEN),2))
    X_9=((X_7-(REF(X_7,1)-X_7)-LOW)/LOW*100)
    X_10=((X_3+(X_3-REF(X_3,1))-LOW)/LOW*100)
    X_11=(X_7-(REF(X_7,1)-X_7))
    X_12=(X_3+(X_3-REF(X_3,1)))
    X_13=(np.where(X_8,X_10,X_9))
    X_14=(REF(EVERY(CLOSE>=OPEN,4),1) ) * ( REF(EVERY((CLOSE+OPEN)/2>X_7,4),1) ) * ( (CLOSE+OPEN)/2<X_7)
    X_15=(NOT(X_14) ) * ( (X_7-OPEN)/OPEN*1000>5 ) * ( OPEN<REF(np.minimum(CLOSE,OPEN),1))
    X_16=(sma(LOW,4,3))
    X_17=(sma(HIGH,4,3))
    X_18=(sma(np.minimum(CLOSE,OPEN),4,3))
    X_19=(REF(LOW,1)-hhv(np.maximum(REF(LOW,2),REF(LOW,1))-np.minimum(REF(LOW,2),REF(LOW,1)),2))
    X_20=(REF((CLOSE+OPEN)/2,1)>REF((CLOSE+OPEN)/2,2))
    X_21=(LOW<=X_19+X_19*0.01 ) * ( OPEN>REF(OPEN,2) ) * ( OPEN>=REF(LOW,3) ) * ( count(CLOSE>OPEN,7)>3)
    X_22=((X_19-LOW)/LOW*100)
    X_23=((REF(LOW,1)-X_19)/X_19*100)
    X_24=((X_16-LOW)/LOW>5/1000)
    X_25=(X_24)
    X_26=((REF(HIGH,1)-OPEN)/OPEN*100-(REF(np.maximum(CLOSE,OPEN),2)-REF(np.maximum(CLOSE,OPEN),1))/REF(np.maximum(CLOSE,OPEN),1)*100)
    X_27=(np.where(X_20,X_22,X_26))
    X_28=(X_22+X_27>2)
    X_29=(np.where(X_20,X_21,X_28) ) * ( X_13>2)
    X_30=(X_1 ) * ( X_25 ) * ( X_29)
    X_31=(REF(X_30,1) ) * ( LOW<REF(LOW/0.995,1))
    X_32=(100*(hhv(HIGH,10)-CLOSE)/(hhv(HIGH,10)-llv(LOW,10)))
    X_33=(100*(hhv(HIGH,6)-CLOSE)/(hhv(HIGH,6)-llv(LOW,6)))
    X_34=((X_32>95) ) + ( (X_33>95
))
    X_35=(MA(CLOSE,12))
    X_36=(MA(CLOSE,26))
    X_37=(CLOSE<OPEN)
    X_38=(X_35>X_36 ) * ( X_37)
    a=(REF(X_34,1) ) * ( (REF(CLOSE,1)-CLOSE)/REF(CLOSE,1)*X_31 ) * ( X_38)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
