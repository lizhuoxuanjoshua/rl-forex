...
#开始
    ...
    X_1=(REF(CLOSE,1)<REF(CLOSE,2))
    X_2=(OPEN<CLOSE)
    X_3=(CLOSE/REF(CLOSE,1)>1.005)
    X_4=(HIGH-CLOSE>(OPEN-LOW)*1.5)
    X_5=(HIGH/OPEN>1.01 ) * ( HIGH/OPEN<1.09)
    X_6=(HIGH<REF(HIGH,2)*0.995)
    X_7=(LOW<REF(LOW,2)*0.995)
    X_8=(REF(CLOSE,2)/REF(CLOSE,1)>1.034)
    X_9=(VOL>=hhv(VOL,30)*0.2)
    X_10=(CLOSE/REF(CLOSE,1)>1.024)
    X_11=(CLOSE/REF(CLOSE,1)<1.0995)
    X_12=(HIGH/CLOSE<1.06 ) * ( HIGH/REF(HIGH,1)<1.015)
    X_13=(5)
    X_14=(10)
    X_15=(20)
    X_16=(60)
    ...
    X_19=(MA(VOL,X_15))
    X_20=(MA(VOL,X_16))
    X_21=(np.minimum(REF(VOL,1),REF(VOL,2)))
    X_22=(np.minimum(REF(VOL,3),REF(VOL,4)))
    X_23=(np.minimum(REF(VOL,5),REF(VOL,6)))
    X_24=(np.minimum(REF(VOL,7),REF(VOL,8)))
    X_25=(np.minimum(REF(VOL,9),REF(VOL,10)))
    X_26=(np.minimum(REF(VOL,11),REF(VOL,12)))
    X_27=(np.minimum(REF(VOL,13),REF(VOL,14)))
    X_28=(np.minimum(REF(VOL,15),REF(VOL,15)))
    X_29=(np.minimum(np.minimum(X_21,X_22),np.minimum(X_23,X_24)))
    X_30=(np.minimum(np.minimum(X_25,X_26),np.minimum(X_27,X_28)))
    X_31=(np.minimum(X_29,X_30))
    X_32=(VOL<=X_31*4)
    X_33=(VOL<MA(VOL,60))
    X_34=(REF(np.maximum(OPEN,CLOSE)/np.minimum(OPEN,CLOSE),3)<1.04)
    a=(X_1 ) * ( X_2 ) * ( X_4 ) * ( X_5 ) * ( X_6 ) * ( X_8 ) * ( X_9 ) * ( X_10 ) * ( X_7 ) * ( X_12 ) * ( X_32 ) * ( X_34)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
