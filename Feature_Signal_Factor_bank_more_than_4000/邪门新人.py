...
def 策略(合成周期,ktime,high,o,low,c,volume,amount,均线,开盘,收盘,切片位置,subindex,turn):
    ...
    X_1 = np.where(CLOSE > REF(CLOSE, 1), 1, 0)
    X_2 = np.where((CLOSE / REF(CLOSE, 1) > 1.05) * (HIGH / CLOSE < 1.01) * (X_1 == 1), 1, 0)
    X_3 = 1
    X_4 = (HIGH + LOW + CLOSE * 2) / 4
    X_5 = X_4 * 3 + X_1 * 2 - (X_2 + X_1) * 2 + X_2 - X_4
    X_6 = X_5 + X_2 - X_4
    X_7 = 4 * sma((CLOSE - llv(LOW, X_5)) / (hhv(HIGH, X_5) - llv(LOW, X_5)) * 100, 5, X_6) - 3 * sma(
        sma((CLOSE - llv(LOW, X_5)) / (hhv(HIGH, X_5) - llv(LOW, X_5)) * 100, 5, 1), 3.2, 1)
    X_8 = (X_6 - MA(X_6, 14)) / (0.015 * AVEDEV(X_6, 14))
    X_9 = (2 * CLOSE + HIGH + LOW + OPEN) / 5
    X_10 = (X_9 - MA(X_9, 14)) * 100 / (1.5 * AVEDEV(X_9, 14))
    X_11 = sma(np.maximum(CLOSE - REF(CLOSE, 1), 0), 6, 1) / sma(np.abs(CLOSE - REF(CLOSE, 1)), 6, 1) * 100
    X_A = CROSS(X_7, X_11)
    X_B = CROSS(X_11, X_7)
    X_12 = REF(X_8, 1)
    X_13 = REF(X_8, 2)
    X_14 = ((CLOSE - MA(CLOSE, 6)) / MA(CLOSE, 6) * 100 + (CLOSE - MA(CLOSE, 24)) / MA(CLOSE, 24) * 100 + (
                CLOSE - MA(CLOSE, 2)) / MA(CLOSE, 32) * 100) / 3
    X_15 = (X_8 < -100) + (X_12 < -100)
    X_16 = (X_8 > 100) + (X_12 > 100)
    X_17 = 20 * np.maximum(X_8, X_16) * DMA(EMA(CLOSE, 6), count(VOL, 60) / VOL) / np.exp(2 * HIGH - LOW / 3)
    X_C = CROSS(X_14, X_17)
    X_D = CROSS(X_17, X_14)
    关键点 = (X_A) * (X_B) * (X_C) * (X_D)
    X_E = 关键点
    X_18 = ((X_8 > X_12) * (X_12 < X_13)) + (X_E)
    X_19 = ((X_8 < X_12) * (X_12 > X_13)) + (X_E)
    买入 = (X_15) * (X_18)



    #确定买入和测试信号------------------------------------------------------
    测试信号 = np.where(买入,1,0)
