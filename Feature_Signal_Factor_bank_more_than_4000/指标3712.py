...
#开始
    ...
    T=(barslast(C>REF(C,1)*1.095 ) * ( C==H))
    TJ=np.divide(V<=REF(V,T),8)
    TJ1=(np.maximum(C,O)/np.minimum(C,O)<1.005 ) * ( H>np.maximum(C,O) ) * ( L<np.minimum(C,O))
    a=(TJ ) * ( count(TJ,T)==1 ) * ( L==llv(L,10) ) * ( TJ1)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
