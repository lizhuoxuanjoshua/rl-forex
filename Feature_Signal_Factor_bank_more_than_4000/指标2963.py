...
#开始
    ...
    VAR2=(2*np.abs(VAR1))
    VAR3=(100-3*sma((CLOSE-llv(LOW,90))/(hhv(HIGH,90)-llv(LOW,90))*100,20,1)+2*sma(sma((CLOSE-llv(LOW,90))/(hhv(HIGH,90)-llv(LOW,90))*100,20,1),15,1))
    活筹OAMV=(VAR3)
    核能=(np.where(VAR1>0,VAR2,0))
    LC1 =( REF(CLOSE,1))
    RSI1=(sma(np.maximum(CLOSE-LC1,0),14,1)/sma(np.abs(CLOSE-LC1),14,1)*100)
    TYP =( (HIGH + LOW + CLOSE)/3)
    V1=(count(np.where(TYP>REF(TYP,1),TYP*VOL,0),14)/count(np.where(TYP<REF(TYP,1),TYP*VOL,0),14))
    MFI1=(100-(100/(1+V1)))
    黄龙RSI=(RSI1)
    青龙MFI=(MFI1)
    a=((黄龙RSI>青龙MFI) )  * ((黄龙RSI>青龙MFI) ) * ((活筹OAMV<REF(活筹OAMV,1)) * ( 核能>REF(核能,1)))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
