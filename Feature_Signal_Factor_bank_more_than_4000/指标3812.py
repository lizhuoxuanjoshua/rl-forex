...
#开始
    ...
    D=(sma(K,3,1))
    J=(3*K-2*D)
    A=(((REF(C,3)-REF(C,1))/C)*100>6)
    A1=( CLOSE>REF(CLOSE,1)*1.03)
    A2=(CROSS(J,MA(J,2)) ) * ( C>O)
    A3=( REF(((H-O)/C)*100<2,1))
    A4=(REF(C<O,2) ) * ( REF(C<O,1))
    A5=(((H-C)/C)*100<2 ) * ( H>REF(H,1))
    A6=(((REF(O,1)-REF(C,1))/C)*100>1)
    A7=(((REF(O,2)-REF(C,2))/C)*100<9)
    a=(A ) * ( A1 ) * ( A2 ) * ( A3 ) * ( A4 ) * ( A5 ) * ( A7)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
