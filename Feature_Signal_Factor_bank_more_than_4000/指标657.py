...
#开始
    ...
    VAR4=(EMA(np.where(CLOSE*1.3,VAR3*10,VAR3/10),3)*VAR1)
    VAR5=(llv(LOW,30)*VAR1)
    VAR6=(hhv(VAR4,30)*VAR1)
    VAR7=(np.where(MA(CLOSE,58),1,0)*VAR1)
    VAR8=(EMA(np.where(LOW<=VAR5,(VAR4+VAR6*2)/2,0),3)/618*VAR7*VAR1)
    AA=(np.where(VAR8>100,100,VAR8)*VAR1)
    a=(AA>-150 ) * ( REF(AA,1)<=-150)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
