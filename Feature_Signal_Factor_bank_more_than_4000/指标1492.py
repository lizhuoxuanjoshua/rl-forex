...
#开始
    ...
    震仓=(45)
    底线=(20)
    PP=(操作线<震仓 ) * ( REF(操作线,1)>震仓)
    PP3=(操作线<底线 ) * ( REF(操作线,1)>底线*VAB3)
    a=(((((((count((X31 == 1),4) == 3) ) * ( (X31 == 0))) * ( O<C ) * ( (((C-REF(C,1))/REF(C,1))>0.065))*10) >=10))) + ((((C-REF(C,1))/REF(C,1)>0.065 )) * ( (REF(PP3,1) ) + ( (REF(PP,1)))) ) * ( VAB3==1,)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
