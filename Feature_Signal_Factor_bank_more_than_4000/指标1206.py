...
#开始
    ...
    AAB=(np.power(MA(CLOSE,3)*10000,3))
    MA5=(MA(CLOSE,5))
    MA15=(MA(CLOSE,15))
    MA34=(MA(CLOSE,34))
    AA10=(MA(C,10))
    AA=(np.arctan((AA10-REF(AA10,1)))*180/3.1416)
    AA20=(MA(C,20))
    AAA=(np.arctan((AA20-REF(AA20,1)))*180/3.1416)
    a=(AAB*(CLOSE<MA5 ) * ( np.abs(MA5-MA15)/MA15<0.05 ) * ( AA<AAA ) * ( AAA>0 ) * ( AA>0))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
