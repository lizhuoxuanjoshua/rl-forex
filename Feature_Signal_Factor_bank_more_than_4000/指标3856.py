...
#开始
    ...
    X_1=(EMA(np.maximum(np.maximum(HIGH-LOW,np.abs(HIGH-REF(CLOSE,1))),np.abs(REF(CLOSE,1)-LOW)),14))
    X_2=(HIGH-REF(HIGH,1))
    X_3=(REF(LOW,1)-LOW)
    X_4=(EMA((X_2>0 ) * ( X_2>X_3),14))
    X_5=(EMA((X_3>0 ) * ( X_3>X_2),14))
    X_6=(X_4*100/X_1)
    X_7=(X_5*100/X_1)
    X_8=(EMA(np.abs(X_7-X_6)/(X_7+X_6)*100,6))
    X_9=(EMA(X_8,6))
    X_10=(np.minimum(X_7,X_8))
    X_11=(np.maximum(X_7,X_8))
    X_12=(np.minimum(X_8,X_9))
    X_13=(np.maximum(X_8,X_9))
    X_14=(np.minimum(X_10,X_12))
    X_15=(np.maximum(X_11,X_13))
    X_16=(REF(X_6<X_14,1))
    X_17=(X_6>X_15)
    X_18=(VOL>hhv(VOL,25)*0.5)
    X_19=(HIGH/CLOSE<1.005)
    X_20=(CLOSE>REF(CLOSE,25))
    X_21=(HIGH<=hhv(HIGH,35) ) * ( LOW>llv(LOW,35))
    X_22=(X_16 ) * ( X_17 ) * ( X_18 ) * ( X_19 ) * ( X_20 ) * ( X_21)
    X_23=(CLOSE>MA(CLOSE,5) ) * ( CLOSE>MA(CLOSE,10) ) * ( CLOSE>MA(CLOSE,50))
    X_24=(REF(CLOSE,1)<MA(CLOSE,5) ) * ( REF(CLOSE,1)<MA(CLOSE,10) ) * ( REF(CLOSE,1)<MA(CLOSE,50))
    X_25=(np.minimum(np.minimum(MA(CLOSE,5),MA(CLOSE,10)),MA(CLOSE,50)))
    X_26=(np.maximum(np.maximum(MA(CLOSE,5),MA(CLOSE,10)),MA(CLOSE,50)))
    X_27=(X_26/X_25<=1.05)
    X_28=(OPEN/REF(CLOSE,1)<1.02)
    X_29=(X_23 ) * ( X_24 ) * ( X_27 ) * ( X_28)
    a=(count(X_22,3) ) * ( X_29)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
