...
#开始
    ...
    VARC2=(sma(np.abs(L-REF(L,1)),3,1)/sma(np.maximum(L-REF(L,1),0),3,1))
    VARD2=(EMA(np.where(C<EMA(C,250),VARC2*1000,VARC2*10),3))
    VAA=(EMA(np.where(L<=llv(L,30),VARD2/2+hhv(VARD2,30),0),3)/900)
    a=(REF(VAA,1)>20 ) * ( VAA<REF(VAA,1))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
