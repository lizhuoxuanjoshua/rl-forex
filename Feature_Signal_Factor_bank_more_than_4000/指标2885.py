...
#开始
    ...
    VAR3K=(llv(VAR1K,240))
    VAR4K=(MA(CLOSE,33))
    VAR5K=(EMA(CLOSE,13)-EMA(CLOSE,26))
    VAR6K=(EMA(VAR5K,9))
    VAR7K=(2*(VAR5K-VAR6K))
    VAR8K=((CLOSE-llv(LOW,9))/(hhv(HIGH,9)-llv(LOW,9))*100)
    VARBK=(sma(VAR8K,3,1))
    VAR12K=(VARBK-REF(VARBK,1))
    固定筹码=( EMA((VAR1K-VAR3K)/VAR3K*VAR4K,13))
    移动筹码=( EMA((VAR2K-VAR1K)/VAR1K*VAR4K,13))
    多空对比=(固定筹码-移动筹码)
    VAR13K=(VAR7K>=-0.035 ) * ( VAR7K<=0.045 ) * ( CLOSE>MA(CLOSE,5) ) * ( CLOSE>MA(CLOSE,10) ) * ( 多空对比>-100)
    a=((VAR13K ) * ( VAR12K>7 ) * ( REF(VAR12K,1)<0 ) * ( (VAR12K-REF(VAR12K,1)>9)))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
