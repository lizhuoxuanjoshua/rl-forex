...
#开始
    ...
    HJ_1=((hhv(HIGH,8)-CLOSE)/(hhv(HIGH,8)-llv(LOW,8))*100-70)
    HJ_2=(sma(HJ_1,8,1))
    HJ_3=((CLOSE-llv(LOW,8))/(hhv(HIGH,8)-llv(LOW,8))*100)
    HJ_4=(sma(HJ_3,3,1))
    HJ_5=(sma(HJ_4,3,1))
    HJ_6=(HJ_5+100-(HJ_2+100))
    HJ_7=(HJ_6/REF(HJ_6,1))
    HJ_8=(100*(CLOSE-llv(LOW,5))/(hhv(CLOSE,5)-llv(LOW,5)))
    HJ_9=(100*(CLOSE-llv(LOW,30))/(hhv(CLOSE,30)-llv(LOW,30)))
    HJ_10=(HJ_9/REF(HJ_9,1))
    a=(CLOSE<REF(CLOSE,1) ) * ( HJ_8>=REF(HJ_8,1) ) * ( HJ_8<20 ) * ( ((HJ_10>=REF(HJ_10,1)) ) + ( (HJ_7>=REF(HJ_7,1))))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
