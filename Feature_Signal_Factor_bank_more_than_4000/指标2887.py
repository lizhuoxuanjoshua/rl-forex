...
#开始
    ...
    VAR33=((CLOSE-REF(OPEN,29))/REF(OPEN,29)*100)
    VARF=((HIGH+LOW+CLOSE)/3)
    VAR10=((VARF-MA(VARF,14))/(0.015*AVEDEV(VARF,14)))
    VAR11=((CLOSE-llv(LOW,45))/(hhv(HIGH,45)-llv(LOW,45))*100)
    VAR12=(sma(VAR11,3,1))
    VAR13=(sma(VAR12,3,1))
    VAR14=(3*VAR12-2*VAR13)
    VAR15=(CLOSE< REF(CLOSE,1))
    VAR16=(OPEN< REF(CLOSE,1))
    VAR17=((VAR13< 10 ) * ( VAR33< -30 ) * ( VAR22>6 ) * ( VAR10<-107))
    Q=(( VAR17) ) + ( (count(VAR17,10) )) * ( VAR15 ) + ( (count(VAR17,10) )) * ( VAR16)
    AA=(FILTER(Q,20))
    RSV=( (CLOSE-llv(LOW,25))/(hhv(HIGH,25)-llv(LOW,25))*100)
    FASTK=(sma(RSV,3,1))
    K=(sma(FASTK,3,1))
    WR=(-100*(hhv(HIGH,55)-CLOSE)/(hhv(HIGH,55)-llv(LOW,55)))
    a=( CROSS(WR,-45) ) * ( CLOSE>MA5 ) * ( MA3>REF(MA3,1) ) * ( K>REF(K,1))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
