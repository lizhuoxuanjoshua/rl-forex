...
#开始
    ...
    QJJ=(VOL/((HIGH-LOW)*2-np.abs(CLOSE-OPEN)))
    XVL=((np.where(CLOSE>OPEN,QJJ*(HIGH-LOW),np.where(CLOSE<OPEN,QJJ*(HIGH-OPEN+CLOSE-LOW),VOL/2))+np.where(CLOSE>OPEN,0-QJJ*(HIGH-CLOSE+OPEN-LOW),np.where(CLOSE<OPEN,0-QJJ*(HIGH-LOW),0-VOL/2))))
    HSL=((XVL/3)/1.15)
    攻击=(((HSL*0.55+(REF(HSL,1)*0.33))+(REF(HSL,2)*0.22)))
    LLJX=(EMA(攻击,3))
    B1=(np.where(count(LLJX,1)>REF(MA(count(LLJX,1),1),1),count(LLJX,1),0))
    B2=(np.where(count(LLJX,2)>REF(MA(count(LLJX,2),1),1),count(LLJX,2),0))
    B3=(np.where(count(LLJX,3)>REF(MA(count(LLJX,3),1),1),count(LLJX,3),0))
    B4=(np.where(count(LLJX,4)>REF(MA(count(LLJX,4),1),1),count(LLJX,4),0))
    B5=(np.where(count(LLJX,5)>REF(MA(count(LLJX,5),1),1),count(LLJX,5),0))
    B6=(np.where(count(LLJX,6)>REF(MA(count(LLJX,6),1),1),count(LLJX,6),0))
    B7=(np.where(count(LLJX,7)>REF(MA(count(LLJX,7),1),1),count(LLJX,7),0))
    B8=(np.where(count(LLJX,8)>REF(MA(count(LLJX,8),1),1),count(LLJX,8),0))
    B9=(np.where(count(LLJX,9)>REF(MA(count(LLJX,9),1),1),count(LLJX,9),0))
    B10=(np.where(count(LLJX,10)>REF(MA(count(LLJX,10),1),1),count(LLJX,10),0))
    a=(B1 ) * ( B2 ) * ( B3 ) * ( B4 ) * ( B5 ) * ( B6 ) * ( B7 ) * ( B8 ) * ( B9 ) * ( B10 ) * ( (CLOSE-OPEN)>=(UPR-BBIBOLL) ) * ( V/REF(V,1)>=1.9)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
