...
#开始
    ...
    W=(C-MA(C,30))/MA(C,30)*100
    W2=(W<REF(W,1))
    W1=(CROSS(0,(C-MA(C,5))/MA(C,5)*100))
    W3=MA(C,10)>REF(MA(C,10),1)
    W8=C>REF(C,3)
    W4=((W1 ) * ( W2 ) * ( W3))
    W5=(2*CLOSE+HIGH+LOW)/4
    W7=(sma(C,6.5,1))
    W6=(MA(EMA((W5-llv(LOW,5))/(hhv(HIGH,5)-llv(LOW,5))*100,5),1))
    Q=(REF(W4,2) ) * ( CROSS(C,EMA(CLOSE,5)) ) * ( W8)
    Q1=(W6>REF(W6,1) ) * ( REF(W6,1)<REF(W6,2))
    Q2=(W7>REF(W7,1) ) * ( REF(W7,1)<REF(W7,2))
    a=(Q ) * ( Q1 ) * ( Q2)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
