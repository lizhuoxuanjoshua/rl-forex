...
#开始
    ...
    TTJ1=(VOL==hhv(VOL,10) ) * ( VOL>2*REF(VOL,1) ) * ( CLOSE>短趋)
    立桩量=(FILTER(TTJ1,5))
    tt1=(barslast(立桩量)==1 ) * ( CLOSE>REF(HIGH,1))
    tt2=(barslast(立桩量)==2 ) * ( CLOSE>REF(HIGH,2))
    tt3=(barslast(立桩量)==3 ) * ( CLOSE>REF(HIGH,3))
    TTJ2=(np.where(tt1,1,np.where(tt2,1,np.where(tt3,1,0))))
    突破=(TTJ2==1)
    tt4=(barslast(立桩量)==1 ) * ( CLOSE>REF(LOW,1))
    tt5=(barslast(立桩量)==2 ) * ( CLOSE>REF(LOW,2))
    tt6=(barslast(立桩量)==3 ) * ( CLOSE>REF(LOW,3))
    TTJ3=(np.where(tt4,1,np.where(tt5,1,np.where(tt6,1,0))))
    三天法则=(TTJ3==1)
    成立=(三天法则 ) * ( 突破)
    a=(FILTER(成立,3))
#结束
