...
#开始
    ...
    ZAR18=(4*sma((CLOSE-llv(LOW,11))/(hhv(HIGH,11)-llv(LOW,11))*100,5,1)-3*sma(sma((CLOSE-llv(LOW,11))/(hhv(HIGH,11)-llv(LOW,11))*100,5,1),3.2,1))
    SSRCJL=(EMA(VOL,13))
    SSRCJE=(EMA(AMOUNT,13))
    SSRCBJX=(((SSRCJE / SSRCJL)))
    SSRGL=((((CLOSE - SSRCBJX) / SSRCBJX) * 100))
    P2=(SSRGL<(0-6) ) * ( SSRGL<(0-10))
    P3=(ZAR18<=8 ) * ( L<= llv(L,30))
    PX=(P2+P3)
    a= ( FILTER(PX>=1,3))
#结束
    买入 = a
