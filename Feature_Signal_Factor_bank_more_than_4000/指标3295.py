...
#开始
    ...
    波浪线=(sma((VAR11-50)*2,3,1)+sma((VAR10-50)*2,3,1)/2)
    VAR12=((VAR11-50)*6-2*sma((VAR11-50)*2,3,1))
    VAR13=(波浪线>REF(波浪线,1)) * ( 波浪线<-20)
    VAR14=(波浪线<REF(波浪线,3)) * ( 波浪线>REF(波浪线,1)) * ( 波浪线<-10)
    VAR15=(FILTER(波浪线<REF(波浪线,1),10))
    VAR16=(波浪线<REF(波浪线,1)) * ( 波浪线>110)
    VAR17=(VAR11<REF(VAR11,3) ) * ( VAR11>REF(VAR11,2)) * ( VAR11<30)
    a=(((VAR13) ) + ( (VAR14) ) + ( (VAR17)) ) * ( VARA>REF(VARA,1))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
