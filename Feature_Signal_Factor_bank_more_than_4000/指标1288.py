...
#开始
    ...
    VAR5G=(VOL>1.05*MA(VOL,5) ) * ( VOL<5*REF(VOL,1))
    VAR6G=((CLOSE-OPEN)/OPEN)
    VAR7G=((HIGH-CLOSE)/(CLOSE-OPEN))
    VAR8G=((OPEN-LOW)/(CLOSE-OPEN))
    VAR9G=(VAR7G<0.8 ) * ( VAR8G<0.8)
    VARA2=(VAR6G>0.04)
    VARB2=((CROSS(MA(CLOSE,5),MA(CLOSE,10))) ) + ( (MA(CLOSE,5)-MA(CLOSE,10)<0 )) * ( np.abs(MA(CLOSE,5)-MA(CLOSE,10))/MA(CLOSE,10)<0.02 ) * ( MA(CLOSE,5)-MA(CLOSE,10)>REF(MA(CLOSE,5)-MA(CLOSE,10),1))
    VARC2=(VAR4G ) * ( VAR5G ) * ( VAR9G ) * ( VARA2 ) * ( VARB2)
