...
#开始
    ...
    U1=(REF(C,1)/C>1.07)
    U2=(C/L>1.05 ) * ( O/L>1.05 ) * ( C<REF(C,1))
    U3=(REF(U1,1) ) * ( U2)
    U4=(REF(C,4)/REF(L,1)>1.05)
    AA=(REF(H,barslast(U1)))
    BB=(REF(L,barslast(U2)))
    CL=(CROSS(C,AA) ) * ( REF(count(BB,59),1)  ) * ( count(U3,60) ) * ( C/REF(C,1)>1.06)
    a=(FILTER(CL,18))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
