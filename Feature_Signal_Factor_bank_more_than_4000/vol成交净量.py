...
#开始
    ...
    量=(VOL/((HIGH-LOW)*2-np.abs(CLOSE-OPEN)))
    买量=(np.where(CLOSE>OPEN,量*(HIGH-LOW),np.where(CLOSE<OPEN,量*((HIGH-OPEN)+(CLOSE-LOW)),VOL/2)))
    卖量=((np.where(CLOSE>OPEN,0-量*((HIGH-CLOSE)+(OPEN-LOW)),np.where(CLOSE<OPEN,0-量*(HIGH-LOW),0-VOL/2)))*-1)
    总量=(买量+卖量)
    净量=(买量-卖量)
    五日净量=(count(净量,5))
    十日净量=(count(净量,10))
