...
#开始
    ...
    指标王G=(EMA(((2*CLOSE+HIGH+LOW)/4-llv(LOW,5))/(hhv(HIGH,4)-llv(LOW,5))*100,4))
    指标王H=(EMA(0.667*REF(指标王G,1)+0.333*指标王G,2))
    指标王D=(np.arctan((EMA(C,5)/REF(EMA(C,5),1)-1)*100)*180/3.1416)
    指标王E=(np.where(指标王D>0,指标王D,指标王D))
    a=(指标王E>REF(指标王E,1) ) * ( 指标王E<=-40)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
