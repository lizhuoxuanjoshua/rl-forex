...
#开始
    ...
    M55=(MA(C,55))
    RSV1=((CLOSE-llv(LOW,23))/(hhv(HIGH,23)-llv(LOW,23))*100)
    RSV=((CLOSE-llv(LOW,196))/(hhv(HIGH,196)-llv(LOW,196))*100)
    快线=(sma(RSV,30,1))
    参照快线=(sma(RSV1,6,1))
    参照慢线=(sma(参照快线,3,1))
    a=(CROSS(参照快线,参照慢线) ) * ( 快线>=REF(快线,1) ) * ( M55>=REF(M55,1) ) * ( 参照快线<76 ) * ( np.abs(MA(C,5)-MA(C,10))<0.3)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
