...
#开始
    ...
    X_1=(1)
    X_2=((2*CLOSE+HIGH+LOW)/4)
    X_3=(llv(LOW,5))
    X_4=(hhv(HIGH,4))
    X_5=(EMA((X_2-X_3)/(X_4-X_3)*100,4)*X_1)
    X_6=(EMA(0.667*REF(X_5,1)+0.333*X_5,2)*X_1)
    X_7=(CROSS(X_5,X_6))
    X_8=(EMA(CLOSE,1))
    X_9=(EMA(CLOSE,5))
    X_10=(CROSS(X_8,X_9))
    X_11=(LOW<MA(CLOSE,13))
    X_12=(MA(CLOSE,13)>MA(CLOSE,25) ) * ( MA(CLOSE,25)>MA(CLOSE,55) ) * ( MA(CLOSE,25)>REF(MA(CLOSE,25),1))
    X_13=(count(LOW<MA(CLOSE,13),3)==2)
    a= ( CROSS(CLOSE,MA(CLOSE,13)) ) * ( X_10 ) * ( X_7 ) * ( CLOSE/REF(CLOSE,1)>1.05)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
