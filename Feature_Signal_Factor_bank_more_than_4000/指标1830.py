...
#开始
    ...
    VAR4=((VAR2>=150 ) * ( VAR2<200 ) * ( VAR3>=150 ) * ( VAR3<200)*(-10))
    VAR5=((VAR2<=-150 ) * ( VAR2>-200 ) * ( VAR3<=-150 ) * ( VAR3>-200)*10)
    VAR6=(sma((CLOSE-llv(LOW,10))/(hhv(HIGH,10)-llv(LOW,10))*100,4,1))
    VAR60=(sma((VAR6-50)*2,3,1))
    VAR7=(sma((CLOSE-llv(LOW,11))/(hhv(HIGH,11)-llv(LOW,11))*100,4,1))
    VAR70=(sma((VAR7-50)*2,3,1))
    VAR71=((sma((VAR7-50)*2,3,1)+sma((VAR6-50)*2,3,1))/2)
    VAR8=(sma((CLOSE-llv(LOW,12))/(hhv(HIGH,12)-llv(LOW,12))*100,4,1))
    VAR80=(sma((VAR8-50)*2,3,1))
    VAR81=((sma((VAR8-50)*2,3,1)+sma((VAR7-50)*2,3,1))/2)
    VAR9=(sma((CLOSE-llv(LOW,13))/(hhv(HIGH,13)-llv(LOW,13))*100,4,1))
    VARA=(sma((VAR9-50)*2,3,1))
    VARB=((sma((VAR9-50)*2,3,1)+sma((VAR8-50)*2,3,1))/2)
    VAR10=(sma((CLOSE-llv(LOW,14))/(hhv(HIGH,14)-llv(LOW,14))*100,4,1))
    VAR100=(sma((VAR10-50)*2,3,1))
    VAR11=(sma((CLOSE-llv(LOW,15))/(hhv(HIGH,15)-llv(LOW,15))*100,4,1))
    AB=(sma((VAR11-50)*2,3,1)+sma((VAR10-50)*2,3,1)/2)
    a=(AB>REF(AB,1))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
