...
#开始
    ...
    A15=(np.where((A14>100),(A14-100),0))
    A16=((EMA((((A3-A4)/(A5-A4))*100),3))*A1)
    A17=((EMA(A16,5))*A1)
    A18=((EMA(CLOSE,2))*A1)
    A19=((EMA((((SLOPE(CLOSE,21))*20)+CLOSE),36))*A1)
    A20=((hhv((MA(HIGH,13)),13))*A1)
    A21=((llv((MA(HIGH,13)),1))*A1)
    A22=((EMA(CLOSE,4))*A1)
    A23=(((((A18>=A19) ) * ( (A21==A20)) ) * ( (A18>A22)) ) * ( (CLOSE>A21)) ) * ( ((REF(A21,1))<(REF(A20,1))))
    A24=(barslast(A23)+1)
    A25=((llv(LOW,A24))*1.3)
    A26=((llv(LOW,20))*1.418)
    A27=((((((A18>=A19) ) * ( (A21==A20)) ) * ( ((CLOSE/(REF(CLOSE,1)))>=1.07)) ) * ( (A24<=30)) ) * ( (CLOSE<=A25)) ) * ( (CLOSE<=A26))
    A28=(A27 ) * ( (FILTER(A27,A24)))
    A30=(((((((A18>=A19) ) * ( (A21==A20)) ) * ( ((CLOSE/(REF(CLOSE,1)))>=1.535)) ) * ( (A24<=30)) ) * ( (CLOSE<=A25)) ) * ( (CLOSE<=A26)))
    A31=(A30 ) * ( (FILTER(A30,30)))
    a=((A28) ) + ( (A31
))
#结束
    买入 = a
