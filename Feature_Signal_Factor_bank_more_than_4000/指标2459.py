...
#开始
    ...
    QJ1=(np.where(0,QJ0*(JJ-np.minimum(CLOSE,OPEN)),QJ0*np.where(HIGH==LOW,1,(np.minimum(OPEN,CLOSE)-LOW))))
    QJ2=(np.where(0,QJ0*(np.minimum(OPEN,CLOSE)-LOW),QJ0*np.where(HIGH==LOW,1,(JJ-np.minimum(CLOSE,OPEN)))))
    QJ3=(np.where(0,QJ0*(HIGH-np.maximum(OPEN,CLOSE)),QJ0*np.where(HIGH==LOW,1,(HIGH-np.maximum(OPEN,CLOSE)))))
    QJ4=(np.where(0,QJ0*(np.maximum(CLOSE,OPEN)-JJ),QJ0*np.where(HIGH==LOW,1,(np.maximum(CLOSE,OPEN)-JJ))))
    DDX=(((QJ1+QJ2)-(QJ3+QJ4))/np.where(0,1000000,10000))
    DX=(MA(DDX,5)*N)
    DDX1=(sma(DX,3,1))
    DDX2=(sma(DDX1,3,1))
    DDX3=(EMA(DDX2,5))
    a=(DDX>0 ) * ( MA(V,5)>MA(MA(V,5),5) ) * ( CROSS(DDX1,DDX2))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
