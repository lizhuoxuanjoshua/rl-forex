...
#开始
    ...
    DEA=(EMA(DIFF,9))
    MACD=(2*(DIFF-DEA))
    FH1=(REF(MACD,1)<REF(MACD,2) ) * ( REF(MACD,2)<REF(MACD,3) ) * ( REF(MACD,3)<REF(MACD,4))
    FH2=(MACD>=0 ) * ( count(MACD>REF(MACD,1),1)==1)

    FH4=((CROSS(C,EMA(C,30))) ) + ( (C>EMA(C,30)
))
    FH5=(FH1 ) * ( FH2 )  * ( FH4)
    FH6=(FH1 ) * ( FH2 )  * ( FH4 ) * ( np.abs((DIFF-DEA)/C)<0.018)
    FH7=(FH1 ) * ( FH2 )  * ( FH4 ) * ( MACD<0.10)
    a=((FH5) ) + ( (FH6) ) + ( (FH7))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
