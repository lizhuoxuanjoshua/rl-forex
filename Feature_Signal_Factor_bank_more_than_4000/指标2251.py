...
#开始
    ...
    ZM=(np.where(CLOSE>OPEN,VAR1*(HIGH-LOW),np.where(CLOSE<OPEN,VAR1*((HIGH-OPEN)+(CLOSE-LOW)),VOL/2)))
    ZM1=(np.where(CLOSE>OPEN,0-VAR1*((HIGH-CLOSE)+(OPEN-LOW)),np.where(CLOSE<OPEN,0-VAR1*(HIGH-LOW),0-VOL/2)))
    MMB=((count(ZM,30)+count(ZM1,30))/10)
    SG=(MMB>0 ) * ( REF(MMB,1)<MMB)
    AA=(count(SG,5))
    JSC=(AA<1)
    a=(count(JSC,5)>3 ) * ( (JSC)==0)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
