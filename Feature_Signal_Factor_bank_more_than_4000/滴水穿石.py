...
#开始
    ...
    VARC=sma(np.abs(LOW-VARB),3,1)/sma(np.maximum(LOW-VARB,0),3,1)*100

    VARD=EMA(np.where(CLOSE*1.35<=VARA,VARC*10,VARC/10),3)

    VARE=llv(LOW,30)

    VARF=hhv(VARD,30)

    VAR10=np.where(MA(CLOSE,58),1,0)

    QZG=EMA(np.where(LOW<=VARE,(VARD+VARF*2)/2,0),3)/618*VAR10

    QZG2=EMA(np.where(LOW<=VARE,(VARD+VARF*2)/2,0),3)/-618*VAR10

    a=(REF(QZG2,2)>REF(QZG2,1)) * (QZG2>REF(QZG2,1))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
