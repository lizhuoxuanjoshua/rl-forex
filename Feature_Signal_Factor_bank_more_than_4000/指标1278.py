...
#开始
    ...
    VAR10=(EMA(VAR9,13))
    VAR11=(2*(VAR9-VAR10))
    AAC=(np.power(VAR11,3)*0.1+np.power(VAR11,1))
    AAB=(CROSS(AAC,0))
    AA=(VOL/((HIGH-LOW)*2-np.abs(CLOSE-OPEN)))
    买量=(np.where(CLOSE>OPEN,AA*(HIGH-LOW),np.where(CLOSE<OPEN,AA*((HIGH-OPEN)+(CLOSE-LOW)),VOL/2)))
    卖量=(np.where(CLOSE>OPEN,0-AA*((HIGH-CLOSE)+(OPEN-LOW)),np.where(CLOSE<OPEN,0-AA*(HIGH-LOW),0-VOL/2)))
