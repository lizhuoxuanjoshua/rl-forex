...
#开始
    ...
    TR1=(EMA(np.maximum(np.maximum(HIGH-LOW,np.abs(HIGH-REF(CLOSE,1))),np.abs(REF(CLOSE,1)-LOW)),21))
    HD =(HIGH-REF(HIGH,1))
    LD =(REF(LOW,1)-LOW)
    DMP=(EMA(HD,21))
    DMM=(EMA(LD,21))
    ...
    DM2=(REF(CLOSE,DM1+1)>CLOSE ) * ( REF(ADX1,DM1+1)<ADX1 ) * ( CROSS(ADX1,ADXR1))
    a=(FILTER(DM2>0,5))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
