...
#开始
    ...
    VAR1=(HIGH/REF(LOW,1)> 0.98 ) * ( np.abs(hhv(HIGH,60)-llv(LOW,60))/CLOSE> 0.66 ) * ( np.abs(CLOSE-REF(CLOSE,10))/CLOSE> 0.2)
    VAR2=(CLOSE/MA(CLOSE,40)*100< 78)
    VAR3=(CLOSE/MA(CLOSE,60)*100< 74)
    VAR4=(HIGH> LOW*1.051)
    VAR5=(VAR4 ) * ( count(VAR4,5)> 2)
    VAR6=(VAR5 ) * ( ((VAR2) ) + ( (VAR3)))
    VAR7=(DMA(CLOSE,VOL/count(VOL,34)))
    VAR8=(CLOSE/MA(CLOSE,50)< 0.75)
    VAR9=(CLOSE/VAR7< 0.74)
    VARA=  ( MA(VOL,16))
    VARB=(np.minimum(VAR8,VAR9))
    VARC=(VARA ) * ( VARB)
    a=((VARC) ) + ( (VAR6) ) + ( (VAR1))
#结束
    买入 = a
