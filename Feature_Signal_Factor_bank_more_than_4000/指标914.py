...
#开始
    ...
    AJ1=(0.211916*C+0.192951*REF(C,1)+0.158082*REF(C,2)+0.122217*REF(C,3)+0.091137*REF(C,4)+0.066337*REF(C,5)+0.047475*REF(C,6)+0.033564*REF(C,7)+0.023519*REF(C,8)+0.016373*REF(C,9)+0.011344*REF(C,10)+0.007832*REF(C,11)+0.005395*REF(C,12)+0.00371*REF(C,13)+0.002549*REF(C,14)+0.001751*REF(C,15)+0.001203*REF(C,16)+0.000826*REF(C,17)+0.000568*REF(C,18)+0.000391*REF(C,19)+0.000269*REF(C,20))
    YYU=(AJ1+0.000185*REF(C,21)+0.000128*REF(C,22)+0.000088*REF(C,23)+0.000061*REF(C,24)+0.000042*REF(C,25)+0.000029*REF(C,26)+0.00002*REF(C,27)+0.000014*REF(C,28)+0.000009*REF(C,29)+0.000006*REF(C,30)+0.000003*REF(C,31)+0.000002*REF(C,32)+0.000001*REF(C,33)+0.000001*REF(C,34))
    X00=(C)
    ...
    AA=(((EMA((((EMA(X01,22)+EMA(X01,23))+EMA(X01,24))/3),2))+(EMA((((EMA(X01,24)+EMA(X01,25))+EMA(X01,26))/3),2))+(EMA((((EMA(X01,26)+EMA(X01,27))+EMA(X01,28))/3),2))+(EMA((((EMA(X01,28)+EMA(X01,29))+EMA(X01,30))/3),2))+(EMA((((EMA(X01,30)+EMA(X01,31))+EMA(X01,32))/3),2))+(EMA((((EMA(X01,32)+EMA(X01,33))+EMA(X01,34))/3),2)))/6)
    BB1=((X01*5+X02*3+X03*1)/9)
    主升线=(EMA(AA,10)-EMA(YYU,22))
    DIF=(EMA(BB1+SLOPE(BB1,10),10)-EMA(YYU+SLOPE(YYU,22),22))
    RSI5=(((sma(np.maximum((CLOSE - REF(CLOSE,1)),0),5,1) / sma(np.abs((CLOSE - REF(CLOSE,1))),5,1)) * 100))
    TR1=(count(np.maximum(np.maximum((HIGH - LOW),np.abs((HIGH - REF(CLOSE,1)))),np.abs((LOW - REF(CLOSE,1)))),10))
    HD=((HIGH - REF(HIGH,1)))
    LD=((REF(LOW,1) - LOW))
    DMP=(count(np.where(((HD > 0) ) * ( (HD > LD)),HD,0),10))
    DMM=(count(np.where(((LD > 0) ) * ( (LD > HD)),LD,0),10))
    PDI=(((DMP * 100) / TR1))
    MDI=(((DMM * 100) / TR1))
    ADX=(MA(((np.abs((MDI - PDI)) / (MDI + PDI)) * 100),5))
    AV=((RSI5 + ADX))
    DXR=((((ADX + REF(ADX,5)) / 2) + RSI5))
    WR10=(((100 * (hhv(HIGH,10) - CLOSE)) / (hhv(HIGH,10) - llv(LOW,10))))
    NEWVOL=((RSI5 - WR10))
    回调买=((AV + NEWVOL))
    a=(CROSS(回调买,0) ) * ( DIF>主升线)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
