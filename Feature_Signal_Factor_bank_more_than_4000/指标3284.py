...
#开始
    ...
    AB=(EMA((VAR1-VAR3)/VAR3*VAR4,13))
    AC=(EMA((VAR2-VAR1)/VAR1*VAR4,13))
    AA=(AB-AC)
    VAR13=(VAR7>=-0.035 ) * ( VAR7<=0.045 ) * ( CLOSE>MA(CLOSE,5) ) * ( CLOSE>MA(CLOSE,10) ) * ( AA>-100)
    a=(VAR13 ) * ( VAR12>7 ) * ( REF(VAR12,1)<0 ) * ( VAR12-REF(VAR12,1)>9)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
