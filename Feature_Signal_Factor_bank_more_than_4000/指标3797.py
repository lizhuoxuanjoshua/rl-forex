...
#开始
    ...
    XA_16=(np.arctan((XA_2/REF(XA_2,1)-1)*100)*180/3.1416)
    XA_17=( CLOSE>OPEN ) * ( XA_15>50 ) * ( XA_16>70)
    XA_18=(FILTER(XA_17,13))
    XA_19=(MA(LOW+(HIGH-LOW)/2,26)*1.15)
    XA_20=(MA(LOW+(HIGH-LOW)/2,26)*0.95)
    XA_21=((XA_19+XA_20)/2)
    XA_22=((MA(CLOSE,5)+MA(OPEN,5))/2*1.06)
    XA_23=((MA(CLOSE,5)+MA(OPEN,5))/2*0.98)
    XA_24=((XA_22+XA_23)/2)
    XA_25=(XA_22>XA_20)
    XA_26=(XA_22>XA_21)
    XA_27=(XA_22>XA_19)
    ...
    XA_32=(XA_23>XA_21)
    XA_33=(XA_23>XA_19)
    XA_34=(np.where(XA_25,10,0-10)+np.where(XA_26,10,0-10)+np.where(XA_27,10,0-10)+np.where(XA_28,10,0-10)+np.where(XA_29,10,0-10)+np.where(XA_30,10,0-10)+np.where(XA_31,10,0-10)+np.where(XA_32,10,0-10)+np.where(XA_33,10,0-10))
    XA_35=(EMA(EMA(EMA(CLOSE,13),13),13))
    XA_36=(np.arctan((XA_35/REF(XA_35,1)-1)*100)*180/3.1416)
    XA_37=(XA_36>0)
    XA_38=(barslast(NOT(XA_37)))
    XA_39=((CLOSE-llv(LOW,21))/(hhv(HIGH,21)-llv(LOW,21))*100)
    XA_40=((sma(XA_39,13,8)))
    XA_41=(XA_40<10 ) * ( XA_14==(-60) ) * ( XA_15<(-60))
    XA_42=(barslast(NOT(XA_41<1)))
    a=(XA_38==0 ) * ( XA_34<10 ) * ( XA_18==1)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
