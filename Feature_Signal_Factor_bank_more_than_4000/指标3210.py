...
#开始
    ...
    BB05=(np.arctan((AA05/REF(AA05,1)-1)*100)*180/3.1416)
    速度5=(sma(EMA((AA05-REF(AA05,1))/REF(AA05,1),3)*100,3,1))
    加速度5=(EMA((速度5-REF(速度5,1)),3))
    AA10=MA(C,10)
    AA20=MA(C,20)
    BB10=(np.arctan((AA10/REF(AA10,1)-1)*100)*180/3.1416)
    AA30=(MA(C,30))
    三拾日乖离率=((C-AA30)/AA30*100)
    VAR1=((AA20-AA30)>REF((AA20-AA30),1) ) * ( AA20>REF(AA20,1) ) * ( AA30>REF(AA30,1))
    VAR2=((AA30-AA20)<REF((AA30-AA20),1) ) * ( AA20<REF(AA20,1) ) * ( AA30<REF(AA30,1))
    BB30=(np.arctan((AA30/REF(AA30,1)-1)*100)*180/3.1416)
    a=(count(CROSS(BB05,30),5)>=1 ) * ( AA05>REF(AA05,1) ) * ( 三拾日乖离率>REF(三拾日乖离率,1) ) * ( AA10>REF(AA10,1) ) * ( 加速度5>REF(加速度5,1) ) * ( 速度5>REF(速度5,1))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
