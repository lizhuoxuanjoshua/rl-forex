...
#开始
    ...
    BBB=(MA(AAA,3))
    CCC=((3*C+L+O+H)/6)
    牛线=((20*CCC+19*REF(CCC,1)+18*REF(CCC,2)+17*REF(CCC,3)+16*REF(CCC,4)+15*REF(CCC,5)+14*REF(CCC,6)+13*REF(CCC,7)+12*REF(CCC,8)+11*REF(CCC,9)+10*REF(CCC,10)+9*REF(CCC,11)+8*REF(CCC,12)+7*REF(CCC,13)+6*REF(CCC,14)+5*REF(CCC,15)+4*REF(CCC,16)+3*REF(CCC,17)+2*REF(CCC,18)+REF(CCC,20))/2102)
    马线=(MA(牛线,5))
    a=(count(C>REF(C,1)*1.096,55)>0 ) * ( (((CROSS(C,AAA) ) * ( CROSS(C,BBB) ) * ( CROSS(C,牛线) ) * ( CROSS(C,马线))) ) + ( ((CROSS(C,AAA) ) * ( CROSS(C,BBB) ) * ( 牛线>REF(牛线,1) ) * ( BBB>马线)) ) + ( ((CROSS(C,牛线) ) * ( CROSS(C,马线) ) * ( 牛线>REF(牛线,1)))))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
