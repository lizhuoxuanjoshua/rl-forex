...
#开始
    ...
    VAR1=(VOL/((H-L)*2-np.abs(C-O)))
    主买1=(np.where(C>O,VAR1*(H-L),np.where(C<O,VAR1*((H-O)+(C-L)),VOL/2)))
    主卖1=(np.where(C>O,0-VAR1*((H-C)+(O-L)),np.where(C<O,0-VAR1*(H-L),0+V/2)))
    DDX=(主买1+主卖1)
    DX=(MA(DDX,5)*5)
    DDX1=(sma(DX,3,1))
    DDX2=(sma(DDX1,3,1))
    X1=((3*C+L+H+O)/6)
    VAR40=((20*X1+19*REF(X1,1)+18*REF(X1,2)+17*REF(X1,3)+16*REF(X1,4)+15*REF(X1,5)+14*REF(X1,6)+13*REF(X1,7)+12*REF(X1,8)+11*REF(X1,9)+10*REF(X1,10)+9*REF(X1,11)+8*REF(X1,12)+7*REF(X1,13)+6*REF(X1,14)+5*REF(X1,15)+4*REF(X1,16)+3*REF(X1,17)+2*REF(X1,18)+REF(X1,20))/210)
    MTM=(C-REF(C,1))
    DX1=(100*EMA(EMA(MTM,6),6)/EMA(EMA(np.abs(MTM),6),6))
    CB=(EMA(VAR40,13))
    买1=(llv(DX1,2)==llv(DX1,7) ) * ( count(DX1<0,2) ) * ( CROSS(DX1,MA(DX1,2)))
    a=(DDX1>=DDX2 ) * ( 买1==1)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
