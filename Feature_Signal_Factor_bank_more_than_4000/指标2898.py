...
#开始
    ...
    DEA=(EMA(DIFF,9))
    MACD=(2*(DIFF-DEA))
    MM=(np.where(MACD< 0,MACD,0))
    JJ=(MM> REF(MM,1))
    ZZ=(REF(JJ,1)==1)
    JH=(hhv(np.maximum(K,np.maximum(D,J)),A))
    DH=(hhv(np.maximum(DIFF,DEA),A))
    SH=((JH/DH)/2)
    DIFF50=(SH*DIFF+50)
    DEA50=(SH*DEA+50)
    AA=((1.5*(DIFF50-DEA50)<0 ) * ( REF(1.5*(DIFF50-DEA50),1)< REF(1.5*(DIFF50-DEA50),2) ) * ( 1.5*(DIFF50-DEA50)>REF(1.5*(DIFF50-DEA50),1)))
    a=(FILTER(AA,3) ) * ( AA)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
