...
#开始
    ...
    趋势线=(EMA(-100*(hhv(HIGH,34)-CLOSE)/(hhv(HIGH,34)-llv(LOW,34)),4)+100)
    波段线=(EMA(-100*(hhv(HIGH,34)-CLOSE)/(hhv(HIGH,34)-llv(LOW,34)),19)+100)
    短期线=(-100*(hhv(HIGH,14)-CLOSE)/(hhv(HIGH,14)-llv(LOW,14))+100)
    AA=((波段线<12 ) * ( 趋势线<8 ) * ( ((短期线<7.2) ) + ( (REF(短期线,1)<5)) ) * ( (趋势线>REF(趋势线,1) ) + ( (短期线>REF(短期线,1)))) ) + ( ((波段线<8 ) * ( 趋势线<7 ) * ( 短期线<15 ) * ( 短期线>REF(短期线,1))) ) + ( ((波段线<10 ) * ( 趋势线<7 ) * ( 短期线<1)
))
    a=(count(AA,7)>=1 ) * ( CROSS(趋势线,10))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
