...
#开始
    ...
    AHC1=(CLOSE*VOL)
    AHC22=(EMA((EMA(AHC1,23)/EMA(VOL,23)+EMA(AHC1,54)/EMA(VOL,54)+EMA(AHC1,108)/EMA(VOL,108)+EMA(AHC1,316)/EMA(VOL,316))/4,34))
    AA=1.0668*AHC22
    a=(CROSS(C,AA) ) * ( REF(C,1)*1.05-C<0.008 ) * ( C>O*0.5)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
