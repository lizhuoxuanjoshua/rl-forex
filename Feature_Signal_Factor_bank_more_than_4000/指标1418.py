...
#开始
    ...
    JJ=((HIGH+LOW+CLOSE)/3)
    QJ0=(AMOUNT/np.where(HIGH==LOW,4,HIGH-LOW))
    买1=(QJ0*(np.minimum(OPEN,CLOSE)-LOW))
    买2=(QJ0*(JJ-np.minimum(CLOSE,OPEN)))
    卖1=(QJ0*(np.maximum(CLOSE,OPEN)-JJ))
    卖2=(QJ0*(HIGH-np.maximum(OPEN,CLOSE)))
    DT=(((买1+买2)-(卖1+卖2))/10000)
    CC=(np.abs((2*CLOSE+HIGH+LOW)/4-MA(CLOSE,20))/MA(CLOSE,20))
    DD=(EMA(CLOSE,CC))


    a=(DT>0 ) * ( REF(DT>0,1))* ( VOL>REF(VOL,1) ) * ( VOL/REF(VOL,1)<2 )  * ( REF(C/O,1)>1 ) * ( HSL>2)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
