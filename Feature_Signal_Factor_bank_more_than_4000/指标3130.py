...
#开始
    ...
    MA3=(MA(C,N3))
    MA30=(MA(C,30))
    GX=(np.maximum(MA1,np.maximum(MA2,MA3)))
    DX=(np.minimum(MA1,np.minimum(MA2,MA3)))
    QSXS=(DX>MA30 ) * ( MA30>REF(MA30,1))
    均线粘合=(np.where(np.abs(GX/DX-1)*100<=3,1,0))
    FL=((V>REF(V,1)) ) + ( (V>MA(V,5)
))
    SZ=(((CROSS(C,MA(C,3))) ) + ( (CROSS(C,MA(C,5)))) ) * ( C>=O)
    放量上涨=((FL ) * ( SZ))
    a=(count(均线粘合,2) ) * ( count(放量上涨,2) ) * ( C>O ) * ( QSXS)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
