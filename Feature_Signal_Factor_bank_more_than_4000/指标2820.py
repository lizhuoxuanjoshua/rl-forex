...
#开始
    ...
    CN03=(C==O)
    CN04=(((count(V,5)/CN1)))
    CN05=(np.where(CN04>0.3,1,0))
    CN06=((CN2 ) * ( CN05))
    CN07=((CN02 ) * ( CN05))

    LJLR=(count(CN07,9)/10000)
    LJLC=(count(CN06,9)/10000)
    JLR=((LJLR-LJLC))
    A5=(LJLR>REF(LJLR,1) ) * ( REF(LJLR,1)>REF(LJLR,2) ) * ( JLR>REF(JLR,1) ) * ( REF(JLR,1)>REF(JLR,2)  ) * ( C/REF(C,1)>1.03)
    a=(A5 ) * ( LJLR>0 ) * ( LJLR<100)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
