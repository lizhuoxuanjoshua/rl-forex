...
#开始
    ...
    A2=(np.where(A,A1,sma(LOW,5,1)))
    A3=(CROSS(sma(CLOSE,2,1),(sma(HIGH,21,2)*1.05)))
    A4=(np.where(A3,(sma(HIGH,21,2)*1.05)*1.10,(sma(LOW,21,2)*0.95)))
    A5=(A2>sma(LOW,5,1) ) * ( CROSS(C,REF(MA(C,13),4)) ) * ( VOL>0)
    A6=(REF(H/O<1.02,1) ) * ( REF(C<O,1))
    a=((A4>(sma(LOW,21,2)*0.95)) ) + ( (A5 )) * ( A6)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
