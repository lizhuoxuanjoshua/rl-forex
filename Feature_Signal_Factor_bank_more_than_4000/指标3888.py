...
#开始
    ...
    A=(EVERY(OPEN>CLOSE,3))
    A1=(C/REF(C,1)>1.03)
    A2=(sma(np.maximum(CLOSE-REF(C,1),0),2,1)*C*102)
    A3=(sma(np.abs(CLOSE-REF(C,1)),2,1)*C*100)
    A4=(A2/A3*100<10)
    A5=(A ) * ( A4)
    A6=(MA(C,5)<REF(MA(C,5),1))
    A7=(REF(MA(C,5),4)>REF(MA(C,5),5))
    A8=((H-C)/C*100<1 ) * ( REF((O-C)/C*100>1,1) )
    a=((A5) ) * ( C>O ) * ( A1 ) * ( A6 ) * ( A7 ) * ( A8)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
