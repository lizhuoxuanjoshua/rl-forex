...
#开始
    ...
    A=(4*sma((CLOSE-llv(LOW,5))/(hhv(HIGH,5)-llv(LOW,5))*100,5,1)-3*sma(sma((CLOSE-llv(LOW,5))/(hhv(HIGH,5)-llv(LOW,5))*100,5,1),3.2,1))
    A1=(((2)*(CLOSE) + HIGH + LOW + OPEN)/(5))
    A2=(((A1 - MA(A1,14))*(100))/((1.5)*(AVEDEV(A1,14))))
    A3=(((CLOSE-MA(CLOSE,6))/MA(CLOSE,6)*100+(CLOSE-MA(CLOSE,24))/MA(CLOSE,24)*100+(CLOSE-MA(CLOSE,32))/MA(CLOSE,32)*100)/3)
    A4=(A3<-9 ) * ( A3> REF(A3,1))
    A5=(((sma(np.maximum(CLOSE - REF(CLOSE,1),0),6,1))/(sma(np.abs(CLOSE - REF(CLOSE,1)),6,1)))*(100))
    A6=(((A2 < (-100))) ) + ( ((A5 < 25)) ) + ( (A4
))
    B=(AMOUNT/VOL)
    B1=(L<B*0.9)
    B2=((C-REF(C,1))/REF(C,1)*100>1.2)
    B3=(L<MA(C,6)*0.921)
    B4=(VOL<MA(V,5)*1.5)
    a=(B1 ) * ( B2 ) * ( B3 ) * ( B4 )
#结束
