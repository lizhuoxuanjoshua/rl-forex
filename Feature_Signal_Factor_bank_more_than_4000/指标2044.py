...
#开始
    ...
    LC=(REF(CLOSE,1))
    RSI5=(sma(np.maximum(CLOSE-LC,0),5,1)/sma(np.abs(CLOSE-LC),5,1))
    WR10=((hhv(HIGH,10)-CLOSE)/(hhv(HIGH,10)-llv(LOW,10)))
    NEWVOL=(RSI5-WR10)
    a=((NEWVOL>REF(NEWVOL,1)) ) * ( ( REF(NEWVOL,1)<REF(NEWVOL,2) ) * ( NEWVOL<0 ))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
