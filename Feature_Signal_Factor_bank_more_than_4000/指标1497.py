...
#开始
    ...
    RSVV=((((CLOSE - llv(LOW,9)) / (hhv(HIGH,9) - llv(LOW,9))) * 100))
    K=(sma(RSVV,3,1))
    DS=(sma(K,3,1))
    JS=(3*K-2*DS)
    X=(llv(JS,2)==llv(JS,8))
    RE=(CROSS(JS,REF(JS+0.01,1)) ) * ( X ) * ( JS<10)
    AA=(REF(CLOSE,1))
    BBB=(sma(np.maximum(CLOSE-AA,0),5,1)/sma(np.abs(CLOSE-AA),5,1)*1000)
    HHF=(BBB-llv(BBB,10))
    SSS=((MA(HHF,2)*3+HHF*13)/16)
    AAA=(np.where(SSS>13,MA(SSS,2),SSS)/6)
    AB=(30*(CROSS(AAA,1) ) * ( (AAA<20)))
    CSS=(((CLOSE-llv(LOW,8))/(hhv(HIGH,8)-llv(LOW,8))*100))
    HH=(CROSS(15,EMA(CSS,3)))
    CE=(REF(HH,1))
    a=(RE ) * ( AB ) * ( CE)
#结束
    买入 = a
