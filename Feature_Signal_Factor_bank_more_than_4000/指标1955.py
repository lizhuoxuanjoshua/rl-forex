...
#开始
    ...
    A1=(REF(CLOSE,1))
    A2=(((sma((np.maximum((CLOSE - A1),0)),5,1)) / (sma(np.abs((CLOSE - A1)),5,1))) * 100)
    A3=(count((np.maximum((np.maximum((HIGH - LOW),np.abs((HIGH - A1)))),np.abs((LOW - A1)))),10))
    A4=(HIGH - (REF(HIGH,1)))
    A5=((REF(LOW,1)) - LOW)
    条件1=(((A4 > 0) ) * ( (A4 > A5)))
    条件2=(((A5 > 0) ) * ( (A5 > A4)))
    A6=(count((np.where(条件1,A4,0)),10))
    A7=(count((np.where(条件2,A5,0)),10))
    A8=((A6 * 100) / A3)
    A9=((A7 * 100) / A3)
    A10=(MA(((np.abs((A9 - A8)) / (A9 + A8)) * 100),5))
    A11=(A2 + A10)
    A12=(((A10 + (REF(A10,5))) / 2) + A2)
    A13=((100 * ((hhv(HIGH,10)) - CLOSE)) / ((hhv(HIGH,10)) - (llv(LOW,10))))
    A14=(A2 - A13)
    A15=(A11 + A14)
    A=(llv(A15,1))
    B=(np.where((A < 0),0,A))
