...
#开始
    ...
    C1=(O>REF(C,1) ) * ( L>REF(H,1))

    C3=((H/np.maximum(C,O)>np.maximum(C,O)/np.minimum(C,O)) ) + ( (H/np.maximum(C,O)>1.03
))
    C4=(np.minimum(C,O)/L<np.maximum(C,O)/np.minimum(C,O) ) * ( np.minimum(C,O)/L<1.02)
    C6=(C>MA(C,5) ) * ( C>MA(C,10) ) * ( C>MA(C,20))
    C7=(C==hhv(C,7))
    昨天=(C1 )  * ( C3 ) * ( C4 ) * ( C6 ) * ( C7)
    D1=(O/REF(C,1)>0.98)
    D2=(L>REF(H,2))
    D3=(C/REF(np.maximum(C,O),1)>1.01)
    今天=(D1 ) * ( D2 ) * ( D3)
    a=(REF(昨天,1) ) * ( 今天)
#结束
    买入 = a
