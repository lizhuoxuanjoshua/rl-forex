...
#开始
    ...
    VAR23=(AMOUNT/VOL)
    VAR24=(count(AMOUNT,5)/count(VOL,5))
    ...
    VAR30=((EMA(EMA(EMA(VAR2F,15),10),5)-25)*2)
    VAR31=((VAR1F+VAR20+VAR21*30)/32/5-15)
    VAR32=(EMA(VAR31,13))
    VAR33=((VAR2F+VAR31)/2/1.1)
    VAR34=(VAR33)
    AA=(EMA(VAR34,1))
    AB=(EMA((VAR30+VAR32)/2,2))
    ...
    VAR3B=(MA(VAR3A,3))
    a=(CROSS(AA,AB) ) * (( count(VAR3B<N,2)>=1) ) + ( (CROSS(AA,0) )) * ( count(VAR3B<N,1)>=1)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
