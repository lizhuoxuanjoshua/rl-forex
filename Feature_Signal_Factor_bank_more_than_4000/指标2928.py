...
#开始
    ...
    长期线=(MA(-100*(hhv(HIGH,34)-CLOSE)/(hhv(HIGH,34)-llv(LOW,34)),19)+100)
    短期线=(-100*(hhv(HIGH,14)-CLOSE)/(hhv(HIGH,14)-llv(LOW,14))+100)
    中期线=(EMA(-100*(hhv(HIGH,34)-CLOSE)/(hhv(HIGH,34)-llv(LOW,34)),4)+100)
    a=((长期线<12 ) * ( 中期线<8 ) * ( ((短期线<7.2) ) + ( (REF(短期线,1)<5)) ) * ( (中期线>REF(中期线,1) ) + ( (短期线>REF(短期线,1))))) + ( ((长期线<8 ) * ( 中期线<7 ) * ( 短期线<15 ) * ( 短期线>REF(短期线,1))) ) + ( ((长期线<10 ) * ( 中期线<7 ) * ( 短期线<1) ))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
