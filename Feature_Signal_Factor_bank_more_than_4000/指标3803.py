...
#开始
    ...
    V1=(sma(sma((CLOSE-llv(LOW,80))/(hhv(HIGH,80)-llv(LOW,80))*200,3,1),3,1))
    V2=(3*sma((CLOSE-llv(LOW,80))/(hhv(HIGH,80)-llv(LOW,80))*200,3,1)-2*V1)
    V3=(EMA((CLOSE-llv(LOW,5))/(hhv(HIGH,5)-llv(LOW,5))*100,4)*2)
    V4=(EMA(0.667*REF(V3,1)+0.333*V3,2))
    V5=(3*V3-2*V4)
    VA=(np.power(MA(CLOSE,5),2)+MA(CLOSE,5))
    VB=(np.power(MA(LOW,5),2)+MA(LOW,5))
    VC=(np.power(MA(HIGH,5),2)+MA(HIGH,5))
    VD=((VA-llv(VB,64))/(hhv(VC,64)-llv(VB,64))*150+65-10)
    VE=(sma(VD,3,1)*1.5-100)
    VF=(sma(VE,3,1))
    V6=(3*VE-2*VF)
    V7=(((HIGH+LOW+CLOSE*2)/4-EMA((HIGH+LOW+CLOSE*2)/4,13))*100/STD((HIGH+LOW+CLOSE*2)/4,13))
    V8=(EMA(EMA(V7,5),13)+200-100)
    V9=(EMA(V8,10))
    V10=(3*V8-2*V9)
    V1A=(sma((CLOSE-llv(LOW,80))/(hhv(HIGH,80)-llv(LOW,80))*200,3,1)+V3+VE+V8)
    V1B=(V1+V4+VF+V9)
    V1C=(V2+V5+V6+V10)
    V1D=((HIGH+LOW+CLOSE)/3)
    V11=((V1D-MA(V1D,34))/(0.015*AVEDEV(V1D,34)))
    V12=(EMA((HIGH+LOW+CLOSE+V1D)/4,24))
    V13=(STD((HIGH+LOW+CLOSE+V1D)/4,24))
    V14=(((HIGH+LOW+CLOSE+V1D)/4-V12)/V13*100)
    V15=((V1D-MA(V1D,24))/(0.015*AVEDEV(V1D,24)))
    V16=(EMA(V14,5))
    V2A=(((V16+V15+V11)/3+165)/4)
    V2B=((EMA(EMA(EMA(V2A,15),10),5)-25)*2+DMA(AMOUNT/VOL/100,1))
    V2C=((V1A+V1B+V1C*30)/32/5-15)
    V2D=(EMA(V2C,13))
    V2E=((V2A+V2C)/2/1.1)
    短线王=(EMA(V2E,3))
    波段王=(EMA(短线王,5))
    超级主力=(EMA((V2B+V2D)/2,2))
    a=(CROSS(短线王,波段王) ) * ( CROSS(波段王,超级主力))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
