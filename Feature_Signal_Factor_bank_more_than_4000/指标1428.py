...
#开始
    ...
    LC=(REF(CLOSE,1))
    RSI5=(((sma(np.maximum((CLOSE - LC),0),5,1) / sma(np.abs((CLOSE - LC)),5,1)) * 100))
    ...
    NEWVOL=((RSI5 - WR10))
    潜龙=((AV + NEWVOL))
    AA=(MA(C,40)>REF(MA(C,40),1) ) * ( REF(MA(C,40),1)>REF(MA(C,40),2))
    BB=(MA(C,60)>REF(MA(C,60),1) ) * ( REF(MA(C,60),1)>REF(MA(C,60),2))
    CC=(MA(C,20)>REF(MA(C,20),1) ) * ( REF(MA(C,20),1)>REF(MA(C,20),2))
    EE=(((AA) ) + ( (BB) ) + ( (CC)))
    BOLL=(MA(CLOSE,19))
    LOWER=(BOLL-1.6*STD(CLOSE,19))
    FF=(L<=LOWER)
    a=(CROSS(潜龙,0) ) * ( EE ) * ( FF)
#结束
    买入 = a
