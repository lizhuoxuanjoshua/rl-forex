...
#开始
    ...
    VAR3=sma(np.abs(LOW-VAR2),13,1)/sma(np.maximum(LOW-VAR2,0),13,1)*100 

    

    VAR4=EMA(np.where(CLOSE*1.2,VAR3*13,VAR3/13),13) 
    ...
    VAR11=EMA(np.where(LOW<=VAR5,(VAR4+VAR6*2)/2,0),3)/618*VAR7*(-1) 
    ...
    XL3=(REF(XL1,1)<XL2) * (REF(XL1,1)<XL1) * (REF(XL1,1)<REF(XL1,2)) 

    

    启动=np.where(XL3,0,VAR8*3) 
