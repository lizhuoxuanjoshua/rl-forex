...
#开始
    ...
    X_1=(OPEN>CLOSE)
    X_2=(LAST(X_1,5,4))
    X_3=(REF(CLOSE,2)<REF(CLOSE,1)*0.975)
    X_4=(OPEN>CLOSE)
    X_5=(REF(CLOSE,7)/HIGH>1.025)
    X_6=(llv(LOW,9))
    X_7=(hhv(HIGH,9))
    X_8=(EMA((CLOSE-X_6)/(X_7-X_6)*100,7))
    X_9=(EMA(X_8,3))
    X_10=(EMA(X_9,3))
    X_11=(X_10<55)
    X_12=(VOL<=llv(VOL,20)*1.4)
    X_13=(VOL<REF(VOL,5))
    X_14=(LOW>REF(LOW,3)*0.94 ) * ( LOW<REF(LOW,3)*1.01)
    X_15=(MA(CLOSE,20)/CLOSE>1.08)
    X_16=(HIGH>REF(HIGH,2)*0.98 ) * ( HIGH<REF(HIGH,3)*1.05)
    X_17=(OPEN<REF(CLOSE,1))
    X_18=(count(X_17,10)>1)
    X_19=(REF(CLOSE,4)/REF(CLOSE,1)<1.025)
    X_20=(OPEN<REF(CLOSE,1)*0.997)
    X_21=(np.maximum(OPEN,CLOSE)/np.minimum(OPEN,CLOSE)>1.005)
    X_22=(X_2 ) * ( X_3 ) * ( X_4 ) * ( X_5 ) * ( X_11 ) * ( X_12 ) * ( X_13 ) * ( X_14 ) * ( X_15 ) * ( X_16 ) * ( X_19 ) * ( X_19 ) * ( X_20 ) * ( X_21)
    X_23=(OPEN>CLOSE)
    X_24=(LAST(X_23,5,4) ) * ( LAST(OPEN<CLOSE,6,6))
    X_25=(REF(CLOSE,2)<REF(CLOSE,1)*1.005)
    X_26=(OPEN>CLOSE)
    X_27=(REF(CLOSE,8)/HIGH>1.01)
    X_28=(llv(LOW,9))
    X_29=(hhv(HIGH,9))
    X_30=(EMA((CLOSE-X_28)/(X_29-X_28)*100,7))
    X_31=(EMA(X_30,3))
    X_32=(EMA(X_31,3))
    X_33=(X_32<55)
    X_34=(VOL<=llv(VOL,20)*2.9)
    X_35=(VOL<REF(VOL,5))
    X_36=(LOW>REF(LOW,3)*0.85 ) * ( LOW<REF(LOW,3)*1.035)
    X_37=(MA(CLOSE,20)/CLOSE>1.07)
    X_38=(HIGH>REF(HIGH,2)*0.978 ) * ( HIGH<REF(HIGH,3)*1.05)
    X_39=(REF(CLOSE,4)/REF(CLOSE,1)<1.13)
    X_40=(OPEN<REF(CLOSE,1)*0.987)
    X_41=((HIGH+LOW+CLOSE+OPEN)/4)
    X_42=((X_41-EMA(X_41,16))/(0.015*AVEDEV(X_41,16)))
    X_43=(X_42<120)
    X_44=(X_24 ) * ( X_25 ) * ( X_26 ) * ( X_27 ) * ( X_33 ) * ( X_34 ) * ( X_35 ) * ( X_36 ) * ( X_37 ) * ( X_38 ) * ( X_39 ) * ( X_40 ) * ( X_43)
    X_45=(HIGH<EMA(HIGH,6))
    X_46=(LAST(X_45,6,1))
    X_47=(REF(HIGH,1)>REF(EMA(CLOSE,3),1)*0.98)
    X_48=(HIGH>EMA(CLOSE,4))
    X_49=(VOL<=llv(VOL,20)*1.2)
    X_50=(REF(VOL,1)<REF(VOL,2))
    X_51=(REF(HIGH,5)/HIGH>1.05)
    X_52=(EMA(VOL,65)/VOL*2)
    X_53=(X_52>1.5)
    X_54=(EMA(CLOSE,5)/HIGH>1.005)
    X_55=(count(X_54,10)>=2)
    X_56=(EMA(CLOSE,90)/CLOSE>1.05)
    X_57=(EMA(CLOSE,20)/CLOSE>1.02)
    X_58=(HIGH/LOW<1.15)
    X_59=(REF(HIGH/LOW,2)<1.09)
    X_60=(OPEN/CLOSE>1.01)
    X_61=(count(X_60,10)>=1)
    X_62=((HIGH+LOW+CLOSE+OPEN)/4)
    X_63=((X_62-EMA(X_62,16))/(0.015*AVEDEV(X_62,16)))
    X_64=(X_63>(-120) ) * ( X_63<(-40))
    X_65=(X_63>REF(X_63,2)*1.02)
    X_66=(llv(LOW,9))
    X_67=(hhv(HIGH,9))
    X_68=(EMA((CLOSE-X_66)/(X_67-X_66)*100,3))
    X_69=(EMA(X_68,6))
    X_70=(MA(X_69,3))
    X_71=(X_69<65)
    X_72=(REF(HIGH,1)>REF(LOW,4)*0.99)
    X_73=(REF(X_69,1)<REF(X_70,4)*1.9)
    X_74=(np.maximum(OPEN,CLOSE)-np.minimum(OPEN,CLOSE)>(np.minimum(OPEN,CLOSE)-LOW)*0.5)
    X_75=(CLOSE/LOW<1.07 ) * ( HIGH/REF(LOW,2)>1.005)
    X_76=(LOW<REF(LOW,3)*0.999 ) * ( OPEN<REF(HIGH,1)*0.989)
    X_77=(REF(VOL,6)>VOL*0.5 ) * ( REF(VOL,5)<VOL*3.5)
    X_78=(REF(VOL,4)>VOL*1.19 ) * ( REF(VOL,4)<VOL*2.5)
    X_79=(REF(LOW,1)>REF(LOW,2)*1.007)
    X_80=(CLOSE<REF(CLOSE,2)*1.005)
    X_81=(OPEN<REF(OPEN,1)*1.03)
    X_82=(X_47 ) * ( X_46 ) * ( X_49 ) * ( X_65 ) * ( X_51 ) * ( X_55 ) * ( X_56 ) * ( X_57 ) * ( X_58 ) * ( X_59 ) * ( X_64 ) * ( X_72 ) * ( X_73 ) * ( X_74 ) * ( X_75 ) * ( X_76 ) * ( X_77 ) * ( X_78 ) * ( X_79 ) * ( X_80 ) * ( X_80)
    a=(X_22 ) + ( ((X_44 )) * ( X_81) ) + ( X_82)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
