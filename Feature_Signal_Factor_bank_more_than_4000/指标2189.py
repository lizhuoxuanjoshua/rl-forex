...
#开始
    ...
    X=((3*C+2*L+O+H)/7)
    多空线=((20*X+19*REF(X,1)+18*REF(X,2)+17*REF(X,3)+16*REF(X,4)+15*REF(X,5)+14*REF(X,6)+13*REF(X,7)+12*REF(X,8)+11*REF(X,9)+10*REF(X,10)+9*REF(X,11)+8*REF(X,12)+7*REF(X,13)+6*REF(X,14)+5*REF(X,15)+4*REF(X,16)+3*REF(X,17)+2*REF(X,18)+REF(X,20))/210)
    A0=((L+H+C*2)/4)
    工作=(EMA(A0,14))
    ...
    ZLL=HSL
    HSL1=(np.where((ZLL > 10),10,ZLL))
    HSL=(((XVL / 20) / 1.15))
    攻击流量=((((HSL * 0.55) + (REF(HSL,1) * 0.33)) + (REF(HSL,2) * 0.22)))
    LLJX=(EMA(攻击流量,3))
    资金流入=(np.where((HSL*0.4+LLJX)>0,(HSL*0.4+LLJX),0))
    a=(CROSS(A1X,0) ) * ( 资金流入>0 ) * ( L<多空线 ) * ( C>多空线 ) * ( C-O>(H-C)*0.99)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
