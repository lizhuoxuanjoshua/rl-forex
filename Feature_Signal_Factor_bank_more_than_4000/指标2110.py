...
#开始
    ...
    VAR1=(1)
    VAR2=((CLOSE-llv(LOW,34))/(hhv(HIGH,34)-llv(LOW,34))*100*VAR1)
    火龙=(sma(VAR2,3,1)*VAR1)
    VAR3=((2*CLOSE+HIGH+LOW)/4*VAR1)
    VAR4=(llv(LOW,34)*VAR1)
    VAR5=(hhv(HIGH,34)*VAR1)
    青龙=(EMA((VAR3-VAR4)/(VAR5-VAR4)*100,13)*VAR1)
    a=(CROSS(火龙,青龙) ) * ( 火龙<34 ) * ( (CROSS(火龙,青龙) ) * ( 火龙<34))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
