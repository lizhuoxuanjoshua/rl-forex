...
#开始
    ...
    A4=(MA(CLOSE,60))
    A5=(MA(CLOSE,250))
    VAR11=(((4 * sma((((CLOSE - llv(LOW,5)) / (hhv(HIGH,5) - llv(LOW,5))) * 100),5,1)) - (3 * sma(sma((((CLOSE - llv(LOW,5)) / (hhv(HIGH,5) - llv(LOW,5))) * 100),5,1),3.2,1))))
    DD=(np.where((VAR11 <= 8),0.5,0))
    XG1=(((((((A1 > A2) ) * ( (A2 > A3)) ) * ( (A3 > A4)) ) * ( (A4 > A5)) ) * ( AA) ) * ( np.where(DD,1,0)))
    XG2=(((((((A2 > A1) ) * ( (A1 > A3)) ) * ( (A3 > A4)) ) * ( (A4 > A5)) ) * ( AA) ) * ( np.where(DD,1,0)))
    a=((XG1) ) + ( (XG2))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
