...
#开始
    ...
    ABV=(MA(count(np.where(CLOSE>REF(CLOSE,1),VOL,np.where(CLOSE<REF(CLOSE,1),-VOL,0)),0)/25000,2))
    M1=(EMA(ABV,12))
    M2=(EMA(ABV,26))
    MTM=(CLOSE-REF(CLOSE,1))
    MMS=(((100)*(EMA(EMA(MTM,6),6)))/(EMA(EMA(np.abs(MTM),6),6)))
    MMM=(((100)*(EMA(EMA(MTM,12),12)))/(EMA(EMA(np.abs(MTM),12),12)))
    MML=(((100)*(EMA(EMA(MTM,26),26)))/(EMA(EMA(np.abs(MTM),26),26)))
    MTGJ=(C-REF(C,1))
    ZLGJ=(100*EMA(EMA(MTGJ,9),9)/EMA(EMA(np.abs(MTGJ),9),9))
    MAZL=(MA(ZLGJ,5))

    X=(1)
    V1=((HIGH+OPEN+LOW+(2)*(CLOSE))/(5))
    V2=(REF(V1,1))
    V3=(np.maximum(V1-V2,0))
    V4=(np.abs(V1-V2))
    V5=(sma(V3,10,1))
    V6=(sma(V4,10,1))
    V8=(count(((V5)/(V6)<0.2),5))
    V9=(count((llv(V1,10)==V1),10))
    精准买卖=(V8>=1 ) * ( V9>=1 ) * ( CLOSE>OPEN ) * ( REF(CLOSE,1)>REF(OPEN,1) ) * ( (VOL>REF(VOL,1)) ) * ( MMS>MML ) * ( CROSS(ABV,M2))
    短线买点=(V8>=1 ) * ( V9>=1 ) * ( CLOSE>OPEN ) * ( REF(CLOSE,1)>REF(OPEN,1) ) * ( (VOL>REF(VOL,1)) ) * ( CROSS(MMS,MML) ) * ( ABV>REF(ABV,1))
    中线买点=(V8>=1 ) * ( V9>=1 ) * ( CLOSE>OPEN ) * ( REF(CLOSE,1)>REF(OPEN,1) ) * ( (VOL>REF(VOL,1)) ) * ( ABV>REF(ABV,1) ) * ( MMS>MML ) * ( CROSS(MMM,MML) ) * ( MMS>REF(MMS,1) ) * ( MMM>REF(MMM,1) ) * ( MML>REF(MML,1))
    主进主买=(ABV>M2 ) * ( ZLGJ>MAZL ) * ( CROSS(MMS,MML) ) * ( MMS>REF(MMS,1) ) * ( MMM>REF(MMM,1) ) * ( MML>REF(MML,1))
    a=((精准买卖) ) + ( (短线买点) ) + ( (中线买点) ) + ( (主进主买
))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
