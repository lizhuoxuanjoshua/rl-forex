...
#开始
    ...
    a=(count(CLOSE<REF(CLOSE,1),13)/13>6/10 ) * ( count(SK>SD,6) ) * ( REF(LOW,5)==llv(LOW,120) ) * ( REF(CLOSE>=OPEN,4) ) * ( REF(CLOSE>OPEN,3) ) * ( REF(CLOSE>OPEN,2) ) * ( REF(OPEN>CLOSE,1) ) * ( OPEN>REF(CLOSE,1))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
