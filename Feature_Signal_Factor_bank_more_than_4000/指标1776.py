...
#开始
    ...
    HLM11=(sma(np.abs(L-REF(L,1)),3,1)/sma(np.maximum(L-REF(L,1),0),3,1)*100)
    HLM12=(MA(np.where(C*1.35<=HLM10,HLM11*10,HLM11/10),3))
    HLMH=(hhv(HLM12,30))
    AA=(MA(np.where(L<=llv(L,42),(HLMH+HLM11*2)/3,0),3)/200)
    a=(CROSS(3.8,AA) ) * ( AA<REF(AA,1))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
