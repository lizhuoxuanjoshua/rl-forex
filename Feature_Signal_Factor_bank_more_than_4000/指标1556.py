...
#开始
    ...
    TTR=(count(np.maximum(np.maximum((HIGH - LOW),np.abs((HIGH - REF(CLOSE,1)))),np.abs((LOW - REF(CLOSE,1)))),10))
    HD=((HIGH - REF(HIGH,1)))
    LD=((REF(LOW,1) - LOW))
    DMP=(count(HD,10))
    DMM=(count(LD,10))
    ...
    阶段底部=((AV + NEWVOL))
    AA=(REF(CLOSE,1))
    BBB=(sma(np.maximum(CLOSE-AA,0),21,1)/sma(np.abs(CLOSE-AA),21,1)*1000)
    HHH=(BBB-llv(BBB,27))
    CCC=(llv(BBB,27)-BBB)
    QQ=((MA(HHH,2)*3+HHH*13)/16)
    SS=((MA(CCC,2)*3+CCC*13)/16)
    GG=(MA(HHH,5))
    MM=(MA(CCC,5))
    WW=(np.where(QQ<GG ,SS,MM))
    EE=(np.where(SS<MM ,GG,QQ))
    买=(EE-WW<5.6)
