...
#开始
    ...
    A1=(C-REF(C,1))
    A2=(100*EMA(EMA(A1,6),6)/EMA(EMA(np.abs(A1),6),6))
    AA=(llv(A2,2)==llv(A2,7) ) * ( count(A2<0,2) ) * ( CROSS(A2,MA(A2,2)))
    VAR1=((CLOSE*2+HIGH+LOW)/4*10)
    VAR2=(EMA(VAR1,13)-EMA(VAR1,34))
    VAR3=(EMA(VAR2,5))
    VAR4=(2*(VAR2-VAR3)*5.5)
    ZLC=(np.where(VAR4<=0,VAR4,0))
    DIF=(EMA(CLOSE,12)-EMA(CLOSE,26))
    DEA=(EMA(DIF,9))
    MACD=((DIF-DEA)*2)

    a=(CROSS(A2,MA(A2,2)) ) * ( AA ) * ( ZLC>REF(ZLC,1))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
