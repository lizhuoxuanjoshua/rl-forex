...
#开始
    ...
    VAR5=(llv(LOW,8))
    VAR6=(hhv(HIGH,8))
    VAR7=(MA((CLOSE-VAR5)/(VAR6-VAR5)*100,4)*2)
    VAR8=(MA(0.667*REF(VAR7,1)+0.333*VAR7,2))
    VAR9=(3*VAR7-2*VAR8)
    VARA=(np.power(MA(CLOSE,9),2)+MA(CLOSE,9))
    VARB=(np.power(MA(LOW,9),2)+MA(LOW,9))
    VARC=(np.power(MA(HIGH,9),2)+MA(HIGH,9))
    VARD=((VARA-llv(VARB,75))/(hhv(VARC,75)-llv(VARB,75))*160)
    VARE=(MA(VARD,3)*1.5-100)
    VARF=(MA(VARE,3))
    VAR10=(3*VARE-2*VARF)
