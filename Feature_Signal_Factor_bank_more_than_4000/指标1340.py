...
#开始
    ...
    AAC=(((3*sma((CLOSE-llv(LOW,27))/(hhv(HIGH,27)-llv(LOW,27))*100,5,1)-2*sma(sma((CLOSE-llv(LOW,27))/(hhv(HIGH,27)-llv(LOW,27))*100,5,1),3,1)-50)*1.032+50))
    VAR2=((2*CLOSE+HIGH+LOW+OPEN)/5)
    VAR3=(llv(LOW,34))
    VAR4=(hhv(HIGH,34))
    VAR1=(hhv(HIGH,90))
    AAB=(EMA((VAR2-VAR3)/(VAR4-VAR3)*100,13))
    AA=(np.sqrt(np.sqrt(np.sqrt(MA(1/(CLOSE/VAR1)*100,4)/10000)))*5)
    VAR5=(CROSS(AAC,AAB) ) * ( AAB<25)
    a=(VAR5 ) * ( AA>0)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
