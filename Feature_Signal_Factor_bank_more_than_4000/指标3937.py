...
#开始
    ...
    VAR2=(sma(np.abs(LOW-VAR1),20,1)/sma(np.maximum(LOW-VAR1,0),10,1))
    VAR3=(EMA(VAR2,10))
    VAR4=(llv(LOW,60))
    VAR5=(EMA(np.where(LOW<=VAR4,VAR3,0),3))
    VAR6=(hhv(HIGH,60))
    VAR7=(llv(LOW,30))
    VAR8=(np.where(VAR5<REF(VAR5,1),VAR5,0))
    VAR9=(sma((CLOSE-VAR7)/(VAR6-VAR7),5,1)*100)
    VAR10=((CLOSE-MA(CLOSE,90))/MA(CLOSE,90)*100)
    VAR12=(REF(C,1)/REF(C,2)<1.0618)
    a=(CROSS(VAR9,20) ) * ( VAR8>1  ) * ( REF(VAR9,6)<20 )  * ( VAR12)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
