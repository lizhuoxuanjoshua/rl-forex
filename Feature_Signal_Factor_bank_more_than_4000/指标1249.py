...
#开始
    ...
    K1=sma(RSV,3,1)
    D1=sma(K1,3,1)
    AAA=( 慢线1<Q ) * ( VOL>1.5*MA(VOL,5) ) * ( CROSS(D1,慢线1))
    VAR2=((HIGH+LOW+OPEN+2*CLOSE)/5)
    VAR3=(REF(VAR2,1))
    VAR4=(sma(np.maximum(VAR2-VAR3,0),10,1)/sma(np.abs(VAR2-VAR3),10,1)*100)
    AA=(count(VAR4<20,5)>=1 ) * ( count(VAR2==llv(VAR2,5),10)>=1 ) * ( CLOSE>OPEN ) * ( CLOSE>REF(OPEN,1) ) * ( VOL>1.5*MA(VOL,5))
    a=((AA) ) + ( (AAA))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
