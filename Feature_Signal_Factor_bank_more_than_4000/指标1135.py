...
#开始
    ...
    M22=(MA(M2,13))
    M32=(MA(M3,26))
    Mmax=(np.maximum(np.maximum(M12,M22),M32))
    Mmin=(np.minimum(np.minimum(M12,M22),M32))
    AA=(100-(Mmax-Mmin)/(Mmax+Mmin)*100)
    a=(AA>90 ) * ( M3<=REF(M3,1) ) * ( M1>REF(M1,1) ) * ( MA(C,5)>MA(C,13))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
