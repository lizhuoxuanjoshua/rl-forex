...
#开始
    ...
    AA=(3*sma((C-llv(L,55))/(hhv(H,55)-llv(L,55))*100,5,1)-2*sma(sma((C-llv(L,55))/(hhv(H,55)-llv(L,55))*100,5,1),3,1))
    AAC=(EMA(AA,3)-10)
    AAB=((AAC-REF(AAC,1))/REF(AAC,1)*100)
    a=(FILTER((AAC<=13 ) * ( AAB>13),10))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
