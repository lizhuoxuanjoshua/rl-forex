...
#开始
    ...
    QQ=(EMA(C,12)-EMA(C,26))
    Q=((C-llv(L,9))/(hhv(H,9)-llv(L,9))*100)
    Q1=(sma(Q,3,1))
    Q2=(sma(Q1,2,1))
    Q3=((C-llv(L,9))/(hhv(H,9)-llv(L,9))*100)
    Q4=(sma(Q3,3,1))
    Q5=(sma(Q4,3,1))
    Q6=(3*Q1-2*Q5)
    a=(REF(Q1<REF(Q1,1) ) * ( Q2<REF(Q2,1) ) * ( Q4<REF(Q4,1) ) * ( Q5<REF(Q5,1) ) * ( Q6<REF(Q6,1)) ) * ( Q1>REF(Q1,1) ) * ( Q2>REF(Q2,1) ) * ( Q4>REF(Q4,1) ) * ( Q5>REF(Q5,1) ) * ( Q6>REF(Q6,1) ) * ( llv(L,10)==L ) * ( QQ>REF(QQ,1))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
