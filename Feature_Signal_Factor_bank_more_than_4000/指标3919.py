...
#开始
    ...
    A1=(CROSS(3*sma(RSV,3,1)-2*sma(sma(RSV,3,1),3,1),MA(3*sma(RSV,3,1)-2*sma(sma(RSV,3,1),3,1),2))) * ( C>O)
    A2=(((REF(C,3)-REF(C,1))/C)*100>6)
    A3=( CLOSE>REF(CLOSE,1)*1.03)
    A4=( REF(((H-O)/C)*100<2,1))
    B1=(REF(C<O,2) ) * ( REF(C<O,1))
    B2=(((H-C)/C)*100<2 ) * ( H>REF(H,1))
    B3=(((REF(O,1)-REF(C,1))/C)*100>1)
    B4=(((REF(O,2)-REF(C,2))/C)*100<9)
