...
#开始
    ...
    XA_1=(1)
    XA_2=(1)
    XA_3=(CLOSE/MA(CLOSE,40)*100<78)
    XA_4=(CLOSE/MA(CLOSE,60)*100<74)
    XA_5=(HIGH>LOW*1.051)
    XA_6=(XA_5 ) * ( count(XA_5,5)>1)
    XA_7=(XA_6 ) * ( ((XA_3) ) + ( (XA_4)))
    XA_8=(CLOSE/REF(CLOSE,25)<=1.1)
    XA_9=(sma(np.maximum(CLOSE-REF(CLOSE,2),0),7,1)/sma(np.abs(CLOSE-REF(CLOSE,2)),7,1)*100<15)
    XA_10=((CLOSE-llv(LOW,8))/(hhv(HIGH,8)-llv(LOW,8))*100)
    XA_11=(sma(XA_10,2,1))
    XA_12=(sma(XA_11,2,1))
    XA_13=((XA_11>REF(XA_11,1) ) * ( REF(XA_11,1)<REF(XA_11,2) ) * ( XA_11<23)*XA_2)
    XA_14=(llv(LOW,4)==llv(LOW,120))
    XA_15=((CLOSE-MA(CLOSE,58))/MA(CLOSE,58)<(-0.07)*XA_2)
    XA_17=1
    XA_18=(MA(CLOSE,27))
    XA_19=((CLOSE-XA_18)/XA_18*100)
    XA_20=(MA(XA_19,2)*XA_1)
    XA_21=(barslast(CROSS((-10),XA_20)==1)*XA_2)
    XA_22=(XA_20<(-10) ) * ( XA_21>3)
    XA_23=(np.abs(np.where(XA_22,XA_20,0)))
    XA_24=(XA_23>0)
    XA_25=(MA(CLOSE,80)-MA(CLOSE,13)/3)
    XA_26=(MA((CLOSE-XA_25)/XA_25,1)*XA_1)
    XA_27=(EMA(XA_26,12)*0.5)
    XA_28=(EMA(XA_26,3))
    XA_29=(np.abs(np.where(llv(XA_28-XA_27,12)>0,0,1)))
    XA_30=(CLOSE/(EMA(EMA((CLOSE+LOW+HIGH)/3,3),26)*0.9)<0.95)
    XA_31=(MA((CLOSE-MA(CLOSE,21))/MA(CLOSE,21),3)*100)
    XA_32=(XA_31<(-15)*XA_2)
    XA_33=((CLOSE-MA(CLOSE,28))/MA(CLOSE,28)*100<(-23)*XA_2)
    XA_34=(XA_8 ) * ( XA_7 ) * ( XA_9 ) * ( XA_13)
    XA_35=(XA_32>0 ) * ( XA_30>0 ) * ( XA_29>0 ) * ( XA_24>0 ) * ( XA_15>0 ) * ( XA_17>0 ) * ( XA_33>0 ) * ( CLOSE>REF(CLOSE,1)*XA_2)
    a=(XA_35 ) * ( XA_34)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
