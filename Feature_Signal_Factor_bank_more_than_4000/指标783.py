...
#开始
    ...
    VAR3=(REF(HIGH,1)*1.1)
    VAR4=(HIGH*1.1)
    VAR5=((VAR4*VOL+VAR3*(VOL)))
    VAR6=(REF(LOW,1)*0.9)
    VAR7=(LOW*0.9)
    VAR8=((VAR7*VOL+VAR6*(VOL)))
    VAR9=(EMA(VAR8,30))
    VARA=(EMA(VAR5,30))
    VARB=(VARA-2)
    VARC=(np.where(C<=VAR9,VARB,VARA))
    a=(VARC-VARB==0 ) * ( CROSS((EMA(EMA(EMA(C,2),2),2)-REF(EMA(EMA(EMA(C,2),2),2),1))/REF(EMA(EMA(EMA(C,2),2),2),1)*100,MA((EMA(EMA(EMA(C,2),2),2)-REF(EMA(EMA(EMA(C,2),2),2),1))/REF(EMA(EMA(EMA(C,2),2),2),1)*100,3)))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
