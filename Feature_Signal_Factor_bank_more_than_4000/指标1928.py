...
#开始
    ...
    AA=(((C-llv(L,21))/(hhv(H,21)-llv(L,21)))*100)
    DD=(sma(AA,13,8))
    EE=(sma(np.maximum(C-REF(C,2),0),7,1)/sma(np.abs(C-REF(C,2)),7,1)*100)
    FF=(800/(sma(DD,13,8)))
    a=(FF<REF(FF,1) ) * ( REF(FF,1)>REF(FF,2) ) * ( count(FF>REF(FF,1),6)>=4 ) * ( EE>REF(EE,1) ) * ( REF(EE,1)<REF(EE,2))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
