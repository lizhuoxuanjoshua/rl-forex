...
#开始
    ...
    LH=(80)
    VAR1=(REF(CLOSE,1))
    VAR2=(sma(np.maximum(CLOSE-VAR1,0),18,1)/sma(np.abs(CLOSE-VAR1),13,1)*100)
    VAR3=(sma(np.maximum(CLOSE-VAR1,0),24,1)/sma(np.abs(CLOSE-VAR1),20,1)*100)
    VAR4=(barslast(VAR3<48)<=3 ) * ( CROSS(VAR2,VAR3))
    ...
    VARA=(1011131)
    VARB=(LH)
    a=(VAR4 ) * ( VAR5 ) * ( CROSS(VAR8,VAR9) ) * ( VAR8<=25)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
