...
#开始
    ...
    AA=(100*(hhv(HIGH,2)-CLOSE)/(hhv(HIGH,2)-llv(LOW,2)))
    M=(count(VOL,1))
    量=count(VOL*CLOSE,1)/count(VOL,1)
    BBB=((量-REF(量,1)))
    AAA=(EMA(BBB,1))
    F=(100*(C-REF(C,1))/REF(C,1))
    P1=((C<REF(O,1) ) * ( F<0))
    P2=((C<REF(O,2) ) * ( F<0))
    TT3=(P1==0 ) * ( REF(P1,1)==1 ) * ( P2==0 ) * ( REF(P2,1)==1)
    a=(AA<REF(AA,1) ) * ( REF(AA,1)>85 ) * ( AAA>REF(AAA,1) ) * ( TT3)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
