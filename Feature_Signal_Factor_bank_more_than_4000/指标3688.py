...
#开始
    ...
    X_1=(EMA(EMA((CLOSE-OPEN)*2+OPEN,18),5)-EMA(EMA((CLOSE-OPEN)*2+OPEN,3),5))
    X_2=((EMA(CLOSE,14)-EMA(CLOSE,4))/EMA(CLOSE,4))
    a=(X_1>0 ) * ( REF(X_1,1)<0 ) * ( X_2>0 ) * ( REF(X_1,2)>REF(X_1,3) ) * ( REF(X_1,3)>REF(X_1,4))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
