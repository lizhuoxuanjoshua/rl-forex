...
#开始
    ...
    AA1=(REF(C,2)<REF(C,1)*1.01)
    AA2=(REF(C,1)/C<1.09)
    AA3=(O>REF(C,1)*0.95 ) * ( O<REF(C,1)*1.015)
    AA4=(V<REF(V,1)*1.2)
    AA5=((C-L)<(O-C)*2.2)
    AA6=(H>O)
    AA7=(V<llv(V,16)*9.5)
    AA8=(H/C>1.021)
    AA9=(L>REF(C,3)*1.03)
    AA10=(H>REF(C,1)*1.005)
    AA11=(L>REF(O,2)*0.99)
    AA12=((C-L)<(O-C)*0.6)
    AA13=(REF(C,1)/REF(C,2)<1.0995)
    AA14=(REF(H,1)/C<1.07)
    AA17=(L/REF(L,2)>1.03)
    AA18=(REF(C,1)/L<1.035)
    AA19=(H-O>(C-L))
    AA20=(REF(H/C,1)>1.001 ) * ( REF(H/C,2)>1.001)
    AA21=(EMA(C,5)>MA(C,14)*1.017)
    AA22=(V>MA(V,15))
    AA23=(REF(H/np.minimum(O,C),3)>1.011)
    AA24=(L>REF(H,3)*1.003)
    a=(AA1 ) * ( AA2 ) * ( AA3 ) * ( AA4 ) * ( AA5 ) * ( AA8 ) * ( AA9 ) * ( AA10 ) * ( AA12 ) * ( AA17 ) * ( AA18 ) * ( AA21 ) * ( AA23 ) * ( AA24)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
