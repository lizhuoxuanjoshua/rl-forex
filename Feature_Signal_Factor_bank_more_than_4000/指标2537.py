...
#开始
    ...
    A=((3*C+L+O+H)/6)
    VAR4=((20*A+19*REF(A,1)+18*REF(A,2)+17*REF(A,3)+16*REF(A,4)+15*REF(A,5)+14*REF(A,6)+13*REF(A,7)+12*REF(A,8)+11*REF(A,9)+10*REF(A,10)+9*REF(A,11)+8*REF(A,12)+7*REF(A,13)+6*REF(A,14)+5*REF(A,15)+4*REF(A,16)+3*REF(A,17)+2*REF(A,18)+REF(A,20))/210)
    MTM=(C-REF(C,1))
    DX=(100*EMA(EMA(MTM,6),6)/EMA(EMA(np.abs(MTM),6),6))
    AA=(llv(DX,2)==llv(DX,7) ) * ( count(DX<0,2) ) * ( CROSS(DX,MA(DX,2)))
    a=(FILTER(AA==1,5))
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
