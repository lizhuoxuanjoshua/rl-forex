...
#开始
    ...
    XA_5=(MA(CLOSE,60))
    XA_6=(MA(CLOSE,250))
    XA_7=(EMA(100*(CLOSE-llv(LOW,34))/(hhv(HIGH,34)-llv(LOW,34)),3))
    XA_9=1
    XA_10=(count(CLOSE>REF(CLOSE,1),5)/30*100)
    XA_11=(MA(XA_10,3))
    XA_12=(XA_9 ) * ( CROSS(CLOSE,XA_4) ) * ( XA_5>XA_6 ) * ( XA_4>XA_5 ) * ( XA_2>XA_3 ) * ( XA_3>XA_4)
    XA_13=(27)
    XA_14=(23)
    XA_15=(100*(hhv(HIGH,XA_13)-CLOSE)/(hhv(HIGH,XA_13)-llv(LOW,XA_13)))
    XA_16=(100*(hhv(HIGH,XA_14)-CLOSE)/(hhv(HIGH,XA_14)-llv(LOW,XA_14)))
    XA_17=(CLOSE*VOL)
    XA_18=(EMA((EMA(XA_17,3)/EMA(VOL,3)+EMA(XA_17,6)/EMA(VOL,6)+EMA(XA_17,12)/EMA(VOL,12)+EMA(XA_17,24)/EMA(VOL,24))/4,13))
    XA_19=(1.08*XA_18)
    XA_20=(EMA(XA_19-(EMA(CLOSE,3)-XA_19),5))
    XA_21=(np.where(XA_20<=XA_19,XA_20,XA_19))
    XA_22=(2*XA_19-XA_15)
    XA_23=(np.where(XA_22>=XA_19,XA_22,XA_19))
    a=(XA_16<=0 ) * ( REF(XA_15>0,1) ) * ( LOW<XA_21)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
