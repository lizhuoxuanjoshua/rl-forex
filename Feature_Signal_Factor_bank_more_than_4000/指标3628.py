...
#开始
    ...
    多空=((MA(LOW,6)+MA(LOW,13)+MA(LOW,26))/3)
    相对高位=(np.where(HIGH==hhv(HIGH,5),HIGH==hhv(HIGH,5),CLOSE==hhv(CLOSE,5)))
    相对低位=(np.where(LOW==llv(LOW,5),LOW==llv(LOW,5),CLOSE==llv(CLOSE,5)))
    TJ6=(((REF(相对低位,1)) ) + ( (REF(相对低位,2))))
    TJ7=(((REF(相对高位,1)) ) + ( (REF(相对高位,2))))
    a=(TJ6 ) * ( REF(CLOSE,1)<REF(MA(CLOSE,3),1) ) * ( CLOSE>MA(CLOSE,3) ) * ( CLOSE>=多空)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
