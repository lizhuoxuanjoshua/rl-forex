...
#开始
    ...
    XA_1=(OPEN>CLOSE)
    XA_3=(OPEN>CLOSE)
    XA_4=(REF(OPEN,3)/CLOSE>1.09)
    XA_5=(VOL>REF(VOL,1)*1.2)
    XA_6=(VOL<REF(VOL,2)*1.4)
    XA_7=(LOW<REF(LOW,2)*1)
    XA_8=(np.maximum(OPEN,CLOSE)/np.minimum(OPEN,CLOSE)>1.035)
    XA_9=(OPEN/CLOSE>1.035)
    XA_10=(count(XA_8,6)>=1)
    XA_11=(HIGH<REF(LOW,4)*0.998)
    XA_12=(CLOSE/LOW<1.02)
    XA_13=(REF(CLOSE,1)/CLOSE>1.001)
    XA_14=(REF(OPEN,6)/CLOSE>=1.02)
    XA_15=(OPEN<REF(LOW,1)*1.02)
    XA_16=1
    XA_17=(REF(HIGH,1)<REF(MA(CLOSE,5),2)*1.01)
    XA_18=(REF(MA(CLOSE,5),1)/REF(HIGH,1)>=1.001)
    XA_19=(EMA(CLOSE,16)/OPEN>1.025)
    XA_20=(REF(CLOSE-LOW,1)<REF(OPEN-CLOSE,1))
    XA_21=((HIGH+LOW+CLOSE)/3)
    XA_22=((XA_21-MA(XA_21,14))/(0.015*AVEDEV(XA_21,14)))
    XA_23=(REF(XA_22,1)-XA_22<20)
    XA_24=(REF(OPEN>CLOSE,3) ) * ( REF(HIGH,2)<REF(MA(CLOSE,5),2))
    XA_25=(OPEN/CLOSE>1.0995)
    XA_26=(count(XA_25,7)==0)
    XA_27=(hhv(HIGH,19)/llv(LOW,20)>=1.2)
    XA_29=(HIGH<MA(CLOSE,7))
    XA_31=(REF(CLOSE,1)>REF(MA(CLOSE,3),1))
    XA_32=(LOW>REF(LOW,2))
    XA_33=(VOL>REF(VOL,1)*0.6 ) * ( VOL<REF(VOL,1)*1.3)
    XA_34=(REF(LOW,1)>REF(LOW,2)*0.985)
    XA_35=(VOL<=MA(VOL,60))
    XA_36=(REF(CLOSE,1)/REF(MA(CLOSE,5),1)<1.05)
    XA_37=(HIGH<REF(HIGH,5))
    XA_38=(np.maximum(OPEN,CLOSE)/np.minimum(OPEN,CLOSE)<1.06)
    XA_39=(MA(CLOSE,20)/CLOSE>1.09)
    XA_40=(EMA(CLOSE,60)/CLOSE>1.08 ) * ( EMA(CLOSE,60)/CLOSE<1.25)
    XA_41=(REF(CLOSE,5)/CLOSE>1.025)
    XA_42=(CLOSE/LOW<1.035)
    XA_43=(REF(VOL,5)>VOL*0.8)
    XA_44=(REF(VOL,4)<VOL*2)
    XA_45=(REF(OPEN,1)<=REF(CLOSE,2)*1.02)
    XA_46=(OPEN<REF(CLOSE,1)*1.002)
    XA_47=(np.maximum(OPEN,CLOSE)/np.minimum(OPEN,CLOSE)<1.04)
    XA_48=(CLOSE-REF(CLOSE,10))
    XA_49=(EMA(XA_48,6))
    XA_50=(XA_49>REF(XA_48,1)*0.97)
    XA_51= ( XA_31 ) * ( XA_33 ) * ( XA_34 ) * ( XA_38 ) * ( XA_39 ) * ( XA_41 ) * ( XA_42 ) * ( XA_43 ) * ( XA_44 ) * ( XA_40 ) * ( XA_45 ) * ( XA_46 ) * ( XA_50)
    a= ( XA_51
)
#结束
    买入 = a
