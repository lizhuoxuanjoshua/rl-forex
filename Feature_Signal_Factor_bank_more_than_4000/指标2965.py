...
#开始
    ...
    V8=((V6>=150 ) * ( V6<200 ) * ( V7>=150 ) * ( V7<200))
    VA=((C-MA(C,13))/MA(C,13)*100)
    VB=(100-np.abs(VA))
    顶线=(np.where(VB<90,VB,100))
    多方=(np.where(V1>0,V1,0))
    预警系统=(((V7>=200 ) * ( V6>=150)) ) + ( CROSS(65,多方))
    T3=(多方>60 ) * ( 预警系统>65 ) * ( 顶线==100)
    T4=(多方>48 ) * ( 预警系统>65 ) * ( 顶线<100)
    T7=(V3>REF(V4,1) ) * ( V>REF(V,1) ) * ( C>REF(C,1))
    a=(T7 ) * ( count(T7,30)==1)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
