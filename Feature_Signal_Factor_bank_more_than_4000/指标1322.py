...
#开始
    ...
    PVAR2=(sma(np.abs(LOW-PVAR1),13,1)/sma(np.maximum(LOW-PVAR1,0),10,1))
    PVAR3=(EMA(PVAR2,10))
    PVAR4=(llv(LOW,33))
    PVAR5=(EMA(np.where(LOW<=PVAR4,PVAR3,0),3))
    PVAAA=(np.where(PVAR5<REF(PVAR5,1),PVAR5,0))
    a=(PVAAA>0.01 ) * ( PVAAA<0.05)
#结束
    买入 = a
    #确定买入和测试信号------------------------------------------------------
    测试信号 =买入
